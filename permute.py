...
def adjudicate_adoption(content, src_path):
    """Authoritative from-scratch measurement gating an adoption while warm
    screening is active. Returns (full_size, full_models)."""
    size, _, _, models = measure_in_verify_dir(content, src_path, 'full')
    return size, models
    ...
    def _on_done(self, future, worker_id, total_bar, group_bar, stats_bar,
        ...
        try:
            ...
            if is_stale:
                ...
                tag = f"(verify rejected: full {fmt_size(full_size)})"
            else:
                tag = ""
            group_bar.write(
...
def main():
    ...
    for run in range(1, num_runs + 1):
        ...
        pass_best = {
            ...
            'best': screen_initial,
            'best_full': initial_size,
            'best_content': text,
        }

        engine.best_size = screen_initial
        engine.best_full = initial_size
