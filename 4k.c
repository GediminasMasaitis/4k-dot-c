...
static void move_str(char* restrict str, const Move* restrict move,
  ...
  for (i32 i = 0; i < 2; i++) {
    G(6, str[i * 2] = 'a' + (&move->from)[i] % 8;)
      G(6, str[i * 2 + 1] = '1' + ((&move->from)[i] / 8 ^ 7 * flip);)
  }
...
[[nodiscard]] static i32 is_attacked(const Position* const restrict pos,
  ...
  const u64 blockers = pos->colour[0] | pos->colour[1];
  return G(8, rook(bb, blockers) & theirs &
    (pos->pieces[Rook] | pos->pieces[Queen])) ||
    G(8, knight(bb) & theirs & pos->pieces[Knight]) ||
    G(8, king(bb) & theirs & pos->pieces[King]) ||
    G(8, bishop(bb, blockers) & theirs &
...
i32 makemove(Position* const restrict pos, const Move* const restrict move) {
  ...
  G(9, // Castling
    ...
    G(9, pos->colour[0] ^= mask;)

    // Move the piece
    G(9, pos->pieces[piece] ^= mask;)
    G(9, // Captures
  ...
  G(10, // Pawn double move
    ...
    G(10, // Update castling permissions
...
static Move* generate_piece_moves(H(11, 1, Move* restrict movelist),
  H(11, 1, const Position* restrict pos),
  H(11, 1, const u64 to_mask)) {
...
[[nodiscard]] static i32 movegen(const Position* const restrict pos,
  ...
  if (!only_captures && pos->castling[0] && !(all & 0x60ull) &&
    !is_attacked(pos, 1ULL << 4, true) &&
    !is_attacked(pos, 1ULL << 5, true)) {
    *movelist++ =
      (Move){ .from = 4, .to = 6, .promo = None, .takes_piece = None };
  }
  if (!only_captures && pos->castling[1] && !(all & 0xEull) &&
    !is_attacked(pos, 1ULL << 4, true) &&
    !is_attacked(pos, 1ULL << 3, true)) {
    *movelist++ =
      (Move){ .from = 4, .to = 2, .promo = None, .takes_piece = None };
  }
  movelist = generate_piece_moves(H(11, 2, movelist), H(11, 2, pos), H(11, 2, to_mask));
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  H(12, 1, i16 material[6];)
  H(12, 1, i8 pst_rank[64];)
  H(12, 1, i8 pst_file[64];)
    H(12, 1, i8 mobilities[4];)
    H(13, 1, i8 king_attacks[4];)
    H(13, 1, i8 open_files[6];)
    H(13, 1, i8 bishop_pair;)
    H(13, 1, i8 tempo;)
} EvalParams;

typedef struct [[nodiscard]] __attribute__((packed)) {
  H(12, 2, i32 material[6];)
  H(12, 2, i32 pst_rank[64];)
  H(12, 2, i32 pst_file[64];)
    H(12, 2, i32 mobilities[4];)
    H(13, 2, i32 king_attacks[4];)
    H(13, 2, i32 open_files[6];)
    H(13, 2, i32 bishop_pair;)
    H(13, 2, i32 tempo;)
} EvalParamsMerged;

G(14, static const EvalParams mg = ((EvalParams) {
...
G(14, static const EvalParams eg = ((EvalParams) {
...
static i32 eval(Position* const restrict pos) {
  G(16, i32 score = eval_params.tempo;)
    G(16, i32 phase = 0;)

    for (i32 c = 0; c < 2; c++) {

      G(17, const u64 opp_king_zone = king(pos->colour[1] & pos->pieces[King]);)

        G(17, const u64 own_pawns = pos->colour[0] & pos->pieces[Pawn];)
        G(17, // BISHOP PAIR
        ...
        for (i32 p = Pawn; p <= King; p++) {
          ...
          while (copy) {
            ...
            G(18, const int rank = sq >> 3;)
              G(18, const int file = sq & 7;)
              ...
              G(19, score += eval_params.pst_file[(p - 1) * 8 + file];)
              G(19, score += eval_params.pst_rank[(p - 1) * 8 + rank];)

              G(19, if ((north(0x101010101010101ULL << sq) & own_pawns) == 0) {
...
typedef struct [[nodiscard]] {
  G(21, Move killer;)
    G(21, i32 static_eval;)
    G(21, Move best_move;)
    G(21, i32 num_moves;)
    G(21, Move moves[max_moves];)
    G(21, u64 position_hash;)
  } SearchStack;

typedef struct [[nodiscard]] __attribute__((packed)) {
  G(22, Move move;)
    G(22, u16 partial_hash;)
    G(22, u8 flag;)
    G(22, i8 depth;)
    G(22, i16 score;)
} TTEntry;
...
G(23, static i32 move_history[2][6][64][64];)
G(23, static TTEntry tt[tt_length];)
G(23, static size_t start_time;)
G(23, static size_t max_time;)
...
#error "Unsupported architecture: get_hash only for x86_64 and aarch64"
  ...
  static i16 search(H(24, 1, Position* const restrict pos), H(24, 1, const i32 ply), H(24, 1, i32 depth),
    H(24, 1, i32 alpha), H(25, 1, const i32 beta),
#ifdef FULL
    u64* nodes,
#endif
    H(25, 1, SearchStack* restrict stack), H(25, 1, const i32 pos_history_count),
    H(25, 1, const bool do_null)) {
  assert(alpha < beta);
  assert(ply >= 0);
  ...
  // TT PROBING
  G(26, TTEntry * tt_entry = &tt[tt_hash % tt_length];)
    G(26, const u16 tt_hash_partial = tt_hash / tt_length;)
    G(26, stack[ply].best_move = (Move){ 0 };)
    if (tt_entry->partial_hash == tt_hash_partial) {
      stack[ply].best_move = tt_entry->move;

      // TT PRUNING
      if (G(27, alpha == beta - 1) &&
        G(27, tt_entry->flag != tt_entry->score <= alpha) &&
        G(27, tt_entry->depth >= depth)) {
  ...
  if (G(28, tt_entry->partial_hash == tt_hash_partial) &&
    G(28, tt_entry->flag != static_eval > tt_entry->score)) {
    static_eval = tt_entry->score;
  }

  // QUIESCENCE
  if (G(29, in_qsearch) && G(29, static_eval > alpha)) {
    if (static_eval >= beta) {
      return static_eval;
    }
    alpha = static_eval;
  ...
  if (G(30, !in_check) && G(30, alpha == beta - 1)) {
    if (G(31, depth < 8) && G(31, !in_qsearch)) {
    ...
    // NULL MOVE PRUNING
    if (G(33, do_null) && G(33, depth > 2) && G(33, static_eval >= beta)) {
      Position npos = *pos;
      flip_pos(&npos);
      npos.ep = 0;
      const i32 score =
        -search(H(24, 2, &npos), H(24, 2, ply + 1), H(24, 2, depth - 3 - depth / 4), H(24, 2, -beta), H(25, 2, -alpha),
#ifdef FULL
          nodes,
#endif
          H(25, 2, stack), H(25, 2, pos_history_count), H(25, 2, false));
      if (score >= beta) {
        return score;
      }
    }
    ...
    for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
      ...
      if (G(37, depth < 8) &&
        G(37, static_eval + 128 * depth +
          ...
          alpha) &&
        G(37, moves_evaluated) && G(37, !in_check)) {
        break;
      }
      ...
      i32 score;
      while (true) {
        score = -search(H(24, 3, &npos), H(24, 3, ply + 1), H(24, 3, depth - reduction), H(24, 3, low), H(25, 3, -alpha),
#ifdef FULL
          nodes,
#endif
          H(25, 3, stack), H(25, 3, pos_history_count), H(25, 3, true));
      ...
      if (score > alpha) {
        ...
        if (score >= beta) {
          ...
          G(39, if (stack[ply].best_move.takes_piece == None) {
...
#endif
    i32 score = search(H(24, 4, pos), H(24, 4, 0), H(24, 4, depth), H(24, 4, -inf), H(25, 4, inf),
#ifdef FULL
      nodes,
#endif
      H(25, 4, stack), H(25, 4, pos_history_count), H(25, 4, false));
