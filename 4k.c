...
S(1) void get_piece_hashes(const Position *const pos, u64 hashes[5]) {
  for (i32 c = 0; c < 2; c++) {
    ...
    for (i32 p = Pawn; p <= King; p++) {
      const u64 mixed =
          (G(185, G(274, pos->pieces[p]) & G(274, pos->colour[c])) *
           G(185, 0x9E3779B97F4A7C15ULL)) >>
          48;
      hashes[p == Pawn ? 0 : 1 + c] =
          G(272, hashes[p == Pawn ? 0 : 1 + c] * 9) + G(272, mixed);
      if (p == Knight || p == Bishop) {
        hashes[3] = G(273, hashes[3] * 9) + G(273, mixed);
      }
    }
  }
}
...
i32 search(
  ...
  G(271, corr_entries[5] =
             &data->corr_cont[pos->flipped][0][stack[ply + 1].move_key];)
  G(271, corr_entries[6] =
             &data->corr_cont[pos->flipped][1][stack[ply].move_key];)
  for (i32 i = 0; i < 7; i++) {
  ...
  G(
      ...
      if (G(243,
          ...
          G(243, !in_check) && G(243, !in_qsearch)) {
