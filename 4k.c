...
G(7, [[nodiscard]] S(1) i32 iabs(i32 x) { return (x ^ (x >> 31)) - (x >> 31); })
...
S(0) i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    G(125, const i32 own_king_sq =
               lsb(G(126, pos->colour[0]) & G(126, pos->pieces[King]));)
    G(125, const i32 opp_king_sq =
               lsb(G(126, pos->colour[1]) & G(126, pos->pieces[King]));)
      ...
      while (copy) {
        ...
        G(
            ...
            if (G(139, p == Pawn) &&
              ...
              G(141, {
                const i32 king_sqs[] = {own_king_sq, opp_king_sq};
                for (i32 ki = 0; ki < 2; ki++) {
                  const i32 dr = iabs((king_sqs[ki] >> 3) - rank - 1);
                  const i32 df = iabs((king_sqs[ki] & 7) - file);
                  score += eval_params.pawn_passed_king_distance[ki] *
                           (rank - 1) * (dr > df ? dr : df);
                }
              })
            })
