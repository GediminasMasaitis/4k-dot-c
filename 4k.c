...
G(62, static const EvalParams mg = ((EvalParams){
          .material = {72, 318, 299, 402, 902, 0},
          .pst_rank =
              {
                  0,   -16, -11, -10, 4,  33, 127, 0,   // Pawn
                  -35, -21, 3,   15,  31, 55, 26,  -76, // Knight
                  -11, 2,   11,  11,  16, 22, -1,  -51, // Bishop
                  -2,  -13, -18, -21, -2, 19, 17,  19,  // Rook
                  13,  15,  8,   -4,  -8, -1, -20, -3,  // Queen
                  -15, -7,  -27, -25, 12, 71, 87,  99,  // King
              },
          .pst_file =
              {
...
G(62, static const EvalParams eg = ((EvalParams){
          .material = {87, 302, 280, 535, 965, 0},
          .pst_rank =
              {
                  0,   -10, -15, -15, -4, 44, 113, 0,   // Pawn
                  -35, -15, -1,  20,  24, 9,  -1,  -1,  // Knight
                  -7,  -6,  -1,  0,   3,  1,  1,   8,   // Bishop
                  -14, -17, -13, 2,   9,  9,  15,  9,   // Rook
                  -56, -42, -20, 9,   27, 28, 37,  16,  // Queen
                  -31, -1,  11,  21,  26, 21, 7,   -37, // King
              },
          .pst_file =
              {
                  12,  13, 0,  -6, -6, -6, -1, -7,  // Pawn
                  -22, -4, 11, 17, 17, 7,  -3, -22, // Knight
                  -3,  0,  -1, 2,  3,  2,  2,  -4,  // Bishop
                  1,   4,  5,  0,  -5, 0,  -1, -4,  // Rook
                  -18, -4, 3,  9,  12, 10, -3, -9,  // Queen
                  -15, -3, 9,  20, 15, 18, 0,  -23, // King
              },
          .mobilities = {5, 2, 1, 0},
          .king_attacks = {-3, -6, 6, 0},
          .open_files = {32, -1, 10, 15, 27, 10},
          .bishop_pair = 53,
          .tempo = 8});)
...
static i32 eval(Position *const restrict pos) {
  G(65, i32 phase = 0;)
  G(65, i32 score = eval_params.tempo;)

  for (i32 c = 0; c < 2; c++) {

    G(66, const u64 opp_king_zone = king(pos->colour[1] & pos->pieces[King]);)

    G(66, const u64 own_pawns = pos->colour[0] & pos->pieces[Pawn];)
    G(
        66, // BISHOP PAIR
    ...
    for (i32 p = Pawn; p <= King; p++) {
      ...
      while (copy) {
        G(67, phase += phases[p];)
        G(67, const i32 sq = lsb(copy);)
        G(68, copy &= copy - 1;)
        G(68, const int rank = sq >> 3;)
        G(68, const int file = sq & 7;)
...
G(71, static i32 move_history[2][6][64][64];)
G(71, static TTEntry tt[tt_length];)
G(71, static size_t start_time;)
G(71, static size_t max_time;)
...
static i16 search(H(72, 1, const i32 ply),
                  H(72, 1, Position *const restrict pos), H(72, 1, i32 alpha),
                  H(72, 1, i32 depth), H(73, 1, const bool do_null),
#ifdef FULL
                  u64 *nodes,
#endif
                  H(73, 1, SearchStack *restrict stack),
                  H(73, 1, const i32 pos_history_count),
                  H(73, 1, const i32 beta)) {
  assert(alpha < beta);
  assert(ply >= 0);
  ...
  for (i32 i = G(74, pos_history_count) + G(74, ply);
       G(75, i > 0) && G(75, do_null); i -= 2) {
  ...
  // TT PROBING
  G(76, stack[ply].best_move = (Move){0};)
  G(76, const u16 tt_hash_partial = tt_hash / tt_length;)
  G(76, TTEntry *tt_entry = &tt[tt_hash % tt_length];)
  if (tt_entry->partial_hash == tt_hash_partial) {
    stack[ply].best_move = tt_entry->move;

    // TT PRUNING
    if (G(77, alpha == beta - 1) &&
        G(77, tt_entry->flag != tt_entry->score <= alpha) &&
        G(77, tt_entry->depth >= depth)) {
  ...
  if (G(78, tt_entry->partial_hash == tt_hash_partial) &&
      G(78, tt_entry->flag != static_eval > tt_entry->score)) {
    static_eval = tt_entry->score;
  }

  // QUIESCENCE
  if (G(79, static_eval > alpha) && G(79, in_qsearch)) {
    if (static_eval >= beta) {
      return static_eval;
    }
    alpha = static_eval;
  ...
  if (G(80, !in_check) && G(80, alpha == beta - 1)) {
    if (G(81, depth < 8) && G(81, !in_qsearch)) {
    ...
    // NULL MOVE PRUNING
    if (G(83, do_null) && G(83, depth > 2) && G(83, static_eval >= beta)) {
      Position npos = *pos;
      flip_pos(&npos);
      npos.ep = 0;
      const i32 score = -search(
          H(72, 2, ply + 1), H(72, 2, &npos), H(72, 2, -beta),
          H(72, 2, depth - 3 - depth / 4), H(73, 2, false),
#ifdef FULL
          nodes,
#endif
          H(73, 2, stack), H(73, 2, pos_history_count), H(73, 2, -alpha));
      if (score >= beta) {
        return score;
      }
    }
  ...
  G(72, i32 moves_evaluated = 0;)
  G(72, stack[pos_history_count + ply + 2].position_hash = tt_hash;)
  G(72, stack[ply].num_moves = movegen(
            H(55, 3, pos), H(55, 3, stack[ply].moves), H(55, 3, in_qsearch));)
  G(72, i32 best_score = in_qsearch ? static_eval : -inf;)
  G(72, i32 quiets_evaluated = 0;)
  G(72, u8 tt_flag = Upper;)
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      const i32 order_move_score =
          G(73, // MOST VALUABLE VICTIM
            stack[ply].moves[order_index].takes_piece * 921) +
          G(73, // HISTORY HEURISTIC
            move_history[pos->flipped]
                        [stack[ply].moves[order_index].takes_piece]
                        [stack[ply].moves[order_index].from]
                        [stack[ply].moves[order_index].to]) +
          G(73, // PREVIOUS BEST MOVE FIRST
            (move_equal(G(84, &stack[ply].best_move),
                        G(84, &stack[ply].moves[order_index]))
             << 30)) +
          G(73, // KILLER MOVE
            move_equal(G(85, &stack[ply].killer),
                       G(85, &stack[ply].moves[order_index])) *
                915);
      if (order_move_score > move_score) {
        G(86, move_score = order_move_score;)
        G(86, swapmoves(G(87, &stack[ply].moves[move_index]),
                        G(87, &stack[ply].moves[order_index]));)
      }
    }
    ...
    if (G(88, depth < 8) &&
        G(88,
          G(89, static_eval + 128 * depth) +
                  G(89,
                    max_material[stack[ply].moves[move_index].takes_piece]) +
                  G(89, max_material[stack[ply].moves[move_index].promo]) <
              alpha) &&
        G(88, !in_check) && G(88, moves_evaluated)) {
      break;
    }
    ...
    i32 reduction =
        G(90, depth > 1) && G(90, moves_evaluated > 6)
            ? 1 + (alpha == beta - 1) + moves_evaluated / 11 + !improving
            : 1;

    i32 score;
    while (true) {
      score = -search(H(72, 3, ply + 1), H(72, 3, &npos), H(72, 3, low),
                      H(72, 3, depth - reduction), H(73, 3, true),
#ifdef FULL
                      nodes,
#endif
                      H(73, 3, stack), H(73, 3, pos_history_count),
                      H(73, 3, -alpha));
    ...
    if (score > alpha) {
      G(91, stack[ply].best_move = stack[ply].moves[move_index];)
      G(91, alpha = score;)
      G(91, tt_flag = Exact;)
      if (score >= beta) {
        tt_flag = Lower;
        ...
        G(
            92, if (stack[ply].best_move.takes_piece == None) {
              stack[ply].killer = stack[ply].best_move;
            })
        G(92, const i32 bonus = depth * depth;
          G(93,
            i32 *const this_hist =
            ...
            *this_hist += bonus - bonus * *this_hist / 1024;)
              G(
                  93, for (i32 prev_index = 0; prev_index < move_index;
                           prev_index++) {
    ...
    // LATE MOVE PRUNING
    if (G(94, alpha == beta - 1) &&
        G(94, quiets_evaluated > 1 + depth * depth >> !improving) &&
        G(94, !in_check)) {
      break;
    }
  }
...
#endif
    H(95, 1, const i32 pos_history_count),
    H(95, 1, SearchStack *restrict stack),
    H(95, 1, Position *const restrict pos)) {
  start_time = get_time();
#ifdef FULL
...
#endif
    i32 score =
        search(H(72, 4, 0), H(72, 4, pos), H(72, 4, -inf), H(72, 4, depth),
               H(73, 4, false),
#ifdef FULL
               nodes,
#endif
               H(73, 4, stack), H(73, 4, pos_history_count), H(73, 4, inf));
...
static void bench() {
  ...
  iteratively_deepen(20, &nodes, H(95, 2, pos_history_count), H(95, 2, stack),
                     H(95, 2, &pos));
...
static void run() {
  ...
  G(96, char line[4096];)
  G(96, __builtin_memset(move_history, 0, sizeof(move_history));)
  G(96, init();)
  G(96, Position pos;)
  G(96, // #ifdef LOWSTACK
        //  SearchStack *stack = malloc(sizeof(SearchStack) * 1024);
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      max_time = 99999999999;
      iteratively_deepen(max_ply, &nodes, H(95, 3, pos_history_count),
                         H(95, 3, stack), H(95, 3, &pos));
...
#endif
    G(97, if (line[0] == 'q') { exit_now(); })
    else G(
        97, if (line[0] == 'i') { puts("readyok"); }) else if (line[0] == 'p') {
      G(98, pos = start_pos;)
      G(98, pos_history_count = 0;)
      while (true) {
      ...
      iteratively_deepen(max_ply, &nodes, H(95, 4, pos_history_count),
                         H(95, 4, stack), H(95, 4, &pos));
#else
      for (i32 i = 0; i < (pos.flipped ? 4 : 2); i++) {
        getl(line);
        max_time = atoi(line) / 2;
      }
      iteratively_deepen(H(95, 5, pos_history_count), H(95, 5, stack),
                         H(95, 5, &pos));
#endif
    }
  }
