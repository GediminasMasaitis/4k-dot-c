...
G(
    43, S(0) Move *generate_pawn_moves(H(44, 1, const i32 offset),
                                       H(44, 1, u64 to_mask),
                                       H(44, 1, Move *restrict movelist),
                                       H(44, 1, const Position *const pos)) {
      while (to_mask) {
...
[[nodiscard]] S(0) i32 movegen(H(61, 1, const Position *const restrict pos),
  ...
  movelist = generate_pawn_moves(
      ...
      H(44, 3, movelist), H(44, 3, pos));
  movelist = generate_pawn_moves(
      H(44, 4, -7),
      H(44, 4,
        nw(pos->colour[0] & pos->pieces[Pawn]) & (pos->colour[1] | pos->ep)),
      H(44, 4, movelist), H(44, 4, pos));
  movelist = generate_pawn_moves(
      H(44, 5, -9),
      H(44, 5,
        ne(pos->colour[0] & pos->pieces[Pawn]) & (pos->colour[1] | pos->ep)),
      H(44, 5, movelist), H(44, 5, pos));
  if (G(63, !(all & 0x60ull)) && G(63, pos->castling[0]) &&
      G(63, !only_captures) &&
  ...
  if (G(65, !(all & 0xEull)) && G(65, pos->castling[1]) &&
      G(65, !only_captures) &&
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    while (true) {
      score = -search(H(96, 3, &npos), H(96, 3, low), H(96, 3, ply + 1),
                      ...
                      nodes,
#endif
                      H(97, 3, stack), H(97, 3, pos_history_count),
                      H(97, 3, -alpha));
