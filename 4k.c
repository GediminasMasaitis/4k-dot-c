...
TUNE_PARAMETER(lmr_history_divisor, 256, 0, 768, 32)
TUNE_PARAMETER(ffp_depth, 8, 5, 12, 0.5)
TUNE_PARAMETER(ffp_margin, 142, 72, 192, 6)
TUNE_PARAMETER(msp_margin, 128, 72, 192, 6)
...
i32 search(H(175, 1, const i32 beta), H(175, 1, SearchStack *restrict stack),
  ...
  if (G(189, !in_check) && G(189, G(190, alpha) == G(190, beta - 1))) {
    if (G(191, !in_qsearch) && G(191, depth < rfp_depth)) {
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    G(
        ...
        if (G(209, depth < ffp_depth) &&
            G(209,
              G(210, static_eval) + G(210, G(211, ffp_margin) * G(211, depth)) +
    ...
    G(
        ...
        if (G(212, move_score < G(213, -msp_margin) * G(213, depth)) &&
    ...
    i32 reduction = G(214, depth > 3) && G(214, move_score <= 0)
                        ? G(215, (move_score < -lmr_history_divisor)) +
                              G(215, (G(216, alpha) == G(216, beta - 1))) +
                              G(215, !improving) + G(215, moves_evaluated / lmr_move_divisor)
                        : 0;
