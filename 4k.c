...
static void swapu64(H(19, 1, u64 *const lhs), H(19, 1, u64 *const rhs)) {
  const u64 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
}

static void swapu32(H(20, 1, u32 *const lhs), H(20, 1, u32 *const rhs)) {
  const u32 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
}

static void swapmoves(H(21, 1, Move *const lhs), H(21, 1, Move *const rhs)) {
  swapu32(H(20, 2, (u32 *)lhs), H(20, 2, (u32 *)rhs));
}
...
static void flip_pos(Position *const restrict pos) {
  ...
  pos->flipped ^= 1;
  for (i32 i = 0; i < 2; i++) {
    swapbool(G(22, &pos->castling[i + 2]), G(22, &pos->castling[i]));
  }
  swapu64(H(19, 2, &pos->colour[0]), H(19, 2, &pos->colour[1]));
}
...
is_attacked(H(24, 1, const Position *const restrict pos),
            H(24, 1, const i32 them), H(24, 1, const u64 bb)) {
  assert(count(bb) == 1);
...
i32 makemove(H(26, 1, Position *const restrict pos),
             H(26, 1, const Move *const restrict move)) {
  assert(move->from >= 0);
  assert(move->from < 64);
  assert(move->to >= 0);
  assert(move->to < 64);
      ...
      if (piece == King) {
        const u64 bb = move->to - move->from == 2   ? 0xa0
                       : move->from - move->to == 2 ? 0x9
                                                    : 0;
        G(30, pos->colour[0] ^= bb;)
        G(30, pos->pieces[Rook] ^= bb;)
      })

  G(29, pos->colour[0] ^= mask;)
  ...
  // En passant
  if (G(32, piece == Pawn) && G(32, to == pos->ep)) {
    G(33, pos->colour[1] ^= to >> 8;)
    G(33, pos->pieces[Pawn] ^= to >> 8;)
  }
  pos->ep = 0;
  ...
  return !is_attacked(H(24, 2, pos), H(24, 2, false),
                      H(24, 2, pos->colour[1] & pos->pieces[King]));
}

static Move *generate_pawn_moves(H(38, 1, const Position *const pos),
                                 H(38, 1, const i32 offset),
                                 H(38, 1, u64 to_mask),
                                 H(38, 1, Move *restrict movelist)
...
static Move *generate_piece_moves(H(39, 1, const Position *restrict pos),
                                  H(39, 1, const u64 to_mask),
                                  H(39, 1, Move *restrict movelist)) {
...
[[nodiscard]] static i32 movegen(H(40, 1, const Position *const restrict pos),
  ...
  G(41, const Move *start = movelist;)
  G(41, const u64 all = pos->colour[0] | pos->colour[1];)
  G(41, const u64 to_mask = only_captures ? pos->colour[1] : ~pos->colour[0];)
  if (!only_captures) {
    movelist = generate_pawn_moves(
        H(38, 2, pos), H(38, 2, -16),
        H(38, 2,
  ...
  if (G(44, !only_captures) && G(44, pos->castling[1]) &&
      G(44, !(all & 0xEull)) &&
      G(45, !is_attacked(H(24, 5, pos), H(24, 5, true), H(24, 5, 1ULL << 4))) &&
      G(45, !is_attacked(H(24, 6, pos), H(24, 6, true), H(24, 6, 1ULL << 3)))) {
    *movelist++ =
        (Move){.from = 4, .to = 2, .promo = None, .takes_piece = None};
  }
  movelist = generate_piece_moves(H(39, 2, pos), H(39, 2, to_mask),
                                  H(39, 2, movelist));
...
[[nodiscard]] static u64 perft(const Position *const restrict pos,
  ...
  for (i32 i = 0; i < num_moves; ++i) {
    ...
    // Check move legality
    if (!makemove(H(26, 2, &npos), H(26, 2, &moves[i]))) {
      continue;
    }
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  ...
  i8 king_attacks[4];
  H(46, 1, i8 tempo;)
  H(46, 1, i8 mobilities[4];)
  H(46, 1, i8 bishop_pair;)
  H(46, 1, i8 open_files[6];)
  H(46, 1, i8 pst_file[64];)
  H(46, 1, i8 pst_rank[64];)
} EvalParams;
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  ...
  i32 king_attacks[4];
  H(46, 2, i32 tempo;)
  H(46, 2, i32 mobilities[4];)
  H(46, 2, i32 bishop_pair;)
  H(46, 2, i32 open_files[6];)
  H(46, 2, i32 pst_file[64];)
  H(46, 2, i32 pst_rank[64];)
} EvalParamsMerged;

G(47, static const EvalParams mg = ((EvalParams){
...
G(47, static const EvalParams eg = ((EvalParams){
...
static i32 combine_eval_param(H(48, 1, const i32 mg_val),
                              H(48, 1, const i32 eg_val)) {
...
static void init() {
  ...
  G(
      ...
      for (i32 i = 0; i < sizeof(mg.material) / sizeof(i16); i++) {
        eval_params.material[i] = combine_eval_param(H(48, 2, mg.material[i]),
                                                     H(48, 2, eg.material[i]));
      })
  G(
      ...
      for (i32 i = 0; i < sizeof(mg) - sizeof(mg.material); i++) {
        ...
        ((i32 *)&eval_params)[offset / sizeof(*mg.material) + i] =
            combine_eval_param(H(48, 3, ((i8 *)&mg)[offset + i]),
                               H(48, 3, ((i8 *)&eg)[offset + i]));
      })
}
...
static i32 eval(Position *const restrict pos) {
  G(49, i32 phase = 0;)
  G(49, i32 score = eval_params.tempo;)

  for (i32 c = 0; c < 2; c++) {

    G(50, const u64 opp_king_zone = king(pos->colour[1] & pos->pieces[King]);)
    ...
    G(50, const u64 own_pawns = pos->colour[0] & pos->pieces[Pawn];)
    ...
    for (i32 p = Pawn; p <= King; p++) {
      ...
      while (copy) {
        ...
        G(51, const int rank = sq >> 3;)
        G(51, const int file = sq & 7;)

        G(38, // SPLIT PIECE-SQUARE TABLES FOR FILE
...
typedef struct [[nodiscard]] {
  G(46, Move best_move;)
  G(46, i32 static_eval;)
  G(46, i32 num_moves;)
  G(46, Move killer;)
  G(46, Move moves[max_moves];)
  G(46, u64 position_hash;)
} SearchStack;

typedef struct [[nodiscard]] __attribute__((packed)) {
  G(53, i16 score;)
  G(53, u16 partial_hash;)
  G(53, Move move;)
  G(53, i8 depth;)
  G(53, u8 flag;)
} TTEntry;
...
G(54, static i32 move_history[2][6][64][64];)
G(54, static TTEntry tt[tt_length];)
G(54, static size_t start_time;)
G(54, static size_t max_time;)
...
static i16 search(H(55, 1, const i32 ply),
                  H(55, 1, Position *const restrict pos), H(55, 1, i32 alpha),
                  H(55, 1, i32 depth), H(56, 1, const bool do_null),
#ifdef FULL
                  u64 *nodes,
#endif
                  H(56, 1, SearchStack *restrict stack),
                  H(56, 1, const i32 pos_history_count),
                  H(56, 1, const i32 beta)) {
  assert(alpha < beta);
  assert(ply >= 0);

  const bool in_check =
      is_attacked(H(24, 7, pos), H(24, 7, true),
                  H(24, 7, pos->colour[0] & pos->pieces[King]));
  ...
  // TT PROBING
  G(57, stack[ply].best_move = (Move){0};)
  G(57, const u16 tt_hash_partial = tt_hash / tt_length;)
  G(57, TTEntry *tt_entry = &tt[tt_hash % tt_length];)
  if (tt_entry->partial_hash == tt_hash_partial) {
    stack[ply].best_move = tt_entry->move;

    // TT PRUNING
    if (G(58, alpha == beta - 1) &&
        G(58, tt_entry->flag != tt_entry->score <= alpha) &&
        G(58, tt_entry->depth >= depth)) {
  ...
  if (G(59, tt_entry->partial_hash == tt_hash_partial) &&
      G(59, tt_entry->flag != static_eval > tt_entry->score)) {
    static_eval = tt_entry->score;
  }

  // QUIESCENCE
  if (G(60, static_eval > alpha) && G(60, in_qsearch)) {
    if (static_eval >= beta) {
      return static_eval;
    }
    alpha = static_eval;
  ...
  if (G(61, !in_check) && G(61, alpha == beta - 1)) {
    if (G(62, depth < 8) && G(62, !in_qsearch)) {
    ...
    // NULL MOVE PRUNING
    if (G(64, do_null) && G(64, depth > 2) && G(64, static_eval >= beta)) {
      Position npos = *pos;
      flip_pos(&npos);
      npos.ep = 0;
      const i32 score = -search(
          H(55, 2, ply + 1), H(55, 2, &npos), H(55, 2, -beta),
          H(55, 2, depth - 3 - depth / 4), H(56, 2, false),
#ifdef FULL
          nodes,
#endif
          H(56, 2, stack), H(56, 2, pos_history_count), H(56, 2, -alpha));
      if (score >= beta) {
        return score;
      }
    }
  ...
  G(55, i32 moves_evaluated = 0;)
  G(55, stack[pos_history_count + ply + 2].position_hash = tt_hash;)
  G(55, stack[ply].num_moves = movegen(
            H(40, 3, pos), H(40, 3, stack[ply].moves), H(40, 3, in_qsearch));)
  G(55, i32 best_score = in_qsearch ? static_eval : -inf;)
  G(55, i32 quiets_evaluated = 0;)
  G(55, u8 tt_flag = Upper;)
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      if (order_move_score > move_score) {
        ...
        G(65, swapmoves(H(21, 2, &stack[ply].moves[move_index]),
                        H(21, 2, &stack[ply].moves[order_index]));)
      }
    }
    ...
    if (G(66, depth < 8) &&
        G(66,
          G(67, static_eval + 128 * depth) +
                  G(67,
                    max_material[stack[ply].moves[move_index].takes_piece]) +
                  G(67, max_material[stack[ply].moves[move_index].promo]) <
              alpha) &&
        G(66, !in_check) && G(66, moves_evaluated)) {
      break;
    }
    ...
    (*nodes)++;
#endif
    if (!makemove(H(26, 3, &npos), H(26, 3, &stack[ply].moves[move_index]))) {
      continue;
    }
    ...
    i32 reduction =
        G(68, depth > 1) && G(68, moves_evaluated > 6)
            ? 1 + (alpha == beta - 1) + moves_evaluated / 11 + !improving
            : 1;

    i32 score;
    while (true) {
      score = -search(H(55, 3, ply + 1), H(55, 3, &npos), H(55, 3, low),
                      H(55, 3, depth - reduction), H(56, 3, true),
#ifdef FULL
                      nodes,
#endif
                      H(56, 3, stack), H(56, 3, pos_history_count),
                      H(56, 3, -alpha));
    ...
    if (score > alpha) {
      ...
      if (score >= beta) {
        ...
        G(
            69, if (stack[ply].best_move.takes_piece == None) {
    ...
    // LATE MOVE PRUNING
    if (G(70, alpha == beta - 1) &&
        G(70, quiets_evaluated > 1 + depth * depth >> !improving) &&
        G(70, !in_check)) {
      break;
    }
  }
...
#endif
    H(71, 1, const i32 pos_history_count),
    H(71, 1, SearchStack *restrict stack),
    H(71, 1, Position *const restrict pos)) {
  start_time = get_time();
#ifdef FULL
...
#endif
    i32 score =
        search(H(55, 4, 0), H(55, 4, pos), H(55, 4, -inf), H(55, 4, depth),
               H(56, 4, false),
#ifdef FULL
               nodes,
#endif
               H(56, 4, stack), H(56, 4, pos_history_count), H(56, 4, inf));
...
static void bench() {
  ...
  iteratively_deepen(20, &nodes, H(71, 2, pos_history_count), H(71, 2, stack),
                     H(71, 2, &pos));
...
static void run() {
  ...
  G(72, char line[4096];)
  G(72, __builtin_memset(move_history, 0, sizeof(move_history));)
  G(72, init();)
  G(72, Position pos;)
  G(72, // #ifdef LOWSTACK
        //  SearchStack *stack = malloc(sizeof(SearchStack) * 1024);
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      max_time = 99999999999;
      iteratively_deepen(max_ply, &nodes, H(71, 3, pos_history_count),
                         H(71, 3, stack), H(71, 3, &pos));
    ...
    if (line[0] == 'q') {
      ...
      while (true) {
        ...
        for (i32 i = 0; i < num_moves; i++) {
          ...
          if (move_string_equal(H(8, 3, line), H(8, 3, move_name))) {
            ...
            makemove(H(26, 4, &pos), H(26, 4, &stack[0].moves[i]));
            break;
          }
        }
        if (!line_continue) {
      ...
      iteratively_deepen(max_ply, &nodes, H(71, 4, pos_history_count),
                         H(71, 4, stack), H(71, 4, &pos));
#else
      for (i32 i = 0; i < (pos.flipped ? 4 : 2); i++) {
        getl(line);
        max_time = atoi(line) / 2;
      }
      iteratively_deepen(H(71, 5, pos_history_count), H(71, 5, stack),
                         H(71, 5, &pos));
#endif
    }
  }
