...
G(35, [[nodiscard]] S(1) u64 knight(const u64 bb) {
  G(800, const u64 r1 = east(bb);)
    ...
    return G(802, h2 << 8) | G(802, h1 >> 16) | G(802, h1 << 16) | G(802, h2 >> 8);
})
...
G(55, S(1) void move_str(H(62, 1, char* restrict str),
  ...
  G(63, str[5] = '\0';)
    ...
    G(63, str[4] = "\0\0nbrq"[move->promo];)
    G(63, // Hack to save bytes, technically UB but works on GCC 14.2
      for (i32 i = 0; i < 2; i++) {
        G(64, str[i * 2] = 'a' + (&move->from)[i] % 8;)
          G(64, str[i * 2 + 1] = '1' + ((&move->from)[i] / 8 ^ 7 * flip);)
      })
})
...
G(87, S(0) Move* generate_pawn_moves(H(106, 1, const i32 offset),
  ...
  G(136, i32 score = eval_params.tempo;)
    ...
    for (i32 c = 0; c < 2; c++) {
      ...
      G(137, const u64 opp_king_zone =
        ...
        G(137, const u64 own_pawns = G(139, pos->pieces[Pawn]) & G(139, pos->colour[0]);)
        G(137, const u64 opp_pawns = G(140, pos->pieces[Pawn]) & G(140, pos->colour[1]);
      const u64 attacked_by_pawns =
        G(141, se(opp_pawns)) | G(141, sw(opp_pawns));
      G(142, const u64 no_passers = G(143, opp_pawns) | G(143, attacked_by_pawns);)
...
#error "Unsupported architecture: get_hash only for x86_64 and aarch64"
  ...
  G(199, i32 moves_evaluated = 0;)
    ...
    for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
      ...
      G(208, // FORWARD FUTILITY PRUNING / DELTA PRUNING
        if (G(209, depth < 8) &&
          G(209, G(210, static_eval) + G(210, G(211, 142) * G(211, depth)) +
            G(210, initial_params.eg.material
              [stack[ply].moves[move_index].takes_piece]) +
            G(210, initial_params.eg
              .material[stack[ply].moves[move_index].promo]) <
            alpha) &&
          G(209, !in_check) && G(209, moves_evaluated)) {
          break;
        })

        G(208, // MOVE SCORE PRUNING
          if (G(212, move_score < G(213, -128) * G(213, depth)) &&
            G(212, moves_evaluated)) {
            break;
          })
        ...
        if (score > alpha) {
          G(218, tt_flag = Exact;)
            ...
            if (score >= beta) {
              ...
              G(219, tt_flag = Lower;)
                G(219, if (stack[ply].best_move.takes_piece == None) {
                ...
                *this_hist +=
                  ...
                  G(220, for (i32 prev_index = 0; prev_index < move_index;
                    prev_index++) {
...
S(1) void init() {
  G(93, // MERGE EVAL PARAMS
    for (i32 i = 0; i < sizeof(EvalParamsMerged) / sizeof(i32); i++) {
      ((i32*)&eval_params)[i] =
        combine_eval_param(H(134, 2, i < 6 ? initial_params.mg.material[i]
          : ((i8*)&initial_params.mg)[6 + i]),
          H(134, 2, i < 6 ? initial_params.eg.material[i]
    ...
    G(93, // INIT DIAGONAL MASKS
      for (i32 sq = 0; sq < 64; sq++) {
        const u64 bb = 1ULL << sq;
        G(228, u64 sw_bb = sw(bb);)
          G(228, u64 ne_bb = ne(bb);)
          for (i32 i = 6; i > 0; i--) {
            G(229, sw_bb |= sw(sw_bb);)
              G(229, ne_bb |= ne(ne_bb);)
          }
        diag_mask[sq] = G(230, sw_bb) | G(230, ne_bb);
      })
}
...
#endif
  H(231, 1, Position* const restrict pos),
  H(231, 1, SearchStack* restrict stack),
  H(231, 1, const i32 pos_history_count)) {
  G(232, start_time = get_time();)
    G(232, i32 score = 0;)
#ifdef FULL
    ...
    for (i32 depth = 1; depth < max_ply; depth++) {
      ...
      // ASPIRATION WINDOWS
      G(233, i32 window = 15;)
        G(233, size_t elapsed;)
        while (true) {
          G(234, const i32 alpha = score - window;)
            G(234, const i32 beta = G(235, score) + G(235, window);)
            score =
          ...
          G(236, window *= 2;)
            G(236, if (G(237, elapsed > max_time) ||
              G(237, (G(238, score > alpha) && G(238, score < beta)))) {
            break;
          })
        }
...
S(1) void bench() {
  ...
  iteratively_deepen(23, &nodes, H(231, 2, &pos), H(231, 2, stack),
    H(231, 2, pos_history_count));
...
S(1) void run() {
  ...
  G(239, char line[4096];)
    G(239, Position pos;)
    G(239, i32 pos_history_count;)
    G(239, // #ifdef LOWSTACK
      //  SearchStack *stack = malloc(sizeof(SearchStack) * 1024);
      // #else
      SearchStack stack[1024];
  // #endif
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      max_time = -1ll;
      iteratively_deepen(max_ply, &nodes, H(231, 3, &pos), H(231, 3, stack),
        H(231, 3, pos_history_count));
    }
    ...
    G(240, if (G(243, line[0]) == G(243, 'i')) { puts("readyok"); })
      ...
      iteratively_deepen(max_ply, &nodes, H(231, 4, &pos), H(231, 4, stack),
        H(231, 4, pos_history_count));
#else
      for (i32 i = 2 << pos.flipped; i > 0; i--) {
        getl(line);
        max_time = (u64)atoi(line) << 19; // Roughly /2 time
      }
      iteratively_deepen(H(231, 5, &pos), H(231, 5, stack),
        H(231, 5, pos_history_count));
#endif
    })
