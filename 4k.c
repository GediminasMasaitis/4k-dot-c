  ...
  if (G(104, !in_check) && G(104, alpha == beta - 1)) {
    if (G(105, !in_qsearch) && G(105, depth < 8)) {
      ...
      G(106, {
        ...
        if (static_eval - 53 * depth >= beta) {
          return static_eval;
        }
      })

      G(106, // RAZORING
        in_qsearch = static_eval + 133 * depth <= alpha;)
    }
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      const i32 order_move_score =
          ...
          G(97, // MOST VALUABLE VICTIM
            stack[ply].moves[order_index].takes_piece * 851) +
