...
G(10, [[nodiscard]] S(1) bool move_string_equal(G(11, const char* restrict lhs),
...
G(19, [[nodiscard]] S(1) u64 sw(const u64 bb) {
...
G(35, [[nodiscard]] S(1) u64 knight(const u64 bb) {
  return G(41, G(42, (G(43, bb << 17) | G(43, bb >> 15))) &
    G(42, ~0x101010101010101ull)) |
    G(41, G(44, (G(45, bb << 15 | G(45, bb >> 17)))) &
      G(44, ~0x8080808080808080ull)) |
    G(41, G(46, (G(47, bb << 6) | G(47, bb >> 10))) &
      G(46, 0x3F3F3F3F3F3F3F3Full)) |
    G(41, G(48, (G(49, bb >> 6) | G(49, bb << 10))) &
      G(48, 0xFCFCFCFCFCFCFCFCull));
})

G(35, [[nodiscard]] S(1) u64 king(const u64 bb) {
...
G(55, S(1) void swapu32(G(56, u32* const rhs), G(56, u32* const lhs)) {
  const u32 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
}
...
G(55, [[nodiscard]] S(1) bool move_equal(G(59, Move* const rhs),
  G(59, Move* const lhs)) {
  return G(60, *(u32*)lhs) == G(60, *(u32*)rhs);
})

G(55, S(1) void swapu64(G(61, u64* const rhs), G(61, u64* const lhs)) {
  const u64 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
})

G(55, S(1) void move_str(H(62, 1, char* restrict str),
  H(62, 1, const Move* restrict move),
  H(62, 1, const i32 flip)) {
  assert(move->from >= 0);
  assert(move->from < 64);
  assert(move->to >= 0);
  assert(move->to < 64);
...
G(55, S(1) void swapu16(G(62, u16* const restrict lhs),
  G(62, u16* const restrict rhs)) {
  const u16 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
})

G(67, [[nodiscard]] S(1) i32 is_attacked(H(68, 1, const Position* const restrict pos),
  H(68, 1, const u64 bb)) {
  assert(count(bb) == 1);
  ...
  G(69, const u64 pawns = theirs & pos->pieces[Pawn];)
    ...
    G(70, G(74, king(bb)) & G(74, theirs) & G(74, pos->pieces[King])) ||
...
G(67, S(0) void flip_pos(Position* const restrict pos) {
  G(77, swapu64(G(78, &pos->colour[0]), G(78, &pos->colour[1]));)

    G(77, // Hack to flip the first 10 bitboards in Position.
...
G(67, [[nodiscard]] S(1) u64 get_mobility(H(80, 1, const Position* pos),
  ...
  G(81, if (piece == King) { moves = king(bb); })
    ...
    G(83, if (G(84, piece == Queen) || G(84, piece == Bishop)) {
      moves |= bishop(H(36, 3, blockers), H(36, 3, bb));
    })
      G(83, if (G(85, piece == Queen) || G(85, piece == Rook)) {
...
G(87, S(1) Move* generate_piece_moves(H(88, 1, const u64 to_mask),
  ...
  for (u8 piece = Knight; piece <= King; piece++) {
    ...
    u64 copy = G(89, pos->pieces[piece]) & G(89, pos->colour[0]);
    while (copy) {
        ...
        while (moves) {
          ...
          G(92, *movelist++ = ((Move) {
            .from = from,
              .to = to,
              .promo = None,
              .takes_piece = piece_on(H(65, 2, pos), H(65, 2, to))
          });)
            G(92, moves &= moves - 1;)
        }
    }
...
G(87, S(0) i32 makemove(H(93, 1, Position* const restrict pos),
  ...
  assert(move->takes_piece == piece_on(H(65, 3, pos), H(65, 3, move->to)));
...
G(87, S(0) Move* generate_pawn_moves(H(106, 1, const Position* const pos),
  H(106, 1, Move* restrict movelist),
  H(106, 1, u64 to_mask),
  H(106, 1, const i32 offset)) {
  while (to_mask) {
    ...
    assert(to >= 0);
    assert(to < 64);
    assert(piece_on(H(65, 5, pos), H(65, 5, from)) == Pawn);
    const u8 takes = piece_on(H(65, 6, pos), H(65, 6, to));
    if (to > 55) {
...
[[nodiscard]] S(1) i32 movegen(H(108, 1, const Position* const restrict pos),
  ...
  G(109, const u64 all = G(110, pos->colour[0]) | G(110, pos->colour[1]);)
    ...
    G(111, // PAWN PROMOTIONS
      if (!only_captures) {
        movelist = generate_pawn_moves(
          H(106, 2, pos), H(106, 2, movelist),
          H(106, 2, G(112, north(G(113, north(G(114, pos->colour[0]) &
    ...
    G(111, // PAWN DOUBLE MOVES
      movelist = generate_pawn_moves(
        H(106, 3, pos), H(106, 3, movelist),
        H(106, 3, north(G(115, G(116, pos->colour[0]) & G(116, pos->pieces[Pawn]))) &
          G(115, (only_captures ? 0xFF00000000000000ull : ~0ull)) &
          G(115, ~all)),
        H(106, 3, -8));)
    G(111, // PAWN WEST CAPTURES
      movelist = generate_pawn_moves(
        H(106, 4, pos), H(106, 4, movelist),
        H(106, 4, G(117, nw(G(118, pos->colour[0]) & G(118, pos->pieces[Pawn]))) &
          G(117, (G(119, pos->colour[1]) | G(119, pos->ep)))),
        H(106, 4, -7));)
    G(111, // PAWN EAST CAPTURES
      movelist = generate_pawn_moves(
        H(106, 5, pos), H(106, 5, movelist),
        H(106, 5, G(120, ne(G(121, pos->colour[0]) & G(121, pos->pieces[Pawn]))) &
          G(120, (G(122, pos->colour[1]) | G(122, pos->ep)))),
        H(106, 5, -9));)
    G(111, // LONG CASTLE
      if (G(123, !only_captures) && G(123, pos->castling[0]) &&
        G(123, !(G(124, all) & G(124, 0x60ull))) &&
        G(125, !is_attacked(H(68, 3, pos), H(68, 3, 1ULL << 5))) &&
        G(125, !is_attacked(H(68, 4, pos), H(68, 4, 1ULL << 4)))) {
        *movelist++ =
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i16 material[6];
  H(129, 1, H(130, 1, i8 pawn_attacked_penalty[2];) H(130, 1, i8 mobilities[5];)
    ...
    H(129, 1, H(131, 1, i8 passed_pawns[6];) H(131, 1, i8 king_attacks[5];)
      H(131, 1, i8 pst_rank[48];) H(131, 1, i8 bishop_pair;)
      H(131, 1, i8 protected_pawn;) H(131, 1, i8 phalanx_pawn;))
} EvalParams;

typedef struct [[nodiscard]] __attribute__((packed)) {
  i32 material[6];
  H(129, 2, H(130, 2, i32 pawn_attacked_penalty[2];) H(130, 2, i32 mobilities[5];)
    ...
    H(129, 2, H(131, 2, i32 passed_pawns[6];) H(131, 2, i32 king_attacks[5];)
      H(131, 2, i32 pst_rank[48];) H(131, 2, i32 bishop_pair;)
      H(131, 2, i32 protected_pawn;) H(131, 2, i32 phalanx_pawn;))
...
G(133, __attribute__((aligned(8))) S(1)
  ...
  G(133, [[nodiscard]] S(1) i32 combine_eval_param(H(134, 1, const i32 mg_val),
...
S(1) i32 eval(Position* const restrict pos) {
  G(136, i32 score = eval_params.tempo;)
    ...
    for (i32 c = 0; c < 2; c++) {
      ...
      G(137, const u64 opp_king_zone =
        ...
        G(137, const u64 opp_pawns = G(139, pos->pieces[Pawn]) & G(139, pos->colour[1]);
      const u64 attacked_by_pawns =
        G(140, se(opp_pawns)) | G(140, sw(opp_pawns));
      G(141, const u64 no_passers = G(142, opp_pawns) | G(142, attacked_by_pawns);)
        G(141, // PROTECTED PAWNS
          score -=
          G(143, eval_params.protected_pawn) *
          G(143, count(G(144, opp_pawns) & G(144, attacked_by_pawns)));)
        G(141, // PHALANX PAWNS
          score -=
          G(145, eval_params.phalanx_pawn) *
          G(145, count(G(146, opp_pawns) & G(146, west(opp_pawns))));))
        G(137, // BISHOP PAIR
          if (count(G(147, pos->pieces[Bishop]) & G(147, pos->colour[0])) > 1) {
            score += eval_params.bishop_pair;
          })
        G(137, const u64 own_pawns = G(148, pos->pieces[Pawn]) & G(148, pos->colour[0]);)
        ...
        for (u8 p = Pawn; p <= King; p++) {
          ...
          while (copy) {
            ...
            G(150, phase += initial_params.phases[p];)
              ...
              G(106, // OPEN FILES / DOUBLED PAWNS
              ...
              G(106, // PASSED PAWNS
                if (G(153, p == Pawn) &&
                  G(153, !(G(154, (0x101010101010101ULL << sq)) &
                    G(154, no_passers)))) {
                  G(155, if (G(156, north(1ULL << sq)) & G(156, pos->colour[1])) {
              ...
              G(106, // SPLIT PIECE-SQUARE TABLES FOR FILE
                score +=
                eval_params
                .pst_file[G(157, G(158, (p - 1)) * G(158, 8)) + G(157, file)];)

              G(106, // SPLIT PIECE-SQUARE TABLES FOR RANK
                score +=
                eval_params
                .pst_rank[G(159, G(160, (p - 1)) * G(160, 8)) + G(159, rank)];)

              G(106, // MATERIAL
                score += eval_params.material[p];)

              G(106, if (p > Pawn) {
              G(161, // PIECES ATTACKED BY PAWNS
  ...
  return (G(170, (i16)score) * G(170, phase) +
    G(171, (128 - stronger_side_pawns_missing * stronger_side_pawns_missing)) *
...
#error "Unsupported architecture: get_hash only for x86_64 and aarch64"
    ...
    if (G(196, depth > 2) && G(196, static_eval >= beta) && G(196, do_null)) {
      ...
      G(197, flip_pos(&npos);)
        ...
        const i32 score = -search(
          H(175, 2, -alpha), H(175, 2, stack),
          H(175, 2, depth - G(198, 4) - G(198, depth / 4)), H(175, 2, false),
          H(175, 2, -beta),
#ifdef FULL
          nodes,
#endif
  ...
  G(199, i32 moves_evaluated = 0;)
    ...
    G(199, stack[ply].num_moves = movegen(H(108, 3, pos), H(108, 3, stack[ply].moves),
      H(108, 3, in_qsearch));)
    G(199, u8 tt_flag = Upper;)
    G(199, i32 best_score = in_qsearch ? static_eval : -inf;)
    G(199, stack[G(200, pos_history_count) + G(200, ply) + G(200, 2)].position_hash =
      tt_hash;)
    ...
    for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
      ...
      if (G(208, depth < 8) &&
        G(208, G(209, static_eval) + G(209, G(210, 142) * G(210, depth)) +
      ...
      i32 score;
      while (true) {
        score = -search(H(175, 3, -alpha), H(175, 3, stack),
          H(175, 3, depth - G(214, 1) - G(214, reduction)),
          H(175, 3, true), H(175, 3, low),
#ifdef FULL
          nodes,
#endif
      ...
      if (score > best_score) {
        ...
        if (score > alpha) {
          G(215, tt_flag = Exact;)
            G(215, stack[ply].best_move = stack[ply].moves[move_index];)
            G(215, alpha = score;)
            if (score >= beta) {
              assert(stack[ply].best_move.takes_piece ==
                piece_on(H(65, 8, pos), H(65, 8, stack[ply].best_move.to)));
              G(216, tt_flag = Lower;)
                G(216, if (stack[ply].best_move.takes_piece == None) {
                ...
                *this_hist +=
                  bonus - G(218, bonus) * G(218, *this_hist) / 1024;)
                  G(217, for (i32 prev_index = 0; prev_index < move_index;
                    prev_index++) {
                  ...
                  i32* const prev_hist =
                    ...
                    [prev.from][prev.to];
                  *prev_hist -=
                    bonus + G(219, bonus) * G(219, *prev_hist) / 1024;
                })
              })
                break;
            }
  ...
  if (G(223, best_score) == G(223, -inf)) {
    return G(224, (ply - mate)) * G(224, in_check);
  }
...
S(1) void init() {
  G(93, // CLEAR HISTORY
    ...
    G(93, // MERGE NON-MATERIAL VALUES
      for (i32 i = 0;
    ...
    ((i32*)&eval_params)[G(226, offset / sizeof(*initial_params.mg.material)) +
      G(226, i)] =
      combine_eval_param(
        H(134, 3, ((i8*)&initial_params.mg)[G(227, offset) + G(227, i)]),
        H(134, 3, ((i8*)&initial_params.eg)[G(228, offset) + G(228, i)]));
  })
}
...
void iteratively_deepen(
    ...
    for (i32 depth = 1; depth < max_ply; depth++) {
      ...
      G(231, i32 window = 15;)
        ...
        while (true) {
          ...
          G(234, window *= 2;)
            G(234, if (G(235, (G(236, score < beta) && G(236, score > alpha))) ||
              G(235, elapsed > max_time)) {
            break;
          })
        }
  ...
  char move_name[8];
  move_str(H(62, 3, move_name), H(62, 3, &stack[0].best_move),
    H(62, 3, pos->flipped));
  putl("bestmove ");
  puts(move_name);
    }
    ...
    G(238, if (G(239, line[0]) == G(239, 'g')) {
      ...
      G(243, pos_history_count = 0;)
        ...
        while (true) {
          ...
          for (i32 i = 0; i < num_moves; i++) {
            char move_name[8];
            move_str(H(62, 4, move_name), H(62, 4, &stack[0].moves[i]),
              H(62, 4, pos.flipped));
