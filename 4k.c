...
search(
  ...
  for (A(3, u8, i16, u16, i32, u32, i64, u64) move_index = 0; move_index < ss->num_moves; move_index++) {
    ...
    for (A(5, u8, i16, u16, i32, u32, i64, u64) order_index = move_index; order_index < ss->num_moves; order_index++) {
      ...
      const A(1, i32, i64) order_move_score = G(187, // HISTORY HEURISTIC
                                              ...
                                              G(187, // MOST VALUABLE VICTIM
                                                G(219, moves[order_index].takes_piece) * G(219, mvv_weight)) +
                                              G(187, // KILLER MOVE
                                                G(216, move_equal(G(217, &moves[order_index]), G(217, &ss->killer))) * G(216, killer_weight)) +
                                              G(187, // COUNTER MOVE
                                                G(327, move_equal(G(328, &moves[order_index]), G(328, &counter))) * G(327, counter_weight)) +
    ...
    G(
        ...
        if (G(223, moves_evaluated) && G(223, move_score < G(224, msp_margin) * G(224, depth))) { break; })
    ...
    if (score > best_score) {
      ...
      if (score > alpha) {
        ...
        if (score >= beta) {
          ...
          G(
              233, if (!in_qsearch) {
                const A(2, i16, u16, i32, i64) bonus = history_bonus * depth;
...
S(1) void run() {
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      PRINT_TUNE_OPTION(history_bonus);
      puts("uciok");
      ...
      READ_TUNE_OPTION(history_bonus)
      ...
      PRINT_TUNE_INPUT(history_bonus);
