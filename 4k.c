...
static i32 eval(const Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    const u64 own_pawns = pos->colour[c] & pos->pieces[Pawn];
    score += 4 * count(west(own_pawns) & own_pawns);
