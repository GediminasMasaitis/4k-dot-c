  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    for (i32 p = Pawn; p <= King; p++) {
      ...
      while (copy) {
        ...
        if (p == Pawn) {
          score -= 12 * (count((0x101010101010101ULL << file) & pos->colour[c] &
                               pos->pieces[Pawn]) -
                         1);
        }
      }
