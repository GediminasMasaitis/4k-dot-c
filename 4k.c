...
typedef struct [[nodiscard]] __attribute__((packed)) {
  ...
  H(116, 1,
    H(118, 1, i8 passed_pawns[6];) H(118, 1, i8 bishop_pair;)
        H(118, 1, i8 phalanx_pawn[6];) H(118, 1, i8 protected_pawn[6];)
            H(118, 1, i8 pst_rank[48];) H(118, 1, i8 king_attacks[5];))
} EvalParams;
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  ...
  H(116, 2,
    H(118, 2, i32 passed_pawns[6];) H(118, 2, i32 bishop_pair;)
        H(118, 2, i32 phalanx_pawn[6];) H(118, 2, i32 protected_pawn[6];)
            H(118, 2, i32 pst_rank[48];) H(118, 2, i32 king_attacks[5];))
} EvalParamsMerged;
...
G(121, // EVAL PARAMETERS
  __attribute__((aligned(8))) S(1)
      C(1)
          EvalParamsInitial initial_params =
              {.phases = {0, 0, 1, 1, 2, 4},
               .mg =
                   {
                       .material = {0, 64, 282, 321, 367, 815},
                       .pst_rank =
                           {
                               0,   -14, -18, -3,  7,   28,  76,  0,   // Pawn
                               -20, -6,  8,   23,  30,  35,  12,  -82, // Knight
                               -6,  11,  17,  19,  17,  17,  -9,  -66, // Bishop
                               7,   -2,  -7,  -11, 4,   12,  -2,  -1,  // Rook
                               13,  15,  7,   1,   -3,  -2,  -19, -12, // Queen
                               0,   9,   -2,  -23, -31, -12, -5,  16,  // King
                           },
                       .pst_file =
                           {
                               -8,  -12, -7,  2,   7,   12,  11, -5,  // Pawn
                               -29, -11, -1,  12,  14,  14,  7,  -5,  // Knight
                               -14, 0,   3,   1,   4,   -1,  10, -3,  // Bishop
                               -12, -8,  -2,  7,   12,  7,   6,  -11, // Rook
                               -14, -8,  -4,  0,   3,   4,   13, 6,   // Queen
                               -5,  16,  -11, -30, -28, -12, 11, 2,   // King
                           },
                       .mobilities = {6, 5, 2, 3, -3},
                       .king_attacks = {0, 14, 20, 14, 0},
                       .pawn_threat = {-17, -8, -9, -8, -1},
                       .open_files = {6,
                                      -12, -11, 15, -3, -22, 5, -9, -11, 32, -11, -58},
                       .passed_pawns = {-7, -10, -11, 6, 22, 76},
                       .passed_blocked_pawns = {-3, -7, -4, 2, -1, -42},
                       .passed_king_distance = {1, -2},
                       .protected_pawn = {0, 20, 12, 18, 30, 127},
                       .phalanx_pawn = {7, 12, 20, 37, 127, 127},
                       .bishop_pair = 26,
                       .bishop_pawns = {-6, -5},
                       .king_shield = {29, 18},
                       .pawn_attacked_penalty = {-56, -112},
                       .piece_threats = {24, 16},
                       .tempo = 29,
                   },
               .eg =
                   {
                       .material = {0, 93, 426, 461, 771, 1437},
                       .pst_rank =
                           {
                               0,   -1,  -8,  -7, -2, 18, 123, 0,   // Pawn
                               -28, -16, -6,  16, 21, 6,  1,   5,   // Knight
                               -14, -12, -1,  2,  5,  1,  5,   13,  // Bishop
                               -17, -19, -11, 3,  9,  9,  18,  7,   // Rook
                               -56, -41, -12, 13, 29, 25, 29,  13,  // Queen
                               -42, -9,  -1,  16, 39, 49, 38,  -31, // King
                           },
                       .pst_file =
                           {
                               3,   7,  -2, -8, -2, 2,  3,  -3,  // Pawn
                               -19, -5, 9,  18, 17, 4,  -3, -22, // Knight
                               -11, -1, 5,  11, 12, 5,  -4, -16, // Bishop
                               2,   6,  6,  -2, -6, 0,  0,  -4,  // Rook
                               -23, -8, 4,  9,  16, 15, -1, -10, // Queen
                               -34, -2, 12, 19, 20, 15, 4,  -31, // King
                           },
                       .mobilities = {4, 3, 4, 1, -4},
                       .king_attacks = {0, -3, -7, 9, 0},
                       .pawn_threat = {-2, -7, -16, -12, -6},
                       .open_files = {10, 6, 16, 13, 26, 9, 26, -7, 3, 14, 42,
                                      8},
                       .passed_pawns = {11, 5, 29, 65, 127, 123},
                       .passed_blocked_pawns = {-9, 1, -10, -26, -61, -68},
                       .passed_king_distance = {-6, 9},
                       .protected_pawn = {0, 16, 16, 20, 49, 39},
                       .phalanx_pawn = {5, 15, 26, 83, 127, 127},
                       .bishop_pair = 60,
                       .bishop_pawns = {-10, 0},
                       .king_shield = {-9, -8},
...
S(0) i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    G(125, const u64 opp_king_zone =
               king(G(127, pos->pieces[King]) & G(127, pos->colour[1]));)
                ...
                G(
                    ...
                    if (p < Queen) {
                      score +=
                          ...
                          G(289, eval_params
                                     .piece_threats[G(292, p) == G(292, Rook)]);
                    })
                    ...
                    G(170, // MOBILITY
                      ...
                      G(171, eval_params.mobilities[p - 2]);)
                        ...
                        G(170, // KING ATTACKS
                          score += G(173, count(G(174, mobility) &
                                                G(174, opp_king_zone))) *
  ...
  i32 scale = 128 - stronger_side_pawns_missing * stronger_side_pawns_missing;

  // OPPOSITE COLOURED BISHOP ENDGAMES
  if (G(330, !(pos->pieces[Knight] | pos->pieces[Rook] | pos->pieces[Queen])) &&
      G(330, count(pos->pieces[Bishop]) == 2) &&
      G(330, count(pos->pieces[Bishop] & pos->colour[0]) == 1) &&
      G(330, count(pos->pieces[Bishop] & 0xAA55AA55AA55AA55ULL) == 1)) {
    scale /= 2;
  }

  return (G(304, G(176, (i16)score) * G(176, phase)) +
          G(304, G(177, ((score + 0x8000) >> 16)) * G(177, scale) / 128 *
                     (24 - phase))) /
         24;
}
...
S(1)
void get_piece_hashes(H(300, 1, const Position *const pos),
...
i32 search(
    ...
    u64 *nodes,
#endif
    H(186, 1, H(187, 1, ThreadData *data), H(187, 1, Position *const pos),
  ...
  const bool improving =
      G(301, static_eval > ss[-2].static_eval) && G(301, ply > 1);
  if (G(199, tt_hit) &&
  ...
  if (G(203, !in_check) && G(203, G(204, alpha) == G(204, beta - 1))) {
    if (G(205, depth < 10) && G(205, !in_qsearch)) {
      ...
      G(206, // RAZORING
        in_qsearch =
            G(208, static_eval) + G(208, G(209, 83) * G(209, depth)) <= alpha;)
    }
    ...
    if (G(210, depth > 2) && G(210, do_null) && G(210, static_eval >= beta)) {
      ...
      const i32 score = -search(
          ...
          nodes,
#endif
          H(186, 2, H(187, 2, data), H(187, 2, &npos), H(187, 2, -alpha),
            H(187, 2, depth - G(212, 4) - G(212, depth / 4))),
  ...
  for (i32 move_index = 0; move_index < ss->num_moves; move_index++) {
    ...
    G(
        ...
        if (G(225, depth < 5) &&
            G(225,
              G(226, G(227, 176) * G(227, depth)) + G(226, static_eval) +
                      G(226,
    ...
    while (true) {
      score = -search(
          ...
          nodes,
#endif
          H(186, 3, H(187, 3, data), H(187, 3, &npos), H(187, 3, -alpha),
    ...
    // LATE MOVE PRUNING
    if (G(237, quiets_evaluated > (G(239, 1) + G(239, depth * depth)) >>
                   !improving) &&
        G(237, G(238, alpha) == G(238, beta - 1)) && G(237, !in_check)) {
      break;
    }
  }
  ...
  G(
      ...
      if (G(243, G(244, tt_flag) != G(244, (best_score < ss->static_eval))) &&
          G(243, G(245, ss->best_move.takes_piece) == G(245, None))) {
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    while (true) {
      ...
      score = search(
          ...
          &data->nodes,
#endif
          H(186, 4, H(187, 4, data), H(187, 4, &data->pos), H(187, 4, beta),
            H(187, 4, depth)),
