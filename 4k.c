...
G(
    32, S(1) void flip_pos(Position *const restrict pos) {
      G(35, swapu64(G(36, &pos->colour[0]), G(36, &pos->colour[1]));)
...
G(
    32, [[nodiscard]] S(1)
            u64 get_mobility(H(38, 1, const i32 piece), H(38, 1, const i32 sq),
              ...
              G(39, if (piece == Knight) { moves = knight(bb); })
              else G(39, if (piece == King) { moves = king(bb); }) else {
                const u64 blockers = pos->colour[0] | pos->colour[1];
                G(
...
G(
    43, S(0) i32 makemove(H(44, 1, Position *const restrict pos),
      ...
      G(46, const i32 piece = move->promo & 0xF; assert(piece != None);)
      G(46, const u64 mask = from | to;)

      G(
          47, // Captures
          if (move->takes_piece != None) {
            G(48, pos->colour[1] ^= to;)
            G(48, pos->pieces[move->takes_piece] ^= to;)
          })
        ...
        while (copy) {
          ...
          while (moves) {
            ...
            G(59, *movelist++ = ((Move){
                      ...
                      .to = to,
                      .promo = piece,
                      .takes_piece = piece_on(H(31, 3, pos), H(31, 3, to))});)
          }
        }
      }
...
G(
    43, S(0) Move *generate_pawn_moves(H(60, 1, const Position *const pos),
      ...
      while (to_mask) {
        ...
        assert(to >= 0);
        assert(to < 64);
        assert(piece_on(H(31, 4, pos), H(31, 4, from)) == Pawn);
        const u8 takes = piece_on(H(31, 5, pos), H(31, 5, to));
        if (to > 55) {
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i16 material[6];
  H(67, 1,
    H(68, 1, i8 king_attacks[5];) H(68, 1, i8 passed_blocked_pawns[6];)
        H(68, 1, i8 mobilities[5];) H(68, 1, i8 tempo;)
            H(68, 1, i8 passed_pawns[6];))
  H(67, 1,
    H(69, 1, i8 bishop_pair;) H(69, 1, u8 pawn_attacked_penalty[2];)
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i32 material[6];
  H(67, 2,
    H(68, 2, i32 king_attacks[5];) H(68, 2, i32 passed_blocked_pawns[6];)
        H(68, 2, i32 mobilities[5];) H(68, 2, i32 tempo;)
            H(68, 2, i32 passed_pawns[6];))
  H(67, 2,
    H(69, 2, i32 bishop_pair;) H(69, 2, i32 pawn_attacked_penalty[2];)
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      assert(
          ...
          piece_on(H(31, 6, pos), H(31, 6, stack[ply].moves[order_index].to)));
    ...
    if (G(113, depth < 8) &&
        G(113,
          G(114, static_eval + 136 * depth) +
                  G(114,
                    max_material[stack[ply].moves[move_index].promo >> 4]) +
                  G(114,
    ...
    if (score > best_score) {
      ...
      if (score > alpha) {
        G(118, tt_flag = Exact;)
        G(118, stack[ply].best_move = stack[ply].moves[move_index];)
        G(118, alpha = score;)
        if (score >= beta) {
          assert(stack[ply].best_move.takes_piece ==
                 piece_on(H(31, 7, pos), H(31, 7, stack[ply].best_move.to)));
          G(119, tt_flag = Lower;)
          G(
          ...
          G(119, const i32 bonus = depth * depth;
            G(120,
              ...
              *this_hist += bonus - bonus * *this_hist / 1024;)
                G(
                    120, for (i32 prev_index = 0; prev_index < move_index;
                              prev_index++) {
    ...
    // LATE MOVE PRUNING
    if (G(121, !in_check) && G(121, alpha == beta - 1) &&
        G(121, quiets_evaluated > 1 + depth * depth >> !improving)) {
      break;
    }
  }
...
#endif
    H(122, 1, Position *const restrict pos),
    H(122, 1, SearchStack *restrict stack),
    H(122, 1, const i32 pos_history_count)) {
  start_time = get_time();
#ifdef FULL
...
S(1) void display_pos(Position *const pos) {
  ...
  for (i32 rank = 7; rank >= 0; rank--) {
    for (i32 file = 0; file < 8; file++) {
      i32 sq = rank * 8 + file;
      u64 bb = 1ULL << sq;
      i32 piece = piece_on(H(31, 8, &npos), H(31, 8, sq));
      if (bb & npos.colour[0]) {
        if (piece == Pawn) {
          putl("P");
...
S(1) void bench() {
  ...
  iteratively_deepen(20, &nodes, H(122, 2, &pos), H(122, 2, stack),
                     H(122, 2, pos_history_count));
...
S(1) void run() {
  ...
  G(123, char line[4096];)
  G(123, Position pos;)
  G(123, i32 pos_history_count;)
  G(123, // #ifdef LOWSTACK
         //  SearchStack *stack = malloc(sizeof(SearchStack) * 1024);
         // #else
    SearchStack stack[1024];
    // #endif
  )
  G(123, __builtin_memset(move_history, 0, sizeof(move_history));)
  G(123, init();)
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      max_time = 99999999999;
      iteratively_deepen(max_ply, &nodes, H(122, 3, &pos), H(122, 3, stack),
                         H(122, 3, pos_history_count));
...
#endif
    G(124, if (line[0] == 'i') { puts("readyok"); })
    else G(124, if (line[0] == 'q') { exit_now(); }) else if (line[0] == 'p') {
      G(125, pos_history_count = 0;)
      G(125, pos = start_pos;)
      while (true) {
        ...
        for (i32 i = 0; i < num_moves; i++) {
          ...
          if (move_string_equal(G(126, line), G(126, move_name))) {
      ...
      iteratively_deepen(max_ply, &nodes, H(122, 4, &pos), H(122, 4, stack),
                         H(122, 4, pos_history_count));
#else
      for (i32 i = 0; i < (pos.flipped ? 4 : 2); i++) {
        getl(line);
        max_time = atoi(line) / 2;
      }
      iteratively_deepen(H(122, 5, &pos), H(122, 5, stack),
                         H(122, 5, pos_history_count));
#endif
    }
  }
}
