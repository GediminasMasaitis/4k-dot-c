  ...
  __attribute__((aligned(8))) S(1)
      const EvalParamsInitial initial_params =
          {.phases = {0, 0, 1, 1, 2, 4},
           .mg =
               {
                   .material = {0, 71, 289, 328, 380, 836},
                   .pst_rank =
                       {
                           0,   -15, -17, -5,  7,  29, 127, 0,   // Pawn
                           -19, -7,  6,   20,  30, 41, 19,  -90, // Knight
           ...
           .eg =
               {
                   .material = {0, 96, 399, 442, 736, 1366},
                   .pst_rank =
                       {
                           0,   -4,  -8,  -7, 0,  19, 95, 0,   // Pawn
                           -30, -18, -6,  17, 21, 6,  1,  10,  // Knight
                           -13, -11, 0,   4,  6,  2,  3,  9,   // Bishop
                           -17, -17, -11, 4,  10, 9,  17, 4,   // Rook
                           -58, -41, -13, 14, 29, 25, 30, 15,  // Queen
                   ...
                   .open_files = {8, 4, 17, 9, 25, 8, 15, -8, 1, 13, 43, 7},
                   .passed_pawns = {14, 2, 21, 49, 101, 95},
...
S(0) i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    // DOUBLED PAWNS
    score -= eval_params.doubled_pawn * count(pawns[1] & north(pawns[1]));
