...
S(1) i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    G(
        78, // BISHOP PAIR
        if (count(G(82, pos->pieces[Bishop]) & G(82, pos->colour[0])) > 1) {
...
i16 search(H(99, 1, const i32 ply), H(99, 1, i32 alpha), H(99, 1, i32 depth),
  ...
  if (G(107, !in_check) && G(107, alpha == beta - 1)) {
    ...
    if (G(110, do_null) && G(110, depth > 2) && G(110, static_eval >= beta)) {
      ...
      const i32 score = -search(
          ...
          nodes,
#endif
          H(100, 2, stack), H(100, 2, -alpha), H(100, 2, pos_history_count),
          H(100, 2, find_in_check(&npos)));
      if (score >= beta) {
        return score;
      }
    }
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    i32 reduction =
        G(119, depth > 1) && G(119, !gives_check) && G(119, moves_evaluated > 6)
    ...
    while (true) {
      score = -search(H(99, 3, ply + 1), H(99, 3, low),
                      ...
                      H(100, 3, pos_history_count), H(100, 3, gives_check));
    ...
    // LATE MOVE PRUNING
    if (G(125, !in_check) && G(125, !gives_check) &&
        G(125, alpha == beta - 1) &&
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    i32 score =
        search(H(99, 4, 0), H(99, 4, -inf), H(99, 4, depth), H(99, 4, pos),
               ...
               nodes,
#endif
               H(100, 4, stack), H(100, 4, inf), H(100, 4, pos_history_count),
               H(100, 4, find_in_check(pos)));
