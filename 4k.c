...
__attribute__((aligned(8))) static const i16 material[] = {91,  264, 289,
                                                           449, 864, 0};
__attribute__((aligned(8))) static const i8 pst_rank[] = {
    0,   -10, -11, -11, -2, 34, 105, 0,   // Pawn
    -29, -16, -1,  12,  23, 26, 8,   -22, // Knight
    -21, -5,  4,   8,   12, 13, 1,   -11, // Bishop
    -17, -21, -19, -7,  8,  17, 22,  17,  // Rook
    -21, -14, -9,  -2,  7,  16, 7,   15,  // Queen
    -17, -11, -5,  5,   16, 21, 11,  -14, // King
};
__attribute__((aligned(8))) static const i8 pst_file[] = {
    -4,  2,  -4, -2, 0,  2,  12, -7,  // Pawn
    -24, -7, 6,  13, 12, 11, 1,  -12, // Knight
    -11, -1, 2,  5,  5,  1,  4,  -6,  // Bishop
    -6,  1,  6,  7,  5,  2,  -1, -15, // Rook
    -19, -8, 2,  5,  4,  5,  5,  5,   // Queen
...
static i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    const u64 own_pawns = pos->colour[1] & pos->pieces[Pawn];
    score -= count(own_pawns << 8 & own_pawns) * 25;
