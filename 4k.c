...
S(1)
i16 search(H(99, 1, i32 alpha), H(99, 1, const i32 beta), H(99, 1, i32 depth),
           H(99, 1, const bool do_null), H(99, 1, SearchStack *restrict stack),
#ifdef FULL
           u64 *nodes,
#endif
           H(99, 1, Position *const restrict pos),
           H(99, 1, const i32 pos_history_count), H(99, 1, const i32 ply)) {
  assert(alpha < beta);
  assert(ply >= 0);
  ...
  if (G(106, !in_check) && G(106, alpha == beta - 1)) {
    ...
    if (G(109, depth > 2) && G(109, static_eval >= beta) && G(109, do_null)) {
      ...
      const i32 score = -search(
          ...
          nodes,
#endif
          H(99, 2, &npos), H(99, 2, pos_history_count), H(99, 2, ply + 1));
      if (score >= beta) {
        return score;
      }
    }
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    i32 score;
    while (true) {
      score = -search(H(99, 3, low), H(99, 3, -alpha),
                      H(99, 3, depth - G(120, 1) - G(120, reduction)),
                      H(99, 3, true), H(99, 3, stack),
#ifdef FULL
                      nodes,
#endif
                      H(99, 3, &npos), H(99, 3, pos_history_count),
                      H(99, 3, ply + 1));
...
void iteratively_deepen(
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    while (true) {
      ...
      score = search(H(99, 4, alpha), H(99, 4, beta), H(99, 4, depth),
                     H(99, 4, false), H(99, 4, stack),
#ifdef FULL
                     nodes,
#endif
                     H(99, 4, pos), H(99, 4, pos_history_count), H(99, 4, 0));
      elapsed = get_time() - start_time;
      G(
          998, if (G(999, (score > alpha && score < beta)) ||
                   G(999, elapsed > max_time)) { break; })
      G(998, window *= 2;)
    }
