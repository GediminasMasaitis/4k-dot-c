...
i32 search(
  ...
  if (G(203, !in_check) && G(203, G(204, alpha) == G(204, beta - 1))) {
    if (G(205, depth < 10) && G(205, !in_qsearch)) {
      ...
      G(206, {
        ...
        if (static_eval - G(207, 68) * G(207, (depth - improving)) >= beta) {
          return static_eval;
        }
      })
      ...
      G(206, // RAZORING
        in_qsearch =
            G(208, static_eval) + G(208, G(209, 126) * G(209, depth)) <= alpha;)
    }
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      const i32 order_move_score =
          ...
          G(187, // MOST VALUABLE VICTIM
            G(219, moves[order_index].takes_piece) * G(219, 556)) +
    ...
    G(
        ...
        if (G(223, moves_evaluated) &&
            G(223, move_score < G(224, -211) * G(224, depth))) { break; })
    ...
    i32 reduction = G(228, depth > 3) && G(228, move_score <= 0)
                        ? G(229, !improving) + G(229, (move_score / -387)) +
                              G(229, (G(230, alpha) == G(230, beta - 1))) +
                              G(229, moves_evaluated / 8)
                        : 0;
    ...
    if (score > best_score) {
      ...
      if (score > alpha) {
        ...
        if (score >= beta) {
          ...
          G(
              233, if (!in_qsearch) {
                const i32 bonus = 364 * depth * depth / 256;
                G(234, i32 *const this_hist =
                  ...
                  *this_hist +=
                  bonus - G(235, bonus) * G(235, *this_hist) / 978;)
                G(
                    234, for (i32 prev_index = 0; prev_index < move_index;
                      ...
                      i32 *const prev_hist =
                          &move_history[pos->flipped][prev.takes_piece]
                                       [prev.from][prev.to];
                      *prev_hist -=
                          bonus + G(236, bonus) * G(236, *prev_hist) / 978;
                    })
              })
          break;
        }
  ...
  G(
      ...
      if (G(243,
        ...
        G(246, i32 dd = depth * depth; if (dd > 87) { dd = 87; })
        G(246, i32 target = best_score - stack[ply].static_eval; G(
              247, if (target < -161) { target = -161; })
              G(247, if (target > 161) { target = 161; }))

        for (i32 i = 0; i < 4; i++) {
          *corr_entries[i] =
              (G(248, G(249, *corr_entries[i]) * G(249, (547 - dd))) +
...
void iteratively_deepen(
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    if (stop || elapsed > data->max_time / 15) {
      break;
    }
  }
}
