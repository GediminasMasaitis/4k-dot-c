      ...
      for (i32 piece = Knight; piece <= King; piece++) {
        ...
        while (copy) {
          ...
          G(98, u64 moves = G(99, to_mask) &
                            G(99, get_mobility(H(75, 2, piece), H(75, 2, from),
                                               H(75, 2, pos)));)
...
S(1) i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    G(132,
      const u64 opp_pawns = G(133, pos->pieces[Pawn]) & G(133, pos->colour[1]);
        ...
        const u64 no_passers = G(136, opp_pawns) | G(136, attacked_by_pawns);)
          G(135, // PROTECTED PAWNS
            ...
            eval_params.protected_pawn * count(opp_pawns & attacked_by_pawns);)
              G(135, // PHALANX PAWNS
                score -=
                eval_params.phalanx_pawn * count(opp_pawns & west(opp_pawns));))
    G(
        132, // BISHOP PAIR
...
i32 search(H(165, 1, i32 depth), H(165, 1, i32 alpha),
  ...
  if (G(179, !in_check) && G(179, G(180, alpha) == G(180, beta - 1))) {
    ...
    if (G(186, depth > 2) && G(186, static_eval >= beta) && G(186, do_null)) {
      ...
      const i32 score = -search(
          ...
          H(165, 2, false), H(165, 2, -alpha), H(165, 2, stack),
#ifdef FULL
          nodes,
#endif
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    i32 reduction = G(201, depth > 3) && G(201, move_score <= 0)
                        ? G(202, (move_score < -256)) +
                              G(202, (G(203, alpha) == G(203, beta - 1))) +
                              G(202, !improving) + G(202, moves_evaluated / 10)
                        : 0;
    // DO NOT ALLOW REDUCTION TO QSEARCH
    if (G(765, reduction) && G(765, reduction >= depth - 1)) {
      reduction = depth - 2;
    }
    ...
    while (true) {
      score = -search(
          ...
          H(165, 3, true), H(165, 3, -alpha), H(165, 3, stack),
#ifdef FULL
          nodes,
#endif
          H(166, 3, &npos), H(166, 3, pos_history_count), H(166, 3, ply + 1));
...
void iteratively_deepen(
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    while (true) {
      ...
      G(224, window *= 2;)
      G(
          224, if (G(225, (G(226, score < beta) && G(226, score > alpha))) ||
