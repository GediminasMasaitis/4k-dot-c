...
G(7, [[nodiscard]] S(1) T(1, i32) lsb(u64 bb) { return __builtin_ctzll(bb); })
...
G(
    7, [[nodiscard]] S(1) T(2, i32)
           count(const u64 bb) { return __builtin_popcountll(bb); })
...
G(
    46, [[nodiscard]] S(0) T(4, i32)
            piece_on(H(55, 1, const Position *const restrict pos),
                     H(55, 1, const i32 sq)) {
              assert(sq >= 0);
              assert(sq < 64);
              for (T(5, i32) i = Pawn; i <= King; ++i) {
...
G(
    57,
    [[nodiscard]] S(0) T(3, i32) is_attacked(
        H(58, 1, const Position *const restrict pos), H(58, 1, const u64 bb)) {
      assert(count(bb) == 1);
...
S(0) T(3, i32) find_in_check(const Position *restrict pos) {
  return is_attacked(
      H(58, 2, pos),
...
G(
    ...
    S(1) Move *generate_piece_moves(H(75, 1, const u64 to_mask),
      ...
      for (T(8, u8) piece = Knight; piece <= King; piece++) {
...
G(
    74, S(0) T(7, i32) makemove(H(80, 1, Position *const restrict pos),
      ...
      G(82,
        const T(6, u32) piece = piece_on(H(55, 4, pos), H(55, 4, move->from));
        assert(piece != None);)
...
G(
    74, S(0) Move *generate_pawn_moves(H(93, 1, const T(32, i32) offset),
                                       H(93, 1, u64 to_mask),
...
[[nodiscard]] S(1) T(9, i32)
    movegen(H(95, 1, const Position *const restrict pos),
            H(95, 1, const T(31, i64) only_captures),
  ...
  const T(9, i32) num_moves = movelist - start;
...
[[nodiscard]] S(1) u64
    perft(const Position *const restrict pos, const T(21, i32) depth) {
  if (depth == 0) {
    return 1;
  }
  ...
  const T(9, i32) num_moves =
      movegen(H(95, 2, pos), H(95, 2, false), H(95, 2, moves));

  for (T(10, u32) i = 0; i < num_moves; ++i) {
    Position npos = *pos;
...
S(0) T(14, i32) eval(Position *const restrict pos) {
  ...
  for (T(17, i16) c = 0; c < 2; c++) {
    ...
    for (T(8, u8) p = Pawn; p <= King; p++) {
      u64 copy = G(136, pos->colour[0]) & G(136, pos->pieces[p]);
      while (copy) {
        const T(16, u64) sq = lsb(copy);
        G(137, const T(18, i32) rank = sq >> 3;)
        G(137, const u64 piece_bb = 1ULL << sq;)
        G(137, phase += initial_params.phases[p];)
        G(137, const T(18, i32) file = G(138, sq) & G(138, 7);)
        ...
        G(
            ...
            if (G(143, p == Pawn) &&
              ...
              for (T(17, i16) i = 0; i < 2; i++) {
                const T(16, u64) king_sq =
                    lsb(G(148, pos->colour[i]) & G(148, pos->pieces[King]));
                G(149, const T(19, i32) rank_distance = __builtin_abs(
                           king_sq / 8 - G(150, rank) - G(150, 1));)
                G(149, const T(19, i32) file_distance =
                           __builtin_abs(king_sq % 8 - file);)
                score +=
  ...
  const T(20, i32) stronger_side_pawns_missing =
...
typedef struct [[nodiscard]] {
  ...
  G(119, Move killer;)
  G(119, T(9, i32) num_moves;)
  G(119, T(13, i32) static_eval;)
} SearchStack;
...
search(
  ...
  T(25, i32)(*const move_history)[6][64][64] = data->move_history;
  ...
  if (G(203, !in_check) && G(203, G(204, alpha) == G(204, beta - 1))) {
    if (G(205, depth < 9) && G(205, !in_qsearch)) {
      ...
      G(206, {
        ...
        if (static_eval - G(207, 57) * G(207, (depth - improving)) >= beta) {
          return static_eval;
        }
      })
  ...
  G(213, T(12, i32) best_score = in_qsearch ? static_eval : -inf;)
  G(213, stack[G(214, ply) + G(214, 2)].position_hash = tt_hash;)
  G(213, T(23, u8) quiets_evaluated = 0;)
  G(213, T(23, u8) moves_evaluated = 0;)
  G(213, u8 tt_flag = Upper;)

  for (T(10, u32) move_index = 0; move_index < stack[ply].num_moves;
       move_index++) {
    // MOVE ORDERING
    ...
    G(
        ...
        if (G(223, moves_evaluated) &&
            G(223, move_score < G(224, -125) * G(224, depth))) { break; })

    G(
        222, // FORWARD FUTILITY PRUNING / DELTA PRUNING
        if (G(225, depth < 8) &&
            G(225,
              G(226, static_eval) + G(226, G(227, 154) * G(227, depth)) +
                      G(226,
    ...
    T(12, i32) low = moves_evaluated == 0 ? -beta : -alpha - 1;
    moves_evaluated++;
        ...
        if (score >= beta) {
          ...
          G(
              233, if (!in_qsearch) {
                ...
                G(
                    234, for (T(10, u32) prev_index = 0;
                              prev_index < move_index; prev_index++) {
                      const Move prev = moves[prev_index];
                      T(25, i32) *const prev_hist =
...
void iteratively_deepen(
  ...
  T(12, i32) score = 0;
#ifdef FULL
  for (T(21, i32) depth = 1; depth < maxdepth; depth++) {
#else
  for (T(21, i32) depth = 1; depth < max_ply; depth++) {
#endif
    // ASPIRATION WINDOWS
    G(254, T(12, i32) window = 16;)
    G(254, size_t elapsed;)
    while (true) {
      G(255, const T(12, i32) alpha = score - window;)
      G(255, const T(12, i32) beta = G(256, score) + G(256, window);)
      score = search(
#ifdef FULL
          &data->nodes,
#endif
...
void run_smp() {
  ...
  for (T(29, i32) i = 0; i < thread_count - 1; i++) {
  ...
  for (T(29, i32) i = 1; i < thread_count; i++) {
...
S(1) void run() {
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      char depth_str[4];
      getl(depth_str);
      const T(21, i32) depth = atoi(depth_str);
      const u64 start = get_time();
...
#endif
#else
      for (T(17, i16) i = 2 << main_data->pos.flipped; i > 0; i--) {
        getl(line);
        main_data->max_time = (u64)atoi(line) << 19; // Roughly /2 time
      }
      start_time = get_time();
      ...
      bg_stop();
        ...
        if (!strcmp(line, "fen")) {
          ...
          line_continue = get_fen(&main_data->pos, line);
            ...
            {
              Move moves[max_moves];
              const T(9, i32) num_moves = movegen(
                  H(95, 4, &main_data->pos), H(95, 4, false), H(95, 4, moves));
              for (T(10, u32) i = 0; i < num_moves; i++) {
                char move_name[8];
...
#endif
      else G(262, if (G(268, line[0]) == G(268, 'i')) {
              puts("readyok");
