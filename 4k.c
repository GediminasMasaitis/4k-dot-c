...
G(1, [[nodiscard]] S(1) i32 strlen(const char* const restrict string) {
  i32 length = 0;
  while (string[length]) {
    length++;
  }
...
G(10, [[nodiscard]] S(1)
  ...
  G(13, [[nodiscard]] S(1) u64 nw(const u64 bb) {
...
G(14, [[nodiscard]] S(0) u64 xattack(H(16, 1, const u64 dir_mask),
  H(16, 1, const u64 bb),
...
G(22, S(1) void swapu32(G(23, u32* const rhs), G(23, u32* const lhs)) {
  const u32 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
}
...
G(22, [[nodiscard]] S(1) bool move_equal(G(26, Move* const rhs),
  G(26, Move* const lhs)) {
  return *(u32*)lhs == *(u32*)rhs;
})

G(22, S(1) void swapu64(G(27, u64* const rhs), G(27, u64* const lhs)) {
  const u64 temp = *lhs;
  *lhs = *rhs;
  *rhs = temp;
})

G(22, S(1) void move_str(H(28, 1, char* restrict str),
...
G(22, S(1) void swapbool(G(28, bool* const restrict rhs),
...
G(32, S(1) void flip_pos(Position* const restrict pos) {
  G(37, for (i32 i = 0; i < 2; i++) {
    swapbool(G(38, &pos->castling[i + 2]), G(38, &pos->castling[i]));
  })

    G(37, pos->flipped ^= 1;)
    G(37, swapu64(G(39, &pos->colour[0]), G(39, &pos->colour[1]));)
    G(37, // Hack to flip the first 10 bitboards in Position.
      // Technically UB but works in GCC 14.2
      u64 * pos_ptr = (u64*)pos;
  for (i32 i = 0; i < 10; i++) { pos_ptr[i] = flip_bb(pos_ptr[i]); })
})

G(32, [[nodiscard]] S(1) u64 get_mobility(H(40, 1, const Position* pos),
  H(40, 1, const i32 sq),
  ...
  G(41, if (piece == King) { moves = king(bb); })
    ...
    G(42, if (G(43, piece == Queen) || G(43, piece == Bishop)) {
      moves |= bishop(H(18, 3, blockers), H(18, 3, bb));
    })
      G(42, if (G(44, piece == Queen) || G(44, piece == Rook)) {
...
G(45, S(0) i32 makemove(H(46, 1, Position* const restrict pos),
...
G(45, S(1) Move* generate_piece_moves(H(58, 1, const u64 to_mask),
...
G(45, S(0) Move* generate_pawn_moves(H(62, 1, const Position* const pos),
  ...
  G(64, const u64 all = pos->colour[0] | pos->colour[1];)
    ...
    if (!only_captures) {
      movelist = generate_pawn_moves(
        ...
        H(62, 2, north(north(pos->colour[0] & pos->pieces[Pawn] & 0xFF00) & ~all) &
          ~all),
        H(62, 2, -16));
    }
  movelist = generate_pawn_moves(
    H(62, 3, pos), H(62, 3, movelist),
    H(62, 3, north(pos->colour[0] & pos->pieces[Pawn]) & ~all &
      (only_captures ? 0xFF00000000000000ull : ~0ull)),
    H(62, 3, -8));
  movelist = generate_pawn_moves(
    H(62, 4, pos), H(62, 4, movelist),
    H(62, 4, nw(pos->colour[0] & pos->pieces[Pawn]) & (pos->colour[1] | pos->ep)),
    H(62, 4, -7));
  movelist = generate_pawn_moves(
    H(62, 5, pos), H(62, 5, movelist),
    H(62, 5, ne(pos->colour[0] & pos->pieces[Pawn]) & (pos->colour[1] | pos->ep)),
    H(62, 5, -9));
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i16 material[6];
  H(69, 1, H(70, 1, i8 mobilities[5];) H(70, 1, i8 tempo;)
    H(70, 1, i8 passed_blocked_pawns[6];) H(70, 1, i8 passed_pawns[6];)
    H(70, 1, i8 king_attacks[5];))
    H(69, 1, H(71, 1, i8 bishop_pair;) H(71, 1, u8 pawn_attacked_penalty[2];)
      H(71, 1, i8 open_files[2][6];) H(71, 1, i8 pst_file[64];)
      H(71, 1, i8 pst_rank[64];))
} EvalParams;

typedef struct [[nodiscard]] __attribute__((packed)) {
  i32 material[6];
  H(69, 2, H(70, 2, i32 mobilities[5];) H(70, 2, i32 tempo;)
    H(70, 2, i32 passed_blocked_pawns[6];) H(70, 2, i32 passed_pawns[6];)
    H(70, 2, i32 king_attacks[5];))
    H(69, 2, H(71, 2, i32 bishop_pair;) H(71, 2, i32 pawn_attacked_penalty[2];)
      H(71, 2, i32 open_files[2][6];) H(71, 2, i32 pst_file[64];)
      H(71, 2, i32 pst_rank[64];))

} EvalParamsMerged;

G(72, [[nodiscard]] S(1) i32 combine_eval_param(H(73, 1, const i32 mg_val),
...
G(72, S(1) const EvalParams eg = ((EvalParams) {
  .material = { 71, 303, 299, 545, 988, 0 },
    .pst_rank =
  {
      0,   -4,  -6,  -5, 0,  14, 76, 0,   // Pawn
      -32, -19, -5,  17, 23, 9,  3,  5,   // Knight
      -11, -11, -3,  1,  5,  4,  5,  11,  // Bishop
      -17, -18, -12, 4,  11, 10, 16, 7,   // Rook
      -57, -43, -17, 13, 30, 28, 33, 13,  // Queen
      -37, -1,  9,   19, 25, 21, 8,  -44, // King
  },
  .pst_file =
  {
      8,   11, -1, -9, -4, -2, 2,  -6,  // Pawn
      -17, -3, 9,  15, 15, 4,  -2, -19, // Knight
      -6,  -1, 1,  5,  5,  3,  0,  -7,  // Bishop
      1,   4,  4,  -2, -5, 1,  0,  -4,  // Rook
      -19, -5, 2,  6,  11, 12, 0,  -6,  // Queen
      -24, 0,  13, 25, 19, 19, 0,  -32, // King
  },
  .mobilities = { 2, 4, 3, 1, -3 },
  .king_attacks = { 0, -3, -6, 6, 0 },
  .open_files =
  {
      {22, 2, 12, 6, 23, 13},
      {29, -8, 2, 10, 40, 8},
  },
  .passed_pawns = { 0, 4, 24, 46, 83, 76 },
  .pawn_attacked_penalty = { -10, -128 },
  .tempo = 7
});)

G(72, S(1) const EvalParams mg = ((EvalParams) {
  .material = { 73, 296, 300, 403, 908, 0 },
    .pst_rank =
  {
      0,   -10, -10, -8,  3,  25, 93,  0,   // Pawn
      -24, -13, 0,   13,  23, 44, 28,  -71, // Knight
      -10, 5,   12,  12,  15, 17, 0,   -51, // Bishop
      1,   -10, -16, -18, 3,  19, 9,   12,  // Rook
      13,  15,  7,   -4,  -7, -1, -18, -4,  // Queen
      -12, -9,  -34, -35, 7,  71, 85,  90,  // King
  },
  .pst_file =
  {
      -21, -10, -10, -1,  6,   21,  24, -10, // Pawn
      -25, -10, 1,   14,  12,  10,  3,  -5,  // Knight
      -10, 3,   5,   3,   6,   -4,  3,  -5,  // Bishop
      -8,  -7,  0,   9,   12,  4,   -1, -10, // Rook
      -10, -7,  -1,  2,   3,   0,   7,  6,   // Queen
      -17, 25,  -2,  -44, -17, -38, 19, -1,  // King
  },
  .mobilities = { 7, 6, 2, 3, -9 },
  .king_attacks = { 0, 15, 19, 15, 0 },
  .open_files =
  {
      {22, -9, -9, 18, -1, -26},
      {24, -14, -14, 33, -14, -59},
  },
  .passed_pawns = { -15, -19, -11, 10, 33, 93 },
  .passed_blocked_pawns = { -1, -6, 1, 11, 11, -35 },
  .bishop_pair = 24,
...
S(1) void init() {
  ...
  G(46, for (i32 sq = 0; sq < 64; sq++) {
    ...
    G(46, // MERGE NON-MATERIAL VALUES
...
G(76, __attribute__((aligned(8))) S(1)
  const i16 max_material[] = { 0, 73, 303, 300, 545, 988 };)
  G(76, __attribute__((aligned(8))) S(1) const i8 phases[] = { 0, 0, 1, 1, 2, 4, 0 };)
  ...
  G(77, i32 score = eval_params.tempo;)
    ...
    for (i32 c = 0; c < 2; c++) {
      ...
      G(78, const u64 own_pawns = G(79, pos->pieces[Pawn]) & G(79, pos->colour[0]);)

        G(78, const u64 opp_king_zone = king(pos->colour[1] & pos->pieces[King]);)
        G(78, const u64 opp_pawns = G(80, pos->colour[1]) & G(80, pos->pieces[Pawn]);
      ...
      const u64 no_passers = G(82, opp_pawns) | G(82, attacked_by_pawns);)
        G(78, // BISHOP PAIR
          if (count(G(83, pos->pieces[Bishop]) & G(83, pos->colour[0])) > 1) {
        ...
        for (i32 p = Pawn; p <= King; p++) {
          ...
          while (copy) {
            ...
            G(85, const int file = sq & 7;)
              ...
              G(62, // OPEN FILES / DOUBLED PAWNS
                const u64 ahead = north(0x101010101010101ULL << sq);
            if ((ahead & own_pawns) == 0) {
              score += eval_params.open_files[(ahead & opp_pawns) == 0][p - 1];
            })
              ...
              G(62, if (p > Pawn) {
              G(86, // PIECES ATTACKED BY PAWNS
                if (1ULL << sq & no_passers) {
                  score += eval_params.pawn_attacked_penalty[c];
                })

                G(86, const u64 mobility =
                  get_mobility(H(40, 3, pos), H(40, 3, sq), H(40, 3, p));

              G(87, // MOBILITY
                score +=
                G(88, count(G(89, mobility) & G(89, ~attacked_by_pawns) &
                  G(89, ~pos->colour[0]))) *
                G(88, eval_params.mobilities[p - 2]);)
              ...
              G(62, // PASSED PAWNS
                if (G(92, !(G(93, (0x101010101010101ULL << sq)) & G(93, no_passers))) &&
                  G(92, p == Pawn)) {
                  G(94, if (north(1ULL << sq) & pos->colour[1]) {
...
#error "Unsupported architecture: get_hash only for x86_64 and aarch64"
  ...
  i16 search(H(97, 1, Position* const restrict pos), H(97, 1, i32 alpha),
    ...
    u64* nodes,
#endif
    H(98, 1, const i32 pos_history_count), H(98, 1, const i32 beta),
  ...
  if (G(103, tt_entry->partial_hash == tt_hash_partial) &&
    G(103, tt_entry->flag != static_eval > tt_entry->score)) {
    ...
    if (G(108, depth > 2) && G(108, static_eval >= beta) && G(108, do_null)) {
      ...
      G(109, flip_pos(&npos);)
        ...
        const i32 score = -search(
          ...
          H(97, 2, depth - 3 - depth / 4), H(98, 2, false),
#ifdef FULL
          nodes,
#endif
          H(98, 2, pos_history_count), H(98, 2, -alpha), H(98, 2, stack));
      if (score >= beta) {
        return score;
      }
    }
  ...
  G(97, i32 quiets_evaluated = 0;)
    ...
    for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
      ...
      swapmoves(G(114, &stack[ply].moves[best_index]),
        G(114, &stack[ply].moves[move_index]));

      // FORWARD FUTILITY PRUNING / DELTA PRUNING
      if (G(115, depth < 8) &&
        G(115, G(116, static_eval + 136 * depth) +
          G(116, max_material[stack[ply].moves[move_index].promo]) +
          G(116, max_material[stack[ply].moves[move_index].takes_piece]) <
          alpha) &&
        G(115, !in_check) && G(115, moves_evaluated)) {
        break;
      }
      ...
      while (true) {
        score = -search(
          ...
          H(97, 3, depth - G(119, 1) - G(119, reduction)), H(98, 3, true),
#ifdef FULL
          nodes,
#endif
          H(98, 3, pos_history_count), H(98, 3, -alpha), H(98, 3, stack));
        ...
        if (score > alpha) {
          G(120, tt_flag = Exact;)
            ...
            if (score >= beta) {
              ...
              G(121, tt_flag = Lower;)
                G(121, if (stack[ply].best_move.takes_piece == None) {
                ...
                *this_hist += bonus - bonus * *this_hist / 1024;)
                  G(122, for (i32 prev_index = 0; prev_index < move_index;
                    prev_index++) {
...
#endif
  H(124, 1, Position* const restrict pos),
  H(124, 1, SearchStack* restrict stack),
  H(124, 1, const i32 pos_history_count)) {
  start_time = get_time();
#ifdef FULL
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    i32 score =
      search(H(97, 4, pos), H(97, 4, -inf), H(97, 4, 0), H(97, 4, depth),
        ...
        nodes,
#endif
        H(98, 4, pos_history_count), H(98, 4, inf), H(98, 4, stack));
...
S(1) void bench() {
  ...
  iteratively_deepen(20, &nodes, H(124, 2, &pos), H(124, 2, stack),
    H(124, 2, pos_history_count));
...
S(1) void run() {
  ...
  G(125, char line[4096];)
    G(125, Position pos;)
    G(125, i32 pos_history_count;)
    G(125, __builtin_memset(move_history, 0, sizeof(move_history));)
    G(125, // #ifdef LOWSTACK
      //  SearchStack *stack = malloc(sizeof(SearchStack) * 1024);
      // #else
      SearchStack stack[1024];
  // #endif
  ...
  while (true) {
    ...
    if (!strcmp(line, "uci")) {
      ...
      max_time = 99999999999;
      iteratively_deepen(max_ply, &nodes, H(124, 3, &pos), H(124, 3, stack),
        H(124, 3, pos_history_count));
    }
...
#endif
    G(126, if (line[0] == 'i') { puts("readyok"); })
    else G(126, if (line[0] == 'q') { exit_now(); }) else if (line[0] == 'p') {
      G(127, pos_history_count = 0;)
        G(127, pos = start_pos;)
        while (true) {
          ...
          for (i32 i = 0; i < num_moves; i++) {
            ...
            if (move_string_equal(G(128, line), G(128, move_name))) {
      ...
      iteratively_deepen(max_ply, &nodes, H(124, 4, &pos), H(124, 4, stack),
        H(124, 4, pos_history_count));
#else
      for (i32 i = 0; i < (pos.flipped ? 4 : 2); i++) {
        getl(line);
        max_time = atoi(line) / 2;
      }
      iteratively_deepen(H(124, 5, &pos), H(124, 5, stack),
        H(124, 5, pos_history_count));
#endif
    }
  }
}
