...
G(
    22, static void move_str(H(23, 1, const i32 flip),
                             H(23, 1, const Move *restrict move),
                             H(23, 1, char *restrict str)) {
      assert(move->from >= 0);
      assert(move->from < 64);
      assert(move->to >= 0);
      assert(move->to < 64);
...
G(
    22,
    static void swapu32(G(27, u32 *const lhs), G(27, u32 *const rhs)) {
      const u32 temp = *lhs;
      *lhs = *rhs;
      *rhs = temp;
    }
...
G(
    22, static void swapu64(G(31, u64 *const lhs), G(31, u64 *const rhs)) {
      const u64 temp = *lhs;
      *lhs = *rhs;
      *rhs = temp;
    })
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i16 material[6];
  H(63, 1,
    H(64, 1, i8 mobilities[4];) H(64, 1, i8 tempo;)
        H(64, 1, i8 king_attacks[4];) H(64, 1, i8 passed_blocked_pawns[4];)
            H(64, 1, i8 passed_pawns[4];))
  H(63, 1,
    H(65, 1, i8 bishop_pair;) H(65, 1, i8 open_files[2][6];)
...
typedef struct [[nodiscard]] __attribute__((packed)) {
  i32 material[6];
  H(63, 2,
    H(64, 2, i32 mobilities[4];) H(64, 2, i32 tempo;)
        H(64, 2, i32 king_attacks[4];) H(64, 2, i32 passed_blocked_pawns[4];)
            H(64, 2, i32 passed_pawns[4];))
  H(63, 2,
    H(65, 2, i32 bishop_pair;) H(65, 2, i32 open_files[2][6];)
...
G(66, static const EvalParams eg = ((EvalParams){
          .material = {71, 313, 301, 546, 992, 0},
          .pst_rank =
              {
                  0,   -4,  -6,  -5, 0,  14, 76, 0,   // Pawn
                  -38, -19, -2,  20, 25, 11, 2,  1,   // Knight
                  -12, -11, -3,  1,  6,  4,  5,  10,  // Bishop
                  -17, -18, -12, 4,  11, 10, 15, 7,   // Rook
                  -57, -42, -17, 13, 30, 28, 32, 12,  // Queen
                  -37, -1,  9,   19, 25, 21, 8,  -45, // King
              },
          .pst_file =
              {
                  8,   11, -1, -9, -4, -2, 2,  -6,  // Pawn
                  -22, -4, 12, 19, 18, 6,  -4, -24, // Knight
                  -6,  -1, 1,  5,  5,  3,  0,  -7,  // Bishop
                  1,   4,  4,  -1, -4, 0,  0,  -4,  // Rook
                  -19, -5, 1,  7,  12, 12, -1, -7,  // Queen
                  -24, 1,  13, 25, 19, 19, 0,  -32, // King
              },
          .mobilities = {4, 3, 1, -4},
          ...
          .passed_pawns = {24, 46, 83, 76},
          .passed_blocked_pawns = {27, 52, 90, 97},
          .bishop_pair = 53,
          .tempo = 8});)
...
G(66, static const EvalParams mg = ((EvalParams){
          .material = {70, 319, 300, 401, 902, 0},
          .pst_rank =
              {
                  0,   -11, -9,  -8,  3,  26, 94,  0,   // Pawn
                  -33, -19, 1,   13,  30, 56, 28,  -75, // Knight
                  -10, 4,   12,  12,  15, 17, 1,   -50, // Bishop
                  1,   -11, -16, -19, 2,  19, 10,  13,  // Rook
                  13,  14,  6,   -5,  -7, -1, -17, -3,  // Queen
                  -11, -8,  -33, -34, 8,  72, 87,  92,  // King
              },
          .pst_file =
              {
                  -23, -11, -9, 0,   6,   22,  25, -10, // Pawn
                  -31, -9,  6,  17,  14,  14,  2,  -13, // Knight
                  -11, 3,   6,  3,   5,   -3,  3,  -6,  // Bishop
                  -7,  -7,  0,  9,   11,  4,   -1, -8,  // Rook
                  -10, -7,  -1, 2,   2,   -1,  7,  7,   // Queen
                  -15, 25,  0,  -43, -16, -37, 19, 1,   // King
              },
          .mobilities = {6, 2, 3, -8},
          ...
          .passed_blocked_pawns = {1, -10, -10, 34},
          .bishop_pair = 25,
          .tempo = 16});)
...
static i32 eval(Position *const restrict pos) {
  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    G(72,
      const u64 own_pawns = G(73, pos->pieces[Pawn]) & G(73, pos->colour[0]);)

    G(
        72, // BISHOP PAIR
        if (count(G(74, pos->pieces[Bishop]) & G(74, pos->colour[0])) > 1) {
      ...
      while (copy) {
        ...
        G(
            40, if (p > Knight) {
              ...
              G(84, // MOBILITY
                score +=
                G(85, count(G(86, ~attacked_by_pawns) & G(86, mobility) &
                            G(86, ~pos->colour[0]))) *
                G(85, eval_params.mobilities[p - 3]);)

              G(84, // KING ATTACKS
                score += G(87, count(G(88, opp_king_zone) & G(88, mobility))) *
                         G(87, eval_params.king_attacks[p - 3]);)
            })
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    for (i32 order_index = move_index; order_index < stack[ply].num_moves;
      ...
      const i32 order_move_score =
          G(92, // KILLER MOVE
            move_equal(G(103, &stack[ply].killer),
    ...
    if (G(107, !in_check) &&
        G(107,
          G(108, max_material[stack[ply].moves[move_index].promo]) +
              ...
              alpha) &&
        G(107, moves_evaluated) && G(107, depth < 8)) {
      break;
    }
...
static void iteratively_deepen(
  ...
  for (i32 depth = 1; depth < max_ply; depth++) {
    ...
    putl(" pv ");
    char move_name[8];
    move_str(H(23, 2, pos->flipped), H(23, 2, &stack[0].best_move),
             H(23, 2, move_name));
    puts(move_name);
#endif
  ...
  char move_name[8];
  move_str(H(23, 3, pos->flipped), H(23, 3, &stack[0].best_move),
           H(23, 3, move_name));
  putl("bestmove ");
  puts(move_name);
}
  ...
  while (true) {
    ...
    G(117, if (line[0] == 'i') { puts("readyok"); })
      ...
      while (true) {
        ...
        for (i32 i = 0; i < num_moves; i++) {
          char move_name[8];
          move_str(H(23, 4, pos.flipped), H(23, 4, &stack[0].moves[i]),
                   H(23, 4, move_name));
