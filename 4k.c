  ...
  for (i32 c = 0; c < 2; c++) {
    ...
    for (i32 p = Pawn; p <= King; p++) {
      ...
      while (copy) {
        ...
        G(16, if ((north(0x101010101010101ULL << sq) & own_pawns) == 0) {
  ...
  for (i32 move_index = 0; move_index < stack[ply].num_moves; move_index++) {
    ...
    if (score > alpha) {
      ...
      if (score >= beta) {
        ...
        G(34, if (stack[ply].best_move.takes_piece == None) {
              stack[ply].killer = stack[ply].best_move;
            })
        G(34, i32 *const this_hist =
