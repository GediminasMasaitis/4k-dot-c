...
.got_bit:
        ret
.init_get_bit:
        pop     rbp             ; load offset of .get_bit, to be used with call ebp
        add     rbp, .get_bit - .get_dibits
.literal:
        movsb                   ; read and write literal byte
.next_command_after_literal:
        push    03H
        pop     rbx             ; set follows_literal(bx) to 3
        
.next_command:
        call    rbp             ; read 'literal or match' bit
        jnc     .literal        ; if 0: literal
                                
                                ; 1x: match
        call    rbp             ; read '8+n bits or other type' bit
...
.got_len:
        push    rsi             ; save esi (current pointer to compressed data)
...
.other:
        xor     ecx,ecx
        call    rbp             ; read '7+1 match or short literal' bit
