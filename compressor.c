...
static void comb_init(Combinable* comb) {
...
static inline int AritSize2(int right_prob, int wrong_prob) {
    ...
    return LogTable[total_prob >> total_shift] - LogTable[right_prob >> right_shift]
...
static void AritCode(AritState* state, unsigned int zero_prob, unsigned int one_prob, int bit) {
...
static int CSVisit(CounterState* states, int* num_states, int max_states,
                   unsigned char count0, unsigned char count1, int new_bit, int saturate) {
    if (new_bit == 0) {
    ...
    states[state_idx].next_state[0] = CSVisit(states, num_states, max_states, count0, count1, 0, saturate);
    states[state_idx].next_state[1] = CSVisit(states, num_states, max_states, count0, count1, 1, saturate);
    return state_idx;
}

static void GenCounterStates(CounterState* states, int max_states, int saturate) {
    ...
    states[0].next_state[0] = CSVisit(states, &num_states, max_states, 1, 0, 0, saturate);
    states[0].next_state[1] = CSVisit(states, &num_states, max_states, 1, 0, 1, saturate);
    states[1].next_state[0] = CSVisit(states, &num_states, max_states, 0, 1, 0, saturate);
    states[1].next_state[1] = CSVisit(states, &num_states, max_states, 0, 1, 1, saturate);
    assert(num_states == max_states);
}
...
static unsigned int ModelHash(const unsigned char* data, int bit_pos, unsigned int mask, int hash_mul) {
    unsigned char data_len = (unsigned char)mask;
...
static void ML_Print(const ModelList4k* ml, FILE* file) {
    for (int m = 0; m < ml->nmodels; m++) {
        fprintf(file, "%s%02X:%d", m ? " " : "", ml->models[m].mask, ml->models[m].weight);
    }
    fprintf(file, "\n");
}
...
static const char* CTName(int compression_type) {
    ...
    case CT_INSTANT:  return "INSTANT";
    case CT_FAST:     return "FAST";
    case CT_SLOW:     return "SLOW";
    case CT_VERYSLOW: return "VERYSLOW";
    }
    return "UNKNOWN";
}
...
static void CSEval_init(CSEval* eval) {
...
static void ChangeWeight_job(int job_idx, void* vctx) {
    ...
    for (int i = 0; i < ctx->packages_per_job && pkg_base + i < ctx->num_packages; i++) {
...
#define DO(I) { \
    ...
    sum_right = _mm_add_ps(sum_right, _mm_mul_ps(_mm_castsi128_ps(_mm_unpacklo_epi16(vec_zero, packed)), vec_delta_weight)); \
    sum_total = _mm_add_ps(sum_total, _mm_mul_ps(_mm_castsi128_ps(_mm_unpackhi_epi16(vec_zero, packed)), vec_delta_weight)); \
...
#undef DO
        ...
        __m128i right_exponent = _mm_srli_epi32(_mm_castps_si128(prod_right), 23);
        __m128i total_exponent = _mm_srli_epi32(_mm_castps_si128(prod_total), 23);
        __m128 right_log = _mm_castsi128_ps(_mm_or_si128(
            _mm_and_si128(_mm_castps_si128(prod_right), mantissa_mask),
            _mm_castps_si128(vec_one)));
        __m128 total_log = _mm_castsi128_ps(_mm_or_si128(
            _mm_and_si128(_mm_castps_si128(prod_total), mantissa_mask),
            _mm_castps_si128(vec_one)));

        right_log = _mm_sub_ps(right_log, vec_one);
        right_log = _mm_mul_ps(
            _mm_add_ps(_mm_mul_ps(_mm_add_ps(
                _mm_mul_ps(log2_coeff2, right_log), log2_coeff1), right_log), log2_coeff0),
            right_log);
        total_log = _mm_sub_ps(total_log, vec_one);
        total_log = _mm_mul_ps(
            _mm_add_ps(_mm_mul_ps(_mm_add_ps(
                _mm_mul_ps(log2_coeff2, total_log), log2_coeff1), total_log), log2_coeff0),
            total_log);

        __m128i frac_part = _mm_cvtps_epi32(
            _mm_mul_ps(_mm_sub_ps(total_log, right_log), vec_precision));
        __m128i new_size = _mm_add_epi32(
            _mm_slli_epi32(_mm_sub_epi32(total_exponent, right_exponent), TABLE_BIT_PRECISION_BITS),
            frac_part);
        new_size = _mm_add_epi32(new_size, _mm_shuffle_epi32(new_size, _MM_SHUFFLE(1, 0, 3, 2)));
        new_size = _mm_add_epi32(new_size, _mm_shuffle_epi32(new_size, _MM_SHUFFLE(2, 3, 0, 1)));
...
static int64_t CSEval_ChangeWeight(CSEval* eval, int model_idx, int delta_weight) {
...
static int64_t CSEval_Evaluate(CSEval* eval, const ModelList4k* ml) {
    ...
    for (int i = 0; i < MAX_MODELS_N; i++) {
        if (new_weights[i] != eval->weights[i]) {
            int64_t delta = CSEval_ChangeWeight(eval, i, new_weights[i] - eval->weights[i]);
            if (eval->weights[i] == 0) {
...
static HashEntry* FindEntry(HashEntry* table, unsigned table_size, unsigned char mask,
                            const unsigned char* data, int bit_pos) {
    ...
    for (unsigned hash = ModelHash(data, bit_pos, mask, HMUL);; hash++) {
        ...
        if (entry->mask == mask && entry->bitnum == bit_num
            && (data_ptr[0] & (0xFF00 >> bit_num)) == (entry->datapos[0] & (0xFF00 >> bit_num))) {
            int match = 1;
            for (int i = 0; i < 8; i++) {
...
static ModelPredictions CS_ApplyModel(const unsigned char* data, int bit_length,
    ...
    int num_packages = 0;
    CompactPackage* packages = (CompactPackage*)aligned_alloc16(max_packages * sizeof(CompactPackage));
    ...
    for (int pkg_idx = 0; pkg_idx < max_packages; pkg_idx++) {
        ...
        for (int bit_offset = 0; bit_offset < PKG_SIZE; bit_offset++) {
            ...
            if (bit_base + bit_offset < bit_length) {
                ...
                HashEntry* entry = FindEntry(hash_table, table_size, mask, data, bit_base + bit_offset);
                int boost = (entry->weights.prob[0] == 0 || entry->weights.prob[1] == 0) ? 2 : 0;
                if (entry->weights.prob[0] || entry->weights.prob[1]) {
                    needs_commit = 1;
                }
                right_prob = (float)(entry->weights.prob[bit] << boost);
                total_prob = (float)((entry->weights.prob[0] + entry->weights.prob[1]) << boost);
                UpdateWeights(&entry->weights, bit, saturate);
            }
            uint16_t* packed = (uint16_t*)&packages[num_packages].prob[bit_offset >> 2];
...
static void ApplyModelJob(int mask, void* vctx) {
    ...
    ctx->models[mask] = CS_ApplyModel(ctx->data, ctx->bit_length, (unsigned char)mask, ctx->saturate);
}

static CState* CState_new(const unsigned char* data, int size, int baseprob,
                           int saturate, CSEval* eval, const unsigned char* context) {
    ...
    ApplyModelCtx apply_ctx = { padded_data + MAX_CTX, cs->size, saturate, cs->models };
...
static void HB_init(HashBits* hb) {
...
static void HB_push_hash(HashBits* hb, unsigned int value) {
    if (hb->hashes_len >= hb->hashes_cap) {
        ...
        hb->hashes = (unsigned*)realloc(hb->hashes, hb->hashes_cap * sizeof(unsigned));
    }
...
static HashBits ComputeHashBits(const unsigned char* data, int size, unsigned char* context,
                                const ModelList4k* models, int first, int finish) {
    ...
    if (copy_size > 0) {
        memcpy(context + MAX_CTX - copy_size, padded_data + size - copy_size, copy_size);
    }
    free(databuf);
    return out;
}
...
static void CompressFromHashBits(AritState* arit_state, const HashBits* hb,
    ...
    uint32_t hash_reciprocal = (uint32_t)(((1ull << (hash_shift + 31)) + hashsize - 1) / hashsize);
    ...
    for (int bp = 0; bp < bit_length; bp++) {
        ...
        for (int m = 0; m < num_models; m++) {
            ...
            unsigned int reduced_hash = hash_val
                - (uint32_t)(((uint64_t)hash_val * hash_reciprocal) >> reciprocal_shift) * hashsize;
            ...
            while (1) {
                if (entry->used == 0) {
                    ...
                    unsigned int shift = (1 - (((entry->prob[0] + 255) & (entry->prob[1] + 255)) >> 8)) * 2
                                       + weight_factor;
...
static int Compress4k(const unsigned char* data, int data_size, unsigned char* out_data,
                      int max_out, ModelList4k* ml, int saturate, int baseprob, int hashsize) {
    unsigned char context[MAX_CTX] = {};
    HashBits hb = ComputeHashBits(data, data_size, context, ml, 1, 1);
    TinyHashEntry* hash_table = (TinyHashEntry*)calloc(hb.tiny_hash_size, sizeof(TinyHashEntry));

    memset(out_data, 0, max_out);
    AritState arit_state;
    AritCodeInit(&arit_state, out_data);
    CompressFromHashBits(&arit_state, &hb, hash_table, baseprob, hashsize, saturate);
...
static unsigned int TryWeights(CState* cs, ModelList4k* ml, int compression_type) {
...
static ModelList4k ApproximateModels4k(const unsigned char* data, int size,
                                       ...
                                       int compression_type, int saturate, int baseprob,
    ...
    ModelList4k* model_sets = (ModelList4k*)calloc(num_sets, sizeof(ModelList4k));
    CSEval eval;
    CSEval_init(&eval);
...
int main(int argc, char* argv[]) {
    ...
    if (!output_file) {
        snprintf(default_output, sizeof(default_output), "%s.compressed", input_file);
    ...
    ModelList4k ml = ApproximateModels4k(data, data_size, context, compression_type,
                                         saturate, baseprob, &estimated_size, ProgressUpdate, NULL);
    ...
    int compressed_size = Compress4k(data, data_size, output_buf, max_output,
                                     &ml, saturate, baseprob, hashsize);

    printf("Compressed:  %d bytes (%.2f%%)\n", compressed_size, 100.0f * compressed_size / data_size);
    ...
    printf("Output:      %s (%d bytes, weightmask %08X)\n",
           output_file, 12 + num_models + compressed_size, weightmask);
