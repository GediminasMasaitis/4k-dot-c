...
static void write_html_report(const char *path, const CompStats *s) {
  ...
  fprintf(f,
    ...
    "position:absolute;right:-24px;top:20px;width:62px;height:5px;"
    "z-index:3;transform-origin:right center;transform:rotate(8deg);"
    "border-radius:3px;"
    ...
    "position:absolute;left:-4px;top:-5px;width:10px;height:15px;"
...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "    }\n"
    "    var now=actx.currentTime,cur=-1;\n"
    "    while(sonQ.length&&sonQ[0].t<=now) cur=sonQ.shift().i;\n"
    "    if(cur>=0) sonMark(cur);\n"
    ...
    "      if(c<1.5) n=ch[0]+24;              /* calm: root */\n"
    "      else if(c<3) n=ch[step%%3]+24;     /* fine: chord tone */\n"
    "      else if(c<5) n=ch[step%%3]+26;     /* tense: color tone */\n"
    "      else if(c<8) n=ch[(step+1)%%3]+25; /* strained: rub */\n"
    "      else{snare(t);n=ch[(step+1)%%3]+25;} /* pain: percussion */\n"
    "    }\n"
    ...
    "  var eggSet={},eggN=0,EGG_TOTAL=26;\n"
