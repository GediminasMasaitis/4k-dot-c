...
/* fp-contract off: -march=native lets gcc fuse these multiply-adds
   into FMAs, which skips intermediate roundings. The report's in-browser
   forward pass re-runs this polynomial rounding after every step, and its
   bit-exact verdict needs the binary to do the same. */
__attribute__((optimize("-ffp-contract=off"))) static inline float
fast_log2f(float x) {
  union {
    float f;
    uint32_t u;
...
static void encode_from_stream_direct(ArithCoder *ac, const HashBitStream *hb,
  ...
  if (collect && verbose) {
    ...
    /* plain strings passed via %s: single percent, not %% */
...
static void emit_model_contrib(FILE *f, const CompStats *s) {
  if (s->num_models > 0) {
    ...
    fprintf(f, "<div id=\"mctrib-wrap\" style=\"display:flex;"
      "align-items:center;gap:24px;padding:8px 0 4px\">\n");
    ...
    for (int r = 0; r < 16; r++) {
      ...
      for (int c2 = 0; c2 < 16; c2++) {
        ...
        if (label[0]) {
          ...
          fprintf(f,
            "<text class=\"bfg\" x=\"%d\" y=\"%d\" text-anchor=\"middle\" "
...
static void emit_bigram(FILE *f, const CompStats *s) {
  if (s->input_data && s->input_size > 1) {
    ...
    fprintf(f,
      ...
      "}\n"
      "/* tuned bytes: both transitions they sit in, in pink */\n"
      "function paintPink(){\n"
      "  if(!window.TUNED||typeof BD==='undefined') return;\n"
      "  ctx.fillStyle='#ff3fd8';\n"
      "  for(var i=0;i<BD.length;i++){\n"
      "    if(!window.TUNED[i]) continue;\n"
      "    var v=parseInt(BD[i].h,16);\n"
      "    if(i>0) ctx.fillRect(v*2,parseInt(BD[i-1].h,16)*2,2,2);\n"
      "    if(i+1<BD.length)\n"
      "      ctx.fillRect(parseInt(BD[i+1].h,16)*2,v*2,2,2);\n"
      "  }\n"
      "}\n"
      ...
      "}\n"
      "if(bF){\n"
      "  bF.addEventListener('click',function(){bgMode='freq';styleBg();paint();paintPink();});\n"
      "  bC.addEventListener('click',function(){\n"
      "    buildCost();\n"
      "    if(!BGC) return;\n"
      "    bgMode='cost';styleBg();paint();paintPink();\n"
      "  });\n"
      "}\n"
      "styleBg();\n"
      "paint();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-bigram',\n"
      "  fn:function(){\n"
      "    var m2=0;\n"
      "    for(var i=0;i<65536;i++) if(BG[i]>m2) m2=BG[i];\n"
      "    logMax=m2>1?Math.log2(m2):1;\n"
      "    BGC=null;BGN=null;BGC_MAX=0; /* cost cache is stale */\n"
      "    if(bgMode==='cost') buildCost();\n"
      "    paint();paintPink();\n"
      "  }});\n"
      "\n"
  ...
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    for (int i = 0; i < nb; i++) {
      ...
      if (cell >= 6 && bval >= 0x20 && bval <= 0x7E && bval != '<'
        ...
        int bright = t < 0.6f;
        fprintf(f,
          "<text class=\"cmg\" x=\"%d\" y=\"%d\" text-anchor=\"middle\" "
...
static void emit_cost_hist(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    fprintf(f, "%s",
      ...
      "var binT=null;\n"
      "if(window.TUNED){\n"
      "  binT=new Array(nbins);\n"
      "  for(var i=0;i<BD.length;i++){\n"
      "    if(!window.TUNED[i]) continue;\n"
      "    var b2=Math.floor(BD[i].c/binW);\n"
      "    if(b2<0)b2=0; if(b2>=nbins)b2=nbins-1;\n"
      "    binT[b2]=1;\n"
      "  }\n"
      "}\n"
      "var maxN=1;\n"
      ...
      "    +(binT&&binT[b]?' stroke=\"#ff3fd8\" stroke-width=\"1\"':'')+'/>';\n"
      "}\n"
      "/* mean + H0 markers */\n"
      "if(mean<=nbins*binW){\n"
      ...
      "}\n"
      "s+='</svg>';\n"
      "document.getElementById('ch-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-costhist',fn:build});\n"
...
static void emit_info_scatter(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    fprintf(f, "%s",
      ...
      "    +(window.TUNEDV&&window.TUNEDV[p.v]\n"
      "      ?'stroke=\"#ff3fd8\" stroke-width=\"1\" ':'')\n"
      ...
      "s+='</svg>';\n"
      "document.getElementById('is-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-infoscatter',fn:build});\n"
...
static void emit_learning_curve(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    fprintf(f, "%s",
      ...
      "  if(b>=NB) b=NB-1;\n"
      "  sum[b]+=BD[i].c; cnt[b]++;\n"
      "  if(window.TUNED&&window.TUNED[i]) tnb[b]=1;\n"
      "}\n"
      "bins=[];\n"
      "for(var b=0;b<NB;b++)\n"
      "  if(cnt[b]) bins.push({b:b,avg:sum[b]/cnt[b],n:cnt[b],t:tnb[b]});\n"
      ...
      "    +(bn.t?' stroke=\"#ff3fd8\" stroke-width=\"1\"':'')+'/>';\n"
      "  var lx=x+bw/2;\n"
      ...
      "s+='</svg>';\n"
      "document.getElementById('lc-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-learn',fn:build});\n"
...
static void emit_strings(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    fprintf(f, "%s",
      ...
      ".replace(/>/g,'&gt;');}\n"
      "var runs,start,str,cost;\n"
      "function build(){\n"
      "runs=[];start=-1;str='';cost=0;\n"
      "function flush(){\n"
      ...
      "}\n"
      "flush();\n"
      "var card2=document.getElementById('sec-strings');\n"
      "if(!runs.length){card2.style.display='none';return;}\n"
      "card2.style.display='';\n"
      ...
      "  var tn3=false;\n"
      "  if(window.TUNED)\n"
      "    for(var q2=r.o;q2<r.o+r.l;q2++)\n"
      "      if(window.TUNED[q2]){tn3=true;break;}\n"
      "  h+='<tr class=\"st-row\" data-o=\"'+r.o+'\" style=\"cursor:pointer'\n"
      "    +(tn3?';box-shadow:inset 2px 0 0 #ff3fd8':'')+'\">'\n"
      "    +'<td class=\"r\">'+r.o+'</td>'\n"
      ...
      "document.getElementById('st-note').textContent=runs.length>show?\n"
      "  '+'+(runs.length-show)+' more strings not shown':'';\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-strings',fn:build});\n"
...
static void emit_hex_view(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0 && s->input_data) {
    ...
    fprintf(f, "%s",
      ...
      "      var tn2=(window.TUNED&&window.TUNED[i])?\n"
      "        'box-shadow:0 0 0 1px #ff3fd8;':'';\n"
      "      hx+='<span class=\"hx\" data-i=\"'+i+'\" style=\"width:20px;'\n"
      "        +'text-align:center;border-radius:2px;cursor:pointer;'+pad+tn2\n"
      ...
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-hex',fn:render});\n"
      ...
      "      if(window.partyEgg)window.partyEgg('h1337');}\n"
      "    else if(idx===404){window.partyToast('OFFSET NOT FOUND');\n"
      "      if(window.partyEgg)window.partyEgg('h404');}\n"
      "    else if(idx===42){window.partyToast('THE ANSWER');\n"
      "      if(window.partyEgg)window.partyEgg('h42');}\n"
      "  }\n"
  ...
  if (s->byte_model_contrib && s->byte_costs && s->num_data_bytes > 0
    ...
    for (int i = 0; i < nb; i++) {
      ...
      if (cell >= 6 && bval >= 0x20 && bval <= 0x7E && bval != '<'
        ...
        fprintf(f,
          "<text class=\"atg\" x=\"%d\" y=\"%d\" text-anchor=\"middle\" "
          "font-size=\"%d\" fill=\"%s\" pointer-events=\"none\" "
          "opacity=\".8\">%c</text>\n",
          x + cell / 2, y + cell / 2 + (cell >= 10 ? 3 : cell >= 8 ? 2 : 2),
          cell >= 10 ? 7 : cell >= 8 ? 6 : 5,
...
static void emit_net_timeline(FILE *f, const CompStats *s) {
  if (s->byte_model_contrib && s->byte_costs && s->num_data_bytes > 1
    ...
    fprintf(f, "%s",
      ...
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-net',\n"
      "  fn:function(){\n"
      "    if(typeof BD==='undefined'||!BD.length||!BD[0].m) return;\n"
      "    var win=Math.max(4,Math.min(64,(nb/64)|0));\n"
      "    var sp=0,sn=0;\n"
      "    for(var i=0;i<npts;i++){\n"
      "      var end=Math.min(i+win,nb),cnt=end-i,cp=0,cn2=0;\n"
      "      for(var m=0;m<nm;m++){\n"
      "        var sum=0;\n"
      "        for(var j=i;j<end;j++) sum+=BD[j].m[m];\n"
      "        var md2=sum/cnt;\n"
      "        mdata[i*nm+m]=md2;\n"
      "        if(md2>0) cp+=md2; else cn2+=-md2;\n"
      "      }\n"
      "      if(cp>sp) sp=cp;\n"
      "      if(cn2>sn) sn=cn2;\n"
      "    }\n"
      "    smaxP=sp<0.01?0.01:sp;\n"
      "    smaxN=sn<0.01?0.01:sn;\n"
      "    upH=ph*smaxP/(smaxP+smaxN);loH=ph-upH;midY=PT+upH;\n"
      "    render();\n"
      "  }});\n"
...
static void emit_context_depth(FILE *f, const CompStats *s) {
  if (s->byte_model_contrib && s->num_data_bytes > 1 && s->num_models > 0) {
    ...
    fprintf(f, "%s",
      ...
      "var win,npts,mdata,smax,stk,cols,dcnt;\n"
      "var W,H,PL,PR,PT,PB,pw,ph;\n"
      "function build(){\n"
      ...
      "win=Math.max(4,Math.min(64,nb>>6));\n"
      "npts=Math.max(2,nb-win+1);\n"
      "var pre=new Float64Array((nb+1)*nd);\n"
      "for(var i=0;i<nb;i++)\n"
      "  for(var d=0;d<nd;d++) pre[(i+1)*nd+d]=pre[i*nd+d]+pb[i*nd+d];\n"
      "mdata=new Float32Array(npts*nd);\n"
      "smax=0;\n"
      "for(var i=0;i<npts;i++){\n"
      ...
      "W=960;H=200;PL=44;PR=12;PT=12;PB=28;\n"
      ...
      "if(window.TUNED){\n"
      "  for(var i=0;i<nb;i++) if(window.TUNED[i])\n"
      "    s+='<line x1=\"'+(PL+i*pw/(nb-1)).toFixed(1)+'\" y1=\"'+(PT+ph-5)\n"
      "      +'\" x2=\"'+(PL+i*pw/(nb-1)).toFixed(1)+'\" y2=\"'+(PT+ph)\n"
      "      +'\" stroke=\"#ff3fd8\" stroke-width=\"1\"/>';\n"
      "}\n"
      "s+='<line id=\"ctxd-scrub\" class=\"scrub-line\" x1=\"0\" y1=\"'+PT\n"
      "  +'\" x2=\"0\" y2=\"'+(PT+ph)+'\"/>';\n"
      "s+='</svg>';\n"
      "document.getElementById('ctxd-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-depth',fn:build});\n"
...
static void emit_correlation(FILE *f, const CompStats *s) {
  if (s->byte_model_contrib && s->num_data_bytes > 1 && s->num_models >= 2) {
    ...
    fprintf(f, "%s",
      ...
      "  var nb2=BD.length;\n"
      "  var mean=new Float64Array(CN);\n"
      "  for(var i=0;i<nb2;i++){\n"
      "    var mm=BD[i].m;\n"
      "    for(var a=0;a<CN;a++) mean[a]+=mm[a];\n"
      "  }\n"
      "  for(var a=0;a<CN;a++) mean[a]/=nb2;\n"
      "  var cov=new Float64Array(CN*CN);\n"
      "  for(var k=0;k<CN*CN;k++){CBH[k]=0;CBT[k]=0;}\n"
      "  for(var i=0;i<nb2;i++){\n"
      "    var mm=BD[i].m;\n"
      "    for(var a=0;a<CN;a++){\n"
      "      var da=mm[a]-mean[a];\n"
      "      for(var b=a;b<CN;b++){\n"
      "        cov[a*CN+b]+=da*(mm[b]-mean[b]);\n"
      "        if(mm[a]>0.01&&mm[b]>0.01) CBH[a*CN+b]++;\n"
      "        if(mm[a]<-0.01&&mm[b]<-0.01) CBT[a*CN+b]++;\n"
      "      }\n"
      "    }\n"
      "  }\n"
      "  for(var a=0;a<CN;a++)\n"
      "    for(var b=a;b<CN;b++){\n"
      "      var dn=Math.sqrt(cov[a*CN+a]*cov[b*CN+b]);\n"
      "      var r2=dn>0?cov[a*CN+b]/dn:0;\n"
      "      CR[a*CN+b]=r2;CR[b*CN+a]=r2;\n"
      "      CBH[b*CN+a]=CBH[a*CN+b];CBT[b*CN+a]=CBT[a*CN+b];\n"
      "    }\n"
      "  var cg2=31,lb2=44,h='';\n"
      "  for(var b=0;b<CN;b++){\n"
      "    var x=lb2+b*cg2+15;\n"
      "    h+='<text x=\"'+x+'\" y=\"'+(lb2-6)+'\" text-anchor=\"start\"'\n"
      "      +' font-size=\"9\" fill=\"var(--fg3)\" transform=\"rotate(-60 '\n"
      "      +x+' '+(lb2-6)+')\">'+MI[b].mask+':'+MI[b].w+'</text>';\n"
      "  }\n"
      "  for(var a=0;a<CN;a++){\n"
      "    var y=lb2+a*cg2;\n"
      "    h+='<text x=\"'+(lb2-5)+'\" y=\"'+(y+18)+'\" text-anchor=\"end\"'\n"
      "      +' font-size=\"9\" fill=\"var(--fg3)\">'+MI[a].mask+':'\n"
      "      +MI[a].w+'</text>';\n"
      "    for(var b=0;b<CN;b++){\n"
      "      var x=lb2+b*cg2;\n"
      "      var r2=CR[a*CN+b],ar=Math.abs(r2);\n"
      "      var op=a===b?0.9:0.05+0.85*ar;\n"
      "      h+='<rect x=\"'+x+'\" y=\"'+y+'\" width=\"30\" height=\"30\"'\n"
      "        +' rx=\"2\" fill=\"'+(r2>=0?'#22d3ee':'#f87171')\n"
      "        +'\" fill-opacity=\"'+op.toFixed(2)+'\" data-a=\"'+a\n"
      "        +'\" data-b=\"'+b+'\" style=\"cursor:pointer\"/>';\n"
      "      if(a!==b)\n"
      "        h+='<text x=\"'+(x+15)+'\" y=\"'+(y+18)+'\" text-anchor=\"middle\"'\n"
      "          +' font-size=\"8\" fill=\"'+(ar>0.55?'#0c0e14':'var(--fg2)')\n"
      "          +'\" pointer-events=\"none\">'+r2.toFixed(2)+'</text>';\n"
      "    }\n"
      "  }\n"
      "  svg.innerHTML=h;\n"
      "}\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-corr',fn:rebuild});\n"
      "})();</script>\n");
...
static void emit_surprises(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 0 && s->num_models > 0) {
    ...
    fprintf(f, "%s",
      "<script>(function(){\n"
      "/* delegated, so rebuilt rows keep working without rewiring */\n"
      "var tbl=document.querySelector('#sec-surprises table');\n"
      "var tip=document.getElementById('ts-tip');\n"
      "var card=document.getElementById('sec-surprises');\n"
      "function hideTip(){tip.style.display='none';}\n"
      "if(tbl){\n"
      "  tbl.addEventListener('click',function(e){\n"
      "    var row=e.target.closest('tr.ts-row'); if(!row) return;\n"
      ...
      "    var row=e.target.closest('tr.ts-row');\n"
      ...
      "function rebuild(){\n"
      "  if(typeof BD==='undefined'||!BD.length) return;\n"
      "  var tb=document.querySelector('#sec-surprises tbody');\n"
      "  if(!tb) return;\n"
      "  var idx=[];\n"
      "  for(var i=0;i<BD.length;i++) idx.push(i);\n"
      "  idx.sort(function(a,b){return BD[b].c-BD[a].c;});\n"
      "  var topn=Math.min(20,idx.length),h='';\n"
      "  for(var i=0;i<topn;i++){\n"
      "    var bi=idx[i],d=BD[bi];\n"
      "    var clr=d.c<3?'#34d399':d.c<6?'#fbbf24':'#f87171';\n"
      "    var ch=(d.ch&&'<>&\\\"'.indexOf(d.ch)<0)?\" '\"+d.ch+\"'\":'';\n"
      "    var ctx2='';\n"
      "    for(var j=Math.max(0,bi-3);j<Math.min(BD.length,bi+4);j++){\n"
      "      var hx3=BD[j].h+' ';\n"
      "      if(j===bi) ctx2+='<span style=\"color:var(--red);'\n"
      "        +'font-weight:700\">'+hx3+'</span>';\n"
      "      else if(window.TUNED&&window.TUNED[j])\n"
      "        ctx2+='<span style=\"color:#ff3fd8\">'+hx3+'</span>';\n"
      "      else ctx2+=hx3;\n"
      "    }\n"
      "    var mb='\\u2014';\n"
      "    if(d.m&&typeof MI!=='undefined'){\n"
      "      var bm=-1,bv=0;\n"
      "      for(var k=0;k<d.m.length;k++)\n"
      "        if(d.m[k]>bv){bv=d.m[k];bm=k;}\n"
      "      if(bm>=0&&MI[bm]) mb=MI[bm].mask+':'+MI[bm].w\n"
      "        +' <span style=\"color:var(--fg3)\">('+bv.toFixed(2)\n"
      "        +' bits)</span>';\n"
      "    }\n"
      "    h+='<tr class=\"ts-row\" data-i=\"'+bi+'\" style=\"cursor:pointer'\n"
      "      +(window.TUNED&&window.TUNED[bi]\n"
      "        ?';box-shadow:inset 2px 0 0 #ff3fd8':'')+'\">'\n"
      "      +'<td class=\"r\">'+(i+1)+'</td><td class=\"r\">'+bi+'</td>'\n"
      "      +'<td style=\"font-family:var(--mono);font-size:11.5px\">0x'\n"
      "      +d.h+ch+'</td>'\n"
      "      +'<td class=\"r\" style=\"color:'+clr+';font-weight:600\">'\n"
      "      +d.c.toFixed(2)+'</td>'\n"
      "      +'<td style=\"font-family:var(--mono);font-size:10px;'\n"
      "      +'color:var(--fg3)\">'+ctx2+'</td>'\n"
      "      +'<td style=\"font-family:var(--mono);font-size:11.5px\">'+mb\n"
      "      +'</td></tr>';\n"
      "  }\n"
      "  tb.innerHTML=h;\n"
      "}\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-surprises',fn:rebuild});\n"
      "})();</script>\n");
...
static void emit_cost_position(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 1) {
    ...
    fprintf(f, "%s",
      "var H2=160,PT2=12,PH2=120;\n"
      "function rebuild(){\n"
      "  if(typeof BD==='undefined'||BD.length<2) return;\n"
      "  var win=Math.max(4,Math.min(64,(nb/64)|0));\n"
      "  R=[];\n"
      "  var rmin=1e9,rmax=-1e9;\n"
      "  for(var i=0;i<npts;i++){\n"
      "    var end=Math.min(i+win,nb),sum=0;\n"
      "    for(var j=i;j<end;j++) sum+=BD[j].c;\n"
      "    var v=sum/(end-i);\n"
      "    R.push(v);\n"
      "    if(v<rmin)rmin=v; if(v>rmax)rmax=v;\n"
      "  }\n"
      "  if(rmax<=rmin) rmax=rmin+1;\n"
      "  /* H0 of the tuned bytes, from the live frequency table */\n"
      "  var h0=0;\n"
      "  if(typeof BF!=='undefined'){\n"
      "    var tot=0,v2;\n"
      "    for(v2=0;v2<256;v2++) tot+=BF[v2];\n"
      "    if(tot) for(v2=0;v2<256;v2++) if(BF[v2])\n"
      "      h0-=BF[v2]/tot*Math.log2(BF[v2]/tot);\n"
      "  }\n"
      "  function Y2(v){return PT2+((PH2*(rmax-v)/(rmax-rmin))|0);}\n"
      "  function X2(i){return PL+((i*pw/(npts-1))|0);}\n"
      "  var s='<rect x=\"'+PL+'\" y=\"'+PT2+'\" width=\"'+pw\n"
      "    +'\" height=\"'+PH2+'\" fill=\"var(--bg3)\" rx=\"4\"/>';\n"
      "  for(var i=0;i<=4;i++){\n"
      "    var val=rmin+(rmax-rmin)*(4-i)/4,y=PT2+((PH2*i/4)|0);\n"
      "    s+='<line x1=\"'+PL+'\" y1=\"'+y+'\" x2=\"'+(PL+pw)+'\" y2=\"'+y\n"
      "      +'\" stroke=\"var(--bdr)\" stroke-width=\"0.5\"/>'\n"
      "      +'<text x=\"'+(PL-5)+'\" y=\"'+(y+3)+'\" text-anchor=\"end\"'\n"
      "      +' font-size=\"9\" fill=\"var(--fg3)\">'+val.toFixed(1)+'</text>';\n"
      ...
      "      +'<text x=\"'+(PL+pw+3)+'\" y=\"'+(hy+3)+'\" font-size=\"9\"'\n"
      "      +' fill=\"#f87171\" opacity=\".8\">H\\u2080</text>';\n"
      "  }\n"
      "  if(8>=rmin&&8<=rmax){\n"
      "    var y8=Y2(8);\n"
      "    s+='<line x1=\"'+PL+'\" y1=\"'+y8+'\" x2=\"'+(PL+pw)+'\" y2=\"'+y8\n"
      "      +'\" stroke=\"var(--fg3)\" stroke-width=\"0.5\"'\n"
      "      +' stroke-dasharray=\"3,3\" opacity=\".4\"/>';\n"
      "  }\n"
      "  s+='<defs><linearGradient id=\"cfill\" x1=\"0\" y1=\"0\"'\n"
      "    +' x2=\"0\" y2=\"1\">'\n"
      "    +'<stop offset=\"0\" stop-color=\"#22d3ee\" stop-opacity=\".2\"/>'\n"
      "    +'<stop offset=\"1\" stop-color=\"#22d3ee\" stop-opacity=\".02\"/>'\n"
      "    +'</linearGradient></defs>';\n"
      "  var d='M'+PL+','+(PT2+PH2)+' ';\n"
      "  for(var i=0;i<npts;i++) d+='L'+X2(i)+','+Y2(R[i])+' ';\n"
      "  s+='<path d=\"'+d+'L'+(PL+pw)+','+(PT2+PH2)\n"
      "    +' Z\" fill=\"url(#cfill)\"/>';\n"
      "  var d2='';\n"
      "  for(var i=0;i<npts;i++) d2+=(i?'L':'M')+X2(i)+','+Y2(R[i])+' ';\n"
      "  s+='<path d=\"'+d2+'\" fill=\"none\" stroke=\"#22d3ee\"'\n"
      "    +' stroke-width=\"1.5\" stroke-linejoin=\"round\"/>';\n"
      "  for(var i=0;i<=5;i++){\n"
      "    var x=PL+((pw*i/5)|0),bp2=((nb-1)*i/5)|0;\n"
      "    s+='<text x=\"'+x+'\" y=\"'+(H2-4)+'\" text-anchor=\"middle\"'\n"
      "      +' font-size=\"9\" fill=\"var(--fg3)\">'+bp2+'</text>';\n"
      "  }\n"
      "  if(window.TUNED){\n"
      "    for(var i=0;i<nb;i++) if(window.TUNED[i]){\n"
      "      var tx3=X2(i<npts?i:npts-1);\n"
      "      s+='<line x1=\"'+tx3+'\" y1=\"'+(PT2+PH2-5)+'\" x2=\"'+tx3\n"
      "        +'\" y2=\"'+(PT2+PH2)+'\" stroke=\"#ff3fd8\"'\n"
      "        +' stroke-width=\"1\"/>';\n"
      "    }\n"
      "  }\n"
      "  s+='<line id=\"cost-scrub\" class=\"scrub-line\" x1=\"0\" y1=\"'+PT2\n"
      "    +'\" x2=\"0\" y2=\"'+(PT2+PH2)+'\"/>';\n"
      "  document.getElementById('cost-svg').innerHTML=s;\n"
      "  /* the scrub line was just replaced; re-point the cached ref */\n"
      "  scrub=document.getElementById('cost-scrub');\n"
      "}\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-cost',fn:rebuild});\n");
...
static void emit_cum_cost(FILE *f, const CompStats *s) {
  if (s->byte_costs && s->num_data_bytes > 1) {
    ...
    fprintf(f, "%s",
      ...
      "function build(){\n"
      "n=BD.length;\n"
      "cum=new Float64Array(n);var t=0;\n"
      "for(var i=0;i<n;i++){t+=BD[i].c;cum[i]=t;}\n"
      "totalB=t/8;var h0B=H0*n/8;\n"
      ...
      "  +'stroke-linejoin=\"round\"/>';\n"
      "if(window.TUNED){\n"
      "  for(var i=0;i<n;i++) if(window.TUNED[i])\n"
      "    s+='<line x1=\"'+X(i).toFixed(1)+'\" y1=\"'+(PT+ph-5)\n"
      "      +'\" x2=\"'+X(i).toFixed(1)+'\" y2=\"'+(PT+ph)\n"
      "      +'\" stroke=\"#ff3fd8\" stroke-width=\"1\"/>';\n"
      "}\n"
      "s+='<line id=\"cc-scrub\" class=\"scrub-line\" x1=\"0\" y1=\"'+PT\n"
      "  +'\" x2=\"0\" y2=\"'+(PT+ph)+'\"/>';\n"
      "s+='</svg>';\n"
      "document.getElementById('cc-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-cumcost',fn:build});\n"
...
static void emit_bit_heatmap(FILE *f, const CompStats *s) {
  if (s->bit_costs && s->num_data_bytes > 1) {
    ...
    fprintf(f, "%s",
      ...
      "  ctx.fillStyle='#ff3fd8';\n"
      "  for(var i=0;i<nb;i++)\n"
      "    if(window.TUNED[i]) ctx.fillRect((i/agg)|0,0,1,1);\n"
      "}\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-bitmap',\n"
      "  fn:function(){\n"
      "    var S=window.FPSTATS;\n"
      "    if(S&&S.bb.length>=nb*8)\n"
      "      for(var i=0;i<nb*8;i++) BB[i]=S.bb[i];\n"
      "    build();\n"
      "  }});\n"
      "cv.addEventListener('mousemove',function(e){\n"
  ...
  if (s->input_data && s->input_size > 4) {
    ...
    if (matched > 0) {
      ...
      fprintf(f, "%s",
        ...
        "lden=Math.log2(1+maxL); if(lden<=0) lden=1;\n"
        "/* rolling cost overlay from BD */\n"
        "haveC=(typeof BD!=='undefined')&&BD.length>=2;\n"
        "R=null;cmax=0;cn=0;\n"
        "if(haveC){\n"
        ...
        "}\n"
        "if(window.TUNED){\n"
        "  for(var i2=0;i2<n&&i2<window.TUNED.length;i2++)\n"
        "    if(window.TUNED[i2]){\n"
        "      var tx4=X(i2).toFixed(1);\n"
        "      s+='<line x1=\"'+tx4+'\" y1=\"'+(PT+ph-5)+'\" x2=\"'+tx4\n"
        "        +'\" y2=\"'+(PT+ph)+'\" stroke=\"#ff3fd8\"'\n"
        "        +' stroke-width=\"1\"/>';\n"
        "    }\n"
        "}\n"
        ...
        "  +'\" x2=\"0\" y2=\"'+(PT+ph)+'\"/>';\n"
        "s+='</svg>';\n"
        "document.getElementById('rep-chart').innerHTML=s;\n"
        "}\n"
        "build();\n"
        "/* the same backward-match scan the C side ran: 4-byte hash\n"
        "   chains, depth 64, length cap 4096 - so this chart, the\n"
        "   echoes backdrop and the tooltips all see the tuned file */\n"
        "function lzScan(){\n"
        "  var nb2=BD.length,by=new Uint8Array(nb2);\n"
        "  for(var i=0;i<nb2;i++) by[i]=parseInt(BD[i].h,16);\n"
        "  var hb2=12;\n"
        "  while((1<<hb2)<nb2*2&&hb2<20) hb2++;\n"
        "  var head=new Int32Array(1<<hb2).fill(-1);\n"
        "  var prev=new Int32Array(nb2);\n"
        "  for(var i=0;i<nb2&&i<ML.length;i++){ML[i]=0;MD[i]=0;}\n"
        "  for(var i=0;i+3<nb2;i++){\n"
        "    var h4=by[i]|(by[i+1]<<8)|(by[i+2]<<16)|(by[i+3]<<24);\n"
        "    var hh=Math.imul(h4,-1640531527)>>>(32-hb2);\n"
        "    var cand=head[hh],depth=0,bl=0,bd2=0;\n"
        "    while(cand>=0&&depth<64){\n"
        "      var mx2=Math.min(4096,nb2-i),l2=0;\n"
        "      while(l2<mx2&&by[cand+l2]===by[i+l2]) l2++;\n"
        "      if(l2>bl){bl=l2;bd2=i-cand;}\n"
        "      cand=prev[cand];depth++;\n"
        "    }\n"
        "    if(bl>=4&&i<ML.length){ML[i]=bl;MD[i]=bd2;}\n"
        "    prev[i]=head[hh];head[hh]=i;\n"
        "  }\n"
        "}\n"
        "if(window.retuneHooks)window.retuneHooks.push({id:'sec-repeat',\n"
        "  fn:function(){\n"
        "    if(typeof BD==='undefined'||BD.length<5) return;\n"
        "    lzScan();\n"
        "    build();\n"
        "  }});\n"
...
static void emit_calibration(FILE *f, const CompStats *s) {
  {
    ...
    if (total_cal > 0) {
      ...
      /* window.CAL, not var: the tuner's rebuild hook replaces it and
         the tooltip closure resolves to the fresh copy */
      fprintf(f, "<script>\n(function(){\nwindow.CAL=[");
      for (int i = 0; i < 20; i++) {
        if (s->calib_count[i])
...
static void emit_occ_growth(FILE *f, const CompStats *s) {
  if (s->occ_nsamples > 1) {
    ...
    fprintf(f, "%s",
      ...
      "  +'\" x2=\"0\" y2=\"'+(PT+ph)+'\"/>';\n"
      "s+='</svg>';\n"
      "document.getElementById('occ-chart').innerHTML=s;\n"
      "}\n"
      "build();\n"
      "if(window.retuneHooks)window.retuneHooks.push({id:'sec-occ',\n"
      "  fn:function(){\n"
      "    var S=window.FPSTATS;\n"
      "    if(S&&S.occN>1){\n"
      "      OS=[];\n"
      "      for(var k2=0;k2<S.occN;k2++) OS.push(S.occS[k2]);\n"
      "    }\n"
      "    build();\n"
      "  }});\n"
...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "    apply(nxt);\n"
    "    if(window.partyDegauss) window.partyDegauss();\n"
    ...
    "      if(window.partyEgg) window.partyEgg('theme');\n"
    "      spamN=0;\n"
    "    }\n"
    "  });\n"
    "})();\n"
    ...
    "  var toast=null,noToast=false;\n"
    "  function showToast(msg){\n"
    "    if(noToast) return; /* the credits do not share the stage */\n"
    ...
    "     sonify mode replaces the arpeggio with the file itself: each\n"
    "     melody note is the next byte's encoding cost - chord tones while\n"
    "     the coder is calm, dissonance as it strains, a noise snare when\n"
    "     a byte truly hurt */\n"
    "  var actx=null,gainM=null,seqTimer=null,muted=false,mstep=0,nextT=0;\n"
    "  var beatDur=.125;\n"
    "  var sonify=false,sonPos=0,noiseBuf=null;\n"
    "  var sonKey=null; /* {r,mi,nm}: listen, pinned to one scale */\n"
    "  /* match echo: the backward-match region the needle is\n"
    "     riding, if any */\n"
    "  var sonEcoEnd=-1,sonEcoLen=0,sonEcoDist=0,sonEcoT=false;\n"
    "  var sonQ=[],sonRaf=null,sonEl=null;\n"
    "  var CHORDS=[[57,60,64],[53,57,60],[48,52,55],[55,59,62]];\n"
    "  /* tape wow: one slow drift shared by every voice - the whole\n"
    "     machine bends together, like tape does */\n"
    "  function mf(n){\n"
    "    var w=syn.ww?Math.sin((actx?actx.currentTime:0)*1.9)*syn.ww:0;\n"
    "    return 440*Math.pow(2,(n-69+syn.tr+w)/12);\n"
    "  }\n"
    "  function nzBuf(){\n"
    "    if(!noiseBuf){\n"
    "      var sr=actx.sampleRate;\n"
    "      noiseBuf=actx.createBuffer(1,(sr*.1)|0,sr);\n"
    "      var d=noiseBuf.getChannelData(0);\n"
    "      for(var i=0;i<d.length;i++) d[i]=Math.random()*2-1;\n"
    "    }\n"
    "    return noiseBuf;\n"
    "  }\n"
    "  function snare(t,p,g){\n"
    "    var s=actx.createBufferSource(),gn=actx.createGain();\n"
    "    s.buffer=nzBuf();\n"
    "    gn.gain.setValueAtTime(g||.055,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.07);\n"
    "    s.connect(gn);gn.connect(panTo(p||0));s.start(t);s.stop(t+.08);\n"
    "    vreg(t,[s]);\n"
    "  }\n"
    "  /* the kit: a kick that thumps, a hat that ticks, a pad that\n"
    "     holds each bar together. the groove is the arrangement the\n"
    "     sonification was missing - the data still picks every pitch */\n"
    "  /* the kick, parameterized: default is the shipped thump;\n"
    "     genres pass their own pitch drop, length and - for the\n"
    "     hard ones - the drive as destination */\n"
    "  function kick(t,g,f0,f1,d2,dest,cl){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    d2=d2||.12;\n"
    "    o.type='sine';\n"
    "    o.frequency.setValueAtTime(f0||120,t);\n"
    "    o.frequency.exponentialRampToValueAtTime(f1||40,t+d2);\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+d2+.02);\n"
    "    o.connect(gn);gn.connect(dest||gainM);\n"
    "    o.start(t);o.stop(t+d2+.03);\n"
    "    vreg(t,[o]);\n"
    "    if(cl){\n"
    "      /* the click: 10ms of noise on the nose. attack is what\n"
    "         separates a kick from a hum */\n"
    "      var s=actx.createBufferSource(),cg=actx.createGain();\n"
    "      s.buffer=nzBuf();\n"
    "      cg.gain.setValueAtTime(cl,t);\n"
    "      cg.gain.exponentialRampToValueAtTime(.001,t+.012);\n"
    "      s.connect(cg);cg.connect(dest||gainM);\n"
    "      s.start(t);s.stop(t+.015);\n"
    "      vreg(t,[s]);\n"
    "    }\n"
    "  }\n"
    "  function hat(t,g,open,p){\n"
    "    var s=actx.createBufferSource(),gn=actx.createGain();\n"
    "    var f=actx.createBiquadFilter();\n"
    "    s.buffer=nzBuf();f.type='highpass';f.frequency.value=7000;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+(open?.09:.03));\n"
    "    s.connect(f);f.connect(gn);\n"
    "    gn.connect(p===undefined?gainM:panTo(p));\n"
    "    s.start(t);s.stop(t+(open?.1:.05));\n"
    "    vreg(t,[s]);\n"
    "  }\n"
    "  function tom(t,g,f0,p){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type='sine';\n"
    "    o.frequency.setValueAtTime(f0,t);\n"
    "    o.frequency.exponentialRampToValueAtTime(f0*.55,t+.09);\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.11);\n"
    "    o.connect(gn);gn.connect(p===undefined?gainM:panTo(p));\n"
    "    o.start(t);o.stop(t+.12);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  function pad(t,ch,sev){\n"
    "    /* sev adds the seventh on top: three voices are a chord,\n"
    "       four are a mood - lofi and ambient take the fourth */\n"
    "    var nv=sev?4:3;\n"
    "    for(var i=0;i<nv;i++){\n"
    "      var o=actx.createOscillator(),g=actx.createGain();\n"
    "      o.type='triangle';\n"
    "      o.frequency.value=mf((i<3?ch[i]:ch[0]+10)+12);\n"
    "      g.gain.setValueAtTime(.0001,t);\n"
    "      g.gain.linearRampToValueAtTime(i<3?.02:.014,t+.5);\n"
    "      g.gain.exponentialRampToValueAtTime(.001,t+beatDur*15);\n"
    "      o.connect(g);g.connect(panTo(i<3?(i-1)*.4:.2));\n"
    "      o.start(t);o.stop(t+beatDur*15+.1);\n"
    "      vreg(t,[o]);\n"
    "    }\n"
    "  }\n"
    "  /* the clap: three grabs and a hold through a 1.2k bandpass.\n"
    "     one burst is a snare; the smear is the hands */\n"
    "  function clap(t,g,p){\n"
    "    var f=actx.createBiquadFilter();\n"
    "    f.type='bandpass';f.frequency.value=1250;f.Q.value=1.4;\n"
    "    f.connect(panTo(p||0));\n"
    "    var dts=[0,.012,.025,.04];\n"
    "    for(var i=0;i<4;i++){\n"
    "      var s=actx.createBufferSource(),gn=actx.createGain();\n"
    "      s.buffer=nzBuf();\n"
    "      var t2=t+dts[i],d2=i<3?.02:.13;\n"
    "      gn.gain.setValueAtTime(i<3?g:g*.8,t2);\n"
    "      gn.gain.exponentialRampToValueAtTime(.001,t2+d2);\n"
    "      s.connect(gn);gn.connect(f);\n"
    "      s.start(t2);s.stop(t2+d2+.01);\n"
    "      vreg(t,[s]);\n"
    "    }\n"
    "  }\n"
    "  /* 909-style metal: six detuned squares through a highpass.\n"
    "     noise pretends to be a cymbal; the stack rings like one */\n"
    "  function hat909(t,g,dur,p){\n"
    "    var f=actx.createBiquadFilter(),gn=actx.createGain();\n"
    "    f.type='highpass';f.frequency.value=6800;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+dur);\n"
    "    f.connect(gn);gn.connect(panTo(p||0));\n"
    "    var fr=[3609,4867,5482,6412,7509,8871],os=[];\n"
    "    for(var i=0;i<6;i++){\n"
    "      var o=actx.createOscillator();\n"
    "      o.type='square';o.frequency.value=fr[i];\n"
    "      o.connect(f);o.start(t);o.stop(t+dur+.02);\n"
    "      os.push(o);\n"
    "    }\n"
    "    vreg(t,os);\n"
    "  }\n"
    "  /* the rimshot: lofi's polite backbeat */\n"
    "  function rim(t,g,p){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type='triangle';o.frequency.value=790;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.035);\n"
    "    o.connect(gn);gn.connect(panTo(p||0));\n"
    "    o.start(t);o.stop(t+.045);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* the rumble: the kick's basement. techno is a room, and\n"
    "     this is the room */\n"
    "  function rumble(t){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type='sine';\n"
    "    o.frequency.setValueAtTime(64,t);\n"
    "    o.frequency.exponentialRampToValueAtTime(36,t+.3);\n"
    "    gn.gain.setValueAtTime(.0001,t);\n"
    "    gn.gain.linearRampToValueAtTime(.055,t+.025);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.34);\n"
    "    o.connect(gn);gn.connect(gainM);\n"
    "    o.start(t);o.stop(t+.36);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* the zap: psytrance punctuation, a saw falling off a cliff */\n"
    "  function zap(t){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type='sawtooth';\n"
    "    o.frequency.setValueAtTime(1600,t);\n"
    "    o.frequency.exponentialRampToValueAtTime(70,t+.16);\n"
    "    gn.gain.setValueAtTime(.035,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.18);\n"
    "    o.connect(gn);gn.connect(gainM);o.start(t);o.stop(t+.2);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* the hoover: detuned saws falling into the note through the\n"
    "     drive - the alpha juno patch that named a decade */\n"
    "  function hoover(t,fq,g,dur){\n"
    "    var gn=actx.createGain();\n"
    "    gn.gain.setValueAtTime(.0001,t);\n"
    "    gn.gain.linearRampToValueAtTime(g,t+.03);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+dur);\n"
    "    gn.connect(drv||gainM);\n"
    "    var dts=[-30,0,30,-1200],os=[];\n"
    "    for(var i=0;i<4;i++){\n"
    "      var o=actx.createOscillator();\n"
    "      o.type='sawtooth';\n"
    "      var f2=fq*Math.pow(2,dts[i]/1200);\n"
    "      o.frequency.setValueAtTime(f2*1.122,t);\n"
    "      o.frequency.exponentialRampToValueAtTime(f2,t+.09);\n"
    "      o.connect(gn);o.start(t);o.stop(t+dur+.02);\n"
    "      os.push(o);\n"
    "    }\n"
    "    vreg(t,os);\n"
    "  }\n"
    "  /* the reese: two saws a hair apart through one lowpass -\n"
    "     the beating IS the bass */\n"
    "  function reese(t,fq,g,dur){\n"
    "    var f=actx.createBiquadFilter(),gn=actx.createGain();\n"
    "    f.type='lowpass';f.frequency.value=520;f.Q.value=1.1;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+dur);\n"
    "    f.connect(gn);gn.connect(gainM);\n"
    "    var os=[];\n"
    "    for(var i=0;i<2;i++){\n"
    "      var o=actx.createOscillator();\n"
    "      o.type='sawtooth';\n"
    "      o.frequency.value=fq*Math.pow(2,(i?12:-12)/1200);\n"
    "      o.connect(f);o.start(t);o.stop(t+dur+.02);\n"
    "      os.push(o);\n"
    "    }\n"
    "    vreg(t,os);\n"
    "  }\n"
    "  /* genre bass with its own snapping filter: the master chain\n"
    "     stays out of it, so the hats stay crisp */\n"
    "  function gbassF(t,fq,g,d2,f0,f1){\n"
    "    var o=actx.createOscillator(),f=actx.createBiquadFilter();\n"
    "    var gn=actx.createGain();\n"
    "    o.type='sawtooth';o.frequency.value=fq;\n"
    "    f.type='lowpass';f.Q.value=2.6;\n"
    "    f.frequency.setValueAtTime(f0,t);\n"
    "    f.frequency.setTargetAtTime(f1,t,.04);\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+d2);\n"
    "    o.connect(f);f.connect(gn);gn.connect(gainM);\n"
    "    o.start(t);o.stop(t+d2+.02);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* the stab: a chord hit that leaves before it overstays.\n"
    "     house offbeats, korg M1 in spirit */\n"
    "  function stab(t,ns,g,dur){\n"
    "    var gn=actx.createGain();\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+dur);\n"
    "    gn.connect(panTo(0));\n"
    ...
    "      var o=actx.createOscillator();\n"
    "      o.type='sawtooth';\n"
    "      o.frequency.value=mf(ns[i]+12)*Math.pow(2,(i*5-5)/1200);\n"
    "      o.connect(gn);o.start(t);o.stop(t+dur+.02);\n"
    "      os.push(o);\n"
    "    }\n"
    "    vreg(t,os);\n"
    "  }\n"
    "  /* dubstep's snare is furniture: noise on top, a falling\n"
    "     tone in the middle */\n"
    "  function bigSnare(t,g){\n"
    "    var s=actx.createBufferSource(),gn=actx.createGain();\n"
    "    s.buffer=nzBuf();s.loop=true;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.17);\n"
    "    s.connect(gn);gn.connect(gainM);\n"
    "    s.start(t);s.stop(t+.18);\n"
    "    var o=actx.createOscillator(),g2=actx.createGain();\n"
    "    o.type='triangle';\n"
    "    o.frequency.setValueAtTime(210,t);\n"
    "    o.frequency.exponentialRampToValueAtTime(165,t+.09);\n"
    "    g2.gain.setValueAtTime(g*.9,t);\n"
    "    g2.gain.exponentialRampToValueAtTime(.001,t+.11);\n"
    "    o.connect(g2);g2.connect(gainM);\n"
    "    o.start(t);o.stop(t+.12);\n"
    "    vreg(t,[s,o]);\n"
    "  }\n"
    "  /* the drone: ambient's floor. two sines a few cents apart,\n"
    "     breathing under everything */\n"
    "  function drone(t,fq,dur){\n"
    "    var gn=actx.createGain();\n"
    "    gn.gain.setValueAtTime(.0001,t);\n"
    "    gn.gain.linearRampToValueAtTime(.02,t+dur*.3);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+dur);\n"
    "    gn.connect(gainM);\n"
    "    var os=[];\n"
    "    for(var i=0;i<2;i++){\n"
    "      var o=actx.createOscillator();\n"
    "      o.type='sine';\n"
    "      o.frequency.value=fq*Math.pow(2,(i?6:0)/1200);\n"
    "      o.connect(gn);o.start(t);o.stop(t+dur+.05);\n"
    "      os.push(o);\n"
    "    }\n"
    "    vreg(t,os);\n"
    "  }\n"
    "  /* visual playhead: mark the byte whose note is sounding NOW.\n"
    "     notes are scheduled ahead on the audio clock, so a rAF loop\n"
    "     drains the queue against actx.currentTime instead of marking\n"
    "     at schedule time */\n"
    "  function sonClear(){\n"
    "    if(sonEl){sonEl.classList.remove('son-now');sonEl=null;}\n"
    "    sonQ=[];\n"
    "  }\n"
    "  function sonMark(i){\n"
    "    if(sonEl) sonEl.classList.remove('son-now');\n"
    "    sonEl=document.querySelector(\n"
    "      '#cmap-svg rect[data-i=\"'+i+'\"]');\n"
    "    if(sonEl) sonEl.classList.add('son-now');\n"
    "    if(window.hexHighlight) window.hexHighlight(i);\n"
    "  }\n"
    "  /* the beat bus: when sonify plays a byte, everyone hears it.\n"
    "     backdrops subscribe with onBeat(fn); fn gets the event the\n"
    "     tracker gets - byte index i, cost c, step s, audio time t.\n"
    "     cost>=8 is the snare, same threshold the drum uses */\n"
    "  var beatFns=[];\n"
    "  function onBeat(fn){beatFns.push(fn);}\n"
    "  function beatEmit(e){\n"
    "    for(var i=0;i<beatFns.length;i++){\n"
    "      try{beatFns[i](e);}catch(x){}\n"
    "    }\n"
    "  }\n"
    "  window.partyOnBeat=onBeat;\n"
    "  function sonTick(){\n"
    "    if(!sonify||!actx||\n"
    "       !document.body.classList.contains('party')){\n"
    "      sonRaf=null;sonClear();return;\n"
    "    }\n"
    "    var now=actx.currentTime,curE=null;\n"
    "    while(sonQ.length&&sonQ[0].t<=now) curE=sonQ.shift();\n"
    "    if(curE){sonMark(curE.i);trkRender(curE);beatEmit(curE);}\n"
    "    sonRaf=requestAnimationFrame(sonTick);\n"
    "  }\n"
    "  /* paqtracker: the pattern view for sonify. the playhead row is\n"
    "     fixed protracker-style and the file scrolls through it. rows\n"
    "     above are history drained from the audio queue (what was\n"
    "     actually heard); rows below are computed, not queued - the\n"
    "     sequence is deterministic from (step, byte cost) */\n"
    "  var trkEl=null,trkRows=null,trkHist=[];\n"
    "  var TRK_N=11;\n"
    "  var TRK_CHD=['AM','F','C','G'];\n"
    "  var TRK_NN=['C-','C#','D-','D#','E-','F-','F#',\n"
    "    'G-','G#','A-','A#','B-'];\n"
    "  function trkName(n){return TRK_NN[n%%12]+((n/12|0)-1);}\n"
    "  /* effect column: command digit by cost band, param = cost as a\n"
    "     hex byte (cost*16 clamped). pain reads as Dxx, like it should */\n"
    "  function trkFx(c){\n"
    "    var p=Math.min(255,(c*16)|0);\n"
    "    var cmd=c<1.5?'0':c<3?'1':c<5?'3':c<8?'4':'D';\n"
    "    return cmd+(p<16?'0':'')+p.toString(16).toUpperCase();\n"
    "  }\n"
    "  /* the ft2 cell, honestly: instrument = the band channel that\n"
    "     earned the most on this byte (01-08, in band order), volume\n"
    "     = its attribution nibble as an ft2 volume (x4, topping out\n"
    "     at 3C). no model earned = no instrument, like a real\n"
    "     pattern's empty columns */\n"
    "  function trkInst(i){\n"
    "    if(typeof BAND==='undefined'||!BAND.ch.length) return null;\n"
    "    var bi=-1,bv=0;\n"
    "    for(var ci=0;ci<BAND.ch.length;ci++){\n"
    "      var q=bandQ(i,ci);\n"
    "      if(q>bv){bv=q;bi=ci;}\n"
    "    }\n"
    "    return bi<0?null:{n:bi+1,q:bv};\n"
    "  }\n"
    "  function trkBuild(){\n"
    "    if(trkEl) return;\n"
    "    trkEl=document.getElementById('paqtrk');\n"
    "    if(!trkEl) return;\n"
    "    trkEl.innerHTML='<div class=\"pt-head\">PAQTRACKER'\n"
    "      +'<span id=\"pt-pat\"></span></div>'\n"
    "      +'<div id=\"pt-rows\"></div>';\n"
    "    trkRows=document.getElementById('pt-rows');\n"
    "    trkEl.querySelector('.pt-head').addEventListener('click',\n"
    "      function(){egg('tracker');showToast('ONE CHANNEL OF TRUTH');});\n"
    "    dragBox(trkEl,trkEl.querySelector('.pt-head'),true);\n"
    "    trkRows.addEventListener('click',function(e){\n"
    "      var r=e.target.closest('[data-i]'); if(!r) return;\n"
    "      /* a click is a seek, like in a real tracker; the queued\n"
    "         future dies and playback lands on the clicked row.\n"
    "         the hex view follows along, as before */\n"
    "      var i2=+r.getAttribute('data-i');\n"
    "      sonPos=i2;\n"
    "      sonFlush();\n"
    "      if(window.hexHighlight) window.hexHighlight(i2);\n"
    "    });\n"
    "  }\n"
    "  /* while the tuner is open, the song is the tuned file: costs,\n"
    "     bytes, band - everything reads the working copy */\n"
    "  function srcC(i){\n"
    "    if(!(tuOn&&tuCN)) return BD[i].c;\n"
    "    return (tuAB&&tuCB0)?tuCB0[i]:tuCN[i]; /* held O = original */\n"
    "  }\n"
    "  function srcH(i){\n"
    "    if(!(tuOn&&tuBytes)) return BD[i].h;\n"
    "    return hx2((tuAB&&tuBase)?tuBase[i]:tuBytes[i]);\n"
    "  }\n"
    "  function trkRow(i,s,c,cur){\n"
    "    var clr=c<3?'#34d399':c<6?'#fbbf24':'#f87171';\n"
    "    /* row number the tracker way: 00-3F within the pattern.\n"
    "       the full offset rides on data-i for the seek and the\n"
    "       hex view. every 4th row shades, every 16th more, and a\n"
    "       pattern boundary rules a line - the meter, visible */\n"
    "    var off=hx2(i&63);\n"
    "    var bt=(i&63)===0?' pb b16':(i&15)===0?' b16'\n"
    "      :(i&3)===0?' b4':'';\n"
    "    if(bandOn&&typeof BAND!=='undefined'){\n"
    "      /* band layout: one column per model channel; dots mean\n"
    "         the model sat this byte out */\n"
    "      var h2='<div class=\"pt-row'+bt+(cur?' cur':'')\n"
    "        +'\" data-i=\"'+i+'\">'\n"
    "        +'<span class=\"o\">'+off+'</span>';\n"
    "      for(var ci=0;ci<BAND.ch.length;ci++){\n"
    "        var q=bandGate(ci,s)?bandQ(i,ci):0;\n"
    "        h2+='<span style=\"width:38px;color:'+BANDC[ci]\n"
    "          +(q?'\">'+(syn.pts?b2Lbl(ci,s,c)\n"
    "            :trkName(bandNote(ci,s)))\n"
    "            :'\";opacity:.35\">...')\n"
    "          +'</span>';\n"
    "      }\n"
    ...
    "    }\n"
    "    var ins=trkInst(i);\n"
    "    return '<div class=\"pt-row'+bt+(cur?' cur':'')+'\" data-i=\"'+i+'\">'\n"
    "      +'<span class=\"o\">'+off+'</span>'\n"
    "      +'<span class=\"nt\">'+trkName(noteFor(s,c))+'</span>'\n"
    "      +'<span class=\"in\">'+(ins?hx2(ins.n):'..')+'</span>'\n"
    "      +'<span class=\"vl\">'+(ins?hx2(ins.q*4):'..')+'</span>'\n"
    "      +'<span class=\"by\">'+srcH(i)+'</span>'\n"
    "      +'<span class=\"co\" style=\"color:'+clr+'\">'+c.toFixed(1)+'</span>'\n"
    "      +'<span class=\"fx\">'+trkFx(c)+'</span>'\n"
    "      +'</div>';\n"
    "  }\n"
    "  function trkRender(e){\n"
    "    if(!trkEl||!trkRows||!trkEl.classList.contains('on')) return;\n"
    "    trkHist.push(e);\n"
    ...
    "      var h=trkHist[k];\n"
    "      out+=trkRow(h.i,h.s,h.c,k===trkHist.length-1);\n"
    "    }\n"
    "    for(var k=1;k<=TRK_N;k++){\n"
    "      var fi=(e.i+k)%%BD.length;\n"
    "      out+=trkRow(fi,e.s+k,srcC(fi),false);\n"
    "    }\n"
    ...
    "      /* 64 rows to a pattern: the file IS the song sheet */\n"
    "      +' PAT '+hx2(e.i>>6)+' ROW '+hx2(e.i&63);\n"
    ...
    "     a phosphor trace. fade instead of clear for persistence, and\n"
    "     trigger on a rising zero-crossing so the waveform holds still\n"
    "     like a real scope. muted = flatline, which is also correct */\n"
    "  var scopeOn=false,scopeAn=null,scopeRaf=null;\n"
    "  var scopeCv=null,scopeCtx=null,scopeBuf=null,scopeG=0;\n"
    "  function scopeToggle(){\n"
    "    if(scopeOn){scopeStop();showToast('SCOPE PACKED AWAY');return;}\n"
    ...
    "        gainM.connect(scopeAn);\n"
    "      }catch(e){scopeAn=null;return;}\n"
    "      scopeBuf=new Uint8Array(scopeAn.fftSize);\n"
    "      scopeCv=document.getElementById('scope-cv');\n"
    "      scopeCtx=scopeCv.getContext('2d');\n"
    "    }\n"
    "    egg('scope');\n"
    "    scopeOn=true;\n"
    "    document.getElementById('paqscope').classList.add('on');\n"
    "    showToast('CALIBRATING PHOSPHOR');\n"
    "    if(!scopeRaf) scopeRaf=requestAnimationFrame(scopeFrame);\n"
    "  }\n"
    "  function scopeStop(){\n"
    "    scopeOn=false;\n"
    "    var p=document.getElementById('paqscope');\n"
    "    if(p) p.classList.remove('on');\n"
    "    if(scopeRaf){cancelAnimationFrame(scopeRaf);scopeRaf=null;}\n"
    "  }\n"
    "  function scopeFrame(){\n"
    "    if(!scopeOn||!document.body.classList.contains('party')){\n"
    "      scopeStop();return;\n"
    "    }\n"
    "    var W=scopeCv.width,H=scopeCv.height;\n"
    "    scopeAn.getByteTimeDomainData(scopeBuf);\n"
    "    scopeCtx.shadowBlur=0;\n"
    "    scopeCtx.fillStyle='rgba(0,0,4,.3)';\n"
    "    scopeCtx.fillRect(0,0,W,H);\n"
    "    /* pt skin: the quadrascope was FFDD00, so the trace obeys */\n"
    "    var qc=ptOn?'#ffdd00':'#34d399';\n"
    "    scopeCtx.fillStyle=ptOn?'rgba(255,221,0,.10)'\n"
    "      :'rgba(52,211,153,.10)';\n"
    "    scopeCtx.fillRect(0,H>>1,W,1);\n"
    "    var n=scopeBuf.length,start=0,peak=0;\n"
    "    for(var i=0;i<n;i++){\n"
    "      var d=scopeBuf[i]-128; if(d<0)d=-d; if(d>peak)peak=d;\n"
    "    }\n"
    "    /* auto-ranging: the chiptune is mixed quiet (a few counts of\n"
    "       deflection at full volume), so normalize the trace to the\n"
    "       current peak, smoothed so the gain breathes instead of\n"
    "       jumping. true silence decays to a flatline rather than\n"
    "       amplifying quantization noise */\n"
    "    var tg=peak>=2?(((H>>1)-4)/peak):0;\n"
    "    if(tg>12) tg=12;\n"
    "    scopeG=scopeG*.85+tg*.15;\n"
    "    for(var i=1;i<(n>>1);i++){\n"
    "      if(scopeBuf[i-1]<128&&scopeBuf[i]>=128){start=i;break;}\n"
    "    }\n"
    "    scopeCtx.strokeStyle=qc;\n"
    "    scopeCtx.shadowColor=qc;scopeCtx.shadowBlur=4;\n"
    "    scopeCtx.lineWidth=1;\n"
    "    scopeCtx.beginPath();\n"
    "    var span=n>>1;\n"
    "    for(var x=0;x<W;x++){\n"
    "      var y=(H>>1)+(scopeBuf[start+((x*span/W)|0)]-128)*scopeG;\n"
    "      if(y<2)y=2; if(y>H-2)y=H-2;\n"
    "      if(x) scopeCtx.lineTo(x,y); else scopeCtx.moveTo(x,y);\n"
    "    }\n"
    "    scopeCtx.stroke();\n"
    "    scopeRaf=requestAnimationFrame(scopeFrame);\n"
    "  }\n"
    "  /* spectrum: the same tap as the scope, read the other way -\n"
    "     bars are energy per log-spaced band, and the yellow caps are\n"
    "     peak-hold, letting each hit down slowly like a hi-fi that\n"
    "     remembers */\n"
    "  var specOn=false,specAn=null,specRaf=null;\n"
    "  var specCv=null,specCtx=null,specBuf=null,specPk=null,specBands=null;\n"
    "  function specBands0(){\n"
    "    var sr=actx.sampleRate,n2=specAn.frequencyBinCount;\n"
    "    var f0=50,f1=Math.min(9000,sr/2),NB=20;\n"
    "    specBands=[];\n"
    "    for(var b=0;b<NB;b++){\n"
    "      var lo=f0*Math.pow(f1/f0,b/NB);\n"
    "      var hi=f0*Math.pow(f1/f0,(b+1)/NB);\n"
    "      var i0=Math.max(1,Math.round(lo*specAn.fftSize/sr));\n"
    "      var i1=Math.max(i0+1,Math.round(hi*specAn.fftSize/sr));\n"
    "      if(i1>n2) i1=n2;\n"
    "      specBands.push([i0,i1]);\n"
    "    }\n"
    "    specPk=new Float32Array(NB);\n"
    "  }\n"
    "  function specToggle(){\n"
    "    if(specOn){specStop();showToast('THE BARS REST');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    ensureAudio();\n"
    "    if(!actx) return;\n"
    "    if(!specAn){\n"
    "      try{\n"
    "        specAn=actx.createAnalyser();\n"
    "        specAn.fftSize=2048;\n"
    "        specAn.smoothingTimeConstant=.7;\n"
    "        gainM.connect(specAn);\n"
    "      }catch(e){specAn=null;return;}\n"
    "      specBuf=new Uint8Array(specAn.frequencyBinCount);\n"
    "      specCv=document.getElementById('spec-cv');\n"
    "      specCtx=specCv.getContext('2d');\n"
    "      specBands0();\n"
    "    }\n"
    "    egg('winamp');\n"
    "    specOn=true;\n"
    "    document.getElementById('paqspec').classList.add('on');\n"
    "    showToast('IT REALLY WHIPS THE LLAMA\\'S ASS');\n"
    "    if(!specRaf) specRaf=requestAnimationFrame(specFrame);\n"
    "  }\n"
    "  function specStop(){\n"
    "    specOn=false;\n"
    "    var p=document.getElementById('paqspec');\n"
    "    if(p) p.classList.remove('on');\n"
    "    if(specRaf){cancelAnimationFrame(specRaf);specRaf=null;}\n"
    "  }\n"
    "  function specFrame(){\n"
    "    if(!specOn||!document.body.classList.contains('party')){\n"
    "      specStop();return;\n"
    "    }\n"
    "    var W=specCv.width,H=specCv.height;\n"
    "    specAn.getByteFrequencyData(specBuf);\n"
    "    specCtx.fillStyle='rgba(0,0,4,.45)';\n"
    "    specCtx.fillRect(0,0,W,H);\n"
    "    var NB=specBands.length,bw=W/NB;\n"
    "    for(var b=0;b<NB;b++){\n"
    "      var i0=specBands[b][0],i1=specBands[b][1],s=0;\n"
    "      for(var i=i0;i<i1;i++) s+=specBuf[i];\n"
    "      var v=s/(i1-i0)/255;\n"
    "      var h=v*(H-6);\n"
    "      /* the cap rides the bar up, then falls on its own */\n"
    "      if(h>specPk[b]) specPk[b]=h;\n"
    "      else{specPk[b]-=1.1;if(specPk[b]<h)specPk[b]=h;\n"
    "        if(specPk[b]<0)specPk[b]=0;}\n"
    "      var x=(b*bw+1)|0,w2=(bw-2)|0;\n"
    "      if(h>=1){\n"
    "        specCtx.fillStyle='rgba(34,211,238,'+(.35+.65*v).toFixed(2)+')';\n"
    "        specCtx.fillRect(x,H-2-h,w2,h);\n"
    "      }\n"
    "      specCtx.fillStyle='#ffe33f';\n"
    "      specCtx.fillRect(x,H-4-specPk[b],w2,2);\n"
    "    }\n"
    "    specRaf=requestAnimationFrame(specFrame);\n"
    "  }\n"
    "  /* spectre: the spectrogram, live. frequency runs up the\n"
    "     column, time slides left, and the palette climbs black -\n"
    "     blue - cyan - pink - yellow, coldest to loudest. about four\n"
    "     seconds of the file's voice, always leaving */\n"
    "  var sgOn=false,sgAn=null,sgRaf=null;\n"
    "  var sgCv=null,sgCtx=null,sgBuf=null,sgCol=null;\n"
    "  var sgRows=null,sgPal=null;\n"
    "  function sgInit(){\n"
    "    var sr=actx.sampleRate;\n"
    "    var H=sgCv.height,f0=50,f1=Math.min(9000,sr/2);\n"
    "    /* top row is the treble; each row one log step down */\n"
    "    sgRows=new Uint16Array(H);\n"
    "    for(var y=0;y<H;y++){\n"
    "      var fr=f0*Math.pow(f1/f0,(H-1-y)/(H-1));\n"
    "      var i=Math.round(fr*sgAn.fftSize/sr);\n"
    "      if(i<1) i=1;\n"
    "      if(i>=sgAn.frequencyBinCount) i=sgAn.frequencyBinCount-1;\n"
    "      sgRows[y]=i;\n"
    "    }\n"
    "    var stops=[[0,0,0,8],[.3,10,40,90],[.6,34,211,238],\n"
    "      [.85,255,63,216],[1,255,227,63]];\n"
    "    sgPal=[];\n"
    "    for(var v=0;v<256;v++){\n"
    "      var t=v/255,k=1;\n"
    ...
    "      sgPal.push([(a[1]+(b[1]-a[1])*u)|0,\n"
    "        (a[2]+(b[2]-a[2])*u)|0,(a[3]+(b[3]-a[3])*u)|0]);\n"
    "    }\n"
    "    sgCol=sgCtx.createImageData(1,H);\n"
    "  }\n"
    "  function sgToggle(){\n"
    "    if(sgOn){sgStop();showToast('THE RIVER RUNS DRY');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    ensureAudio();\n"
    "    if(!actx) return;\n"
    "    if(!sgAn){\n"
    "      try{\n"
    "        sgAn=actx.createAnalyser();\n"
    "        sgAn.fftSize=2048;\n"
    "        /* less smoothing than the bars: a spectrogram wants\n"
    "           time resolution more than it wants calm */\n"
    "        sgAn.smoothingTimeConstant=.55;\n"
    "        gainM.connect(sgAn);\n"
    "      }catch(e){sgAn=null;return;}\n"
    "      sgBuf=new Uint8Array(sgAn.frequencyBinCount);\n"
    "      sgCv=document.getElementById('sg-cv');\n"
    "      sgCtx=sgCv.getContext('2d');\n"
    "      sgCtx.fillStyle='#000004';\n"
    "      sgCtx.fillRect(0,0,sgCv.width,sgCv.height);\n"
    "      sgInit();\n"
    "    }\n"
    "    egg('spectre');\n"
    "    sgOn=true;\n"
    "    document.getElementById('paqsg').classList.add('on');\n"
    "    showToast('FOUR SECONDS OF MEMORY');\n"
    "    if(!sgRaf) sgRaf=requestAnimationFrame(sgFrame);\n"
    "  }\n"
    "  function sgStop(){\n"
    "    sgOn=false;\n"
    "    var p=document.getElementById('paqsg');\n"
    "    if(p) p.classList.remove('on');\n"
    "    if(sgRaf){cancelAnimationFrame(sgRaf);sgRaf=null;}\n"
    "  }\n"
    "  function sgFrame(){\n"
    "    if(!sgOn||!document.body.classList.contains('party')){\n"
    "      sgStop();return;\n"
    "    }\n"
    "    var W=sgCv.width,H=sgCv.height;\n"
    "    sgAn.getByteFrequencyData(sgBuf);\n"
    "    /* the past slides left by one; the present is one column */\n"
    "    sgCtx.drawImage(sgCv,-1,0);\n"
    "    var d=sgCol.data;\n"
    "    for(var y=0;y<H;y++){\n"
    "      var p=sgPal[sgBuf[sgRows[y]]];\n"
    "      var o=y*4;\n"
    "      d[o]=p[0];d[o+1]=p[1];d[o+2]=p[2];d[o+3]=255;\n"
    "    }\n"
    "    sgCtx.putImageData(sgCol,W-1,0);\n"
    "    sgRaf=requestAnimationFrame(sgFrame);\n"
    "  }\n"
    "  /* vectorscope: left against right, the broadcast way. mono is\n"
    "     a diagonal line; type stereo and the field blooms into\n"
    "     figures. the one instrument that can see the pan */\n"
    "  var vecOn=false,vecRaf=null,vecSplit=null;\n"
    "  var vecAnL=null,vecAnR=null,vecBL=null,vecBR=null;\n"
    "  var vecCv=null,vecCtx=null,vecG=0;\n"
    "  function vecToggle(){\n"
    "    if(vecOn){vecStop();showToast('FLATLAND AGAIN');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    ensureAudio();\n"
    "    if(!actx) return;\n"
    "    if(!vecSplit){\n"
    "      try{\n"
    "        vecSplit=actx.createChannelSplitter(2);\n"
    "        vecAnL=actx.createAnalyser();\n"
    "        vecAnR=actx.createAnalyser();\n"
    "        vecAnL.fftSize=1024;vecAnR.fftSize=1024;\n"
    "        gainM.connect(vecSplit);\n"
    "        vecSplit.connect(vecAnL,0);\n"
    "        vecSplit.connect(vecAnR,1);\n"
    "      }catch(e){vecSplit=null;return;}\n"
    "      vecBL=new Uint8Array(vecAnL.fftSize);\n"
    "      vecBR=new Uint8Array(vecAnR.fftSize);\n"
    "      vecCv=document.getElementById('vec-cv');\n"
    "      vecCtx=vecCv.getContext('2d');\n"
    "    }\n"
    "    egg('vector');\n"
    "    vecOn=true;\n"
    "    document.getElementById('paqvec').classList.add('on');\n"
    "    showToast('L MEETS R');\n"
    "    if(!vecRaf) vecRaf=requestAnimationFrame(vecFrame);\n"
    "  }\n"
    "  function vecStop(){\n"
    "    vecOn=false;\n"
    "    var p=document.getElementById('paqvec');\n"
    "    if(p) p.classList.remove('on');\n"
    "    if(vecRaf){cancelAnimationFrame(vecRaf);vecRaf=null;}\n"
    "  }\n"
    "  function vecFrame(){\n"
    "    if(!vecOn||!document.body.classList.contains('party')){\n"
    "      vecStop();return;\n"
    "    }\n"
    "    var W=vecCv.width,H=vecCv.height;\n"
    "    vecAnL.getByteTimeDomainData(vecBL);\n"
    "    vecAnR.getByteTimeDomainData(vecBR);\n"
    "    vecCtx.shadowBlur=0;\n"
    "    vecCtx.fillStyle='rgba(0,0,4,.25)';\n"
    "    vecCtx.fillRect(0,0,W,H);\n"
    "    /* faint axes: the crosshair of the field */\n"
    "    vecCtx.fillStyle='rgba(34,211,238,.10)';\n"
    "    vecCtx.fillRect(0,H>>1,W,1);\n"
    "    vecCtx.fillRect(W>>1,0,1,H);\n"
    "    var n=vecBL.length,peak=0;\n"
    "    for(var i=0;i<n;i++){\n"
    "      var dl=vecBL[i]-128; if(dl<0)dl=-dl; if(dl>peak)peak=dl;\n"
    "      var dr=vecBR[i]-128; if(dr<0)dr=-dr; if(dr>peak)peak=dr;\n"
    "    }\n"
    "    /* auto-range like the scope: quiet chiptune, smoothed gain,\n"
    "       silence decays to a dot instead of amplified noise */\n"
    "    var half=(W<H?W:H)/2;\n"
    "    var tg=peak>=2?((half-6)/peak):0;\n"
    "    if(tg>12) tg=12;\n"
    "    vecG=vecG*.85+tg*.15;\n"
    "    vecCtx.strokeStyle='#22d3ee';\n"
    "    vecCtx.shadowColor='#22d3ee';vecCtx.shadowBlur=4;\n"
    "    vecCtx.lineWidth=1;\n"
    "    vecCtx.beginPath();\n"
    "    for(var i=0;i<n;i++){\n"
    "      var x=(W>>1)+(vecBL[i]-128)*vecG;\n"
    "      var y=(H>>1)-(vecBR[i]-128)*vecG;\n"
    "      if(i) vecCtx.lineTo(x,y); else vecCtx.moveTo(x,y);\n"
    "    }\n"
    "    vecCtx.stroke();\n"
    "    vecRaf=requestAnimationFrame(vecFrame);\n"
    "  }\n"
    "  /* vinyl scratch: the ring is a record, and records are meant\n"
    "     to be touched. dragging takes the platter off the motor -\n"
    "     the scheduler waits, sonPos follows the hand, pitch is\n"
    "     angular velocity. let go and the file plays on from\n"
    "     wherever the needle landed */\n"
    "  var vsRing=document.querySelector('.hero-ring');\n"
    "  var vsOn=false,vsAng=0,vsRem=0,vsRot=0,vsMoved=0,vsEgged=false;\n"
    "  var vsSkip=false,vsPct=null,vsLastB=0;\n"
    "  function vsAngle(e){\n"
    "    var r=vsRing.getBoundingClientRect();\n"
    ...
    "    if(!vsOn) return;\n"
    "    vsOn=false;\n"
    "    /* hand off: the inline pose clears and the motor spins on */\n"
    "    if(vsPct){vsPct.style.animation='';vsPct.style.transform='';}\n"
    "    if(sonify&&typeof BD!=='undefined'&&BD.length)\n"
    "      showToast('NEEDLE DOWN AT '\n"
    "        +Math.round(100*sonPos/BD.length)+'%%');\n"
    "  }\n"
    "  if(vsRing){\n"
    "    vsRing.addEventListener('pointerdown',function(e){\n"
    "      if(!document.body.classList.contains('party')) return;\n"
    "      if(!vsRing.classList.contains('vinyl')) return;\n"
    "      if(typeof BD==='undefined'||!BD.length) return;\n"
    "      vsOn=true;vsAng=vsAngle(e);vsRem=0;\n"
    "      vsPct=vsRing.querySelector('.hero-pct');\n"
    "      /* the platter comes off the motor: an inline transform\n"
    "         outranks the spin animation until the hand lets go */\n"
    "      if(vsPct){vsPct.style.animation='none';\n"
    "        vsPct.style.transform='rotate('+vsRot+'deg)';}\n"
    "      try{vsRing.setPointerCapture(e.pointerId);}catch(x){}\n"
    "      ensureAudio();\n"
    "      e.preventDefault();\n"
    "    });\n"
    "    vsRing.addEventListener('pointermove',function(e){\n"
    "      if(!vsOn) return;\n"
    "      var a=vsAngle(e),d=a-vsAng;\n"
    "      vsAng=a;\n"
    "      if(d>180)d-=360; if(d<-180)d+=360;\n"
    "      if(!d) return;\n"
    "      vsRot+=d;vsSkip=true;\n"
    "      if(vsPct) vsPct.style.transform='rotate('+vsRot+'deg)';\n"
    "      /* one revolution of wax = 192 bytes under the needle */\n"
    "      vsRem+=d*192/360;\n"
    "      var step=vsRem|0;\n"
    "      if(!step) return;\n"
    "      vsRem-=step;\n"
    "      var L=BD.length;\n"
    "      sonPos=((sonPos+step)%%L+L)%%L;\n"
    "      vsMoved+=step<0?-step:step;\n"
    "      if(vsMoved>=48&&!vsEgged){vsEgged=true;egg('scratch');\n"
    "        showToast('HANDS ON THE WAX');}\n"
    "      var now=Date.now();\n"
    "      if(now-vsLastB<24) return; /* one voice, not a pileup */\n"
    "      vsLastB=now;\n"
    "      var c=BD[sonPos].c;\n"
    "      var v=d<0?-d:d; if(v>26)v=26;\n"
    "      /* pitch: the groove's pain bent by how hard the hand\n"
    "         drags; backward spins read flat, like they should */\n"
    "      blip((150+c*60+v*24)*(step<0?.8:1),.04,.06,panOf(sonPos));\n"
    "      var ev={t:actx?actx.currentTime:0,i:sonPos,s:mstep,c:c};\n"
    "      sonMark(sonPos);trkRender(ev);beatEmit(ev);\n"
    "    });\n"
    "    vsRing.addEventListener('pointerup',vsUp);\n"
    "    vsRing.addEventListener('pointercancel',vsUp);\n"
    "  }\n"
    "  /* stereo: a place in the file is a place in the field. byte\n"
    "     zero sits hard left, the last byte hard right, the band\n"
    "     spreads across the desk, and the bass holds the center\n"
    "     like a bass should */\n"
    "  var stereoOn=false;\n"
    "  function panTo(p){\n"
    "    if(!stereoOn||!p) return gainM;\n"
    "    try{\n"
    "      var pn=actx.createStereoPanner();\n"
    "      pn.pan.value=p<-1?-1:p>1?1:p;\n"
    "      pn.connect(gainM);\n"
    "      return pn;\n"
    "    }catch(e){return gainM;}\n"
    "  }\n"
    "  function panOf(i){\n"
    "    return (typeof BD!=='undefined'&&BD.length>1)?\n"
    "      ((i/(BD.length-1))*2-1)*syn.pw:0;\n"
    "  }\n"
    "  function bandPan(ci){\n"
    "    var n2=BAND.ch.length;\n"
    "    return n2>1?((ci/(n2-1))*2-1)*syn.pw:0;\n"
    "  }\n"
    "  function stereoToggle(){\n"
    "    stereoOn=!stereoOn;\n"
    "    if(stereoOn) egg('stereo');\n"
    "    showToast(stereoOn?'IN GLORIOUS STEREO':'BACK TO MONO');\n"
    "  }\n"
    "  /* the voice registry: every scheduled source is remembered\n"
    "     with its start time so a flip can murder the future - a\n"
    "     source stopped before it starts never sounds. this is what\n"
    "     makes A/B and cueing instant instead of one queue late */\n"
    "  var sonV=[];\n"
    "  function vreg(t,ns){sonV.push({t:t,n:ns});}\n"
    "  /* shared amp envelope: optional attack ramp, then the decay.\n"
    "     atk 0 collapses to the old instant pluck */\n"
    "  function envA(gn,pk,t,dur){\n"
    "    gn.gain.setValueAtTime(.0001,t);\n"
    "    gn.gain.linearRampToValueAtTime(pk,t+syn.atk);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+syn.atk+dur);\n"
    "    return t+syn.atk+dur+.01;\n"
    "  }\n"
    "  function sonFlush(){\n"
    "    if(!actx) return;\n"
    "    var now=actx.currentTime;\n"
    "    for(var i=0;i<sonV.length;i++){\n"
    "      var v=sonV[i];\n"
    "      if(v.t>now+0.03){\n"
    "        for(var k=0;k<v.n.length;k++){\n"
    "          try{v.n[k].stop(0);}catch(e){}\n"
    "          try{v.n[k].disconnect();}catch(e){}\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    sonV=[];\n"
    "    sonQ=[]; /* the beat bus forgets the killed future too */\n"
    "    nextT=now+0.03;\n"
    "    tick(); /* refill immediately from the flipped source */\n"
    "  }\n"
    "  /* opl2: two-operator fm. the modulator leans on the carrier's\n"
    "     pitch; how hard it leans is the timbre. cheap bytes ring\n"
    "     nearly pure, expensive bytes go full dial-up. each band\n"
    "     channel gets its own ratio - the integer seats sound like an\n"
    "     organ, the others like bells with something to prove */\n"
    "  var fmOn=false;\n"
    "  var FMR=[1,2,3,1.5,2.01,3.01,1.414,2.5];\n"
    "  function fmVoice(fq,g,dur,t,dest,ratio,idx){\n"
    "    var c2=actx.createOscillator(),m2=actx.createOscillator();\n"
    "    var cg=actx.createGain(),mg=actx.createGain();\n"
    "    c2.type='sine';m2.type='sine';\n"
    "    c2.frequency.value=fq;\n"
    "    m2.frequency.value=fq*ratio;\n"
    "    /* the index decays with the envelope: the classic fm pluck,\n"
    "       bright at the strike, polite by the tail */\n"
    "    mg.gain.setValueAtTime(fq*idx,t);\n"
    "    mg.gain.exponentialRampToValueAtTime(fq*idx*.1+.001,\n"
    "      t+syn.atk+dur);\n"
    "    m2.connect(mg);mg.connect(c2.frequency);\n"
    "    var te=envA(cg,g,t,dur);\n"
    "    c2.connect(cg);cg.connect(dest);\n"
    "    m2.start(t);c2.start(t);\n"
    "    m2.stop(te);c2.stop(te);\n"
    "    vreg(t,[c2,m2]);\n"
    "  }\n"
    "  function fmToggle(){\n"
    "    fmOn=!fmOn;\n"
    "    if(fmOn) egg('opl2');\n"
    "    showToast(fmOn?'TWO OPERATORS, NO WAITING'\n"
    "      :'BACK TO THE SQUARES');\n"
    "  }\n"
    ...
    "  function bandQ(i,ci){\n"
    "    /* tuner open: live attribution from the forward pass, same\n"
    "       15/4 quantizer the C side used to bake BAND.q */\n"
    "    if(tuOn&&tuM&&tuBandMap){\n"
    "      var hm=tuBandMap[ci];\n"
    "      if(hm>=0){\n"
    "        var q2=(tuM[i*tuNM+hm]*3.75+0.5)|0;\n"
    "        return q2<0?0:q2>15?15:q2;\n"
    "      }\n"
    "    }\n"
    "    var p=i*BAND.ch.length+ci;\n"
    "    return p<BAND.q.length?(parseInt(BAND.q[p],16)||0):0;\n"
    "  }\n"
    "  /* the mask IS the drum pattern: bit 7 first, one mask bit\n"
    "     per two steps of the bar - a set bit is permission to\n"
    "     sound. mask 00 has no context to wait for, so it keeps\n"
    "     time on every step */\n"
    "  var bandMk=null;\n"
    "  function bandGate(ci,step){\n"
    "    if(!syn.mg) return 1;\n"
    "    if(!bandMk){\n"
    "      bandMk=[];\n"
    "      for(var k=0;k<BAND.ch.length;k++)\n"
    "        bandMk.push(parseInt(BAND.ch[k],16)||0);\n"
    "    }\n"
    "    var mk=bandMk[ci];\n"
    "    return !mk||((mk>>(7-((step>>1)&7)))&1);\n"
    "  }\n"
    ...
    "  }\n"
    "  /* one band voice: fm when the opl2 is on, else the given wave */\n"
    "  function bandVoice(fq,g,dur,t,ci,wave,fi){\n"
    "    if(fmOn){\n"
    "      fmVoice(fq,g+.001,dur,t,panTo(bandPan(ci)),FMR[ci%%8],fi);\n"
    "      return;\n"
    "    }\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type=wave;o.frequency.value=fq;\n"
    "    var te=envA(gn,g+.001,t,dur);\n"
    "    o.connect(gn);gn.connect(panTo(bandPan(ci)));\n"
    "    o.start(t);o.stop(te);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* what the tracker prints when the band plays parts */\n"
    "  function b2Lbl(ci,step,c){\n"
    "    if(ci===1) return trkName((sonKey?sonKey.r:curChord(step)[0])\n"
    "      -12+(((step&15)&8)?7:0));\n"
    "    if(ci>=5) return (ci&1)?'HH':'TM';\n"
    "    if(ci===0) return trkName(noteFor(step,c));\n"
    "    return trkName(bandNote(ci,step));\n"
    "  }\n"
    "  /* the rack follows the widest instrument: when the band\n"
    "     widens the tracker, every canvas re-strings itself to the\n"
    "     same width. resizing clears a canvas, so each gets its\n"
    "     black felt back; the waterfall re-cuts its bed in ~4s */\n"
    "  /* zoom: the whole rack at twice the size. the transform\n"
    "     scales tracker text and canvases alike, and hit-testing\n"
    "     rides along for free */\n"
    "  var zoomOn=false;\n"
    "  function zoomToggle(){\n"
    "    zoomOn=!zoomOn;\n"
    "    var d=document.getElementById('pt-dock');\n"
    "    if(d) d.classList.toggle('zoom',zoomOn);\n"
    "    if(zoomOn) egg('deejay');\n"
    "    /* rack units that left the dock follow the booth zoom */\n"
    "    var fr=document.querySelectorAll('.rk-free');\n"
    "    for(var k=0;k<fr.length;k++){\n"
    "      fr[k].style.transform=zoomOn?'scale(2)':'';\n"
    "      fr[k].style.transformOrigin='0 0';\n"
    "      fr[k].style.imageRendering=zoomOn?'pixelated':'';\n"
    "    }\n"
    "    showToast(zoomOn?'WELCOME TO THE BOOTH':'BACK TO THE FLOOR');\n"
    "  }\n"
    "  var dockW=248;\n"
    "  function dockSync(){\n"
    "    var w=248;\n"
    "    if(bandOn&&typeof BAND!=='undefined'&&trkEl&&\n"
    "       trkEl.classList.contains('wide')){\n"
    "      /* offsetWidth, not getBoundingClientRect: the rect is\n"
    "         post-transform, and under deejay's scale(2) that would\n"
    "         double the canvases again - 4x, spilling past the\n"
    "         tracker. layout width is the honest one */\n"
    "      var m2=trkEl.offsetWidth;\n"
    "      if(m2>248) w=m2;\n"
    "    }\n"
    "    if(w===dockW) return;\n"
    "    dockW=w;\n"
    "    var ids=['scope-cv','spec-cv','sg-cv','vec-cv'];\n"
    "    for(var k=0;k<ids.length;k++){\n"
    "      var cv=document.getElementById(ids[k]);\n"
    "      if(!cv) continue;\n"
    "      cv.width=w;\n"
    "      cv.style.width=w+'px';\n"
    "      var cx2=cv.getContext('2d');\n"
    "      cx2.fillStyle='#000004';\n"
    ...
    "    }\n"
    "    /* no cold opens: the band only joins a song already\n"
    "       playing - listen first, or pin a key */\n"
    "    if(!sonify){\n"
    "      showToast('THE BAND NEEDS A SONG. LISTEN FIRST');return;}\n"
    "    bandOn=true;trkHist=[];\n"
    "    trkBuild();\n"
    ...
    "  var KEYPC={a:9,b:11,c:0,d:2,e:4,f:5,g:7};\n"
    "  var KEYSC=[[0,2,4,5,7,9,11],[0,2,3,5,7,8,10]];\n"
    "  function keyToggle(kw){\n"
    "    var mi=kw.slice(1)==='minor'?1:0;\n"
    "    var nm=kw[0].toUpperCase()+(mi?'MIN':'MAJ');\n"
    "    if(sonify&&sonKey&&sonKey.nm===nm){\n"
    "      /* the same key twice lifts the needle */\n"
    "      sonify=false;sonKey=null;sonClear();trkShow(false);\n"
    "      var hr=document.querySelector('.hero-ring');\n"
    "      if(hr) hr.classList.remove('vinyl');\n"
    "      showToast('BACK TO THE JUKEBOX');return;\n"
    "    }\n"
    "    sonKey={r:48+KEYPC[kw[0]],mi:mi,nm:nm};\n"
    "    egg('inkey');\n"
    "    if(!sonify){\n"
    ...
    "    }\n"
    "    showToast('THE FILE, IN '+kw[0].toUpperCase()+' '\n"
    "      +(mi?'MINOR':'MAJOR'));\n"
    "  }\n"
    "  function noteFor(step,c){\n"
    "    /* genre mode quantizes to A minor unless a key is pinned:\n"
    "       the dance floor forgives everything but chromatics */\n"
    "    var sk=sonKey||(GENRES[syn.gen]?GKEY:null);\n"
    "    if(sk){\n"
    "      var sc=KEYSC[sk.mi];\n"
    "      var dg=c<syn.t1?0:c<syn.t2?[0,2,4][step%%3]\n"
    "        :c<syn.t3?[1,3,5][step%%3]:6;\n"
    "      return sk.r+sc[dg]+24;\n"
    "    }\n"
    "    var ch=curChord(step);\n"
    ...
    "  }\n"
    "  /* the effect column, executable: the same cost bands that\n"
    "     print 0/1/3/4 in the tracker now drive the lead - calm\n"
    "     arpeggiates the chord (0xy), mild slides up a semitone\n"
    "     (1xx), tense glides in from the previous note (3xx),\n"
    "     strained wobbles (4xy). pain keeps its snare. the\n"
    "     pattern stops describing the music and starts being it.\n"
    "     analog lead only - the fm carrier keeps its own rules */\n"
    "  var fxLast=0;\n"
    "  function fxLead(o,fq,t,dur,c,step){\n"
    "    var prev=fxLast;fxLast=fq;\n"
    "    if(!syn.fx||c<0) return;\n"
    "    try{\n"
    "      if(c<syn.t1){\n"
    "        var x=4,y=7;\n"
    "        var sk3=sonKey||(GENRES[syn.gen]?GKEY:null);\n"
    "        if(sk3){var sc2=KEYSC[sk3.mi];x=sc2[2];y=sc2[4];}\n"
    "        else{var ch2=curChord(step);\n"
    "          x=ch2[1]-ch2[0];y=ch2[2]-ch2[0];}\n"
    "        var d3=dur/3;\n"
    "        o.frequency.setValueAtTime(fq,t);\n"
    "        o.frequency.setValueAtTime(fq*Math.pow(2,x/12),t+d3);\n"
    "        o.frequency.setValueAtTime(fq*Math.pow(2,y/12),t+2*d3);\n"
    "      } else if(c<syn.t2){\n"
    "        o.frequency.setValueAtTime(fq,t);\n"
    ...
    "          o.frequency.exponentialRampToValueAtTime(fq,t+dur*.6);\n"
    "        }\n"
    "      } else if(c<syn.st){\n"
    "        /* per-voice vibrato lfo: it lives and dies with the\n"
    "           note, so nothing leaks */\n"
    "        var vo=actx.createOscillator(),vg=actx.createGain();\n"
    "        vo.type='sine';vo.frequency.value=6;\n"
    "        vg.gain.value=fq*.025;\n"
    "        vo.connect(vg);vg.connect(o.frequency);\n"
    "        vo.start(t);vo.stop(t+dur+.05);\n"
    "        vreg(t,[vo]);\n"
    "      }\n"
    "    }catch(e){}\n"
    "  }\n"
    ...
    "    var ch=curChord(step);\n"
    "    var sp=step&15;\n"
    "    if(!sonify&&sonEcoEnd>=0&&dlG){\n"
    "      /* the needle is up: the match echo lets go */\n"
    "      sonEcoEnd=-1;\n"
    "      try{dlG.gain.setTargetAtTime(syn.dw,t,.2);}catch(e){}\n"
    "    }\n"
    "    var GEN=GENRES[syn.gen];\n"
    "    if(GEN){\n"
    "      /* the genre kit: bit sp of each mask says whether that\n"
    "         voice plays this step. the lead stays the file's */\n"
    "      var sk4=sonKey||GKEY;\n"
    "      var goff=gprog(GEN,step,sk4);\n"
    "      var hasP=goff>=0;if(!hasP)goff=0;\n"
    "      var grt=hasP?sk4.r+goff:(sonify&&sonKey?sonKey.r:ch[0]);\n"
    "      /* the chord under this bar: the progression's triad, or\n"
    "         whatever the chip chords were doing */\n"
    "      var gch=hasP?gtriad(sk4,GEN.prog[(step>>4)&3])\n"
    "        :(sonify&&sonKey?[grt,grt+(sk4.mi?3:4),grt+7]:ch);\n"
    "      var bar2=(step>>4)&7;\n"
    "      var b5=(step>>4)&31;\n"
    "      /* the arrangement: 24 bars of groove, a 4-bar breakdown,\n"
    "         the riser, the drop. a set, not a loop */\n"
    "      var brk=GEN.arr&&b5>=24&&b5<28;\n"
    "      var km=GEN.k;\n"
    "      /* speedcore floods the last two bars of every phrase */\n"
    "      if(GEN.fld&&bar2>=6) km=0xFFFF;\n"
    "      if(!brk){\n"
    "      if((km>>sp)&1){\n"
    "        kick(t,GEN.kp[3],GEN.kp[0],GEN.kp[1],GEN.kp[2],\n"
    "          GEN.kp[4]===2?drv2:GEN.kp[4]?drv:0,GEN.kp[5]);\n"
    "        if(GEN.rmb) rumble(t);\n"
    "        if(syn.pmp>0&&post){\n"
    "          try{\n"
    "            post.gain.setValueAtTime(1,t);\n"
    "            post.gain.linearRampToValueAtTime(\n"
    "              1-syn.pmp*.8,t+.03);\n"
    "            post.gain.setTargetAtTime(1,t+.06,.1);\n"
    "          }catch(e){}\n"
    "        }\n"
    "      }\n"
    "      if((GEN.s>>sp)&1){\n"
    "        if(GEN.clp) clap(t,GEN.sg||.05,0);\n"
    "        else if(GEN.rim) rim(t,.032,0);\n"
    "        else if(GEN.bigsn) bigSnare(t,GEN.sg||.06);\n"
    "        else snare(t,0,GEN.sg||.05);\n"
    "      }\n"
    "      /* lofi hats arrive when they arrive */\n"
    "      var hj=GEN.hm?(Math.random()*.016-.008):0;\n"
    "      var hg=GEN.hm?(.7+Math.random()*.5):1;\n"
    "      if((GEN.hc>>sp)&1){\n"
    "        if(GEN.h9) hat909(t+hj,.011*hg,.045,0);\n"
    "        else hat(t+hj,.013*hg,false);\n"
    "      }\n"
    "      if((GEN.ho>>sp)&1){\n"
    "        if(GEN.h9) hat909(t+hj,.017*hg,.22,0);\n"
    "        else hat(t+hj,.02*hg,true);\n"
    "      }\n"
    "      /* techno's third hand: a 3-against-4 tick */\n"
    "      if(GEN.pp3&&((GEN.pp3>>sp)&1)) tom(t,.02,330,.35);\n"
    "      /* the ride: long metal on the eighths */\n"
    "      if(GEN.rd&&((GEN.rd>>sp)&1)) hat909(t,.007,.32,.3);\n"
    "      /* dnb ghosts: the break chops itself, and the odds\n"
    "         ride the file - hard bytes, busy break */\n"
    "      if(GEN.gh&&((0x0880>>sp)&1)){\n"
    "        var gp=sonify&&typeof BD!=='undefined'&&BD.length\n"
    "          ?Math.min(.92,.3+srcC(sonPos)/9):.6;\n"
    "        if(Math.random()<gp)\n"
    "          snare(t,(Math.random()*2-1)*.4,.01+Math.random()*.012);\n"
    "      }\n"
    "      /* the hoover opens every 8-bar phrase */\n"
    "      if(GEN.hv&&bar2===0&&sp===0&&step>15)\n"
    "        hoover(t,mf(grt-12),.055,beatDur*10);\n"
    "      if(GEN.stb&&((GEN.stb>>sp)&1)) stab(t,gch,.022,.16);\n"
    "      if(GEN.pad&&sp===0) pad(t,gch,GEN.pad7);\n"
    "      if(GEN.drn&&sp===0) drone(t,mf(grt-24),beatDur*17);\n"
    "      /* structure: 8-bar phrases. fills close bars 4 and 8,\n"
    "         the riser climbs the last bar, the crash lands the\n"
    "         drop - only for genres that asked */\n"
    "      if(GEN.fl&&(bar2===3||bar2===7)&&sp>=12)\n"
    "        snare(t,0,.02+.012*(sp-12));\n"
    "      if(GEN.zp&&(bar2===3||bar2===7)&&sp===14) zap(t);\n"
    "      if(GEN.rs&&bar2===7) riser(t,sp);\n"
    "      if(GEN.rs&&!GEN.arr&&bar2===0&&sp===0&&step>15) crash(t);\n"
    "      /* hardstyle reverse bass: the offbeat 'uh', driven */\n"
    "      if(GEN.rb&&(sp&3)===2)\n"
    "        gbass(mf(grt-12),.07,.12,t,'sawtooth',drv);\n"
    "      if(GEN.bs==='ob'){\n"
    "        /* offbeat eighths: the house/techno pulse */\n"
    "        if((0x4444>>sp)&1)\n"
    "          gbass(mf(grt-12),GEN.bg||.08,.2,t,GEN.bw2||'square');\n"
    "      } else if(GEN.bs==='psy'){\n"
    "        /* rolling 16ths on every non-kick step, each through\n"
    "           its own snapping filter: the rubber gallop. exempt\n"
    "           from the tape wow - psy bass does not wobble */\n"
    "        if(!((km>>sp)&1))\n"
    "          gbassF(t,440*Math.pow(2,(grt-12-69+syn.tr)/12),.1,\n"
    "            beatDur*.8,950,260);\n"
    "      } else if(GEN.bs==='roll'){\n"
    "        /* trance rolls 16ths with the accent off the beat */\n"
    "        if(!((km>>sp)&1))\n"
    "          gbassF(t,mf(grt-12),(sp&3)===2?.095:.055,.13,\n"
    "            1700,420);\n"
    "      } else if(GEN.bs==='sub'){\n"
    "        if(sp===0||sp===8) gbass(mf(grt-24),.1,.45,t,'sine');\n"
    "      } else if(GEN.bs==='reese'){\n"
    "        if(sp===0||sp===8) reese(t,mf(grt-12),.09,.5);\n"
    "      } else if(GEN.bs==='ton'){\n"
    "        if((km>>sp)&1) gtail(mf(grt-24),t);\n"
    "      } else if(GEN.bs==='wub'){\n"
    "        /* one long note per half-bar; the wobble lfo on the\n"
    "           master filter does the talking. the sub takes the\n"
    "           post road and stays solid under the wub */\n"
    "        if(sp===0||sp===8){\n"
    "          gbass(mf(grt-24),.11,beatDur*7.5,t,'sawtooth');\n"
    "          if(GEN.sub&&post)\n"
    "            gbass(mf(grt-24),.09,beatDur*7.5,t,'sine',post);\n"
    "        }\n"
    "      }\n"
    "      } else {\n"
    "        /* breakdown: drums out, pad and lead carry it. the\n"
    "           master filter closes and takes four bars to open */\n"
    "        if(sp===0){\n"
    "          pad(t,gch,GEN.pad7);\n"
    "          if(b5===24&&filt&&!syn.wr&&!syn.fe){\n"
    "            try{\n"
    ...
    "        if(b5===27) riser(t,sp);\n"
    "      }\n"
    "      if(GEN.arr&&b5===28&&sp===0) crash(t);\n"
    "      /* dubstep re-aims the wub each half-bar: a musical\n"
    "         division, picked by how hard the file fights */\n"
    "      if(GEN.wq&&lfo&&syn.wa>0&&(sp===0||sp===8)){\n"
    "        var wn=[2,3,4,6,8,12][sonify&&typeof BD!=='undefined'\n"
    "          &&BD.length?Math.min(5,(srcC(sonPos)|0)>>1)\n"
    "          :(Math.random()*6)|0];\n"
    "        try{lfo.frequency.setValueAtTime(\n"
    "          wn/(beatDur*8),t);}catch(e){}\n"
    "      }\n"
    "    } else if(syn.gr){\n"
    "      /* the bar is 16 steps, exactly one chord: kick on the\n"
    "         downbeats, backbeat snare, hats on the sixteenths with\n"
    "         one open at the turnaround, a pad under it all */\n"
    "      if(sp===0||sp===8){\n"
    "        kick(t,.09);\n"
    "        if(syn.pmp>0&&post){\n"
    "          /* sidechain: the thump lands, then the bus ducks and\n"
    "             breathes back - the bar pumps */\n"
    "          try{\n"
    "            post.gain.setValueAtTime(1,t);\n"
    "            post.gain.linearRampToValueAtTime(\n"
    "              1-syn.pmp*.8,t+.03);\n"
    ...
    "        }\n"
    "      }\n"
    "      if(sp===4||sp===12) snare(t,0,.026);\n"
    "      hat(t,(sp&1)?.016:.008,sp===14);\n"
    "      if(sp===0) pad(t,ch);\n"
    "    }\n"
    "    var n=ch[step%%3]+24;\n"
    ...
    "    if(sonify&&typeof BD!=='undefined'&&BD.length){\n"
    "      var si=sonPos;\n"
    "      var c=srcC(si);leadC=c;\n"
    "      /* the needle steps by the knob: forward, backward, or\n"
    "         skipping - the file backwards is a different song */\n"
    "      sonPos=((si+syn.str)%%BD.length+BD.length)%%BD.length;\n"
    "      if(sonPos===0) showToast('THE FILE HAS ENDED. IT BEGINS AGAIN');\n"
    "      n=GEN?gnote(step,c,GEN):noteFor(step,c);\n"
    "      notePan=panOf(si);\n"
    "      if(c>=syn.st) snare(t,notePan); /* pain: percussion */\n"
    "      sonQ.push({t:t,i:si,s:step,c:c});\n"
    "      if(!sonRaf) sonRaf=requestAnimationFrame(sonTick);\n"
    "      /* match echo: while the needle rides a backward match\n"
    "         the delay wakes and the copy plays over the ghost of\n"
    "         its source. short matches get the literal tap - the\n"
    "         source really is dist steps back - long ones take\n"
    "         the synced tap. dry again when the region ends */\n"
    "      if(syn.me&&dl&&dlG&&window.repML&&si<window.repML.length){\n"
    "        if(window.repML[si]>=8){\n"
    "          sonEcoLen=window.repML[si];\n"
    "          sonEcoEnd=si+sonEcoLen;\n"
    "          sonEcoDist=window.repMD[si];\n"
    "        }\n"
    "        var inE=si>=sonEcoEnd-sonEcoLen&&si<sonEcoEnd;\n"
    "        var ewet=inE?Math.min(.5,.14+sonEcoLen/400):0;\n"
    "        try{\n"
    "          dlG.gain.setTargetAtTime(Math.max(syn.dw,ewet),t,\n"
    "            inE?.06:.3);\n"
    "          if(inE){\n"
    "            var dts=sonEcoDist*beatDur;\n"
    "            dl.delayTime.setTargetAtTime(dts<=2\n"
    "              ?Math.max(beatDur,dts)\n"
    "              :Math.min(2,beatDur*syn.dt),t,.08);\n"
    "            if(!sonEcoT){sonEcoT=true;\n"
    "              showToast('DEJA VU. THE DELAY AGREES');}\n"
    "          } else if(syn.dw>0){\n"
    "            /* the knob's own echo gets its time back */\n"
    "            dl.delayTime.setTargetAtTime(\n"
    "              Math.min(2,beatDur*syn.dt),t,.2);\n"
    "          }\n"
    "        }catch(e){}\n"
    "      }\n"
    "      if(bandOn&&typeof BAND!=='undefined'){\n"
    ...
    "          var q=bandQ(si,ci);\n"
    "          if(!q) continue;\n"
    "          if(!bandGate(ci,step)) continue;\n"
    "          if(syn.pts){\n"
    "            /* parts mode: the same gate - you play only when your\n"
    "               model earned on this byte - but each rank now\n"
    "               holds a PART. the star takes the melody, second\n"
    "               chair walks the bass, the middle comps the chord\n"
    "               over a pad, and the deep sparse models become the\n"
    "               percussion section. attribution is arrangement */\n"
    "            var bg2=(.016+.05*(q/15))*syn.bgx;\n"
    "            var rt2=(sonKey?sonKey.r:ch[0])-12;\n"
    "            if(ci===0)\n"
    "              bandVoice(mf(noteFor(step,c)),bg2*1.3,syn.ld*1.5,\n"
    "                t,ci,'sawtooth',(1+3*(q/15))*syn.fmi);\n"
    "            else if(ci===1)\n"
    "              bandVoice(mf(rt2+((sp&8)?7:0)),bg2*1.1,.18,t,ci,\n"
    "                'square',1.2*syn.fmi);\n"
    "            else if(ci===4)\n"
    "              bandVoice(mf(bandNote(ci,step)),bg2*.5,beatDur*3,\n"
    "                t,ci,'sine',.6*syn.fmi);\n"
    "            else if(ci>=5){\n"
    "              if(ci&1) hat(t,bg2*.6,false,bandPan(ci));\n"
    "              else tom(t,bg2*.9,150+50*(ci-4),bandPan(ci));\n"
    "            }\n"
    "            else\n"
    "              bandVoice(mf(bandNote(ci,step)),bg2*.8,syn.ld,t,ci,\n"
    "                'triangle',(1+2*(q/15))*syn.fmi);\n"
    "            continue;\n"
    "          }\n"
    "          if(fmOn){\n"
    "            /* louder take, harder lean: the nibble drives both\n"
    "               volume and modulation index */\n"
    "            fmVoice(mf(bandNote(ci,step)),\n"
    "              (.02+.05*(q/15))*syn.bgx+.001,syn.ld,t,\n"
    "              panTo(bandPan(ci)),FMR[ci%%8],(1+3*(q/15))*syn.fmi);\n"
    "            continue;\n"
    "          }\n"
    "          var o2=actx.createOscillator(),g2=actx.createGain();\n"
    "          o2.type=BANDW[ci%%4];\n"
    "          o2.frequency.value=mf(bandNote(ci,step));\n"
    "          var te2=envA(g2,(.014+.038*(q/15))*syn.bgx+.001,t,syn.ld);\n"
    "          o2.connect(g2);g2.connect(panTo(bandPan(ci)));\n"
    "          o2.start(t);o2.stop(te2);\n"
    "          vreg(t,[o2]);\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    if(!bandLive){\n"
    "      /* phrasing: cheap bytes on the off-beats rest - the hat\n"
    "         keeps their place, so silence itself reads as ORDERED.\n"
    "         downbeats lean in, off-beats play soft */\n"
    "      var rest=syn.gr&&(sp&1)&&\n"
    "        (sonify?(leadC>=0&&leadC<syn.t1):(sp===3||sp===11));\n"
    "      var acc=syn.gr?(sp===0?1.25:(sp&1)?.6:1):1;\n"
    "      /* under a genre the lead signs a contract: it may only\n"
    "         speak on the mask's steps, at the genre's length and\n"
    "         register. the needle still eats one byte per step -\n"
    "         the genre owns WHEN, the file owns WHAT */\n"
    "      var nn=n,ldv=syn.ld;\n"
    "      if(GEN){\n"
    "        rest=GEN.lm!==undefined&&!((GEN.lm>>sp)&1);\n"
    "        /* the accent mask: the genre leans where it leans */\n"
    "        acc=GEN.am?(((GEN.am>>sp)&1)?1.3:.8):1;\n"
    "        if(GEN.oc) nn+=GEN.oc*12;\n"
    "        if(GEN.ln) ldv=GEN.ln;\n"
    "      }\n"
    "      if(!rest){\n"
    "        if(fmOn){\n"
    "          /* cost picks the modulation index: the file's difficulty\n"
    "             becomes timbre, not just pitch */\n"
    "          var fi2=(leadC<0?2:leadC<syn.t1?.4:leadC<syn.t2?1.5\n"
    "            :leadC<syn.t3?4:8)*syn.fmi;\n"
    "          fmVoice(mf(nn),(syn.lg*1.4+.001)*acc,ldv,t,\n"
    "            panTo(notePan),1,fi2);\n"
    "        }else{\n"
    "          var o=actx.createOscillator(),g=actx.createGain();\n"
    "          o.type=syn.wave;o.frequency.value=mf(nn);\n"
    "          /* 303 accent: expensive bytes hit harder and\n"
    "             brighter, like the little silver box meant */\n"
    "          var a3=GEN&&GEN.a303,isAcc=a3&&leadC>=syn.t2;\n"
    "          var te=envA(g,(syn.lg+.001)*acc*(isAcc?1.5:1),t,ldv);\n"
    "          if(a3){\n"
    "            /* per-voice filter: the master stays clean, the\n"
    "               squelch lives and dies with the note */\n"
    "            var bq=actx.createBiquadFilter();\n"
    "            bq.type='lowpass';bq.Q.value=syn.fq;\n"
    "            var fp=380+(isAcc?3200:1200)*\n"
    "              Math.min(1,(leadC<0?2:leadC)/8);\n"
    "            bq.frequency.setValueAtTime(fp,t);\n"
    "            bq.frequency.setTargetAtTime(240,t,.05+ldv*.4);\n"
    "            /* the slide: unaccented neighbours tie together */\n"
    "            if(GEN.sl3&&fxLast&&fxLast!==mf(nn)&&!isAcc){\n"
    "              o.frequency.setValueAtTime(fxLast,t);\n"
    ...
    "          }else{\n"
    "            fxLead(o,mf(nn),t,syn.atk+ldv,leadC,step);\n"
    "            o.connect(g);\n"
    "          }\n"
    "          g.connect(panTo(notePan));o.start(t);\n"
    "          o.stop(te);\n"
    "          vreg(t,[o]);\n"
    "          if(syn.dtn>0){\n"
    "            /* unison twin, a few cents flat: instant width. it\n"
    "               holds steady while the main voice runs the fx\n"
    "               column - drone under arp, the old chip trick */\n"
    "            var o2=actx.createOscillator(),g2=actx.createGain();\n"
    "            o2.type=syn.wave;\n"
    "            o2.frequency.value=mf(nn)*Math.pow(2,-syn.dtn/1200);\n"
    "            var te2=envA(g2,(syn.lg*.6+.001)*acc,t,ldv);\n"
    "            o2.connect(g2);g2.connect(panTo(notePan));\n"
    "            o2.start(t);o2.stop(te2);\n"
    "            vreg(t,[o2]);\n"
    "            if(GEN&&GEN.ss){\n"
    "              /* the third saw, sharp: two twins make a\n"
    "                 supersaw, and a screech when driven */\n"
    "              var o4=actx.createOscillator(),g4=actx.createGain();\n"
    "              o4.type=syn.wave;\n"
    "              o4.frequency.value=mf(nn)*Math.pow(2,syn.dtn/1200);\n"
    "              var te4=envA(g4,(syn.lg*.6+.001)*acc,t,ldv);\n"
    "              o4.connect(g4);g4.connect(panTo(notePan));\n"
    "              o4.start(t);o4.stop(te4);\n"
    "              vreg(t,[o4]);\n"
    "            }\n"
    "          }\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    if(!GEN&&syn.bi&&step%%syn.bi===0){\n"
    "      /* keyed sonify sits on a tonic pedal; under the groove the\n"
    "         bass walks root and fifth like a bass player would */\n"
    "      var bn=(sonify&&sonKey?sonKey.r:ch[0])-12;\n"
    "      if(syn.gr&&(sp&8)) bn+=7;\n"
    "      if(fmOn){\n"
    "        fmVoice(mf(bn),syn.bg*1.4+.001,.22,t,gainM,1,1.2*syn.fmi);\n"
    "      }else{\n"
    "        var b=actx.createOscillator(),bg=actx.createGain();\n"
    "        b.type=syn.bw;\n"
    "        b.frequency.value=mf(bn);\n"
    "        var te3=envA(bg,syn.bg+.001,t,.22);\n"
    "        b.connect(bg);bg.connect(gainM);b.start(t);b.stop(te3);\n"
    "        vreg(t,[b]);\n"
    "      }\n"
    "    }\n"
    "    if(syn.sf>0){\n"
    "      /* surface noise: the shellac wears where the coder pays.\n"
    "         crackle density follows the current byte's cost, so a\n"
    "         compressible passage plays clean vinyl and a hard one\n"
    "         plays a worn 78 */\n"
    "      var pd2=syn.sf*(leadC<0?.3:Math.min(1,.12+leadC/8));\n"
    "      if(Math.random()<pd2)\n"
    "        hat(t+Math.random()*beatDur,.005+Math.random()*.012,\n"
    "          false,(Math.random()*2-1)*syn.pw);\n"
    "    }\n"
    "    if(syn.fe>0&&filt){\n"
    "      /* the acid kick: the filter opens on the note and sighs\n"
    "         back to the knob. expensive bytes open it wider -\n"
    "         difficulty as brightness. push RESONANCE and it is\n"
    "         1993 again */\n"
    "      try{\n"
    "        var fpk=syn.fc+syn.fe*(1500+9000*\n"
    "          Math.min(1,(leadC<0?3:leadC)/8));\n"
    "        filt.frequency.setValueAtTime(Math.min(18000,fpk),t);\n"
    "        filt.frequency.setTargetAtTime(syn.fc,t,.05+syn.ld*.4);\n"
    "      }catch(e){}\n"
    "    }\n"
    "  }\n"
    ...
    "  function tick(){\n"
    "    if(!actx) return;\n"
    "    /* a hand on the platter outranks the motor */\n"
    "    if(vsOn){nextT=actx.currentTime+.09;return;}\n"
    "    var now=actx.currentTime;\n"
    "    if(nextT<now) nextT=now+.05; /* resync after a long stall */\n"
    "    while(sonV.length&&sonV[0].t<now-0.5) sonV.shift();\n"
    "    /* scoring runs in a worker, so the normal shallow queue is\n"
    "       enough; only the no-worker fallback needs the deep one to\n"
    "       ride over its synchronous stalls */\n"
    "    while(nextT<now+((tuOn&&!tuWkOk)?0.8:0.25)){\n"
    "      if(!muted)\n"
    "        schedNote(mstep,nextT+((mstep&1)?beatDur*syn.sw:0));\n"
    "      mstep++;\n"
    "      nextT+=beatDur;\n"
    "    }\n"
    "  }\n"
    ...
    "        crush=actx.createWaveShaper();\n"
    "        filt=actx.createBiquadFilter();filt.type='lowpass';\n"
    "        post=actx.createGain();\n"
    "        comp=actx.createDynamicsCompressor();\n"
    "        gainM.connect(crush);crush.connect(filt);\n"
    "        filt.connect(post);post.connect(comp);\n"
    "        comp.connect(actx.destination);\n"
    "        /* the drive: tanh saturation for genre kicks - gabber\n"
    "           is a sine wave with an opinion */\n"
    "        drv=actx.createWaveShaper();\n"
    "        var dcv=new Float32Array(257);\n"
    "        for(var di=0;di<257;di++)\n"
    "          dcv[di]=Math.tanh(3.5*(di/128-1));\n"
    "        drv.curve=dcv;\n"
    "        drv.connect(gainM);\n"
    "        /* the harder opinion: hardcore and speedcore route\n"
    "           their kicks through this one */\n"
    "        drv2=actx.createWaveShaper();\n"
    "        var dcw=new Float32Array(257);\n"
    "        for(var dj=0;dj<257;dj++)\n"
    "          dcw[dj]=Math.tanh(7*(dj/128-1));\n"
    "        drv2.curve=dcw;\n"
    "        drv2.connect(gainM);\n"
    ...
    "        lfo.start();\n"
    "        dl=actx.createDelay(2.0);\n"
    "        dl2=actx.createDelay(2.0);\n"
    "        dlG=actx.createGain();dlF=actx.createGain();\n"
    "        dlG.gain.value=0;\n"
    "        try{\n"
    "          dlPL=actx.createStereoPanner();dlPL.pan.value=-.8;\n"
    "          dlPR=actx.createStereoPanner();dlPR.pan.value=.8;\n"
    "        }catch(e2){dlPL=null;dlPR=null;}\n"
    "        /* dl's own outputs are dlApply's business (mono vs\n"
    "           ping-pong); only the fixed ends are wired here */\n"
    "        post.connect(dl);dlG.connect(comp);\n"
    "        dlF.connect(dl);\n"
    "        filtApply();dlApply();crushApply();cmpApply();\n"
    "        rvApply(); /* restore a nonzero patch on fresh audio */\n"
    "      }\n"
    "      if(actx.state==='suspended') actx.resume();\n"
    "    }catch(e){}\n"
    "  }\n"
    "  function blip(fr,g,dur,p){\n"
    ...
    "      o.connect(og);og.connect(panTo(p||0));\n"
    ...
    "600);\n"
    "    if(on){\n"
    "      startMusic();startTitle();scrambleIn();flyStart();\n"
    "      bcJoin();\n"
    "      partyT0=Date.now();\n"
    "      if(!endDone&&!endT) endT=setTimeout(showEnd,600000);\n"
    "      if(moon&&moon.classList.contains('lit')) egg('moon');\n"
    "    }\n"
    "    else{\n"
    "      stopMusic();stopTitle();scrambleRestore();flyStop();\n"
    "      crtStop();bcLeave();voidStop();dcStop();hwStop();\n"
    "      wfStop();sbStop();\n"
    "      if(finaleEnd) finaleEnd();\n"
    ...
    "      var fb=document.getElementById('fpbox');\n"
    "      if(fb){fb.remove();e.preventDefault();return;}\n"
    "      if(synOn){synStop();e.preventDefault();return;}\n"
    "      if(auOn){auStop();e.preventDefault();return;}\n"
    "      if(finaleEnd){finaleEnd();e.preventDefault();return;}\n"
    "      if(dcOn){dcStop();e.preventDefault();return;}\n"
    "      if(rcOn){rcStop();e.preventDefault();return;}\n"
    "      if(hwOn){hwStop();e.preventDefault();return;}\n"
    "      if(wfOn){wfStop();e.preventDefault();return;}\n"
    "      if(sbOn){sbStop();e.preventDefault();return;}\n"
    ...
    "    if(dcOn&&e.key===' '){\n"
    "      if(dcPaused){\n"
    "        dcPaused=false;dcSnap=null;\n"
    "        if(dcInsp) dcInsp.style.display='none';\n"
    "        if(dcStat&&!dcDone)\n"
    "          dcStat.textContent='DECODING. NO SIMULATION.';\n"
    "        showToast('ROLLING');\n"
    "      } else {\n"
    "        dcTurbo=!dcTurbo;\n"
    ...
    "    }\n"
    "    if(dcOn&&(e.key==='.'||e.key==='>')){\n"
    "      dcStepOne();e.preventDefault();return;\n"
    "    }\n"
    "    if(dcOn&&(e.key===','||e.key==='<')){\n"
    "      dcStepByte();e.preventDefault();return;\n"
    "    }\n"
    "    if(rcOn&&e.key===' '){\n"
    "      if(rcPaused){\n"
    "        rcPaused=false;\n"
    "        if(rcStat&&!rcDone)\n"
    "          rcStat.textContent='PRESSING. NO SIMULATION.';\n"
    "        showToast('ROLLING');\n"
    "      } else {\n"
    "        rcTurbo=!rcTurbo;\n"
    "        showToast(rcTurbo?'TURBO CRUNCH':'SAVOR IT');\n"
    "      }\n"
    "      e.preventDefault();return;\n"
    "    }\n"
    "    if(rcOn&&(e.key==='.'||e.key==='>')){\n"
    "      rcStepOne();e.preventDefault();return;\n"
    "    }\n"
    "    if(rcOn&&(e.key===','||e.key==='<')){\n"
    "      rcStepByte();e.preventDefault();return;\n"
    "    }\n"
    "    if(hwOn&&e.key===' '){\n"
    "      hwTurbo=!hwTurbo;\n"
    "      showToast(hwTurbo?'FAST FORWARD THE WAR':'REAL TIME');\n"
    "      e.preventDefault();return;\n"
    "    }\n"
    "    if(wfOn&&e.key===' '){\n"
    "      wfTurbo=!wfTurbo;\n"
    "      showToast(wfTurbo?'FAST FORWARD':'REAL TIME');\n"
    "      e.preventDefault();return;\n"
    "    }\n"
    "    if(sbOn&&e.key===' '){\n"
    "      sbTurbo=!sbTurbo;\n"
    "      showToast(sbTurbo?'FAST FORWARD':'REAL TIME');\n"
    "      e.preventDefault();return;\n"
    "    }\n"
    "    if(e.key.length===1){\n"
    "      /* keywords get first claim on 'm': the mute shortcut runs\n"
    "         only if no keyword completed on this keystroke. words with\n"
    "         a non-final m (amiga, man) refund the transient mute flip\n"
    "         their m caused in passing */\n"
    "      buf=(buf+k).slice(-10); /* longest keyword: protracker (10) */\n"
    "      if(buf.slice(-5)==='iddqd'){\n"
    "        buf='';\n"
    "        setParty(!document.body.classList.contains('party'),false,\n"
    "          '*** DEGREELESSNESS MODE ***');\n"
    "        return;\n"
    "      }\n"
    "      if(buf.slice(-9)==='devcheats'){buf='';cheatsToggle();return;}\n"
    "      if(document.body.classList.contains('party')){\n"
    "        if(buf.slice(-5)==='xyzzy'){buf='';egg('xyzzy');\n"
    "          showToast('NOTHING HAPPENS');return;}\n"
    "        if(buf.slice(-5)==='idkfa'){buf='';egg('idkfa');\n"
    "          showToast('AMMO NOT INCLUDED');return;}\n"
    "        if(buf.slice(-4)==='e2e4'){buf='';egg('e2e4');\n"
    "          showToast('BEST BY TEST');return;}\n"
    "        if(buf.slice(-5)==='uciok'){buf='';egg('uciok');\n"
    "          showToast('READYOK');return;}\n"
    "        if(buf.slice(-8)==='breakout'){buf='';bkStart();return;}\n"
    "        if(buf.slice(-5)==='amiga'){buf='';muted=!muted;\n"
    "          boingToggle();return;}\n"
    ...
    "          showToast(sonify?'YOU ARE LISTENING TO THE FILE'\n"
    "            :'BACK TO THE JUKEBOX');return;}\n"
    "        if(/^[a-g](major|minor)$/.test(buf.slice(-6))){\n"
    "          var kw=buf.slice(-6);buf='';\n"
    "          muted=!muted; /* refund the mid-word m, like amiga */\n"
    "          keyToggle(kw);return;}\n"
    "        if(buf.slice(-6)==='zodiac'){buf='';constToggle();return;}\n"
    "        if(buf.slice(-3)==='crt'){buf='';crtToggle();return;}\n"
    "        if(buf.slice(-5)==='scope'){buf='';scopeToggle();return;}\n"
    "        if(buf.slice(-6)==='winamp'){buf='';\n"
    "          muted=!muted; /* refund the mid-word m, like amiga */\n"
    "          specToggle();return;}\n"
    "        if(buf.slice(-7)==='spectre'){buf='';sgToggle();return;}\n"
    "        if(buf.slice(-4)==='band'){buf='';bandToggle();return;}\n"
    "        if(buf.slice(-6)==='stereo'){buf='';stereoToggle();return;}\n"
    "        if(buf.slice(-6)==='groove'){buf='';\n"
    "          syn.gr=syn.gr?0:1;egg('groove');\n"
    "          showToast(syn.gr?'THE KIT SITS IN':'JUST THE DATA');\n"
    "          return;}\n"
    "        if(buf.slice(-6)==='vector'){buf='';vecToggle();return;}\n"
    "        if(buf.slice(-4)==='opl2'){buf='';fmToggle();return;}\n"
    "        if(buf.slice(-7)==='forward'){buf='';fpStart();return;}\n"
    "        if(buf.slice(-5)==='tuner'){buf='';tuStart();return;}\n"
    "        if(buf.slice(-5)==='synth'){buf='';synStart();return;}\n"
    "        if(buf.slice(-10)==='protracker'){buf='';ptToggle();return;}\n"
    "        if(buf.slice(-8)==='rotozoom'){buf='';rotoToggle();return;}\n"
    "        if(buf.slice(-6)==='deejay'){buf='';zoomToggle();return;}\n"
    ...
    "        if(buf.slice(-4)==='fire'){buf='';fireToggle();return;}\n"
    "        if(buf.slice(-7)==='twister'){buf='';twToggle();return;}\n"
    "        if(buf.slice(-7)==='kefrens'){buf='';kfToggle();return;}\n"
    "        if(buf.slice(-6)==='echoes'){buf='';ecToggle();return;}\n"
    "        if(buf.slice(-7)==='hashwar'){buf='';hwStart();return;}\n"
    "        if(buf.slice(-6)==='whatif'){buf='';wfStart();return;}\n"
    "        if(buf.slice(-8)==='sabotage'){buf='';sbStart();return;}\n"
    "        if(buf.slice(-8)==='recrunch'){buf='';rcStart();return;}\n"
    "        if(buf.slice(-8)==='audition'){buf='';auStart();return;}\n"
    "        if(buf.slice(-4)==='guru'){buf='';egg('guru');\n"
    "          showGuru();return;}\n"
    "        if(buf.slice(-4)==='sudo'){buf='';egg('sudo');\n"
    "          showToast('THIS INCIDENT WILL BE REPORTED');return;}\n"
    "        if(buf.slice(-3)==='man'){buf='';muted=!muted;egg('man');\n"
    ...
    "  var eggSet={},eggN=0,EGG_TOTAL=61;\n"
    "  var finaleDone=false,finaleCyc=null,finaleEnd=null;\n"
    "  function egg(id){\n"
    "    if(!eggSet[id]){eggSet[id]=1;eggN++;}\n"
    "    if(eggN>=EGG_TOTAL&&!finaleDone) setTimeout(showFinale,900);\n"
    "  }\n"
    "  window.partyEgg=egg;\n"
    ...
    "    endT=null;\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(document.getElementById('endpart')) return;\n"
    "    endDone=true;\n"
    "    var ms=partyAccum+(Date.now()-partyT0);\n"
    "    var ov=document.createElement('div');\n"
    "    ov.id='endpart';\n"
    "    function row(k,v){\n"
    "      return '<div class=\"er\"><span>'+k+'</span>'\n"
    "        +'<span>'+v+'</span></div>';\n"
    "    }\n"
    "    ov.innerHTML='<div class=\"ebox\">*** END OF PART 1 ***'\n"
    "      +'<div class=\"ers\">'\n"
    "      +row('TIME PARTIED',fmtMS(ms))\n"
    "      +row('EGGS FOUND',eggN+'/'+EGG_TOTAL)\n"
    "      +row('CUBES KILLED',killedN)\n"
    "      +row('BYTES FED',feedN)\n"
    "      +row('GURUS',guruN)\n"
    "      +row('BRICKS SMASHED',bkBroken)\n"
    "      +'</div>THE PARTY CONTINUES</div>';\n"
    "    ov.addEventListener('click',function(){ov.remove();});\n"
    "    document.body.appendChild(ov);\n"
    "    setTimeout(function(){if(ov.parentNode) ov.remove();},9000);\n"
    "  }\n"
    "  /* the finale: every egg found, so the production rolls credits.\n"
    "     the backdrops take turns bowing behind the roll. one egg\n"
    "     remains, listed below; it cannot be obtained. that is the\n"
    "     point of it */\n"
    "  function showFinale(){\n"
    ...
    "    var ov=document.createElement('div');\n"
    "    ov.id='finale';\n"
    "    ov.innerHTML='<div class=\"roll\">'\n"
    "      +'<div class=\"fh\">*** '+eggN+' / '+EGG_TOTAL+' ***</div>'\n"
    "      +'<div>EVERY EGG FOUND</div>'\n"
    "      +'<div class=\"fs\">A PAQ REPORT PRODUCTION</div>'\n"
    "      +'<div class=\"fs\">CODE</div>'\n"
    "      +'<div>ONE COMPRESSOR, RUN TWICE</div>'\n"
    "      +'<div class=\"fs\">GRAPHICS</div>'\n"
    "      +'<div>THE FILE ITSELF</div>'\n"
    "      +'<div class=\"fs\">MUSIC</div>'\n"
    "      +'<div>FOUR CHORDS AND A SNARE</div>'\n"
    "      +'<div class=\"fs\">DIRECTION</div>'\n"
    "      +'<div>THE ARITHMETIC CODER</div>'\n"
    "      +'<div class=\"fs\">STARRING</div>'\n"
    "      +'<div>THE GLENZ CUBE'\n"
    "      +(killedN?' (DIED '+killedN+'x)':'')+'</div>'\n"
    "      +'<div>THE BOING BALL</div>'\n"
    "      +'<div>THE MOON, WHEN IT SHOWS</div>'\n"
    "      +'<div class=\"fs\">GREETINGS TO</div>'\n"
    "      +'<div>CRINKLER &middot; FARBRAUSCH</div>'\n"
    "      +'<div>CONSPIRACY &middot; MERCURY</div>'\n"
    "      +'<div>LOGICOMA &middot; TBC</div>'\n"
    "      +'<div>AND SHANNON. NO HARD FEELINGS.</div>'\n"
    "      +'<div class=\"fs\">ONE MORE</div>'\n"
    "      +'<div class=\"fy\">EGG '+(EGG_TOTAL+1)+' OF '+EGG_TOTAL+'</div>'\n"
    "      +'<div class=\"fp\">FIND THE OTHER '+EGG_TOTAL+'</div>'\n"
    "      +'<div class=\"fh\" style=\"margin-top:30px\">'\n"
    "      +'*** THE PARTY NEVER ENDS ***</div>'\n"
    "      +'</div>';\n"
    "    document.body.appendChild(ov);\n"
    "    /* the sky takes its bow: each backdrop in turn under the roll */\n"
    "    var seq=[rotoToggle,voxToggle,tnToggle,blkToggle,\n"
    "      fireToggle,twToggle,kfToggle,ecToggle],si=0;\n"
    "    finaleCyc=setInterval(function(){seq[si%%8]();si++;},6800);\n"
    "    function endF(){\n"
    "      if(finaleCyc){clearInterval(finaleCyc);finaleCyc=null;}\n"
    "      finaleEnd=null;\n"
    "      noToast=false;\n"
    "      if(ov.parentNode) ov.remove();\n"
    "      rotoStop();voxStop();tnStop();blkStop();fireStop();twStop();\n"
    "      kfStop();ecStop();\n"
    "      if(document.body.classList.contains('party')){\n"
    "        flyStart();\n"
    "        showToast('NOW GO OUTSIDE');\n"
    "      }\n"
    "    }\n"
    "    finaleEnd=endF;\n"
    "    ov.addEventListener('click',endF);\n"
    "    setTimeout(function(){if(ov.parentNode) endF();},48000);\n"
    "    blip(523,.05,.12);\n"
    "    setTimeout(function(){blip(659,.05,.12);},150);\n"
    "    setTimeout(function(){blip(784,.05,.12);},300);\n"
    "    setTimeout(function(){blip(880,.05,.12);},450);\n"
    "    setTimeout(function(){blip(1046,.08,.4);},600);\n"
    "  }\n"
    "  /* two reports, one browser: they only talk at the party.\n"
    "     BroadcastChannel handshake - hello on entry, hi in reply,\n"
    "     bye on exit. identity = input CRC, smack talk = ratio */\n"
    "  var bcCh=null,bcId=((Math.random()*1e9)|0)+'';\n"
    "  function bcSay(o){if(bcCh){try{bcCh.postMessage(o);}catch(e){}}}\n"
    "  function bcGreet(p){\n"
    "    egg('twins');\n"
    "    var mine=REPORT_ID.ratio,theirs=+p.ratio;\n"
    "    if(p.crc===REPORT_ID.crc){\n"
    "      showToast('ANOTHER ME. HELLO, ME.');return;}\n"
    "    if(mine<theirs-0.05)\n"
    "      showToast('MY '+mine.toFixed(1)+'%% GREETS YOUR '\n"
    "        +theirs.toFixed(1)+'%%. CONDOLENCES.');\n"
    "    else if(theirs<mine-0.05)\n"
    "      showToast('A '+theirs.toFixed(1)+'%% REPORT IS HERE. '\n"
    "        +'WE ARE NOT WORTHY.');\n"
    "    else showToast('RATIO TWINS. '+mine.toFixed(1)+'%%.');\n"
    "  }\n"
    "  function bcJoin(){\n"
    "    if(bcCh||typeof REPORT_ID==='undefined'||\n"
    "       typeof BroadcastChannel==='undefined') return;\n"
    "    try{bcCh=new BroadcastChannel('paq-party');}catch(e){return;}\n"
    "    bcCh.onmessage=function(ev){\n"
    "      var m=ev.data||{};\n"
    "      if(m.id===bcId) return;\n"
    "      if(m.t==='hello'){\n"
    "        bcSay({t:'hi',id:bcId,crc:REPORT_ID.crc,\n"
    "          ratio:REPORT_ID.ratio});\n"
    "        bcGreet(m);\n"
    "      } else if(m.t==='hi') bcGreet(m);\n"
    "      else if(m.t==='bye')\n"
    "        showToast('A REPORT HAS LEFT THE PARTY.');\n"
    "    };\n"
    "    bcSay({t:'hello',id:bcId,crc:REPORT_ID.crc,\n"
    "      ratio:REPORT_ID.ratio});\n"
    "  }\n"
    "  function bcLeave(){\n"
    "    if(!bcCh) return;\n"
    "    bcSay({t:'bye',id:bcId});\n"
    "    try{bcCh.close();}catch(e){}\n"
    "    bcCh=null;\n"
    "  }\n"
    "  /* crt: the report becomes a tube. real barrel warp - an SVG\n"
    "     displacement filter with a runtime-generated radial map bends\n"
    "     the actual pixels of the page; the glass overlay (bezel,\n"
    "     phosphor stripes, refresh band) sits flat on top, because\n"
    "     scanlines live on the glass, not in the picture */\n"
    "  var crtIsOn=false,crtSvg=null,crtHumO=null,crtHumG=null;\n"
    "  var crtScreen=null;\n"
    "  function crtBuildMap(){\n"
    "    var cv=document.createElement('canvas');\n"
    "    cv.width=128;cv.height=128;\n"
    "    var c2=cv.getContext('2d');\n"
    "    var im=c2.createImageData(128,128);\n"
    "    for(var y=0;y<128;y++){\n"
    "      for(var x=0;x<128;x++){\n"
    "        var nx=x/63.5-1,ny=y/63.5-1;\n"
    "        var r2=nx*nx+ny*ny;\n"
    "        var p=(y*128+x)*4;\n"
    "        var r=128+nx*r2*70,g=128+ny*r2*70;\n"
    "        im.data[p]=r<0?0:r>255?255:r|0;\n"
    "        im.data[p+1]=g<0?0:g>255?255:g|0;\n"
    ...
    "    var f=crtSvg.querySelector('filter');\n"
    "    /* scale 90 displaces at most 45px: a tight filter region\n"
    "       halves the pixels the warp has to chew */\n"
    "    f.setAttribute('x',-50);f.setAttribute('y',-50);\n"
    "    f.setAttribute('width',W+100);f.setAttribute('height',H+100);\n"
    "    var fi=crtSvg.querySelector('feImage');\n"
    "    fi.setAttribute('width',W);fi.setAttribute('height',H);\n"
    "  }\n"
    "  function crtToggle(){\n"
    "    if(crtIsOn){crtStop();showToast('FLAT AGAIN. HOW SAD.');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    crtIsOn=true;egg('crt');\n"
    "    var NS='http://www.w3.org/2000/svg';\n"
    "    crtSvg=document.createElementNS(NS,'svg');\n"
    "    crtSvg.setAttribute('width',0);\n"
    "    crtSvg.setAttribute('height',0);\n"
    "    crtSvg.style.position='absolute';\n"
    "    var f=document.createElementNS(NS,'filter');\n"
    "    f.setAttribute('id','crtwarp');\n"
    "    f.setAttribute('filterUnits','userSpaceOnUse');\n"
    "    f.setAttribute('color-interpolation-filters','sRGB');\n"
    "    var fi=document.createElementNS(NS,'feImage');\n"
    "    fi.setAttribute('href',crtBuildMap());\n"
    ...
    "    crtSvg.appendChild(f);\n"
    "    document.body.appendChild(crtSvg);\n"
    "    /* the picture must be viewport-sized for the warp to read as\n"
    "       a tube: move the page into a fixed screen-sized scroller\n"
    "       and filter that. element references survive the move */\n"
    "    var wr=document.querySelector('.wrap');\n"
    "    var sy=window.scrollY||window.pageYOffset;\n"
    "    crtScreen=document.createElement('div');\n"
    "    crtScreen.id='crt-screen';\n"
    "    document.body.appendChild(crtScreen);\n"
    "    if(wr) crtScreen.appendChild(wr);\n"
    "    crtScreen.scrollTop=sy;\n"
    "    crtSize();\n"
    "    window.addEventListener('resize',crtSize);\n"
    "    document.body.classList.add('crt');\n"
    "    /* power-on: the degauss doubles as the switch-on thump */\n"
    "    if(window.partyDegauss) window.partyDegauss();\n"
    "    ensureAudio();\n"
    "    if(actx&&!muted){\n"
    "      try{\n"
    "        crtHumO=actx.createOscillator();\n"
    "        crtHumG=actx.createGain();\n"
    "        crtHumO.type='sine';crtHumO.frequency.value=120;\n"
    "        crtHumG.gain.value=.012;\n"
    "        crtHumO.connect(crtHumG);crtHumG.connect(gainM);\n"
    "        crtHumO.start();\n"
    "      }catch(e){}\n"
    "    }\n"
    "    showToast('CURVED GLASS. AS NATURE INTENDED.');\n"
    "  }\n"
    "  function crtStop(){\n"
    "    crtIsOn=false;\n"
    "    document.body.classList.remove('crt');\n"
    "    window.removeEventListener('resize',crtSize);\n"
    "    if(crtScreen){\n"
    "      var wr=crtScreen.querySelector('.wrap');\n"
    "      var sy=crtScreen.scrollTop;\n"
    "      if(wr) document.body.insertBefore(wr,crtScreen);\n"
    "      crtScreen.remove();crtScreen=null;\n"
    "      window.scrollTo(0,sy);\n"
    "    }\n"
    "    if(crtSvg){crtSvg.remove();crtSvg=null;}\n"
    "    if(crtHumO){try{crtHumO.stop();}catch(e){}\n"
    "      crtHumO=null;crtHumG=null;}\n"
    "  }\n"
    "  /* the amiga remembers, and it keeps count */\n"
    "  var guruN=0;\n"
    "  function showGuru(){\n"
    "    if(document.getElementById('guru')) return;\n"
    "    guruN++;\n"
    "    var code=(3+guruN).toString(16).toUpperCase();\n"
    "    while(code.length<8) code='0'+code;\n"
    "    var g=document.createElement('div');\n"
    "    g.id='guru';\n"
    "    g.innerHTML='<div class=\"box\">Software Failure.&nbsp;&nbsp;"
    "Press left mouse button to continue.<br>"
    "Guru Meditation #00504151.'+code+'</div>';\n"
    "    g.addEventListener('click',function(){g.remove();});\n"
    "    document.body.appendChild(g);\n"
    "  }\n"
    "  /* void: hide the report chrome, keep the sky and the toys.\n"
    "     scroll position is saved because display:none collapses the\n"
    "     page and would otherwise dump the reader at the top */\n"
    "  var voidSY=0;\n"
    "  function voidToggle(){\n"
    "    var on=!document.body.classList.contains('void');\n"
    "    if(on) voidSY=window.scrollY||window.pageYOffset;\n"
    "    document.body.classList.toggle('void',on);\n"
    "    if(!on) window.scrollTo(0,voidSY);\n"
    "    egg('void');\n"
    "    showToast(on?'THE REPORT IS GONE. THE FILE REMAINS'\n"
    "      :'BACK TO WORK');\n"
    "  }\n"
    "  function voidStop(){\n"
    "    if(!document.body.classList.contains('void')) return;\n"
    "    document.body.classList.remove('void');\n"
    "    window.scrollTo(0,voidSY);\n"
    "  }\n"
    "  /* devcheats: the typed commands, in one box. discovery eggs\n"
    "     (clicks, selections, the moon) stay unlisted - cheats only\n"
    "     get you so far */\n"
    "  function cheatsToggle(){\n"
    "    var ex=document.getElementById('cheatbox');\n"
    "    if(ex){ex.remove();return;}\n"
    "    var rows=[\n"
    "      ['iddqd | konami | 7 ring taps','ENTER / LEAVE THE PARTY'],\n"
    "      ['devcheats','THIS BOX. WORKS ANYWHERE'],\n"
    "      ['m','MUTE'],\n"
    "      ['listen','PLAY THE FILE'],\n"
    "      ['drag the ring while it plays','SCRATCH IT'],\n"
    "      ['aminor cmajor &amp;c','LISTEN, PINNED TO A KEY'],\n"
    "      ['scope','OSCILLOSCOPE'],\n"
    "      ['winamp','THE BARS. THE FALLING CAPS'],\n"
    "      ['spectre','LIVE SPECTROGRAM'],\n"
    "      ['band','TOP MODELS, ONE CHANNEL EACH'],\n"
    "      ['stereo','LEFT IS BYTE ZERO'],\n"
    "      ['groove','DRUMS, RESTS AND A PAD UNDER THE DATA'],\n"
    "      ['vector','L MEETS R, PLOTTED'],\n"
    "      ['opl2','FM. DIFFICULTY AS TIMBRE'],\n"
    "      ['forward','THE MODEL RE-PRICES THE FILE'],\n"
    "      ['tuner','EDIT BYTES. THE MODEL GRADES YOU'],\n"
    "      ['synth','THE SONIFICATION, PATCHABLE'],\n"
    "      ['protracker','THE WHOLE REPORT, PT2.3D'],\n"
    "      ['deejay','THE RACK, TWICE THE SIZE'],\n"
    ...
    "      ['fire','THE COST CURVE, BURNING'],\n"
    "      ['twister','ENTROPY WRINGS THE COLUMN'],\n"
    "      ['kefrens','ONE BAR PER MODEL'],\n"
    "      ['echoes','THE FILE REMEMBERS ITSELF'],\n"
    "      ['hashwar','THE TABLE, CONTESTED, LIVE'],\n"
    "      ['whatif','SMALLER TABLES, PRICED LIVE'],\n"
    "      ['sabotage','FLIP ONE BIT, WATCH IT DIE'],\n"
    "      ['recrunch','PRESS THE EDITED FILE INTO A REAL .PAQ'],\n"
    "      ['audition','EDIT THE MODEL SET. THE BILL IS REAL'],\n"
    "      ['zodiac','FREEZE THE SKY'],\n"
    "      ['void','JUST THE SHOW, NO REPORT'],\n"
    "      ['crt','CURVED GLASS'],\n"
    "      ['amiga','THE BALL'],\n"
    "      ['breakout','SMASH THE FILE'],\n"
    "      ['guru','MEDITATE'],\n"
    "      ['xyzzy idkfa sudo man','UNIX SAYS NO'],\n"
    "      ['atari e2e4 uciok','LOYALTY TESTS'],\n"
    "      ['esc','CLOSE VOID / GAME / THIS BOX']\n"
    "    ];\n"
    "    var h='<div class=\"cbx\"><div class=\"ct\">*** DEV CHEATS ***"
    "</div>';\n"
    "    for(var i=0;i<rows.length;i++)\n"
    "      h+='<div class=\"cr\"><span>'+rows[i][0]+'</span><span>'\n"
    "        +rows[i][1]+'</span></div>';\n"
    "    h+='<div class=\"cf\">TYPED COMMANDS ONLY - THE REST YOU EARN'\n"
    "      +'<br>THIS BOX AWARDS NO EGGS</div></div>';\n"
    "    var ov=document.createElement('div');\n"
    "    ov.id='cheatbox';ov.innerHTML=h;\n"
    "    ov.addEventListener('click',function(){ov.remove();});\n"
    "    document.body.appendChild(ov);\n"
    "    dragBox(ov.querySelector('.cbx'),ov.querySelector('.ct'));\n"
    "  }\n"
    "  /* defrag: the compressibility map, compacted before your eyes.\n"
    "     blocks are input bytes on the cost ramp; the cursor consumes\n"
    "     them and the compacted output (navy) grows at the real\n"
    "     compression ratio - the widening gap IS the ratio */\n"
    "  var dfOn=false,dfOv=null,dfCv2=null,dfCtx2=null,dfRaf=null;\n"
    "  var dfPh=0,dfI=0,dfFr=0,dfN=0,dfCols=0,dfBS=3,dfRows=0;\n"
    "  var dfTotC=0,dfCum=0,dfColA=null,dfStatus=null,dfProg=null;\n"
    "  var dfPc=null,dfWin=null,dfKA=0,dfM=0,dfTip=null;\n"
    "  var DFB='font-family:inherit;font-size:11px;padding:3px 14px;'\n"
    "    +'background:#c0c0c0;border:2px solid;'\n"
    "    +'border-color:#fff #404040 #404040 #fff;cursor:pointer;'\n"
    "    +'min-width:60px';\n"
    "  var DFT='background:#c0c0c0;color:#000;border:1px solid;'\n"
    "    +'border-color:#fff #404040 #404040 #fff;padding:0 4px;'\n"
    "    +'cursor:pointer;font-weight:bold;line-height:1.2;'\n"
    "    +'font-size:10px;min-width:10px;text-align:center';\n"
    "  function dfFmt(n){\n"
    "    return String(n).replace(/\\B(?=(\\d{3})+(?!\\d))/g,',');\n"
    "  }\n"
    "  /* defrag sfx bypass the ducked master: the music is background,\n"
    "     the defragmenter is foreground */\n"
    "  function dfBlip(fr,g,dur){\n"
    "    if(!actx||muted) return;\n"
    "    try{\n"
    "      var t=actx.currentTime;\n"
    "      var o=actx.createOscillator(),og=actx.createGain();\n"
    ...
    "  function dfStop(){\n"
    "    dfOn=false;\n"
    "    if(dfRaf){cancelAnimationFrame(dfRaf);dfRaf=null;}\n"
    "    if(dfOv){dfOv.remove();dfOv=null;}\n"
    "    if(actx&&gainM){try{\n"
    "      gainM.gain.setTargetAtTime(.5,actx.currentTime,.3);\n"
    "    }catch(e){}}\n"
    "  }\n"
    "  /* the win95 message box. yes quits. no lets you admire it */\n"
    "  function dfMsg(){\n"
    "    if(!dfOv||document.getElementById('df-msg')) return;\n"
    "    var m=document.createElement('div');\n"
    "    m.id='df-msg';\n"
    "    m.style.cssText='position:absolute;left:50%%;top:38%%;'\n"
    ...
    "      +'color:#000';\n"
    "    m.innerHTML='<div style=\"background:linear-gradient(90deg,'\n"
    "      +'#000080,#1084d0);color:#fff;font-weight:bold;'\n"
    "      +'padding:3px 6px\">Disk Defragmenter</div>'\n"
    "      +'<div style=\"padding:14px 16px;text-align:center\">'\n"
    "      +'Defragmentation of drive PAQ: is complete.<br>'\n"
    "      +'Do you want to quit Defrag?</div>'\n"
    "      +'<div style=\"display:flex;justify-content:center;gap:8px;'\n"
    "      +'padding:0 0 12px\">'\n"
    "      +'<button id=\"df-yes\" style=\"'+DFB+'\">Yes</button>'\n"
    "      +'<button id=\"df-no\" style=\"'+DFB+'\">No</button>'\n"
    "      +'</div>';\n"
    "    dfOv.appendChild(m);\n"
    "    document.getElementById('df-yes')\n"
    "      .addEventListener('click',dfStop);\n"
    "    document.getElementById('df-no')\n"
    "      .addEventListener('click',function(){m.remove();});\n"
    "  }\n"
    "  function dfStart(){\n"
    "    if(dfOn||typeof BD==='undefined'||!BD.length) return;\n"
    "    egg('defrag');ensureAudio();\n"
    "    /* lower the house music: the defragmenter is working */\n"
    "    if(actx&&gainM){try{\n"
    "      gainM.gain.setTargetAtTime(.16,actx.currentTime,.2);\n"
    "    }catch(e){}}\n"
    "    dfOn=true;dfPh=0;dfI=0;dfFr=0;dfCum=0;dfKA=0;\n"
    "    dfN=Math.min(BD.length,12288);\n"
    "    dfTotC=0;var cmax=.01;\n"
    "    for(var i=0;i<dfN;i++){dfTotC+=BD[i].c;\n"
    "      if(BD[i].c>cmax)cmax=BD[i].c;}\n"
    "    if(dfTotC<=0)dfTotC=1;\n"
    "    dfColA=new Array(dfN);\n"
    "    for(var i=0;i<dfN;i++){\n"
    "      var t=BD[i].c/cmax;if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(t<0.5){var u=t*2;r=16+164*u;g=185-45*u;b=129-89*u;}\n"
    "      else{var u=(t-0.5)*2;r=180+68*u;g=140-27*u;b=40+73*u;}\n"
    "      dfColA[i]='rgb('+(r|0)+','+(g|0)+','+(b|0)+')';\n"
    "    }\n"
    "    var win=Math.min(600,window.innerWidth-40);\n"
    "    var cw=win-28;\n"
    "    var szs=[7,6,5,4,3];\n"
    "    dfBS=3;\n"
    "    for(var k2=0;k2<szs.length;k2++){\n"
    "      var c2=(cw/szs[k2])|0;\n"
    "      if(Math.ceil(dfN/c2)*szs[k2]<=280){dfBS=szs[k2];break;}\n"
    "    }\n"
    "    dfCols=(cw/dfBS)|0;\n"
    "    dfRows=Math.ceil(dfN/dfCols);\n"
    "    dfOv=document.createElement('div');\n"
    "    dfOv.id='defrag';\n"
    "    dfOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,10,.72);display:flex;'\n"
    "      +'align-items:center;justify-content:center';\n"
    "    dfOv.innerHTML=\n"
    "      '<div id=\"df-win\" style=\"width:'+win+'px;background:#c0c0c0;'\n"
    "      +'border:2px solid;border-color:#fff #404040 #404040 #fff;'\n"
    "      +'box-shadow:6px 6px 0 rgba(0,0,0,.5);cursor:wait;'\n"
    "      +'font-family:Tahoma,Verdana,sans-serif;font-size:11px;'\n"
    "      +'color:#000\">'\n"
    "      +'<div id=\"df-tb\" style=\"background:linear-gradient(90deg,'\n"
    "      +'#000080,#1084d0);color:#fff;font-weight:bold;padding:3px 6px;'\n"
    "      +'display:flex;justify-content:space-between;'\n"
    "      +'align-items:center;cursor:move;user-select:none\">'\n"
    "      +'<span style=\"display:flex;align-items:center\">'\n"
    "      +'<span style=\"display:inline-flex;gap:1px;margin-right:6px\">'\n"
    "      +'<span style=\"width:4px;height:9px;background:#0ff\"></span>'\n"
    "      +'<span style=\"width:4px;height:9px;background:#00f\"></span>'\n"
    "      +'<span style=\"width:4px;height:9px;background:#fff\"></span>'\n"
    "      +'</span>Disk Defragmenter - PAQ.EXE</span>'\n"
    "      +'<span style=\"display:flex;gap:2px\">'\n"
    "      +'<span style=\"'+DFT+'\">_</span>'\n"
    "      +'<span style=\"'+DFT+'\">\\u25a1</span>'\n"
    "      +'<span id=\"df-x\" style=\"'+DFT+'\">\\u00d7</span>'\n"
    "      +'</span></div>'\n"
    "      +'<div style=\"padding:10px 12px\">'\n"
    "      +'<div id=\"df-st\" style=\"margin-bottom:8px\">'\n"
    "      +'Analyzing drive PAQ:...</div>'\n"
    "      +'<div id=\"df-det\">'\n"
    "      +'<canvas id=\"df-cv\" width=\"'+(dfCols*dfBS)+'\" height=\"'\n"
    "      +(dfRows*dfBS)+'\" style=\"display:block;background:#fff;'\n"
    "      +'border:2px solid;border-color:#404040 #fff #fff #404040\">'\n"
    "      +'</canvas>'\n"
    "      +'<div style=\"display:flex;gap:12px;margin:8px 0;'\n"
    "      +'flex-wrap:wrap\">'\n"
    "      +'<span><span style=\"display:inline-block;width:9px;'\n"
    "      +'height:9px;background:#f87171\"></span> expensive</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:9px;'\n"
    "      +'height:9px;background:#fbbf24\"></span> moderate</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:9px;'\n"
    "      +'height:9px;background:#10b981\"></span> cheap</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:9px;'\n"
    "      +'height:9px;background:#000080\"></span> compacted</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:9px;'\n"
    "      +'height:9px;background:#e8e8e8;border:1px solid #999\">'\n"
    "      +'</span> free</span>'\n"
    "      +'</div></div>'\n"
    "      +'<div style=\"border:2px solid;border-color:#404040 #fff '\n"
    "      +'#fff #404040;height:16px;background:#fff\">'\n"
    "      +'<div id=\"df-pb\" style=\"height:100%%;width:0;'\n"
    "      +'background:repeating-linear-gradient(90deg,#000080 0 7px,'\n"
    "      +'#fff 7px 9px)\"></div></div>'\n"
    "      +'<div id=\"df-pc\" style=\"text-align:center;margin-top:4px\">'\n"
    "      +'0%% Complete</div>'\n"
    "      +'<div style=\"display:flex;justify-content:flex-end;gap:8px;'\n"
    "      +'margin-top:10px\">'\n"
    "      +'<button id=\"df-stop\" style=\"'+DFB+'\">Stop</button>'\n"
    "      +'<button id=\"df-det-btn\" style=\"'+DFB+'\">Hide Details</button>'\n"
    "      +'</div>'\n"
    "      +'</div></div>';\n"
    "    document.body.appendChild(dfOv);\n"
    "    dfWin=document.getElementById('df-win');\n"
    "    dfCv2=document.getElementById('df-cv');\n"
    "    dfCtx2=dfCv2.getContext('2d');\n"
    "    dfStatus=document.getElementById('df-st');\n"
    "    dfProg=document.getElementById('df-pb');\n"
    "    dfPc=document.getElementById('df-pc');\n"
    "    document.getElementById('df-x').addEventListener('click',dfStop);\n"
    "    document.getElementById('df-stop')\n"
    "      .addEventListener('click',dfStop);\n"
    "    var det=document.getElementById('df-det');\n"
    "    var db=document.getElementById('df-det-btn');\n"
    "    db.addEventListener('click',function(){\n"
    "      var on=det.style.display!=='none';\n"
    "      det.style.display=on?'none':'block';\n"
    "      db.textContent=on?'Show Details':'Hide Details';\n"
    "    });\n"
    "    /* windows move. that is the whole point of windows */\n"
    "    var tb=document.getElementById('df-tb');\n"
    "    var drg=false,dx3=0,dy3=0;\n"
    "    tb.addEventListener('pointerdown',function(e){\n"
    "      if(e.target.id==='df-x') return;\n"
    "      var r=dfWin.getBoundingClientRect();\n"
    "      dfOv.style.display='block';\n"
    "      dfWin.style.position='absolute';\n"
    "      dfWin.style.left=r.left+'px';dfWin.style.top=r.top+'px';\n"
    "      drg=true;dx3=e.clientX-r.left;dy3=e.clientY-r.top;\n"
    "      e.preventDefault();\n"
    "    });\n"
    "    dfOv.addEventListener('pointermove',function(e){\n"
    "      if(!drg) return;\n"
    "      dfWin.style.left=(e.clientX-dx3)+'px';\n"
    "      dfWin.style.top=(e.clientY-dy3)+'px';\n"
    "    });\n"
    "    dfOv.addEventListener('pointerup',function(){drg=false;});\n"
    "    /* hover inspector: a proper yellow tooltip with the truth\n"
    "       about the cluster under the pointer */\n"
    "    dfTip=document.createElement('div');\n"
    "    dfTip.style.cssText='position:absolute;display:none;'\n"
    "      +'z-index:10003;background:#ffffe1;border:1px solid #000;'\n"
    "      +'padding:3px 7px;font-family:Tahoma,Verdana,sans-serif;'\n"
    "      +'font-size:11px;color:#000;pointer-events:none;'\n"
    "      +'line-height:1.5;box-shadow:2px 2px 0 rgba(0,0,0,.35)';\n"
    "    dfOv.appendChild(dfTip);\n"
    "    dfCv2.addEventListener('mousemove',function(e){\n"
    "      var r=dfCv2.getBoundingClientRect();\n"
    "      var cx4=((e.clientX-r.left)*(dfCv2.width/r.width)/dfBS)|0;\n"
    "      var cy4=((e.clientY-r.top)*(dfCv2.height/r.height)/dfBS)|0;\n"
    "      var j=cy4*dfCols+cx4;\n"
    "      if(cx4<0||cx4>=dfCols||j<0||j>=dfN){\n"
    "        dfTip.style.display='none';return;\n"
    "      }\n"
    "      var d=BD[j];\n"
    "      var st;\n"
    "      if(dfPh===0) st='analyzing...';\n"
    "      else if(j<dfM) st='compacted';\n"
    "      else if(j<dfI) st='free space';\n"
    "      else st=d.c<3?'optimized (cheap)':d.c<6\n"
    "        ?'fragmented (moderate)':'badly fragmented (expensive)';\n"
    "      dfTip.innerHTML='Cluster '+dfFmt(j)+' &nbsp;(0x'+blkHex(j)\n"
    "        +')<br>'\n"
    "        +'Value: '+d.h+(d.ch?' &quot;'+(d.ch==='<'?'&lt;'\n"
    "        :d.ch==='&'?'&amp;':d.ch)+'&quot;':'')+'<br>'\n"
    "        +'Cost: '+d.c.toFixed(2)+' bits<br>'\n"
    "        +'Status: '+st;\n"
    "      var tx=e.clientX+14,ty2=e.clientY+16;\n"
    "      if(tx+180>window.innerWidth) tx=e.clientX-190;\n"
    "      if(ty2+76>window.innerHeight) ty2=e.clientY-88;\n"
    "      dfTip.style.left=tx+'px';dfTip.style.top=ty2+'px';\n"
    "      dfTip.style.display='block';\n"
    "    });\n"
    "    dfCv2.addEventListener('mouseleave',function(){\n"
    "      dfTip.style.display='none';\n"
    "    });\n"
    "    if(!dfRaf) dfRaf=requestAnimationFrame(dfFrame);\n"
    "  }\n"
    "  function dfFrame(){\n"
    "    if(!dfOn||!document.body.classList.contains('party')){\n"
    "      dfStop();return;\n"
    "    }\n"
    "    var ctx=dfCtx2,bs=dfBS;\n"
    "    var outB=(typeof REPORT_ID!=='undefined')\n"
    "      ?REPORT_ID.outb:(dfTotC/8)|0;\n"
    "    var inB=(typeof REPORT_ID!=='undefined')\n"
    "      ?REPORT_ID.inb:BD.length;\n"
    "    dfFr++;\n"
    "    if(dfPh===0){\n"
    "      if(dfFr>96){dfPh=1;dfFr=0;\n"
    "        if(dfWin)dfWin.style.cursor='default';}\n"
    "    } else if(dfPh===1){\n"
    "      /* fractional stepping: ~21s for the full pass */\n"
    "      dfKA+=dfN/1260;\n"
    "      while(dfKA>=1&&dfI<dfN){dfKA--;dfCum+=BD[dfI].c;dfI++;}\n"
    "      if(dfFr%%4===0) dfBlip(160+Math.random()*760,.03,.03);\n"
    "      if(dfI>=dfN){\n"
    "        dfPh=2;\n"
    "        var sv=(100*(1-outB/inB)).toFixed(1);\n"
    "        dfStatus.textContent='Compaction complete. '\n"
    "          +dfFmt(inB)+' \\u2192 '+dfFmt(outB)+' bytes ('\n"
    "          +sv+'%% recovered).';\n"
    "        showToast('COMPACTION COMPLETE');\n"
    "        dfBlip(523,.06,.12);\n"
    "        setTimeout(function(){dfBlip(659,.06,.12);},140);\n"
    "        setTimeout(function(){dfBlip(784,.09,.22);},280);\n"
    "        setTimeout(function(){if(dfOn)dfMsg();},1400);\n"
    "      }\n"
    "    }\n"
    "    /* output cells for the shown portion, clamped so the navy\n"
    "       head never outruns the cursor on cost-skewed files */\n"
    "    var M=dfPh>0\n"
    "      ?Math.round(dfCum/dfTotC*outB*dfN/inB):0;\n"
    "    if(M>dfI)M=dfI;\n"
    "    dfM=M; /* the hover inspector reads this */\n"
    "    ctx.fillStyle='#fff';\n"
    "    ctx.fillRect(0,0,dfCv2.width,dfCv2.height);\n"
    "    for(var j=0;j<dfN;j++){\n"
    "      var cx2=(j%%dfCols)*bs,cy2=((j/dfCols)|0)*bs;\n"
    "      if(dfPh>0&&j<M) ctx.fillStyle='#000080';\n"
    "      else if(dfPh>0&&j<dfI) ctx.fillStyle='#e8e8e8';\n"
    "      else ctx.fillStyle=dfColA[j];\n"
    "      ctx.fillRect(cx2,cy2,bs-1,bs-1);\n"
    "    }\n"
    "    if(dfPh===0){\n"
    "      var yr=(((dfFr/96)*dfRows)|0)*bs;\n"
    "      ctx.fillStyle='rgba(255,227,63,.5)';\n"
    "      ctx.fillRect(0,yr,dfCv2.width,bs);\n"
    "    } else if(dfPh===1){\n"
    "      var cx3=(dfI%%dfCols)*bs,cy3=((dfI/dfCols)|0)*bs;\n"
    "      ctx.fillStyle=(dfFr&4)?'#ffe33f':'#fff';\n"
    "      ctx.fillRect(cx3,cy3,bs-1,bs-1);\n"
    "      /* read-ahead flicker: the real one jumped all over the disk */\n"
    "      for(var f2=0;f2<2;f2++){\n"
    "        var rj=dfI+((Math.random()*(dfN-dfI))|0);\n"
    "        if(rj<dfN){\n"
    "          ctx.fillStyle='#fff';\n"
    "          ctx.fillRect((rj%%dfCols)*bs,((rj/dfCols)|0)*bs,\n"
    "            bs-1,bs-1);\n"
    "        }\n"
    "      }\n"
    "      dfStatus.textContent='Defragmenting drive PAQ: ('\n"
    "        +dfFmt(inB)+' \\u2192 '\n"
    "        +dfFmt(Math.round(dfCum/dfTotC*outB))+' bytes)';\n"
    "      dfProg.style.width=(100*dfI/dfN)+'%%';\n"
    "      if(dfPc) dfPc.textContent=((100*dfI/dfN)|0)+'%% Complete';\n"
    "    }\n"
    ...
    "  function cBit(d,p){\n"
    "    var i=d.co+(p>>3);\n"
    "    return ((i<d.cd.length?d.cd[i]:0)>>(p&7))&1;\n"
    "  }\n"
    "  /* cell-size search all the instrument canvases share: biggest\n"
    "     cell from sizes whose grid still fits maxH */\n"
    "  function fitCells(n,maxH,sizes){\n"
    "    var cw=Math.min(window.innerWidth-72,600);\n"
    "    var cell=sizes[sizes.length-1];\n"
    "    for(var k=0;k<sizes.length;k++){\n"
    "      var c2=(cw/sizes[k])|0;\n"
    "      if(Math.ceil(n/c2)*sizes[k]<=maxH){cell=sizes[k];break;}\n"
    "    }\n"
    "    var cols=Math.max(1,(cw/cell)|0);\n"
    "    return {cell:cell,cols:cols,rows:Math.max(1,Math.ceil(n/cols))};\n"
    "  }\n"
    "  /* the byte colors the instruments share: a stream byte at rest,\n"
    "     and one the codec has consumed */\n"
    "  function grayB(v){\n"
    "    var t=v/255,g=(26+t*46)|0;\n"
    "    return 'rgb('+g+','+g+','+((26+t*56)|0)+')';\n"
    "  }\n"
    "  function cynB(v){\n"
    "    var t=v/255;\n"
    "    return 'rgb('+((10+t*24)|0)+','+((90+t*121)|0)+','\n"
    "      +((110+t*128)|0)+')';\n"
    "  }\n"
    "  /* one cursor tooltip for all codec overlays: it follows the\n"
    "     overlay it was last shown in and dies with it */\n"
    "  var ovTipEl=null;\n"
    "  function ovTipHide(){if(ovTipEl)ovTipEl.style.display='none';}\n"
    "  function ovTipAt(ov,accent,e,html){\n"
    "    if(!ov) return;\n"
    "    if(!ovTipEl||ovTipEl.parentNode!==ov){\n"
    "      if(ovTipEl&&ovTipEl.parentNode)\n"
    "        ovTipEl.parentNode.removeChild(ovTipEl);\n"
    "      ovTipEl=document.createElement('div');\n"
    "      ovTipEl.style.cssText='position:fixed;display:none;'\n"
    "        +'z-index:10003;pointer-events:none;'\n"
    "        +'background:rgba(0,0,10,.94);border:1px solid var(--acc);'\n"
    "        +'padding:7px 9px;font-family:var(--pix);font-size:7px;'\n"
    "        +'color:var(--fg);line-height:2;letter-spacing:1px;'\n"
    "        +'box-shadow:4px 4px 0 rgba(0,0,0,.55)';\n"
    "      ov.appendChild(ovTipEl);\n"
    "    }\n"
    "    ovTipEl.style.borderColor=accent;\n"
    "    ovTipEl.innerHTML=html;\n"
    "    var tx=e.clientX+14,ty=e.clientY+16;\n"
    "    if(tx+220>window.innerWidth) tx=e.clientX-234;\n"
    "    if(ty+80>window.innerHeight) ty=e.clientY-92;\n"
    "    ovTipEl.style.left=tx+'px';ovTipEl.style.top=ty+'px';\n"
    "    ovTipEl.style.display='block';\n"
    "  }\n"
    "  /* the decode step every instrument that replays the stream\n"
    "     shares: mix, threshold, renorm, counter update, output bit.\n"
    "     d.costs collects per-byte cost when present, d.occ counts\n"
    "     fresh slots when present. insp returns the microscope's\n"
    "     slide instead of null - it only pays for it when looking */\n"
    "  function decStep(d,insp){\n"
    "    var p0=PAQ.bp,p1=PAQ.bp,num=d.num;\n"
    "    var snap=insp?{m:[]}:null;\n"
    "    for(var m=0;m<num;m++){\n"
    ...
    "      p0+=e0<<sh;p1+=e1<<sh;\n"
    "      d.sl[m]=si;\n"
    "      if(snap) snap.m.push({mk:d.ext[m]&255,w:d.weights[m],\n"
    "        e0:e0,e1:e1,sh:sh,a0:e0<<sh,a1:e1<<sh});\n"
    "    }\n"
    "    var total=p0+p1;\n"
    ...
    "    if(snap){\n"
    "      /* the interval at the moment of decision, pre-renorm */\n"
    "      snap.bp=d.bp;snap.r0=d.range;snap.l0=d.low;snap.v0=d.value;\n"
    "      snap.th=th;snap.p0=p0;snap.p1=p1;\n"
    "    }\n"
    "    var bit,rn=0;\n"
    ...
    "      d.low=(d.low<<1)>>>0;\n"
    "      d.range=(d.range<<1)>>>0;\n"
    "      d.value=((d.value<<1)|cBit(d,d.cpos++))>>>0;\n"
    "      rn++;\n"
    "    }\n"
    "    if(snap){snap.bit=bit;snap.rn=rn;}\n"
    "    for(var m=0;m<num;m++){\n"
    "      var si=d.sl[m];\n"
    "      if(d.occ!==undefined&&!(d.dt[si]|d.dt[si+1])) d.occ++;\n"
    "      d.dt[si+bit]=d.dt[si+bit]+1;\n"
    "      if(d.dt[si+1-bit]>1) d.dt[si+1-bit]>>=1;\n"
    "    }\n"
    "    if(d.bp>0){\n"
    "      if(d.costs) d.costs[(d.bp-1)>>3]+=-Math.log2((bit?p1:p0)/total);\n"
    ...
    "  var dcCC=null,dcCCtx=null,dcCDrawn=0,dcCCell=4,dcCCols=64,dcNbc=0;\n"
    "  var dcPaused=false,dcSnap=null,dcInsp=null,dcDiv=-1;\n"
    "  function dcCrc(h,b){\n"
    "    if(!dcCT){\n"
    ...
    "    return s;\n"
    "  }\n"
    "  function dcHex2(v){return (v<16?'0':'')+v.toString(16).toUpperCase();}\n"
    "  /* hex window around the read head: consumed cyan, current\n"
    "     bracketed yellow, the rest still in the future */\n"
    "  function dcInWin(cb2){\n"
    "    if(dcNbc<1) return '';\n"
    "    var lo=Math.max(0,cb2-3),hi=Math.min(dcNbc-1,cb2+4),s='';\n"
    "    for(var i=lo;i<=hi;i++){\n"
    "      var hx=dcHex2(dc.cd[dc.co+i]);\n"
    "      s+=(i===cb2)?('<span style=\"color:var(--yel)\">['+hx+']</span>')\n"
    "        :(i<cb2?('<span style=\"color:var(--acc)\">'+hx+'</span>'):hx);\n"
    "      s+=' ';\n"
    "    }\n"
    "    return s+'&middot; BIT '+(dc.cpos&7);\n"
    "  }\n"
    "  function dcTipHide(){ovTipHide();}\n"
    "  function dcTipAt(e,html){ovTipAt(dcOv,'var(--acc)',e,html);}\n"
    "  /* binary shown MSB-first; the coder eats LSB-first, so consumed\n"
    "     bits light up cyan from the right */\n"
    "  function dcBitsHtml(v,eaten){\n"
    "    var s='';\n"
    "    for(var k=7;k>=0;k--){\n"
    "      var b=(v>>k)&1;\n"
    "      s+=(k<eaten)?('<span style=\"color:var(--acc)\">'+b+'</span>'):b;\n"
    "    }\n"
    "    return s;\n"
    "  }\n"
    "  function dcCellIdx(e,cv,cols,cell){\n"
    "    var r=cv.getBoundingClientRect();\n"
    "    var x=((e.clientX-r.left)*(cv.width/r.width)/cell)|0;\n"
    "    var y=((e.clientY-r.top)*(cv.height/r.height)/cell)|0;\n"
    ...
    "    for(var i=0;i<31;i++) dc.value=((dc.value<<1)|cBit(dc,dc.cpos++))>>>0;\n"
    "  }\n"
    "  /* the microscope: one bit, laid open. the bar is the coder's\n"
    "     interval [low, low+range) at the moment of decision, split at\n"
    "     thresh into the 1-region (cyan) and 0-region (pink), with VAL\n"
    "     marked in yellow - whichever side it fell on IS the bit */\n"
    "  function dcDrawInsp(){\n"
    "    if(!dcSnap||!dcInsp) return;\n"
    "    var s=dcSnap,tot=s.p0+s.p1;\n"
    "    var f1=s.th/s.r0*100;\n"
    "    var fv=((s.v0-s.l0)>>>0)/s.r0*100;\n"
    "    if(fv<0)fv=0;if(fv>100)fv=100;\n"
    "    var cost=-Math.log2((s.bit?s.p1:s.p0)/tot);\n"
    "    var cclr=cost<3?'#34d399':cost<6?'#fbbf24':'#f87171';\n"
    "    var db2=s.bp-1;\n"
    "    var h='<div><span style=\"color:var(--fg3)\">BIT '+s.bp+' '\n"
    "      +(s.bp===0?'(THE STOP BIT)':'(OUT '+(db2>>3)+' &middot; B'\n"
    "      +(7-(db2&7))+')')+'</span> \\u2192 '\n"
    "      +'<span style=\"color:'+(s.bit?'var(--acc)':'var(--pnk)')\n"
    "      +';font-size:10px\">'+s.bit+'</span>'\n"
    "      +' &middot; <span style=\"color:'+cclr+'\">COST '\n"
    "      +cost.toFixed(2)+'</span>'\n"
    "      +(s.rn?' &middot; RENORM &times;2^'+s.rn:'')+'</div>';\n"
    "    h+='<div style=\"position:relative;height:18px;margin:7px 0 3px;'\n"
    "      +'background:rgba(255,63,216,.25);border:1px solid var(--bdr2)\">'\n"
    "      +'<div style=\"position:absolute;left:0;top:0;bottom:0;width:'\n"
    "      +f1.toFixed(2)+'%%;background:rgba(34,211,238,.4)\"></div>'\n"
    "      +'<div style=\"position:absolute;left:'+fv.toFixed(2)\n"
    "      +'%%;top:-4px;bottom:-4px;width:2px;background:var(--yel)\">'\n"
    "      +'</div></div>';\n"
    "    h+='<div style=\"display:flex;justify-content:space-between;'\n"
    "      +'color:var(--fg3)\"><span><span style=\"color:var(--acc)\">1'\n"
    "      +'</span> P '+(100*s.p1/tot).toFixed(1)+'%%</span>'\n"
    "      +'<span style=\"color:var(--yel)\">VAL</span>'\n"
    "      +'<span>P '+(100*s.p0/tot).toFixed(1)\n"
    "      +'%% <span style=\"color:var(--pnk)\">0</span></span></div>';\n"
    "    h+='<div style=\"color:var(--fg3);font-size:7px\">AT DECISION: '\n"
    "      +'LOW '+dcHex8(s.l0)+' RANGE '+dcHex8(s.r0)\n"
    "      +' THRESH '+dcHex8(s.th)+'</div>';\n"
    "    h+='<div style=\"margin-top:6px;max-height:110px;overflow-y:auto\">'\n"
    "      +'<div style=\"display:flex;gap:10px;color:var(--fg3)\">'\n"
    "      +'<span style=\"width:46px\">MODEL</span>'\n"
    "      +'<span style=\"width:64px\">CTR 1/0</span>'\n"
    "      +'<span style=\"width:34px\">SHL</span>'\n"
    "      +'<span style=\"flex:1;text-align:right\">&rarr;P1</span>'\n"
    "      +'<span style=\"flex:1;text-align:right\">&rarr;P0</span></div>';\n"
    "    for(var m=0;m<s.m.length;m++){\n"
    "      var mm=s.m[m];\n"
    "      h+='<div style=\"display:flex;gap:10px\">'\n"
    "        +'<span style=\"width:46px;color:var(--acc)\">'\n"
    "        +dcHex2(mm.mk)+':'+mm.w+'</span>'\n"
    "        +'<span style=\"width:64px\">'+mm.e1+'/'+mm.e0+'</span>'\n"
    "        +'<span style=\"width:34px\">'+mm.sh+'</span>'\n"
    "        +'<span style=\"flex:1;text-align:right;color:'\n"
    "        +(mm.a1?'var(--acc)':'var(--fg3)')+'\">+'+mm.a1+'</span>'\n"
    "        +'<span style=\"flex:1;text-align:right;color:'\n"
    "        +(mm.a0?'var(--pnk)':'var(--fg3)')+'\">+'+mm.a0+'</span></div>';\n"
    "    }\n"
    "    h+='<div style=\"display:flex;gap:10px;color:var(--fg2);'\n"
    "      +'border-top:1px solid var(--bdr);margin-top:2px;'\n"
    "      +'padding-top:2px\">'\n"
    "      +'<span style=\"width:46px\">BASE</span>'\n"
    "      +'<span style=\"width:64px\"></span>'\n"
    "      +'<span style=\"width:34px\"></span>'\n"
    "      +'<span style=\"flex:1;text-align:right\">+'+PAQ.bp+'</span>'\n"
    "      +'<span style=\"flex:1;text-align:right\">+'+PAQ.bp+'</span></div>'\n"
    "      +'<div style=\"display:flex;gap:10px;color:var(--fg)\">'\n"
    "      +'<span style=\"width:46px\">TOTAL</span>'\n"
    "      +'<span style=\"width:64px\"></span>'\n"
    "      +'<span style=\"width:34px\"></span>'\n"
    "      +'<span style=\"flex:1;text-align:right;color:var(--acc)\">'\n"
    "      +s.p1+'</span>'\n"
    "      +'<span style=\"flex:1;text-align:right;color:var(--pnk)\">'\n"
    "      +s.p0+'</span></div></div>';\n"
    "    dcInsp.innerHTML=h;\n"
    "    dcInsp.style.display='block';\n"
    "  }\n"
    "  var dcBlipT=0;\n"
    "  function dcStepTick(){\n"
    "    /* one voice, rate-limited: key autorepeat and byte-stepping\n"
    "       were stacking oscillators on the same instant */\n"
    "    var now=Date.now();\n"
    "    if(now-dcBlipT<40) return;\n"
    "    dcBlipT=now;\n"
    "    blip(340+((dcSnap&&dcSnap.bit)?160:0),.02,.03);\n"
    "  }\n"
    "  function dcStepOne(quiet){\n"
    "    if(!dcOn||dcDone||!dc) return;\n"
    "    if(!dcPaused){dcPaused=true;\n"
    "      showToast('THE CODER HOLDS ITS BREATH');}\n"
    "    egg('microscope');\n"
    "    if(dc.bp<dc.bitlen){\n"
    "      var sn=decStep(dc,true);\n"
    "      if(sn) dcSnap=sn;\n"
    "    }\n"
    "    if(dc.bp>=dc.bitlen&&!dcDone){dcDone=true;dcVerify();}\n"
    "    if(!quiet){dcStepTick();dcDrawInsp();}\n"
    "  }\n"
    "  function dcStepByte(){\n"
    "    if(!dcOn||dcDone||!dc) return;\n"
    "    do{dcStepOne(true);}while(dcOn&&!dcDone&&((dc.bp-1)&7)!==0);\n"
    "    dcStepTick();\n"
    "    dcDrawInsp();\n"
    "  }\n"
    "  function dcRamp(c){\n"
    ...
    "  }\n"
    "  /* the byte the stream SHOULD decode to at i: the recrunched\n"
    "     bytes if a pressing was swapped in, else the pristine\n"
    "     snapshot while tuner edits are live, else BD. -1 = no\n"
    "     reference, the crc alone decides */\n"
    "  function dcRefByte(i){\n"
    "    if(rcBytes) return i<rcBytes.length?rcBytes[i]:-1;\n"
    "    if(window.TUNED)\n"
    "      return (tuBase&&i<tuBase.length)?tuBase[i]:-1;\n"
    "    if(typeof BD!=='undefined'&&i<BD.length)\n"
    "      return parseInt(BD[i].h,16);\n"
    "    return -1;\n"
    "  }\n"
    "  function dcVerify(){\n"
    "    var c=0xFFFFFFFF;\n"
    ...
    "    var mm=0,m0=-1;\n"
    "    /* grade against what the stream actually carries - dcRefByte\n"
    "       picks the same reference the live rot check painted */\n"
    "    var tuned=!!window.TUNED;\n"
    "    for(var i=0;i<dc.nb;i++){\n"
    "      var rb=dcRefByte(i);\n"
    "      if(rb>=0&&rb!==dc.buf[8+i]){if(!mm)m0=i;mm++;}\n"
    "    }\n"
    "    if(hex===want&&!mm){\n"
    "      egg('decrunch');\n"
    "      dcStat.style.color='#34d399';\n"
    "      dcStat.textContent='CRC '+hex+' = INPUT. THE FILE IS REAL.'\n"
    "        +(rcBytes?(tuned?' (YOUR EDITS ARE IN IT)':' (RECRUNCHED)')\n"
    "          :tuned?' (YOUR EDITS ARE NOT IN IT)':'');\n"
    "      showToast(rcBytes?'YOUR PRESSING HOLDS UP'\n"
    "        :tuned?'TRUE. MINUS YOUR EDITS.':'IT WAS ALL TRUE');\n"
    "      blip(523,.05,.12);\n"
    ...
    "         screen, the guru is the only honest response - but the\n"
    "         console gets the full autopsy first */\n"
    "      try{\n"
    "        var tn=0;\n"
    "        if(window.TUNED)\n"
    "          for(var i=0;i<window.TUNED.length;i++)\n"
    "            if(window.TUNED[i]) tn++;\n"
    "        console.error('decrunch verify failed: crc '+hex\n"
    "          +(hex===want?' = input crc (stream decodes correctly)'\n"
    "            :' != expected '+want)\n"
    "          +'; '+mm+'/'+dc.nb+' decoded bytes disagree with '\n"
    "          +(rcBytes?'the recrunched bytes'\n"
    "            :tuned?'the original bytes':'BD')\n"
    "          +(m0>=0?'; first at 0x'+m0.toString(16).toUpperCase()\n"
    "            +': decoded '+dcHex2(dc.buf[8+m0])+' vs '\n"
    "            +dcHex2(dcRefByte(m0)):'')\n"
    "          +(tn?'; TUNED: '+tn+' edited byte(s) active':''));\n"
    "      }catch(e){}\n"
    ...
    "       finished picture stays up. cd stays: the hex readout and the\n"
    "       stream strip still show it */\n"
    "    dc.dt=null;dc.sl=null;\n"
    "  }\n"
    "  function dcStop(){\n"
    "    dcOn=false;dcDone=false;\n"
    "    dcPaused=false;dcSnap=null;dcInsp=null;\n"
    ...
    "    dcDone=false;dcTurbo=false;dcDrawn=0;dcFr=0;dcDiv=-1;\n"
    "    var ft=fitCells(dc.nb,300,[8,7,6,5,4,3,2]);\n"
    "    dcCell=ft.cell;dcCols=ft.cols;\n"
    "    var rows=ft.rows;\n"
    "    dcNbc=dc.cd.length-dc.co;\n"
    "    var f2=fitCells(dcNbc,110,[6,5,4,3,2,1]);\n"
    "    dcCCell=f2.cell;dcCCols=f2.cols;\n"
    "    var crows=f2.rows;\n"
    ...
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:4px\">'\n"
    "      +'COMPRESSED IN &middot; '+dcNbc+' B</div>'\n"
    "      +'<canvas id=\"dc-cc\" width=\"'+(dcCCols*dcCCell)+'\" height=\"'\n"
    "      +(crows*dcCCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);margin-bottom:8px;'\n"
    "      +'cursor:crosshair\"></canvas>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:4px\">'\n"
    "      +'DECODED OUT &middot; '+dc.nb+' B</div>'\n"
    "      +'<canvas id=\"dc-cv\" width=\"'+(dcCols*dcCell)+'\" height=\"'\n"
    "      +(rows*dcCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);cursor:crosshair\"></canvas>'\n"
    "      +'<div id=\"dc-regs\" style=\"margin-top:10px;line-height:2.1;'\n"
    "      +'color:var(--fg2)\"></div>'\n"
    "      +'<div id=\"dc-insp\" style=\"display:none;margin-top:8px;'\n"
    "      +'border-top:1px solid var(--bdr);padding-top:8px;'\n"
    "      +'color:var(--fg2);line-height:1.9\"></div>'\n"
    "      +'<div id=\"dc-st\" style=\"margin-top:4px;color:var(--yel);'\n"
    "      +'min-height:14px\"></div>'\n"
    "      +'<div style=\"margin-top:8px;color:var(--fg3);font-size:7px\">'\n"
    "      +'SPACE = ROLL / TURBO &middot; . = ONE BIT &middot; '\n"
    "      +', = ONE BYTE &middot; ESC = LOOK AWAY</div></div>';\n"
    "    document.body.appendChild(dcOv);\n"
    "    dcCv=document.getElementById('dc-cv');\n"
    "    dcCtx=dcCv.getContext('2d');\n"
    "    dcCC=document.getElementById('dc-cc');\n"
    "    dcCCtx=dcCC.getContext('2d');\n"
    "    dcCDrawn=0;\n"
    "    /* the stream at rest: gray cells, textured by byte value */\n"
    "    var cpad0=dcCCell>2?1:0;\n"
    "    for(var i=0;i<dcNbc;i++){\n"
    "      dcCCtx.fillStyle=grayB(dc.cd[dc.co+i]);\n"
    "      dcCCtx.fillRect((i%%dcCCols)*dcCCell,((i/dcCCols)|0)*dcCCell,\n"
    "        dcCCell-cpad0,dcCCell-cpad0);\n"
    "    }\n"
    "    /* hover truth, both directions */\n"
    "    dcCC.addEventListener('mousemove',function(e){\n"
    "      if(!dc){dcTipHide();return;}\n"
    "      var j=dcCellIdx(e,dcCC,dcCCols,dcCCell);\n"
    "      if(j<0||j>=dcNbc){dcTipHide();return;}\n"
    "      var v=dc.cd[dc.co+j],head=dc.cpos>>3;\n"
    "      var st,eaten;\n"
    "      if(j<head){\n"
    "        st='<span style=\"color:var(--acc)\">CONSUMED</span>';eaten=8;\n"
    "      } else if(j===head&&!dcDone){\n"
    "        st='<span style=\"color:var(--yel)\">UNDER THE HEAD '\n"
    "          +'&middot; BIT '+(dc.cpos&7)+'</span>';\n"
    "        eaten=dc.cpos&7;\n"
    "      } else {st='NOT YET READ';eaten=0;}\n"
    "      dcTipAt(e,'IN 0x'+dcHex8(j).slice(4)\n"
    "        +' <span style=\"color:var(--fg3)\">(FILE 0x'\n"
    "        +dcHex8(dc.co+j).slice(4)+')</span><br>'\n"
    "        +'BYTE '+dcHex2(v)+' &middot; '+dcBitsHtml(v,eaten)\n"
    "        +'<br>'+st);\n"
    "    });\n"
    "    dcCC.addEventListener('mouseleave',dcTipHide);\n"
    "    dcCv.addEventListener('mousemove',function(e){\n"
    "      if(!dc){dcTipHide();return;}\n"
    "      var i=dcCellIdx(e,dcCv,dcCols,dcCell);\n"
    "      if(i<0||i>=dc.nb){dcTipHide();return;}\n"
    "      var done=dc.bp>0?Math.min(dc.nb,(dc.bp-1)>>3):0;\n"
    "      var h;\n"
    "      if(i<done){\n"
    "        var v=dc.buf[8+i],c=dc.costs[i];\n"
    "        var ch=(v>=0x20&&v<=0x7E&&v!==60&&v!==62&&v!==38)\n"
    "          ?(' \\''+String.fromCharCode(v)+'\\''):'';\n"
    "        var clr=c<3?'#34d399':c<6?'#fbbf24':'#f87171';\n"
    "        h='OUT '+i+' &middot; BYTE '+dcHex2(v)+ch+'<br>'\n"
    "          +'<span style=\"color:'+clr+'\">COST '+c.toFixed(2)\n"
    "          +' BITS</span>';\n"
    "        /* the live decoder vs what the report claimed: the same\n"
    "           number, or the guru is owed an explanation */\n"
    "        if(typeof BD!=='undefined'&&BD[i])\n"
    "          h+='<br><span style=\"color:var(--fg3)\">REPORT SAID '\n"
    "            +BD[i].c.toFixed(2)+'</span>';\n"
    "        var rb2=dcRefByte(i);\n"
    "        if(rb2>=0&&rb2!==v)\n"
    "          h+='<br><span style=\"color:#f87171\">REFERENCE SAYS '\n"
    "            +dcHex2(rb2)+'. THE STREAM DISAGREES.</span>';\n"
    "        else if(window.TUNED&&window.TUNED[i]&&\n"
    "           typeof BD!=='undefined'&&BD[i]&&\n"
    "           parseInt(BD[i].h,16)!==v)\n"
    "          h+='<br><span style=\"color:#ff3fd8\">YOUR EDIT IS NOT'\n"
    "            +' IN THIS STREAM. RECRUNCH TO PRESS IT.</span>';\n"
    "      } else {\n"
    "        h='OUT '+i+'<br><span style=\"color:var(--fg3)\">'\n"
    "          +'STILL IN THE FUTURE</span>';\n"
    "      }\n"
    "      dcTipAt(e,h);\n"
    "    });\n"
    "    dcCv.addEventListener('mouseleave',dcTipHide);\n"
    "    dcRegs=document.getElementById('dc-regs');\n"
    "    dcStat=document.getElementById('dc-st');\n"
    "    dcInsp=document.getElementById('dc-insp');\n"
    "    /* armed, not rolling: space releases it */\n"
    "    dcPaused=true;dcSnap=null;\n"
    "    dcStat.textContent=(dc.cd.length-dc.co)\n"
    "      +' PAYLOAD BYTES STAGED. SPACE ROLLS THE TAPE.';\n"
    ...
    "      while(dc.bp<dc.bitlen&&n--) decStep(dc);\n"
    ...
    "      var db=dc.buf[8+dcDrawn],rb=dcRefByte(dcDrawn);\n"
    "      if(rb>=0&&db!==rb){\n"
    "        /* sabotage red: the stream does not decode to the\n"
    "           reference. unreachable outside a real bug */\n"
    "        dcCtx.fillStyle='#c8323c';\n"
    "        dcCtx.fillRect(x,y,dcCell-pad,dcCell-pad);\n"
    "        if(dcDiv<0){\n"
    "          dcDiv=dcDrawn;\n"
    "          dcStat.style.color='#f87171';\n"
    "          dcStat.textContent='DIVERGES FROM THE REFERENCE AT 0x'\n"
    "            +blkHex(dcDrawn)+'. THE VERDICT WILL NOT BE KIND.';\n"
    "          showToast('THE ROT SETS IN AT 0x'+blkHex(dcDrawn));\n"
    "          blip(90,.05,.3);\n"
    "        }\n"
    "      } else {\n"
    "        dcCtx.fillStyle=dcRamp(dc.costs[dcDrawn]);\n"
    "        dcCtx.fillRect(x,y,dcCell-pad,dcCell-pad);\n"
    "        /* a tuner edit the stream does not carry yet: pink dot,\n"
    "           the same convention the rest of the report uses */\n"
    "        if(window.TUNED&&window.TUNED[dcDrawn]&&\n"
    "           typeof BD!=='undefined'&&BD[dcDrawn]&&\n"
    "           parseInt(BD[dcDrawn].h,16)!==db){\n"
    "          dcCtx.fillStyle='#ff3fd8';\n"
    "          dcCtx.fillRect(x,y,2,2);\n"
    "        }\n"
    "      }\n"
    "    }\n"
    ...
    "      dcCtx.fillRect(x,y,dcCell-pad,dcCell-pad);\n"
    "      /* the coder hums its progress: pitch is the last byte's\n"
    "         pain. silent while paused - the microscope gets quiet */\n"
    "      if(!dcPaused&&!(dcFr&3)&&done>0)\n"
    "        blip(160+Math.min(900,dc.costs[done-1]*90),.02,.04);\n"
    "    }\n"
    "    /* the stream being eaten: consumed bytes turn cyan, the byte\n"
    "       under the read head flashes. bits go strictly left to right,\n"
    "       so the playhead is real, not decoration */\n"
    "    var cb2=Math.min(dcNbc,dc.cpos>>3);\n"
    "    var cpad2=dcCCell>2?1:0;\n"
    "    for(;dcCDrawn<cb2;dcCDrawn++){\n"
    "      dcCCtx.fillStyle=cynB(dc.cd[dc.co+dcCDrawn]);\n"
    "      dcCCtx.fillRect((dcCDrawn%%dcCCols)*dcCCell,\n"
    "        ((dcCDrawn/dcCCols)|0)*dcCCell,dcCCell-cpad2,dcCCell-cpad2);\n"
    "    }\n"
    "    if(!dcDone&&cb2<dcNbc){\n"
    "      dcCCtx.fillStyle=(dcFr&8)?'#ffe33f':'#ff3fd8';\n"
    "      dcCCtx.fillRect((cb2%%dcCCols)*dcCCell,\n"
    "        ((cb2/dcCCols)|0)*dcCCell,dcCCell-cpad2,dcCCell-cpad2);\n"
    "    } else if(cb2<dcNbc){\n"
    "      /* done with the tail unread (the decoder prereads 31 bits and\n"
    "         stops at bitlen): put the head cell back to rest, honestly */\n"
    "      dcCCtx.fillStyle=grayB(dc.cd[dc.co+cb2]);\n"
    "      dcCCtx.fillRect((cb2%%dcCCols)*dcCCell,\n"
    "        ((cb2/dcCCols)|0)*dcCCell,dcCCell-cpad2,dcCCell-cpad2);\n"
    "    }\n"
    ...
    "      +(dcPaused?' &middot; <span style=\"color:var(--yel)\">STEP'\n"
    "      +'</span>':'')\n"
    "      +'<br>IN 0x'+dcHex8(Math.min(cb2,dcNbc-1)).slice(4)+' '\n"
    "      +dcInWin(Math.min(cb2,dcNbc-1));\n"
    ...
    "    setTimeout(function(){\n"
    "      document.body.classList.remove('degauss');},520);\n"
    "    ensureAudio();\n"
    "    if(!actx||muted) return;\n"
    "    try{\n"
    "      var t=actx.currentTime;\n"
    ...
    "        var o=actx.createOscillator(),g=actx.createGain();\n"
    "        o.type='sine';\n"
    "        o.frequency.setValueAtTime(55+i*1.7,t);\n"
    "        o.frequency.exponentialRampToValueAtTime(30,t+.4);\n"
    "        g.gain.setValueAtTime(.09,t);\n"
    "        g.gain.exponentialRampToValueAtTime(.001,t+.45);\n"
    "        o.connect(g);g.connect(gainM);o.start(t);o.stop(t+.5);\n"
    "      }\n"
    "    }catch(e){}\n"
    "  };\n"
    "  /* the boing ball. it demands nothing. it bounces */\n"
    "  var boing=null,boingRaf=null,bbx=0,bby=0,bbvx=2.6,bbvy=0;\n"
    "  var bbPhase=0,bbHitT=0,BB_R=52;\n"
    "  function boingToggle(){\n"
    "    if(boing){boingStop();showToast('BALL RETRACTED');return;}\n"
    "    egg('amiga');\n"
    "    boing=document.createElement('canvas');\n"
    "    boing.width=150;boing.height=150;\n"
    "    boing.style.cssText='position:fixed;left:0;top:0;z-index:9998;'\n"
    "      +'pointer-events:none';\n"
    "    document.body.appendChild(boing);\n"
    "    bbx=BB_R+30;bby=window.innerHeight/3;bbvx=2.6;bbvy=0;\n"
    "    showToast('BOING');\n"
    "    if(!boingRaf) boingRaf=requestAnimationFrame(boingFrame);\n"
    "  }\n"
    "  function boingStop(){\n"
    "    if(boingRaf){cancelAnimationFrame(boingRaf);boingRaf=null;}\n"
    "    if(boing){boing.remove();boing=null;}\n"
    "  }\n"
    "  function boingFrame(){\n"
    "    if(!boing||!document.body.classList.contains('party')){\n"
    "      boingStop();return;}\n"
    "    var R=BB_R,W=window.innerWidth,H=window.innerHeight;\n"
    "    bbvy+=.35;bbx+=bbvx;bby+=bbvy;\n"
    "    /* mourning halves the bounce; the ball knows */\n"
    "    var jump=document.body.classList.contains('mourning')?9:14;\n"
    "    if(bby+R>H-6){bby=H-6-R;bbvy=-jump;blip(85,.06,.18);}\n"
    "    if(bbx-R<0){bbx=R;bbvx=-bbvx;blip(140,.04,.1);}\n"
    "    if(bbx+R>W){bbx=W-R;bbvx=-bbvx;blip(140,.04,.1);}\n"
    "    /* the cube is in the way. the cube gets kicked */\n"
    "    var gz=document.getElementById('glenz');\n"
    "    if(gz&&!gz.classList.contains('dead')&&Date.now()-bbHitT>400){\n"
    "      var gr=gz.getBoundingClientRect();\n"
    "      var gcx=gr.left+gr.width/2,gcy=gr.top+gr.height/2;\n"
    "      if(Math.abs(bbx-gcx)<R+gr.width/2&&\n"
    "         Math.abs(bby-gcy)<R+gr.height/2){\n"
    "        bbHitT=Date.now();\n"
    "        if((bbx<gcx&&bbvx>0)||(bbx>gcx&&bbvx<0)) bbvx=-bbvx;\n"
    "        gz.classList.add('kick');\n"
    "        setTimeout(function(){gz.classList.remove('kick');},1500);\n"
    "        blip(500,.04,.08);\n"
    "      }\n"
    "    }\n"
    "    bbPhase+=bbvx>0?.05:-.05;\n"
    "    boing.style.left=(bbx-75)+'px';boing.style.top=(bby-75)+'px';\n"
    "    var ctx=boing.getContext('2d');\n"
    "    ctx.clearRect(0,0,150,150);\n"
    "    ctx.fillStyle='rgba(0,0,0,.3)';\n"
    "    ctx.beginPath();\n"
    "    ctx.ellipse(84,84,R,R*.94,0,0,6.29);ctx.fill();\n"
    "    ctx.save();\n"
    "    ctx.beginPath();ctx.arc(75,75,R,0,6.29);ctx.clip();\n"
    "    ctx.fillStyle='#fff';ctx.fillRect(0,0,150,150);\n"
    "    var tilt=-.32;\n"
    "    function pt(la,lo){\n"
    "      var cl=Math.cos(la);\n"
    "      var x=R*cl*Math.sin(lo),y=R*Math.sin(la);\n"
    "      var z=cl*Math.cos(lo);\n"
    "      return {x:75+x*Math.cos(tilt)-y*Math.sin(tilt),\n"
    "              y:75+x*Math.sin(tilt)+y*Math.cos(tilt),z:z};\n"
    "    }\n"
    "    ctx.fillStyle='#e02020';\n"
    "    for(var la=0;la<8;la++){\n"
    "      for(var lo=0;lo<16;lo++){\n"
    "        if((la+lo)%%2) continue;\n"
    "        var a0=-Math.PI/2+la*Math.PI/8,a1=a0+Math.PI/8;\n"
    "        var o0=lo*Math.PI/8+bbPhase,o1=o0+Math.PI/8;\n"
    "        var p0=pt(a0,o0),p1=pt(a0,o1),p2=pt(a1,o1),p3=pt(a1,o0);\n"
    "        if(p0.z+p1.z+p2.z+p3.z<=0) continue;\n"
    "        ctx.beginPath();\n"
    ...
    "    }\n"
    "    ctx.restore();\n"
    "    boingRaf=requestAnimationFrame(boingFrame);\n"
    "  }\n"
    "  /* breakout: the compressibility map, weaponized. brick HP = cost,\n"
    "     score = bits freed, each level is the next 256 bytes of the file */\n"
    "  var bkOn=false,bkRaf=null,bkOv=null,bkCv=null,bkCtx=null;\n"
    "  var bkBricks=[],bkLvl=0,bkLives=3,bkScore=0,bkAlive=0,bkBroken=0;\n"
    "  var bkBx=0,bkBy=0,bkVx=0,bkVy=0,bkPx=0,bkStuck=true;\n"
    "  var bkW=0,bkH=0,bkSpd=4,bkMsg=null,bkMsgT=0,bkOver=false,bkWon=false;\n"
    "  var BK_COLS=32,BK_PER=256,BK_PW=76,BK_PH=8,BK_R=4;\n"
    "  function bkLevels(){return Math.ceil(BD.length/BK_PER);}\n"
    "  /* same green->amber->red gradient as the compressibility map */\n"
    "  function bkColor(c,hp,hp0){\n"
    "    var t=c/8; if(t>1)t=1;\n"
    "    var r,g,b;\n"
    "    if(t<0.5){var u=t*2;r=16+164*u;g=185-45*u;b=129-89*u;}\n"
    "    else{var u=(t-0.5)*2;r=180+68*u;g=140-27*u;b=40+73*u;}\n"
    "    var d=0.4+0.6*(hp/hp0);\n"
    "    return 'rgb('+((r*d)|0)+','+((g*d)|0)+','+((b*d)|0)+')';\n"
    "  }\n"
    "  function bkLoad(){\n"
    "    var start=bkLvl*BK_PER;\n"
    "    var n=Math.min(BK_PER,BD.length-start);\n"
    "    bkBricks=[];bkAlive=0;\n"
    "    var bw=bkW/BK_COLS,bh=13;\n"
    "    for(var i=0;i<n;i++){\n"
    "      var c=BD[start+i].c;\n"
    "      var hp=c<3?1:c<6?2:3;\n"
    "      bkBricks.push({x:(i%%BK_COLS)*bw,y:34+((i/BK_COLS)|0)*bh,\n"
    "        w:bw-1,h:bh-1,c:c,hp:hp,hp0:hp});\n"
    "      bkAlive++;\n"
    "    }\n"
    "    bkSpd=Math.min(8,4+bkLvl*0.3);\n"
    "    bkStuck=true;bkOver=false;\n"
    "  }\n"
    "  function bkFlash(t){bkMsg=t;bkMsgT=Date.now();}\n"
    "  function bkServe(){\n"
    "    if(bkOver){\n"
    "      if(!bkWon){bkLives=3;bkLoad();bkFlash('TRY AGAIN');}\n"
    "      return;\n"
    "    }\n"
    "    if(bkStuck){\n"
    "      bkStuck=false;\n"
    "      var a=-Math.PI/2+(Math.random()*.6-.3);\n"
    "      bkVx=Math.cos(a)*bkSpd;bkVy=Math.sin(a)*bkSpd;\n"
    "      blip(440,.03,.06);\n"
    "    }\n"
    "  }\n"
    "  function bkStart(){\n"
    "    if(bkOn||typeof BD==='undefined'||!BD.length) return;\n"
    "    egg('breakout');\n"
    "    bkOn=true;bkLvl=0;bkLives=3;bkScore=0;bkWon=false;\n"
    "    bkOv=document.createElement('div');\n"
    "    bkOv.id='breakout';\n"
    "    bkOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,4,.94);display:flex;align-items:center;'\n"
    "      +'justify-content:center;cursor:none';\n"
    "    bkCv=document.createElement('canvas');\n"
    "    bkW=Math.min(window.innerWidth-40,640);\n"
    "    bkH=Math.min(window.innerHeight-40,480);\n"
    "    bkCv.width=bkW;bkCv.height=bkH;\n"
    "    bkOv.appendChild(bkCv);\n"
    "    document.body.appendChild(bkOv);\n"
    "    bkCtx=bkCv.getContext('2d');\n"
    "    bkPx=bkW/2;\n"
    "    bkOv.addEventListener('mousemove',function(e){\n"
    "      var r=bkCv.getBoundingClientRect();\n"
    "      var x=e.clientX-r.left;\n"
    "      if(x<BK_PW/2)x=BK_PW/2;\n"
    "      if(x>bkW-BK_PW/2)x=bkW-BK_PW/2;\n"
    "      bkPx=x;\n"
    "    });\n"
    "    bkOv.addEventListener('click',bkServe);\n"
    "    bkLoad();\n"
    "    bkFlash('LEVEL 1 / '+bkLevels());\n"
    "    if(!bkRaf) bkRaf=requestAnimationFrame(bkFrame);\n"
    "  }\n"
    "  function bkStop(){\n"
    "    bkOn=false;\n"
    "    if(bkRaf){cancelAnimationFrame(bkRaf);bkRaf=null;}\n"
    "    if(bkOv){bkOv.remove();bkOv=null;}\n"
    "  }\n"
    "  /* capture phase: while the game is up, keys belong to the game\n"
    "     (except M, so the music stays mutable) */\n"
    "  document.addEventListener('keydown',function(e){\n"
    "    if(!bkOn) return;\n"
    "    if(e.key==='m'||e.key==='M') return;\n"
    "    if(e.key==='Escape'){bkStop();showToast('COWARD');}\n"
    "    else if(e.key===' '){bkServe();e.preventDefault();}\n"
    "    e.stopPropagation();\n"
    "  },true);\n"
    "  function bkFrame(){\n"
    "    if(!bkOn||!document.body.classList.contains('party')){bkStop();return;}\n"
    "    var ctx=bkCtx,W=bkW,H=bkH,py=H-24;\n"
    "    ctx.fillStyle='#000004';ctx.fillRect(0,0,W,H);\n"
    "    if(bkStuck){bkBx=bkPx;bkBy=py-BK_R-1;}\n"
    "    if(!bkStuck&&!bkOver){\n"
    "      bkBx+=bkVx;bkBy+=bkVy;\n"
    "      if(bkBx<BK_R){bkBx=BK_R;bkVx=-bkVx;}\n"
    "      if(bkBx>W-BK_R){bkBx=W-BK_R;bkVx=-bkVx;}\n"
    "      if(bkBy<BK_R){bkBy=BK_R;bkVy=-bkVy;}\n"
    "      if(bkVy>0&&bkBy>py-BK_R&&bkBy<py+BK_PH&&\n"
    "         bkBx>bkPx-BK_PW/2-BK_R&&bkBx<bkPx+BK_PW/2+BK_R){\n"
    "        var rel=(bkBx-bkPx)/(BK_PW/2);\n"
    "        var ang=-Math.PI/2+rel*1.05;\n"
    "        bkVx=Math.cos(ang)*bkSpd;bkVy=Math.sin(ang)*bkSpd;\n"
    "        bkBy=py-BK_R;\n"
    "        blip(300,.02,.04);\n"
    "      }\n"
    ...
    "        var b=bkBricks[i];\n"
    "        if(!b.hp) continue;\n"
    "        if(bkBx+BK_R<b.x||bkBx-BK_R>b.x+b.w||\n"
    "           bkBy+BK_R<b.y||bkBy-BK_R>b.y+b.h) continue;\n"
    "        var ox=Math.min(bkBx+BK_R-b.x,b.x+b.w-(bkBx-BK_R));\n"
    "        var oy=Math.min(bkBy+BK_R-b.y,b.y+b.h-(bkBy-BK_R));\n"
    "        if(ox<oy) bkVx=-bkVx; else bkVy=-bkVy;\n"
    "        b.hp--;\n"
    "        if(!b.hp){\n"
    "          bkAlive--;bkBroken++;bkScore+=b.c;\n"
    "          blip(200+b.c*70,.035,.07);\n"
    "        } else blip(140,.03,.05);\n"
    "        break;\n"
    "      }\n"
    "      if(bkBy>H+BK_R){\n"
    "        bkLives--;blip(70,.06,.3);\n"
    "        if(bkLives<=0){bkOver=true;bkFlash('OUT OF BALLS');}\n"
    "        else{bkStuck=true;\n"
    "          bkFlash(bkLives+(bkLives===1?' BALL':' BALLS')+' LEFT');}\n"
    "      }\n"
    "      if(!bkAlive){\n"
    "        bkLvl++;\n"
    "        if(bkLvl>=bkLevels()){\n"
    "          bkWon=true;bkOver=true;bkFlash('FILE DELETED');\n"
    "          setTimeout(function(){if(bkOn)bkFlash('... JUST KIDDING');},1500);\n"
    "          setTimeout(function(){\n"
    "            if(bkOn){bkStop();showToast('THE FILE FORGIVES YOU');}\n"
    "          },3100);\n"
    "        } else {\n"
    "          bkLoad();blip(520,.05,.15);\n"
    "          bkFlash('LEVEL '+(bkLvl+1)+' / '+bkLevels());\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    for(var i=0;i<bkBricks.length;i++){\n"
    "      var b=bkBricks[i]; if(!b.hp) continue;\n"
    "      ctx.fillStyle=bkColor(b.c,b.hp,b.hp0);\n"
    "      ctx.fillRect(b.x,b.y,b.w,b.h);\n"
    "    }\n"
    "    ctx.fillStyle='#22d3ee';\n"
    "    ctx.fillRect(bkPx-BK_PW/2,py,BK_PW,BK_PH);\n"
    "    if(!bkOver){\n"
    "      ctx.fillStyle='#ffe33f';\n"
    "      ctx.fillRect(bkBx-BK_R,bkBy-BK_R,BK_R*2,BK_R*2);\n"
    "    }\n"
    "    ctx.font='8px \"Press Start 2P\",monospace';\n"
    "    ctx.textAlign='left';ctx.fillStyle='#63636f';\n"
    "    var st=Math.min(bkLvl,bkLevels()-1)*BK_PER;\n"
    "    ctx.fillText('LVL '+Math.min(bkLvl+1,bkLevels())+'/'+bkLevels()\n"
    "      +'  BYTES '+st+'-'+(Math.min(BD.length,st+BK_PER)-1),8,16);\n"
    "    ctx.textAlign='right';ctx.fillStyle='#ff3fd8';\n"
    "    ctx.fillText('BITS FREED '+bkScore.toFixed(0)\n"
    "      +'  \\u2665'+bkLives,W-8,16);\n"
    "    if(bkMsg&&(bkOver||Date.now()-bkMsgT<1500)){\n"
    "      ctx.textAlign='center';ctx.fillStyle='#22d3ee';\n"
    "      ctx.font='13px \"Press Start 2P\",monospace';\n"
    "      ctx.fillText(bkMsg,W/2,H/2);\n"
    "      if(bkOver&&!bkWon){\n"
    "        ctx.font='8px \"Press Start 2P\",monospace';\n"
    "        ctx.fillStyle='#63636f';\n"
    "        ctx.fillText('CLICK TO RETRY / ESC TO FLEE',W/2,H/2+22);\n"
    "      }\n"
    "    } else if(bkStuck&&!bkOver){\n"
    "      ctx.textAlign='center';ctx.fillStyle='#63636f';\n"
    "      ctx.fillText('CLICK TO SERVE',W/2,py-30);\n"
    "    }\n"
    "    bkRaf=requestAnimationFrame(bkFrame);\n"
    "  }\n"
    ...
    "        if(!shFired){shFired=true;egg('shannon');\n"
    "          showToast('HE CAN\\'T HEAR YOU');}\n"
    "      } else shFired=false;\n"
    "    },350);\n"
    "  });\n"
    ...
    "  }\n"
    "  /* flythrough: every star is a byte of the file, met in file order,\n"
    "     colored by what it cost to encode */\n"
    "  var flyCv=document.getElementById('flyby');\n"
    "  var flyCtx=flyCv?flyCv.getContext('2d'):null;\n"
    "  var flyRaf=null,flyStars=null,flyCur=0,flyConst=false,flyPulse=0;\n"
    "  function flyInit(){\n"
    "    flyCv.width=Math.max(2,window.innerWidth>>1);\n"
    "    flyCv.height=Math.max(2,window.innerHeight>>1);\n"
    "    if(flyConst) constRender();\n"
    "  }\n"
    "  function flyFrame(){\n"
    "    if(!document.body.classList.contains('party')){flyRaf=null;return;}\n"
    "    var W=flyCv.width,H=flyCv.height;\n"
    "    var cx=W/2,cy=H/2,f=W*.7,FAR=900;\n"
    "    flyCtx.fillStyle='rgba(0,0,4,.35)';\n"
    "    flyCtx.fillRect(0,0,W,H);\n"
    "    flyPulse*=.9;\n"
    "    for(var i=0;i<flyStars.length;i++){\n"
    "      var s=flyStars[i];\n"
    "      s.z-=6;\n"
    "      if(s.z<=8){\n"
    "        s.z=FAR;\n"
    "        s.x=(Math.random()*2-1)*900;\n"
    "        s.y=(Math.random()*2-1)*640;\n"
    "        s.b=flyCur;flyCur=(flyCur+1)%%BD.length;\n"
    "      }\n"
    "      var px=cx+s.x/s.z*f,py=cy+s.y/s.z*f;\n"
    "      if(px<0||px>=W||py<0||py>=H) continue;\n"
    "      var c=BD[s.b]?BD[s.b].c:0;\n"
    "      var t=1-s.z/FAR;\n"
    "      var col=c<3?'52,211,153':c<6?'251,191,36':'248,113,113';\n"
    "      flyCtx.fillStyle='rgba('+col+','+(.15+.85*t*t).toFixed(2)+')';\n"
    "      var sz=(1+t*t*(3+flyPulse*3))|0;\n"
    "      flyCtx.fillRect(px|0,py|0,sz,sz);\n"
    "    }\n"
    "    flyRaf=requestAnimationFrame(flyFrame);\n"
    "  }\n"
    "  function flyStart(){\n"
    "    if(!flyCtx||typeof BD==='undefined'||!BD.length) return;\n"
    "    if(!flyStars){\n"
    "      flyStars=[];\n"
    "      for(var i=0;i<220;i++){\n"
    "        flyStars.push({x:(Math.random()*2-1)*900,\n"
    "          y:(Math.random()*2-1)*640,\n"
    "          z:8+Math.random()*892,b:flyCur});\n"
    "        flyCur=(flyCur+1)%%BD.length;\n"
    "      }\n"
    "      window.addEventListener('resize',flyInit);\n"
    "    }\n"
    "    flyInit();\n"
    "    document.body.classList.add('fly');\n"
    "    if(!flyRaf) flyRaf=requestAnimationFrame(flyFrame);\n"
    "  }\n"
    "  function flyStop(){\n"
    ...
    "    flyConst=false;\n"
    "  }\n"
    "  /* constellations: freeze the sky and draw the lines that were\n"
    "     always there - stars are bytes, so stars sharing a dominant\n"
    "     model form a figure. your file has a zodiac */\n"
    "  var CONST_NAMES=['GREAT BEAR','HUNTER','SERPENT','CROWN',\n"
    "    'LYRE','TWINS','SCALES','DRAGON','PHOENIX','RIVER',\n"
    "    'ARCHER','WHALE'];\n"
    "  function bestM(i){\n"
    "    var d=BD[i]; if(!d||!d.m) return -1;\n"
    "    var bi=-1,bv=0.001;\n"
    "    for(var k=0;k<d.m.length;k++) if(d.m[k]>bv){bv=d.m[k];bi=k;}\n"
    "    return bi;\n"
    "  }\n"
    "  function constRender(){\n"
    "    if(!flyCtx||!flyStars) return;\n"
    "    var W=flyCv.width,H=flyCv.height;\n"
    "    var cx=W/2,cy=H/2,f=W*.7;\n"
    "    flyCtx.fillStyle='#000004';\n"
    "    flyCtx.fillRect(0,0,W,H);\n"
    "    var pts=[];\n"
    "    for(var i=0;i<flyStars.length;i++){\n"
    "      var s=flyStars[i];\n"
    "      var px=cx+s.x/s.z*f,py=cy+s.y/s.z*f;\n"
    "      if(px<0||px>=W||py<0||py>=H) continue;\n"
    "      pts.push({x:px,y:py,m:bestM(s.b),z:s.z});\n"
    "    }\n"
    "    /* each star reaches for its nearest same-model sibling:\n"
    "       sparse figures, not hairballs */\n"
    "    flyCtx.lineWidth=0.5;\n"
    "    var seen={};\n"
    "    for(var i=0;i<pts.length;i++){\n"
    "      var p=pts[i]; if(p.m<0) continue;\n"
    "      var bj=-1,bd=1e9;\n"
    "      for(var j=0;j<pts.length;j++){\n"
    "        if(j===i||pts[j].m!==p.m) continue;\n"
    "        var dx=pts[j].x-p.x,dy=pts[j].y-p.y;\n"
    "        var d2=dx*dx+dy*dy;\n"
    "        if(d2<bd){bd=d2;bj=j;}\n"
    "      }\n"
    "      if(bj<0) continue;\n"
    "      var key=i<bj?i+'-'+bj:bj+'-'+i;\n"
    "      if(seen[key]) continue; seen[key]=1;\n"
    "      var pc=(typeof ATTR_PAL!=='undefined'&&ATTR_PAL[p.m])\n"
    "        ||[160,160,160];\n"
    "      flyCtx.strokeStyle='rgba('+pc[0]+','+pc[1]+','+pc[2]+',.45)';\n"
    "      flyCtx.beginPath();\n"
    "      flyCtx.moveTo(p.x,p.y);\n"
    "      flyCtx.lineTo(pts[bj].x,pts[bj].y);\n"
    "      flyCtx.stroke();\n"
    "    }\n"
    "    for(var i=0;i<pts.length;i++){\n"
    "      var p=pts[i];\n"
    "      var pc=p.m>=0\n"
    "        ?((typeof ATTR_PAL!=='undefined'&&ATTR_PAL[p.m])||[255,255,255])\n"
    "        :[140,140,150];\n"
    "      var t=1-p.z/900; if(t<0.2)t=0.2;\n"
    "      flyCtx.fillStyle='rgba('+pc[0]+','+pc[1]+','+pc[2]+','\n"
    "        +(.3+.7*t).toFixed(2)+')';\n"
    "      var sz=1+((t*3)|0);\n"
    "      flyCtx.fillRect(p.x|0,p.y|0,sz,sz);\n"
    "    }\n"
    "    /* name the figures (3+ stars earns a name) */\n"
    "    var groups={};\n"
    "    for(var i=0;i<pts.length;i++){\n"
    "      var p=pts[i]; if(p.m<0) continue;\n"
    "      var g=groups[p.m]||(groups[p.m]={n:0,sx:0,sy:0});\n"
    "      g.n++;g.sx+=p.x;g.sy+=p.y;\n"
    "    }\n"
    "    flyCtx.font='7px \"Press Start 2P\",monospace';\n"
    "    flyCtx.textAlign='center';\n"
    "    for(var m in groups){\n"
    "      var g=groups[m]; if(g.n<3) continue;\n"
    "      var pc=(typeof ATTR_PAL!=='undefined'&&ATTR_PAL[m])\n"
    "        ||[200,200,200];\n"
    "      var name=CONST_NAMES[m%%CONST_NAMES.length];\n"
    "      var mask=(typeof MI!=='undefined'&&MI[m])?MI[m].mask:m;\n"
    "      flyCtx.fillStyle='rgba('+pc[0]+','+pc[1]+','+pc[2]+',.85)';\n"
    "      flyCtx.fillText('THE '+name+' OF 0x'+mask,\n"
    "        g.sx/g.n,g.sy/g.n-8);\n"
    "    }\n"
    "  }\n"
    "  function constToggle(){\n"
    "    if(!flyCtx||!flyStars||typeof BD==='undefined') return;\n"
    "    flyConst=!flyConst;\n"
    "    if(flyConst){\n"
    "      egg('zodiac');\n"
    "      if(flyRaf){cancelAnimationFrame(flyRaf);flyRaf=null;}\n"
    "      constRender();\n"
    "      showToast('THE SKY HOLDS ITS BREATH');\n"
    "    } else {\n"
    "      if(!flyRaf) flyRaf=requestAnimationFrame(flyFrame);\n"
    "      showToast('THE SKY MOVES AGAIN');\n"
    "    }\n"
    "  }\n"
    "  /* rotozoomer: the classic effect, textured with the file's own\n"
    "     bigram matrix - u,v walk the (prev byte, next byte) plane, so\n"
    "     every file spins a different cloth. quarter-res DDA into an\n"
    "     ImageData, upscaled pixelated. replaces the starfield while on */\n"
    "  var rotoOn=false,rotoCv=null,rotoCtx=null,rotoRaf=null;\n"
    "  var rotoTex=null,rotoImg=null,rotoPix=null,rotoT=0,rotoKick=0;\n"
    "  function rotoBuildTex(){\n"
    "    if(rotoTex) return;\n"
    "    rotoTex=new Uint32Array(65536);\n"
    "    var lm=BG_MAX>1?Math.log2(BG_MAX):1;\n"
    "    for(var i=0;i<65536;i++){\n"
    "      var v=BG[i],r=4,g=4,b=8;\n"
    "      if(v>0){\n"
    "        var t=Math.min((Math.log2(v)+1)/(lm+1),1);\n"
    "        if(t<0.75){var u=t/0.75;r=4+30*u;g=4+207*u;b=8+230*u;}\n"
    "        else{var u=(t-0.75)*4;r=34+180*u;g=211+44*u;b=238+17*u;}\n"
    "      }\n"
    "      rotoTex[i]=0xFF000000|((b|0)<<16)|((g|0)<<8)|(r|0);\n"
    "    }\n"
    "  }\n"
    "  function rotoSize(){\n"
    "    if(!rotoCv) return;\n"
    "    rotoCv.width=Math.max(2,window.innerWidth>>2);\n"
    "    rotoCv.height=Math.max(2,window.innerHeight>>2);\n"
    "    rotoImg=rotoCtx.createImageData(rotoCv.width,rotoCv.height);\n"
    "    rotoPix=new Uint32Array(rotoImg.data.buffer);\n"
    "  }\n"
    "  function rotoFrame(){\n"
    "    if(!rotoOn||!document.body.classList.contains('party')){\n"
    "      rotoStop();return;\n"
    "    }\n"
    "    var W=rotoCv.width,H=rotoCv.height;\n"
    "    rotoT+=.016;\n"
    "    rotoKick*=.85;\n"
    "    var sc=1.6+Math.sin(rotoT*.5)*1.1+rotoKick;\n"
    "    var c=Math.cos(rotoT*.35)*sc,s=Math.sin(rotoT*.35)*sc;\n"
    "    var u0=rotoT*40,v0=rotoT*23;\n"
    "    var p=0;\n"
    "    for(var y=0;y<H;y++){\n"
    "      var yy=y-(H>>1);\n"
    "      var u=-(W>>1)*c-yy*s+u0;\n"
    "      var v=-(W>>1)*s+yy*c+v0;\n"
    "      for(var x=0;x<W;x++){\n"
    "        rotoPix[p++]=rotoTex[((v&255)<<8)|(u&255)];\n"
    "        u+=c;v+=s;\n"
    "      }\n"
    "    }\n"
    "    rotoCtx.putImageData(rotoImg,0,0);\n"
    "    rotoRaf=requestAnimationFrame(rotoFrame);\n"
    "  }\n"
    "  function rotoToggle(){\n"
    "    if(rotoOn){rotoStop();flyStart();\n"
    "      showToast('THE ROOM STOPS SPINNING');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BG==='undefined'||!BG_MAX){\n"
    "      showToast('NO BIGRAMS TO SPIN');return;}\n"
    "    egg('rotozoom');\n"
    "    rotoBuildTex();\n"
    "    if(!rotoCv){\n"
    "      rotoCv=document.getElementById('roto');\n"
    "      rotoCtx=rotoCv.getContext('2d');\n"
    "      window.addEventListener('resize',rotoSize);\n"
    "    }\n"
    "    rotoSize();\n"
    "    flyStop();voxStop();tnStop();blkStop();fireStop();twStop();\n"
    "    kfStop();ecStop();\n"
    "    rotoOn=true;\n"
    "    document.body.classList.add('roto');\n"
    "    showToast('DIZZY YET?');\n"
    "    if(!rotoRaf) rotoRaf=requestAnimationFrame(rotoFrame);\n"
    "  }\n"
    "  function rotoStop(){\n"
    "    rotoOn=false;\n"
    "    document.body.classList.remove('roto');\n"
    "    if(rotoRaf){cancelAnimationFrame(rotoRaf);rotoRaf=null;}\n"
    "  }\n"
    "  /* voxel: comanche-style heightmap flyover of the cost terrain.\n"
    "     the compressibility map becomes relief - height and color are\n"
    "     per-byte encoding cost, the file tiles the landscape, and the\n"
    "     camera weaves forward in file order. per depth slice: walk the\n"
    "     view frustum's scanline across the map, project each column,\n"
    "     fill vertical spans against a y-buffer, fog with distance */\n"
    "  var voxOn=false,voxCv=null,voxCtx=null,voxRaf=null;\n"
    "  var voxImg=null,voxPix=null,voxYB=null,voxSky=null;\n"
    "  var voxHf=null,voxCR=null,voxCG=null,voxCB=null;\n"
    "  var voxBC=null,voxBCtx=null,voxFirst=true;\n"
    "  var voxTW=256,voxTH=256,voxT=0,voxY=0,voxCH=90,voxLb=null;\n"
    "  var VOX_HZ=.3; /* horizon as a screen fraction: low = pitched down */\n"
    "  function voxBuild(){\n"
    "    if(voxHf) return;\n"
    "    var nb=BD.length;\n"
    "    /* base grid: 64 bytes per row. raw per-byte cost is noise -\n"
    "       rendered directly it is a forest of one-cell pillars - so\n"
    "       blur it into terrain (expensive REGIONS become ranges),\n"
    "       renormalize so blurring keeps the relief, then bilinearly\n"
    "       upsample x4 into floats. color is NOT blurred (that made\n"
    "       olive mud): it is a palette over final height, and height\n"
    "       is smoothed cost, so the bands stay meaningful */\n"
    "    var BW=64;\n"
    "    var rows=Math.ceil(nb/BW);\n"
    "    var BH=64; while(BH<rows&&BH<512) BH<<=1;\n"
    "    var bn=BW*BH;\n"
    "    var bh=new Float32Array(bn);\n"
    "    var cmax=0.01;\n"
    "    for(var i=0;i<nb;i++) if(BD[i].c>cmax) cmax=BD[i].c;\n"
    "    for(var i=0;i<bn;i++){\n"
    "      var t=BD[i%%nb].c/cmax; if(t>1)t=1;\n"
    "      bh[i]=t;\n"
    "    }\n"
    "    var tmp=new Float32Array(bn);\n"
    "    function blur(a){\n"
    "      for(var y=0;y<BH;y++){\n"
    "        for(var x=0;x<BW;x++){\n"
    "          var s=0;\n"
    "          for(var j=-1;j<=1;j++)\n"
    "            for(var k=-1;k<=1;k++)\n"
    "              s+=a[((y+j)&(BH-1))*BW+((x+k)&(BW-1))];\n"
    "          tmp[y*BW+x]=s/9;\n"
    "        }\n"
    "      }\n"
    "      a.set(tmp);\n"
    "    }\n"
    "    /* heavy smoothing: with light blur every expensive byte\n"
    "       stays a needle-cone, the image becomes mostly silhouette\n"
    "       edges, and every edge sparkles under sub-pixel camera\n"
    "       motion. six passes merge needles into hills */\n"
    "    for(var bp=0;bp<6;bp++) blur(bh);\n"
    "    /* renormalize height so the blur does not flatten the world */\n"
    "    var hmin=1e9,hmax=-1e9;\n"
    "    for(var i=0;i<bn;i++){\n"
    "      if(bh[i]<hmin)hmin=bh[i];\n"
    "      if(bh[i]>hmax)hmax=bh[i];\n"
    "    }\n"
    "    var hs=130/((hmax-hmin)||1);\n"
    "    for(var i=0;i<bn;i++) bh[i]=10+(bh[i]-hmin)*hs;\n"
    "    /* landmarks: every hill and hollow is named for the byte\n"
    "       that made it - the terrain is blurred regions, so a peak\n"
    "       gets its most expensive raw byte, a hollow its cheapest */\n"
    "    voxLb=[];\n"
    "    var cand=[];\n"
    "    for(var y=0;y<BH;y++){\n"
    "      for(var x=0;x<BW;x++){\n"
    "        var v=bh[y*BW+x],mx=1,mn=1;\n"
    "        for(var j=-2;j<=2&&(mx||mn);j++){\n"
    "          for(var k=-2;k<=2;k++){\n"
    "            if(!j&&!k) continue;\n"
    "            var nv=bh[((y+j)&(BH-1))*BW+((x+k)&(BW-1))];\n"
    "            if(nv>=v) mx=0;\n"
    "            if(nv<=v) mn=0;\n"
    "          }\n"
    "        }\n"
    "        if(!mx&&!mn) continue;\n"
    "        var bi=-1,bc=mx?-1:1e9;\n"
    "        for(var j=-2;j<=2;j++){\n"
    "          for(var k=-2;k<=2;k++){\n"
    "            var ci=((y+j)&(BH-1))*BW+((x+k)&(BW-1));\n"
    "            var b2=ci%%nb;\n"
    "            var c2=BD[b2].c;\n"
    "            if(mx?c2>bc:c2<bc){bc=c2;bi=b2;}\n"
    "          }\n"
    "        }\n"
    "        cand.push({x:x*4+2,y:y*4+2,v:v,pk:mx?1:0,\n"
    "          tx:BD[bi].h+(BD[bi].ch?' '+BD[bi].ch:'')});\n"
    "      }\n"
    "    }\n"
    "    cand.sort(function(a,b){return b.v-a.v;});\n"
    "    var pks=0,vls=0;\n"
    "    for(var i=0;i<cand.length;i++)\n"
    "      if(cand[i].pk&&pks<9){voxLb.push(cand[i]);pks++;}\n"
    "    for(var i=cand.length-1;i>=0;i--)\n"
    "      if(!cand[i].pk&&vls<5){voxLb.push(cand[i]);vls++;}\n"
    "    var UP=4;\n"
    "    voxTW=BW*UP;voxTH=BH*UP;\n"
    "    var n=voxTW*voxTH;\n"
    "    voxHf=new Float32Array(n);\n"
    "    for(var y=0;y<voxTH;y++){\n"
    "      var fy=y/UP,y0=fy|0,ty=fy-y0;\n"
    "      var y1=(y0+1)&(BH-1);y0&=BH-1;\n"
    "      for(var x=0;x<voxTW;x++){\n"
    "        var fx=x/UP,x0=fx|0,tx=fx-x0;\n"
    "        var x1=(x0+1)&(BW-1);x0&=BW-1;\n"
    "        voxHf[y*voxTW+x]=\n"
    "          bh[y0*BW+x0]*(1-tx)*(1-ty)+bh[y0*BW+x1]*tx*(1-ty)+\n"
    "          bh[y1*BW+x0]*(1-tx)*ty+bh[y1*BW+x1]*tx*ty;\n"
    "      }\n"
    "    }\n"
    "    /* bake ONE smooth colormap: hypsometric palette (altitude =\n"
    "       cost = the cmap ramp) times slope shading, per cell. the\n"
    "       renderer samples height AND color bilinearly from this, so\n"
    "       nothing can pop per-cell or per-slice */\n"
    "    voxCR=new Uint8Array(n);voxCG=new Uint8Array(n);\n"
    "    voxCB=new Uint8Array(n);\n"
    "    for(var i=0;i<n;i++){\n"
    "      var t=(voxHf[i]-10)/130; if(t<0)t=0; if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(t<0.5){var u=t*2;\n"
    "        r=16+(180-16)*u;g=185+(140-185)*u;b=129+(40-129)*u;}\n"
    "      else{var u=(t-0.5)*2;\n"
    "        r=180+(248-180)*u;g=140+(113-140)*u;b=40+(113-40)*u;}\n"
    "      var sl=1+(voxHf[i]-voxHf[(i+voxTW)%%n])*.05;\n"
    "      if(sl<.5)sl=.5; if(sl>1.4)sl=1.4;\n"
    "      r*=sl;g*=sl;b*=sl;\n"
    "      voxCR[i]=r>255?255:r|0;\n"
    "      voxCG[i]=g>255?255:g|0;\n"
    "      voxCB[i]=b>255?255:b|0;\n"
    "    }\n"
    "  }\n"
    "  function voxSize(){\n"
    "    if(!voxCv) return;\n"
    "    /* half-res, capped, smoothly upscaled */\n"
    "    var sf=Math.min(.5,960/window.innerWidth);\n"
    "    voxCv.width=Math.max(2,(window.innerWidth*sf)|0);\n"
    "    voxCv.height=Math.max(2,(window.innerHeight*sf)|0);\n"
    "    /* render offscreen; the visible canvas composites it over\n"
    "       28%% of the previous frame - temporal persistence that\n"
    "       averages away residual sample flicker */\n"
    "    if(!voxBC){\n"
    "      voxBC=document.createElement('canvas');\n"
    "      voxBCtx=voxBC.getContext('2d');\n"
    "    }\n"
    "    voxBC.width=voxCv.width;voxBC.height=voxCv.height;\n"
    "    voxFirst=true;\n"
    "    voxImg=voxBCtx.createImageData(voxCv.width,voxCv.height);\n"
    "    voxPix=new Uint32Array(voxImg.data.buffer);\n"
    "    voxYB=new Int16Array(voxCv.width);\n"
    "    /* sky: indigo overhead, then a FLAT haze band below the\n"
    "       horizon - distant terrain fogs to exactly this color, so\n"
    "       silhouettes dissolve into it with no visible far edge */\n"
    "    var H=voxCv.height,hz=(H*VOX_HZ)|0;\n"
    "    voxSky=new Uint32Array(H);\n"
    "    for(var y=0;y<H;y++){\n"
    "      var r,g,b;\n"
    "      if(y<hz){var t=y/hz;\n"
    "        r=4+42*t;g=3+17*t;b=10+48*t;\n"
    "      }else{\n"
    "        var u=(y-hz)/((H-hz)||1);\n"
    "        if(u<.55){r=46;g=20;b=58;}\n"
    "        else{var v=(u-.55)/.45;\n"
    "          r=46-34*v;g=20-13*v;b=58-40*v;}\n"
    "      }\n"
    "      voxSky[y]=0xFF000000|((b|0)<<16)|((g|0)<<8)|(r|0);\n"
    "    }\n"
    "  }\n"
    "  function voxFrame(){\n"
    "    if(!voxOn||!document.body.classList.contains('party')){\n"
    "      voxStop();return;\n"
    "    }\n"
    "    var W=voxCv.width,H=voxCv.height;\n"
    "    voxT+=.016;\n"
    "    voxY-=.9; if(voxY<0) voxY+=voxTH;\n"
    "    var camx=(voxTW>>1)+Math.sin(voxT*.17)*30;\n"
    "    var ang=Math.sin(voxT*.11)*.35;\n"
    "    var sinA=Math.sin(ang),cosA=Math.cos(ang);\n"
    "    var mw=voxTW-1,mh=voxTH-1;\n"
    "    /* aerial camera: altitude tracks the ground softly but stays\n"
    "       above every peak, so the world is always BELOW the lens */\n"
    "    var gi=((voxY|0)&mh)*voxTW+((camx|0)&mw);\n"
    "    voxCH+=((voxHf[gi]*.4+130+Math.sin(voxT*.9)*4)-voxCH)*.05;\n"
    "    var camH=voxCH;\n"
    "    var horizon=(H*VOX_HZ)|0;\n"
    "    for(var y=0;y<H;y++) voxPix.fill(voxSky[y],y*W,y*W+W);\n"
    "    var ybuf=voxYB;\n"
    "    for(var x=0;x<W;x++) ybuf[x]=H;\n"
    "    /* small scale = wide vertical FOV: the ground beneath comes\n"
    "       into frame instead of starting at the fog line */\n"
    "    var scale=H*.55;\n"
    "    /* offsets keep sample coords positive so |0 is floor() */\n"
    "    var BX=voxTW*8,BY=voxTH*8;\n"
    "    /* fine multiplicative depth steps. a face is painted as\n"
    "       stacked strips, one per slice - with coarse steps each strip\n"
    "       gets a visibly different fog level and the boundaries crawl\n"
    "       as the camera moves. small steps make strips subpixel */\n"
    "    var z=5,dz=.5,maxZ=600;\n"
    "    while(z<maxZ){\n"
    "      var plx=(-cosA-sinA)*z+camx+BX,ply=(sinA-cosA)*z+voxY+BY;\n"
    "      var prx=(cosA-sinA)*z+camx+BX,pry=(-sinA-cosA)*z+voxY+BY;\n"
    "      var dx=(prx-plx)/W,dy=(pry-ply)/W;\n"
    "      var invz=scale/z;\n"
    "      /* fog saturates to the exact haze color before maxZ, so\n"
    "         far terrain dissolves into the sky band seamlessly */\n"
    "      var af=z/maxZ;af=af*af*1.15;if(af>1)af=1;\n"
    "      var fr=1-af;\n"
    "      var aR=46*af,aG=20*af,aB=58*af;\n"
    "      for(var x=0;x<W;x++){\n"
    "        var ix=plx|0,iy=ply|0;\n"
    "        var fx=plx-ix,fy=ply-iy;\n"
    "        var x0=ix&mw,x1=(ix+1)&mw;\n"
    "        var y0=(iy&mh)*voxTW,y1=((iy+1)&mh)*voxTW;\n"
    "        var w00=(1-fx)*(1-fy),w01=fx*(1-fy);\n"
    "        var w10=(1-fx)*fy,w11=fx*fy;\n"
    "        var i00=y0+x0,i01=y0+x1,i10=y1+x0,i11=y1+x1;\n"
    "        var h=voxHf[i00]*w00+voxHf[i01]*w01\n"
    "          +voxHf[i10]*w10+voxHf[i11]*w11;\n"
    "        var yt=((camH-h)*invz)+horizon|0;\n"
    "        if(yt<0) yt=0;\n"
    "        if(yt<ybuf[x]){\n"
    "          var r=voxCR[i00]*w00+voxCR[i01]*w01\n"
    "            +voxCR[i10]*w10+voxCR[i11]*w11;\n"
    "          var g=voxCG[i00]*w00+voxCG[i01]*w01\n"
    "            +voxCG[i10]*w10+voxCG[i11]*w11;\n"
    "          var b=voxCB[i00]*w00+voxCB[i01]*w01\n"
    "            +voxCB[i10]*w10+voxCB[i11]*w11;\n"
    "          var col=0xFF000000|\n"
    "            (((b*fr+aB)|0)<<16)|\n"
    "            (((g*fr+aG)|0)<<8)|\n"
    "            ((r*fr+aR)|0);\n"
    "          for(var y=yt;y<ybuf[x];y++) voxPix[y*W+x]=col;\n"
    "          ybuf[x]=yt;\n"
    "        }\n"
    "        plx+=dx;ply+=dy;\n"
    "      }\n"
    "      z+=dz;dz*=1.012;\n"
    "    }\n"
    "    voxBCtx.putImageData(voxImg,0,0);\n"
    "    voxCtx.globalAlpha=voxFirst?1:.72;\n"
    "    voxFirst=false;\n"
    "    voxCtx.drawImage(voxBC,0,0);\n"
    "    voxCtx.globalAlpha=1;\n"
    "    /* landmark labels: project each named byte into the scene,\n"
    "       cull peaks against the silhouette buffer, declutter, and\n"
    "       stamp the byte over its hill. drawn after the composite so\n"
    "       this frame's text is crisp; last frame's 28%% ghost reads\n"
    "       as phosphor */\n"
    "    if(voxLb&&voxLb.length){\n"
    "      var vis=[];\n"
    "      for(var i2=0;i2<voxLb.length;i2++){\n"
    "        var lb=voxLb[i2];\n"
    "        var ddx=(lb.x-camx)%%voxTW;\n"
    "        if(ddx>voxTW/2)ddx-=voxTW;\n"
    "        if(ddx<-voxTW/2)ddx+=voxTW;\n"
    "        var ddy=(lb.y-voxY)%%voxTH;\n"
    "        if(ddy>0)ddy-=voxTH; /* the copy ahead of the camera */\n"
    "        var zp=-(ddx*sinA+ddy*cosA);\n"
    "        if(zp<16||zp>300) continue;\n"
    "        var l=ddx*cosA-ddy*sinA;\n"
    "        if(l<-zp*.92||l>zp*.92) continue;\n"
    "        var sx=((l/zp)+1)*.5*W;\n"
    "        var hgt=voxHf[lb.y*voxTW+lb.x];\n"
    "        var sy=(camH-hgt)*scale/zp+horizon;\n"
    "        var sxi=sx|0; if(sxi<0)sxi=0; if(sxi>=W)sxi=W-1;\n"
    "        /* peaks own their silhouette: cull if something nearer\n"
    "           already covers that column top. hollows: near only */\n"
    "        if(lb.pk){ if(sy>ybuf[sxi]+5) continue; }\n"
    "        else { if(zp>150) continue; }\n"
    "        var ty=sy-10; if(ty<8||ty>H-6) continue;\n"
    "        vis.push({x:sx,y:sy,ty:ty,z:zp,pk:lb.pk,tx:lb.tx});\n"
    "      }\n"
    "      vis.sort(function(a,b){return a.z-b.z;});\n"
    "      voxCtx.font='7px \"Press Start 2P\",monospace';\n"
    "      voxCtx.textAlign='center';\n"
    "      var drawn=[],nd=0;\n"
    "      for(var i2=0;i2<vis.length&&nd<10;i2++){\n"
    "        var lv=vis[i2],ok=1;\n"
    "        for(var j2=0;j2<drawn.length;j2++){\n"
    "          if(Math.abs(drawn[j2].x-lv.x)<38&&\n"
    "             Math.abs(drawn[j2].y-lv.ty)<12){ok=0;break;}\n"
    "        }\n"
    "        if(!ok) continue;\n"
    "        drawn.push({x:lv.x,y:lv.ty});nd++;\n"
    "        var al=1.15-lv.z/300;\n"
    "        if(al>1)al=1; if(al<.3)al=.3;\n"
    "        voxCtx.globalAlpha=al;\n"
    "        voxCtx.lineWidth=2;\n"
    "        voxCtx.strokeStyle='rgba(0,0,10,.85)';\n"
    "        voxCtx.fillStyle=lv.pk?'#ffe33f':'#7de3ff';\n"
    "        voxCtx.strokeText(lv.tx,lv.x,lv.ty);\n"
    "        voxCtx.fillText(lv.tx,lv.x,lv.ty);\n"
    "        voxCtx.beginPath();\n"
    "        voxCtx.moveTo(lv.x,lv.ty+2);\n"
    "        voxCtx.lineTo(lv.x,lv.y-1);\n"
    "        voxCtx.strokeStyle=voxCtx.fillStyle;\n"
    "        voxCtx.lineWidth=1;\n"
    "        voxCtx.stroke();\n"
    "      }\n"
    "      voxCtx.globalAlpha=1;\n"
    "    }\n"
    "    voxRaf=requestAnimationFrame(voxFrame);\n"
    "  }\n"
    "  function voxToggle(){\n"
    "    if(voxOn){voxStop();flyStart();showToast('LANDED');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO BYTES NO MOUNTAINS');return;}\n"
    "    egg('landscape');\n"
    "    voxBuild();\n"
    "    if(!voxCv){\n"
    "      voxCv=document.getElementById('voxel');\n"
    "      voxCtx=voxCv.getContext('2d');\n"
    "      window.addEventListener('resize',voxSize);\n"
    "    }\n"
    "    voxSize();\n"
    "    flyStop();rotoStop();tnStop();blkStop();fireStop();twStop();\n"
    "    kfStop();ecStop();\n"
    "    voxOn=true;\n"
    "    document.body.classList.add('voxel');\n"
    "    showToast('TERRAIN IS COST. ALTITUDE IS PAIN');\n"
    "    if(!voxRaf) voxRaf=requestAnimationFrame(voxFrame);\n"
    "  }\n"
    "  function voxStop(){\n"
    "    voxOn=false;\n"
    "    document.body.classList.remove('voxel');\n"
    "    if(voxRaf){cancelAnimationFrame(voxRaf);voxRaf=null;}\n"
    "  }\n"
    "  /* tunnel: per-pixel polar LUT, textured with the file. u is\n"
    "     angle, v is depth plus time; each texture row is a run of\n"
    "     bytes colored by cost, so expensive strata rush past. the\n"
    "     LUT is oversized and window-offset per frame - that is how\n"
    "     the mouth wobbles without recomputing atan2 */\n"
    "  var tnOn=false,tnCv=null,tnCtx=null,tnRaf=null;\n"
    "  var tnImg=null,tnPix=null,tnA=null,tnD=null,tnS=null;\n"
    "  var tnLW=0,tnLH=0,tnMX=0,tnMY=0;\n"
    "  var tnR=null,tnG=null,tnB=null,tnT=0,tnV=0,tnU=0,tnKick=0;\n"
    "  function tnBuildTex(){\n"
    "    if(tnR) return;\n"
    "    var nb=BD.length;\n"
    "    var r0=new Float32Array(65536);\n"
    "    var g0=new Float32Array(65536);\n"
    "    var b0=new Float32Array(65536);\n"
    "    var cmax=.01;\n"
    "    for(var i=0;i<nb;i++) if(BD[i].c>cmax)cmax=BD[i].c;\n"
    "    for(var i=0;i<65536;i++){\n"
    "      var d=BD[i%%nb];\n"
    "      var t=d.c/cmax;if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(t<0.5){var u=t*2;\n"
    "        r=16+(180-16)*u;g=185+(140-185)*u;b=129+(40-129)*u;}\n"
    "      else{var u=(t-0.5)*2;\n"
    "        r=180+(248-180)*u;g=140+(113-140)*u;b=40+(113-40)*u;}\n"
    "      /* byte value modulates brightness: texture inside strata */\n"
    "      var br=.55+.45*(parseInt(d.h,16)/255);\n"
    "      r0[i]=r*br;g0[i]=g*br;b0[i]=b*br;\n"
    "    }\n"
    "    /* one wrapped 3x3 blur: per-byte cost is noise here too */\n"
    "    function bl(src){\n"
    "      var dst=new Uint8Array(65536);\n"
    "      for(var y=0;y<256;y++){\n"
    "        for(var x=0;x<256;x++){\n"
    "          var s=0;\n"
    "          for(var j=-1;j<=1;j++)\n"
    "            for(var k=-1;k<=1;k++)\n"
    "              s+=src[((y+j)&255)*256+((x+k)&255)];\n"
    "          dst[y*256+x]=(s/9)|0;\n"
    "        }\n"
    "      }\n"
    "      return dst;\n"
    "    }\n"
    "    tnR=bl(r0);tnG=bl(g0);tnB=bl(b0);\n"
    "  }\n"
    "  function tnSize(){\n"
    "    if(!tnCv) return;\n"
    "    var sf=Math.min(.5,960/window.innerWidth);\n"
    "    tnCv.width=Math.max(2,(window.innerWidth*sf)|0);\n"
    "    tnCv.height=Math.max(2,(window.innerHeight*sf)|0);\n"
    "    tnImg=tnCtx.createImageData(tnCv.width,tnCv.height);\n"
    "    tnPix=new Uint32Array(tnImg.data.buffer);\n"
    "    var W=tnCv.width,H=tnCv.height;\n"
    "    tnMX=(W*.12)|0;tnMY=(H*.12)|0;\n"
    "    tnLW=W+2*tnMX;tnLH=H+2*tnMY;\n"
    "    tnA=new Uint8Array(tnLW*tnLH);\n"
    "    tnD=new Float32Array(tnLW*tnLH);\n"
    "    tnS=new Float32Array(tnLW*tnLH);\n"
    "    var cx=tnLW/2,cy=tnLH/2;\n"
    "    var Rm=Math.sqrt(cx*cx+cy*cy);\n"
    "    for(var y=0;y<tnLH;y++){\n"
    "      for(var x=0;x<tnLW;x++){\n"
    "        var dx=x-cx,dy=y-cy;\n"
    "        var r=Math.sqrt(dx*dx+dy*dy); if(r<2)r=2;\n"
    "        var i=y*tnLW+x;\n"
    "        tnA[i]=(Math.atan2(dy,dx)*40.743665+256)&255;\n"
    "        tnD[i]=1400/r;\n"
    "        /* depth fade: the far center goes dark, which also hides\n"
    "           the extreme minification aliasing there */\n"
    "        var s=.12+1.35*(r/Rm); if(s>1)s=1;\n"
    "        tnS[i]=s;\n"
    "      }\n"
    "    }\n"
    "  }\n"
    "  function tnFrame(){\n"
    "    if(!tnOn||!document.body.classList.contains('party')){\n"
    "      tnStop();return;\n"
    "    }\n"
    "    var W=tnCv.width,H=tnCv.height;\n"
    "    tnT+=.016;\n"
    "    tnKick*=.85;\n"
    "    tnV+=1.1+tnKick;  /* forward, harder on the snare */\n"
    "    tnU+=.35;  /* slow twist */\n"
    "    var u0=tnU|0;\n"
    "    var ox=(tnMX+Math.sin(tnT*.6)*tnMX*.8)|0;\n"
    "    var oy=(tnMY+Math.cos(tnT*.43)*tnMY*.8)|0;\n"
    "    var p=0;\n"
    "    for(var y=0;y<H;y++){\n"
    "      var li=(y+oy)*tnLW+ox;\n"
    "      for(var x=0;x<W;x++,li++){\n"
    "        var v=((tnD[li]+tnV)|0)&255;\n"
    "        var u=(tnA[li]+u0)&255;\n"
    "        var ti=(v<<8)|u;\n"
    "        var s=tnS[li];\n"
    "        tnPix[p++]=0xFF000000|\n"
    ...
    "    tnRaf=requestAnimationFrame(tnFrame);\n"
    "  }\n"
    "  function tnToggle(){\n"
    "    if(tnOn){tnStop();flyStart();\n"
    "      showToast('OUT THE OTHER END');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO BYTES NO TUNNEL');return;}\n"
    "    egg('tunnel');\n"
    "    tnBuildTex();\n"
    "    if(!tnCv){\n"
    "      tnCv=document.getElementById('tunnel');\n"
    "      tnCtx=tnCv.getContext('2d');\n"
    "      window.addEventListener('resize',tnSize);\n"
    "    }\n"
    "    tnSize();\n"
    "    flyStop();rotoStop();voxStop();blkStop();fireStop();twStop();\n"
    "    kfStop();ecStop();\n"
    "    tnOn=true;\n"
    "    document.body.classList.add('tunl');\n"
    "    showToast('INTO THE FILE');\n"
    "    if(!tnRaf) tnRaf=requestAnimationFrame(tnFrame);\n"
    "  }\n"
    "  function tnStop(){\n"
    "    tnOn=false;\n"
    "    document.body.classList.remove('tunl');\n"
    "    if(tnRaf){cancelAnimationFrame(tnRaf);tnRaf=null;}\n"
    "  }\n"
    "  /* voxel, literally: the compressibility map as minecraft. one\n"
    "     pillar per byte, height and color are cost, the printable\n"
    "     char on the cap and the offset on the flank. the whole city\n"
    "     is prerendered once and the flyover is lattice-tiled blits\n"
    "     at integer offsets - zero per-frame geometry, zero shimmer */\n"
    "  var blkOn=false,blkCv=null,blkCtx=null,blkRaf=null;\n"
    "  var blkW=null,blkT=0,blkP=0,blkDir=1,blkRows=0;\n"
    "  var blkOX=0,blkOY=0;\n"
    "  var blkN=0,blkH=null,blkSel=-1,blkBX=0,blkBY=0,blkTipEl=null;\n"
    "  function blkHex(i){\n"
    ...
    "  function blkBuild(){\n"
    "    if(blkW) return;\n"
    "    var nb=Math.min(BD.length,8192);\n"
    "    var BW2=64,tw=48,th=24,hMax=200;\n"
    "    var rows=Math.ceil(nb/BW2);\n"
    "    blkRows=rows;\n"
    "    blkN=nb;blkH=new Int16Array(nb);\n"
    "    var cmax=.01;\n"
    "    for(var i=0;i<nb;i++) if(BD[i].c>cmax)cmax=BD[i].c;\n"
    "    var ox=rows*tw/2+tw/2;\n"
    "    var oy=hMax+20;\n"
    "    blkOX=ox;blkOY=oy;\n"
    "    blkW=document.createElement('canvas');\n"
    "    blkW.width=((BW2+rows)*tw/2+tw)|0;\n"
    "    blkW.height=((BW2+rows)*th/2+oy+th)|0;\n"
    "    var wc=blkW.getContext('2d');\n"
    "    wc.textAlign='center';\n"
    "    for(var y=0;y<rows;y++){\n"
    "      for(var x=0;x<BW2;x++){\n"
    "        var i=y*BW2+x;\n"
    "        if(i>=nb) break;\n"
    "        var c=BD[i].c,t=c/cmax; if(t>1)t=1;\n"
    "        var r,g,b;\n"
    "        if(t<0.5){var u=t*2;\n"
    "          r=16+(180-16)*u;g=185+(140-185)*u;b=129+(40-129)*u;}\n"
    "        else{var u=(t-0.5)*2;\n"
    ...
    "        var gy=oy+(x+y)*(th/2);\n"
    "        var ty=gy-h;\n"
    "        wc.fillStyle='rgb('+((r*.58)|0)+','+((g*.58)|0)+','\n"
    "          +((b*.58)|0)+')';\n"
    "        wc.beginPath();\n"
    "        wc.moveTo(sx-tw/2,ty);wc.lineTo(sx,ty+th/2);\n"
    "        wc.lineTo(sx,gy+th/2);wc.lineTo(sx-tw/2,gy);\n"
    "        wc.closePath();wc.fill();\n"
    "        wc.fillStyle='rgb('+((r*.4)|0)+','+((g*.4)|0)+','\n"
    "          +((b*.4)|0)+')';\n"
    "        wc.beginPath();\n"
    "        wc.moveTo(sx+tw/2,ty);wc.lineTo(sx,ty+th/2);\n"
    "        wc.lineTo(sx,gy+th/2);wc.lineTo(sx+tw/2,gy);\n"
    "        wc.closePath();wc.fill();\n"
    "        wc.fillStyle='rgb('+r+','+g+','+b+')';\n"
    "        wc.beginPath();\n"
    "        wc.moveTo(sx,ty-th/2);wc.lineTo(sx+tw/2,ty);\n"
    "        wc.lineTo(sx,ty+th/2);wc.lineTo(sx-tw/2,ty);\n"
    "        wc.closePath();wc.fill();\n"
    "        wc.strokeStyle='rgba(0,0,20,.28)';\n"
    "        wc.lineWidth=1;wc.stroke();\n"
    "        /* the byte, on the cap */\n"
    "        if(BD[i].ch){\n"
    "          var lum=(r*299+g*587+b*114)/1000;\n"
    "          wc.fillStyle=lum>120?'rgba(8,12,20,.8)'\n"
    "            :'rgba(255,255,255,.85)';\n"
    "          wc.font='12px \"Press Start 2P\",monospace';\n"
    "          wc.fillText(BD[i].ch,sx,ty+5);\n"
    "        }\n"
    "        /* the offset, on the flank of the tall ones */\n"
    "        if(h>=30){\n"
    "          wc.fillStyle='rgba(255,255,255,.45)';\n"
    "          wc.font='10px Consolas,monospace';\n"
    "          wc.fillText(blkHex(i),sx,ty+th/2+16);\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "  }\n"
    "  function blkSize(){\n"
    "    if(!blkCv) return;\n"
    "    blkCv.width=window.innerWidth;\n"
    "    blkCv.height=window.innerHeight;\n"
    "  }\n"
    "  function blkFrame(){\n"
    "    if(!blkOn||!document.body.classList.contains('party')){\n"
    "      blkStop();return;\n"
    "    }\n"
    "    var W=blkCv.width,H=blkCv.height;\n"
    "    blkT+=.016;\n"
    "    /* one literal map, no tiling: the camera ping-pongs along\n"
    "       the file, following the diamond's spine */\n"
    "    blkP+=blkDir*.05;\n"
    "    if(blkP>blkRows-1){blkP=blkRows-1;blkDir=-1;}\n"
    "    if(blkP<0){blkP=0;blkDir=1;}\n"
    "    var sxc=blkOX+(31.5-blkP)*24;\n"
    "    var syc=blkOY+(31.5+blkP)*12;\n"
    "    var bx=(W/2-sxc+Math.sin(blkT*.15)*60)|0;\n"
    "    var by=(H/2-syc)|0;\n"
    "    blkBX=bx;blkBY=by; /* the picker inverts through these */\n"
    "    if(blkTipEl&&blkTipEl.style.display!=='none'&&\n"
    "       !document.body.classList.contains('void')){\n"
    "      blkTipEl.style.display='none';blkSel=-1;\n"
    "    }\n"
    "    blkCtx.clearRect(0,0,W,H);\n"
    "    blkCtx.drawImage(blkW,bx,by);\n"
    "    if(blkSel>=0&&blkSel<blkN){\n"
    "      var x2=blkSel%%64,y2=(blkSel/64)|0;\n"
    "      var scx=blkOX+(x2-y2)*24+bx;\n"
    "      var gy2=blkOY+(x2+y2)*12+by;\n"
    "      var ty3=gy2-blkH[blkSel];\n"
    "      blkCtx.strokeStyle='#ffe33f';\n"
    "      blkCtx.lineWidth=2;\n"
    "      blkCtx.beginPath();\n"
    "      blkCtx.moveTo(scx,ty3-12);blkCtx.lineTo(scx+24,ty3);\n"
    "      blkCtx.lineTo(scx,ty3+12);blkCtx.lineTo(scx-24,ty3);\n"
    "      blkCtx.closePath();\n"
    "      blkCtx.moveTo(scx-24,ty3);blkCtx.lineTo(scx-24,gy2);\n"
    "      blkCtx.lineTo(scx,gy2+12);blkCtx.lineTo(scx+24,gy2);\n"
    "      blkCtx.lineTo(scx+24,ty3);\n"
    "      blkCtx.moveTo(scx,ty3+12);blkCtx.lineTo(scx,gy2+12);\n"
    "      blkCtx.stroke();\n"
    "    }\n"
    "    blkRaf=requestAnimationFrame(blkFrame);\n"
    "  }\n"
    "  /* reverse-painter hit test: walk cells front to back, test the\n"
    "     pointer against each column's extruded-diamond silhouette.\n"
    "     first hit is the tower actually under the cursor, occlusion\n"
    "     included */\n"
    "  function blkPick(mx,my){\n"
    "    if(!blkH) return -1;\n"
    "    var wx=mx-blkBX,wy=my-blkBY;\n"
    "    for(var j=blkN-1;j>=0;j--){\n"
    "      var x=j%%64,y=(j/64)|0;\n"
    "      var sx=blkOX+(x-y)*24;\n"
    "      var adx=wx-sx; if(adx<0)adx=-adx;\n"
    "      if(adx>24) continue;\n"
    "      var gy=blkOY+(x+y)*12;\n"
    "      var ty=gy-blkH[j];\n"
    "      var edge=12*(1-adx/24);\n"
    "      if(wy>=ty-edge&&wy<=gy+edge) return j;\n"
    "    }\n"
    "    return -1;\n"
    "  }\n"
    "  function blkToggle(){\n"
    "    if(blkOn){blkStop();flyStart();showToast('FLATTENED');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO BYTES NO BLOCKS');return;}\n"
    "    egg('voxel');\n"
    "    blkBuild();\n"
    "    if(!blkCv){\n"
    "      blkCv=document.getElementById('blox');\n"
    "      blkCtx=blkCv.getContext('2d');\n"
    "      window.addEventListener('resize',blkSize);\n"
    "      blkCv.addEventListener('mousemove',function(e){\n"
    "        if(!blkOn) return;\n"
    "        var j=blkPick(e.clientX,e.clientY);\n"
    "        blkSel=j;\n"
    "        if(j<0){\n"
    "          if(blkTipEl)blkTipEl.style.display='none';\n"
    "          return;\n"
    "        }\n"
    "        if(!blkTipEl){\n"
    "          blkTipEl=document.createElement('div');\n"
    "          blkTipEl.style.cssText='position:fixed;display:none;'\n"
    "            +'z-index:10001;pointer-events:none;'\n"
    "            +'background:rgba(0,0,10,.92);'\n"
    "            +'border:1px solid #22d3ee;padding:8px 10px;'\n"
    "            +'font-family:\\'Press Start 2P\\',monospace;'\n"
    "            +'font-size:8px;color:#e8e8f0;line-height:2;'\n"
    "            +'letter-spacing:1px;'\n"
    "            +'box-shadow:4px 4px 0 rgba(0,0,0,.55)';\n"
    "          document.body.appendChild(blkTipEl);\n"
    "        }\n"
    "        var d=BD[j];\n"
    "        var ch2=d.ch?(d.ch==='<'?'&lt;':d.ch==='&'?'&amp;'\n"
    "          :d.ch):'';\n"
    "        blkTipEl.innerHTML=\n"
    "          '<span style=\"color:#ffe33f\">OFFSET 0x'+blkHex(j)\n"
    "          +'</span> ('+j+')<br>BYTE '+d.h\n"
    "          +(ch2?' <span style=\"color:#ff3fd8\">'+ch2+'</span>':'')\n"
    "          +'<br><span style=\"color:'+(d.c<3?'#34d399'\n"
    "          :d.c<6?'#fbbf24':'#f87171')+'\">COST '\n"
    "          +d.c.toFixed(2)+' BITS</span>';\n"
    "        var tx=e.clientX+16,ty2=e.clientY+18;\n"
    "        if(tx+230>window.innerWidth) tx=e.clientX-246;\n"
    "        if(ty2+76>window.innerHeight) ty2=e.clientY-90;\n"
    "        blkTipEl.style.left=tx+'px';\n"
    "        blkTipEl.style.top=ty2+'px';\n"
    "        blkTipEl.style.display='block';\n"
    "      });\n"
    "      blkCv.addEventListener('mouseleave',function(){\n"
    "        blkSel=-1;\n"
    "        if(blkTipEl)blkTipEl.style.display='none';\n"
    "      });\n"
    "    }\n"
    "    blkSize();\n"
    "    flyStop();rotoStop();voxStop();tnStop();fireStop();twStop();\n"
    "    kfStop();ecStop();\n"
    "    blkOn=true;\n"
    "    document.body.classList.add('blox');\n"
    "    showToast('EVERY BYTE A PILLAR');\n"
    "    if(!blkRaf) blkRaf=requestAnimationFrame(blkFrame);\n"
    "  }\n"
    "  function blkStop(){\n"
    "    blkOn=false;blkSel=-1;\n"
    "    document.body.classList.remove('blox');\n"
    "    if(blkTipEl)blkTipEl.style.display='none';\n"
    "    if(blkRaf){cancelAnimationFrame(blkRaf);blkRaf=null;}\n"
    "  }\n"
    "  /* shared hover card for the fire and the twister (void only:\n"
    "     that is when the pointer reaches the backdrop) */\n"
    "  var fwTip=null;\n"
    "  function fwHide(){if(fwTip)fwTip.style.display='none';}\n"
    "  function fwShow(e,html){\n"
    "    if(!fwTip){\n"
    "      fwTip=document.createElement('div');\n"
    "      fwTip.style.cssText='position:fixed;display:none;z-index:10001;'\n"
    "        +'pointer-events:none;background:rgba(0,0,10,.92);'\n"
    "        +'border:1px solid #22d3ee;padding:8px 10px;'\n"
    "        +'font-family:\\'Press Start 2P\\',monospace;font-size:8px;'\n"
    "        +'color:#e8e8f0;line-height:2;letter-spacing:1px;'\n"
    "        +'box-shadow:4px 4px 0 rgba(0,0,0,.55)';\n"
    "      document.body.appendChild(fwTip);\n"
    "    }\n"
    "    fwTip.innerHTML=html;\n"
    "    var tx=e.clientX+16,ty=e.clientY+18;\n"
    "    if(tx+250>window.innerWidth) tx=e.clientX-266;\n"
    "    if(ty+86>window.innerHeight) ty=e.clientY-98;\n"
    "    fwTip.style.left=tx+'px';fwTip.style.top=ty+'px';\n"
    "    fwTip.style.display='block';\n"
    "  }\n"
    "  /* doom fire, but the fuel is the file: x is byte offset and the\n"
    "     heat fed at the bottom is that region's encoding cost. this is\n"
    "     the cost-over-position chart, on fire - expensive regions blaze\n"
    "     white, compressible ones barely smolder, and the priciest\n"
    "     bytes spit sparks */\n"
    "  var fireOn=false,fireCv=null,fireCtx=null,fireRaf=null;\n"
    "  var fireImg=null,firePix=null,fireHeat=null,fireW=0,fireH=0;\n"
    "  var fireBase=null,firePal=null,fireSp=null,fireCmax=8;\n"
    "  function firePalBuild(){\n"
    "    if(firePal) return;\n"
    "    firePal=new Uint32Array(37);\n"
    "    for(var i=1;i<37;i++){\n"
    "      var t=i/36;\n"
    "      var r=Math.min(255,(600*t)|0);\n"
    "      var g=Math.max(0,Math.min(255,(600*(t-.35))|0));\n"
    "      var b=Math.max(0,Math.min(255,(600*(t-.78))|0));\n"
    "      firePal[i]=0xFF000000|(b<<16)|(g<<8)|r;\n"
    "    }\n"
    "    fireCmax=.01;\n"
    "    for(var i=0;i<BD.length;i++)\n"
    "      if(BD[i].c>fireCmax) fireCmax=BD[i].c;\n"
    "  }\n"
    "  function fireSize(){\n"
    "    if(!fireCv) return;\n"
    "    fireW=Math.max(2,window.innerWidth>>2);\n"
    "    fireH=Math.max(2,window.innerHeight>>2);\n"
    "    fireCv.width=fireW;fireCv.height=fireH;\n"
    "    fireImg=fireCtx.createImageData(fireW,fireH);\n"
    "    firePix=new Uint32Array(fireImg.data.buffer);\n"
    "    fireHeat=new Uint8Array(fireW*fireH);\n"
    "    /* fuel map: column x burns its slice of the file */\n"
    "    var nb=BD.length;\n"
    "    fireBase=new Uint8Array(fireW);\n"
    "    for(var x=0;x<fireW;x++){\n"
    "      var lo=(x*nb/fireW)|0,hi=((x+1)*nb/fireW)|0;\n"
    "      if(hi<=lo)hi=lo+1; if(hi>nb)hi=nb;\n"
    "      var s=0;\n"
    "      for(var i=lo;i<hi;i++) s+=BD[i].c;\n"
    "      var t=(s/(hi-lo))/fireCmax; if(t>1)t=1;\n"
    "      fireBase[x]=(3+t*33)|0;\n"
    "    }\n"
    "    /* the twelve most expensive bytes get to spit embers */\n"
    "    var idx=[];\n"
    "    for(var i=0;i<nb;i++) idx.push(i);\n"
    "    idx.sort(function(a,b){return BD[b].c-BD[a].c;});\n"
    "    fireSp=[];\n"
    "    for(var k=0;k<Math.min(12,nb);k++)\n"
    "      fireSp.push((idx[k]*fireW/nb)|0);\n"
    "  }\n"
    "  function fireFrame(){\n"
    "    if(!fireOn||!document.body.classList.contains('party')){\n"
    "      fireStop();return;\n"
    "    }\n"
    "    var W=fireW,H=fireH,h2=fireHeat;\n"
    "    /* stoke the fuel row, with flicker */\n"
    "    var brow=(H-1)*W;\n"
    "    for(var x=0;x<W;x++){\n"
    "      var v=fireBase[x]+((Math.random()*5)|0)-2;\n"
    "      if(v<0)v=0; if(v>36)v=36;\n"
    "      h2[brow+x]=v;\n"
    "    }\n"
    "    if(fireSp.length&&Math.random()<.3){\n"
    "      var sx=fireSp[(Math.random()*fireSp.length)|0];\n"
    "      h2[brow+sx]=36;\n"
    "      if(sx>0)h2[brow+sx-1]=34;\n"
    "      if(sx<W-1)h2[brow+sx+1]=34;\n"
    "    }\n"
    "    /* the doom spread, lateral drift and all */\n"
    "    for(var x=0;x<W;x++){\n"
    "      for(var y=1;y<H;y++){\n"
    "        var src=y*W+x,v=h2[src];\n"
    "        if(!v){h2[src-W]=0;continue;}\n"
    ...
    "      }\n"
    "    }\n"
    "    for(var i=0;i<W*H;i++) firePix[i]=firePal[h2[i]];\n"
    "    fireCtx.putImageData(fireImg,0,0);\n"
    "    fireRaf=requestAnimationFrame(fireFrame);\n"
    "  }\n"
    "  function fireToggle(){\n"
    "    if(fireOn){fireStop();flyStart();showToast('EXTINGUISHED');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO BYTES NO FUEL');return;}\n"
    "    egg('fire');\n"
    "    firePalBuild();\n"
    "    if(!fireCv){\n"
    ...
    "      fireCv.addEventListener('mousemove',function(e){\n"
    "        if(!fireOn){fwHide();return;}\n"
    "        var r=fireCv.getBoundingClientRect();\n"
    "        var x=((e.clientX-r.left)/r.width*fireW)|0;\n"
    "        if(x<0||x>=fireW){fwHide();return;}\n"
    "        var nb=BD.length;\n"
    "        var lo=(x*nb/fireW)|0,hi=((x+1)*nb/fireW)|0;\n"
    "        if(hi<=lo)hi=lo+1; if(hi>nb)hi=nb;\n"
    "        var bi=lo;\n"
    "        for(var i=lo;i<hi;i++) if(BD[i].c>BD[bi].c) bi=i;\n"
    "        var d=BD[bi];\n"
    "        fwShow(e,'BYTES '+lo+'\\u2013'+(hi-1)+'<br>'\n"
    "          +'HOTTEST: 0x'+d.h+' @ '+bi+'<br>'\n"
    "          +'<span style=\"color:'+(d.c<3?'#34d399':d.c<6?'#fbbf24'\n"
    "          :'#f87171')+'\">COST '+d.c.toFixed(2)+' BITS</span>');\n"
    "      });\n"
    "      fireCv.addEventListener('mouseleave',fwHide);\n"
    "    }\n"
    "    fireSize();\n"
    "    flyStop();rotoStop();voxStop();tnStop();blkStop();twStop();\n"
    "    kfStop();ecStop();\n"
    "    fireOn=true;\n"
    "    document.body.classList.add('burn');\n"
    "    showToast('THE EXPENSIVE BYTES BURN BRIGHTEST');\n"
    "    if(!fireRaf) fireRaf=requestAnimationFrame(fireFrame);\n"
    "  }\n"
    "  function fireStop(){\n"
    "    fireOn=false;\n"
    ...
    "  }\n"
    "  /* the twister: the file IS the column. three scanlines per byte,\n"
    "     colored by encoding cost, and the twist RATE is the byte value:\n"
    "     runs of zeros stand straight, entropy wrings the column.\n"
    "     offsets are stamped alongside so you always know where you are */\n"
    "  var twOn=false,twCv=null,twCtx=null,twRaf=null,twT=0,twPos=0,\n"
    "    twKick=0;\n"
    "  var twCum=null,twR=null,twG=null,twB=null;\n"
    "  function twBuild(){\n"
    "    if(twCum) return;\n"
    "    var nb=BD.length;\n"
    "    twCum=new Float32Array(nb+1);\n"
    "    twR=new Uint8Array(nb);twG=new Uint8Array(nb);\n"
    "    twB=new Uint8Array(nb);\n"
    "    var cmax=.01;\n"
    "    for(var i=0;i<nb;i++) if(BD[i].c>cmax)cmax=BD[i].c;\n"
    "    for(var i=0;i<nb;i++){\n"
    "      var v=parseInt(BD[i].h,16);\n"
    "      twCum[i+1]=twCum[i]+.015+(v/255)*.14;\n"
    "      var t=BD[i].c/cmax; if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(t<0.5){var u=t*2;r=16+164*u;g=185-45*u;b=129-89*u;}\n"
    "      else{var u=(t-0.5)*2;r=180+68*u;g=140-27*u;b=40+73*u;}\n"
    "      twR[i]=r|0;twG[i]=g|0;twB[i]=b|0;\n"
    "    }\n"
    "  }\n"
    "  function twSize(){\n"
    "    if(!twCv) return;\n"
    "    var sf=Math.min(.5,960/window.innerWidth);\n"
    "    twCv.width=Math.max(2,(window.innerWidth*sf)|0);\n"
    "    twCv.height=Math.max(2,(window.innerHeight*sf)|0);\n"
    "  }\n"
    "  function twFrame(){\n"
    "    if(!twOn||!document.body.classList.contains('party')){\n"
    "      twStop();return;\n"
    "    }\n"
    "    var W=twCv.width,H=twCv.height,nb=BD.length,ctx=twCtx;\n"
    "    twT+=.016;\n"
    "    twKick*=.9;\n"
    "    twPos+=.5; if(twPos>=nb) twPos-=nb;\n"
    "    ctx.clearRect(0,0,W,H);\n"
    "    var cx0=W/2,R=W*.13,HP=Math.PI/2,twLbl=-1;\n"
    "    for(var y=0;y<H;y++){\n"
    "      var p=twPos+y/3;\n"
    "      var i0=p|0,fr=p-i0;\n"
    "      var i=i0%%nb;\n"
    "      var a=twCum[i]+(twCum[i+1]-twCum[i])*fr\n"
    "        +twT*.6+Math.sin(twT*.7+y*.006)*.5\n"
    "        +twKick*Math.sin(y*.03+twT*9)*.5;\n"
    "      var cx=cx0+Math.sin(twT*.23+y*.002)*W*.06;\n"
    "      var xs=[cx+R*Math.cos(a),cx+R*Math.cos(a+HP),\n"
    "        cx+R*Math.cos(a+2*HP),cx+R*Math.cos(a+3*HP)];\n"
    "      for(var k=0;k<4;k++){\n"
    "        var xa=xs[k],xb=xs[(k+1)&3];\n"
    "        if(xb<=xa) continue;\n"
    "        var sh=.3+.7*(xb-xa)/(R*1.42);\n"
    "        if(k&1)sh*=.82;\n"
    "        ctx.fillStyle='rgb('+((twR[i]*sh)|0)+','+((twG[i]*sh)|0)\n"
    "          +','+((twB[i]*sh)|0)+')';\n"
    ...
    "        ctx.textAlign='left';\n"
    "        ctx.fillStyle='rgba(154,154,173,.75)';\n"
    "        ctx.fillText('0x'+blkHex(i)+' '+BD[i].h\n"
    "          +(BD[i].ch?' '+BD[i].ch:''),cx0+R+16,y+3);\n"
    "        ctx.fillStyle='rgba(99,99,111,.5)';\n"
    "        ctx.fillRect(cx+R+2,y,12,1);\n"
    "      }\n"
    "    }\n"
    "    twRaf=requestAnimationFrame(twFrame);\n"
    "  }\n"
    "  function twToggle(){\n"
    "    if(twOn){twStop();flyStart();showToast('UNWOUND');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO BYTES NO TORQUE');return;}\n"
    "    egg('twister');\n"
    "    twBuild();\n"
    "    if(!twCv){\n"
    "      twCv=document.getElementById('twist');\n"
    "      twCtx=twCv.getContext('2d');\n"
    "      window.addEventListener('resize',twSize);\n"
    "      twCv.addEventListener('mousemove',function(e){\n"
    "        if(!twOn){fwHide();return;}\n"
    "        var r=twCv.getBoundingClientRect();\n"
    "        var y=((e.clientY-r.top)/r.height*twCv.height)|0;\n"
    "        var i=((twPos+y/3)|0)%%BD.length;\n"
    "        var d=BD[i];\n"
    "        var v=parseInt(d.h,16);\n"
    "        fwShow(e,'OFFSET 0x'+blkHex(i)+' ('+i+')<br>'\n"
    "          +'BYTE '+d.h+(d.ch?' <span style=\"color:#ff3fd8\">'\n"
    "          +(d.ch==='<'?'&lt;':d.ch==='&'?'&amp;':d.ch)+'</span>':'')\n"
    "          +' &middot; TWIST +'+(.015+v/255*.14).toFixed(3)+' RAD<br>'\n"
    "          +'<span style=\"color:'+(d.c<3?'#34d399':d.c<6?'#fbbf24'\n"
    "          :'#f87171')+'\">COST '+d.c.toFixed(2)+' BITS</span>');\n"
    "      });\n"
    "      twCv.addEventListener('mouseleave',fwHide);\n"
    "    }\n"
    "    twSize();\n"
    "    flyStop();rotoStop();voxStop();tnStop();blkStop();fireStop();\n"
    "    kfStop();ecStop();\n"
    "    twOn=true;\n"
    "    document.body.classList.add('twst');\n"
    "    showToast('ORDER STANDS STRAIGHT. NOISE TWISTS');\n"
    "    if(!twRaf) twRaf=requestAnimationFrame(twFrame);\n"
    "  }\n"
    "  function twStop(){\n"
    "    twOn=false;\n"
    "    document.body.classList.remove('twst');\n"
    "    fwHide();\n"
    "    if(twRaf){cancelAnimationFrame(twRaf);twRaf=null;}\n"
    "  }\n"
    "  /* kefrens bars: one bar per model, wearing its attribution\n"
    "     color. the classic trick, honestly kept: no clear - each\n"
    "     scanline inherits the line above and the bars stamp over\n"
    "     it, so the columns are trails. the vertical axis is the\n"
    "     file: bar m swings at line y by its net contribution in\n"
    "     that window - dominant models swing wide, dead ones hang\n"
    "     straight, hurting ones blush red */\n"
    "  var kfOn=false,kfCv=null,kfCtx=null,kfRaf=null,kfT=0;\n"
    "  var kfImg=null,kfPix=null,kfVmax=0,kfKick=0,kfKickY=-1;\n"
    "  function kfBuild(){\n"
    "    if(kfVmax||!window.netMdata) return;\n"
    "    var n=window.netNm*window.netNpts;\n"
    "    kfVmax=.001;\n"
    "    for(var i=0;i<n;i++){\n"
    "      var a=Math.abs(window.netMdata[i]);\n"
    "      if(a>kfVmax) kfVmax=a;\n"
    "    }\n"
    "  }\n"
    "  function kfSize(){\n"
    "    if(!kfCv) return;\n"
    "    kfCv.width=Math.max(2,window.innerWidth>>2);\n"
    "    kfCv.height=Math.max(2,window.innerHeight>>2);\n"
    "    kfImg=kfCtx.createImageData(kfCv.width,kfCv.height);\n"
    "    kfPix=new Uint32Array(kfImg.data.buffer);\n"
    "  }\n"
    "  /* where bar m hangs at line y: lane center + global sway +\n"
    "     its own swing, amplitude scaled by |net| in that window */\n"
    "  function kfX(m,y,W,H){\n"
    "    var nm=window.netNm,np=window.netNpts;\n"
    "    var wi=(y*(np-1)/(H-1))|0;\n"
    "    var v=window.netMdata[wi*nm+m]/kfVmax;\n"
    "    if(v>1)v=1; if(v<-1)v=-1;\n"
    "    var at=v<0?-v:v;\n"
    "    var lane=W/nm;\n"
    "    var x=(m+.5)*lane\n"
    "      +Math.sin(kfT*.4+y*.012)*lane*.3\n"
    "      +Math.sin(y*.055+kfT*(2.2+m*.13)+m*2.4)*lane*(.12+1.5*at);\n"
    "    /* the beat reaches the bars: a jolt centered on the\n"
    "       scanline of the byte being played right now */\n"
    "    if(kfKick>.02&&kfKickY>=0){\n"
    "      var d=y-kfKickY;\n"
    "      x+=Math.sin(kfT*23+y*.2)*lane*kfKick/(1+d*d*.004);\n"
    "    }\n"
    "    return {x:x,v:v,at:at,wi:wi};\n"
    "  }\n"
    "  function kfFrame(){\n"
    "    if(!kfOn||!document.body.classList.contains('party')){\n"
    "      kfStop();return;\n"
    "    }\n"
    "    var W=kfCv.width,H=kfCv.height,nm=window.netNm;\n"
    "    var np=window.netNpts,nb=window.netNb;\n"
    "    kfT+=.016;\n"
    "    kfKick*=.88;\n"
    "    for(var x=0;x<W;x++) kfPix[x]=0xFF0E0410;\n"
    "    for(var y=1;y<H;y++){\n"
    "      kfPix.copyWithin(y*W,(y-1)*W,y*W);\n"
    "      for(var m=0;m<nm;m++){\n"
    "        var q=kfX(m,y,W,H);\n"
    "        var pc=(window.netPal&&window.netPal[m])||[160,160,160];\n"
    "        var r=pc[0],g=pc[1],b2=pc[2];\n"
    "        if(q.v<0){\n"
    "          r+=(248-r)*q.at;g+=(60-g)*q.at;b2+=(60-b2)*q.at;\n"
    "        }\n"
    "        var lum=.35+.75*q.at;\n"
    "        var xc=q.x|0;\n"
    "        for(var k=-2;k<=2;k++){\n"
    ...
    "          var sh=lum*(k===0?1.25:(k<-1||k>1)?.4:.85);\n"
    "          var rr=r*sh,gg=g*sh,bb=b2*sh;\n"
    "          if(rr>255)rr=255; if(gg>255)gg=255; if(bb>255)bb=255;\n"
    "          kfPix[y*W+xx]=0xFF000000|((bb|0)<<16)|((gg|0)<<8)|(rr|0);\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    kfCtx.putImageData(kfImg,0,0);\n"
    "    /* the ruler: the vertical axis IS the file - say so */\n"
    "    kfCtx.font='6px \"Press Start 2P\",monospace';\n"
    "    kfCtx.textAlign='left';\n"
    "    kfCtx.fillStyle='rgba(154,154,173,.8)';\n"
    "    var rstep=(H/6)|0;\n"
    "    for(var y2=rstep;y2<H-6;y2+=rstep){\n"
    "      var wi2=(y2*(np-1)/(H-1))|0;\n"
    "      var bp2=(wi2*(nb-1)/(np-1))|0;\n"
    "      kfCtx.fillText('0x'+blkHex(bp2),5,y2+2);\n"
    "      kfCtx.fillRect(0,y2,3,1);\n"
    "    }\n"
    "    /* playhead: the line the music is on, while it plays */\n"
    "    if(kfKick>.05&&kfKickY>0){\n"
    "      kfCtx.fillStyle='rgba(255,227,63,'\n"
    "        +(kfKick>1?1:kfKick).toFixed(2)+')';\n"
    "      kfCtx.fillRect(0,kfKickY|0,W,1);\n"
    "    }\n"
    "    kfRaf=requestAnimationFrame(kfFrame);\n"
    "  }\n"
    "  function kfToggle(){\n"
    "    if(kfOn){kfStop();flyStart();showToast('BARS DOWN');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(!window.netMdata||!window.netNm){\n"
    "      showToast('NO MODELS NO BARS');return;}\n"
    "    egg('kefrens');\n"
    "    kfBuild();\n"
    "    if(!kfCv){\n"
    "      kfCv=document.getElementById('kefrens');\n"
    "      kfCtx=kfCv.getContext('2d');\n"
    "      window.addEventListener('resize',kfSize);\n"
    "      kfCv.addEventListener('mousemove',function(e){\n"
    "        if(!kfOn){fwHide();return;}\n"
    "        var r=kfCv.getBoundingClientRect();\n"
    "        var mx=(e.clientX-r.left)/r.width*kfCv.width;\n"
    "        var my=((e.clientY-r.top)/r.height*kfCv.height)|0;\n"
    "        if(my<1)my=1; if(my>=kfCv.height)my=kfCv.height-1;\n"
    "        var bi=-1,bd=1e9,bq=null;\n"
    "        for(var m=0;m<window.netNm;m++){\n"
    "          var q=kfX(m,my,kfCv.width,kfCv.height);\n"
    "          var d2=q.x>mx?q.x-mx:mx-q.x;\n"
    "          if(d2<bd){bd=d2;bi=m;bq=q;}\n"
    "        }\n"
    "        if(bi<0||bd>kfCv.width/window.netNm){fwHide();return;}\n"
    "        var lbl=(typeof MI!=='undefined'&&MI[bi])\n"
    "          ?MI[bi].mask+':'+MI[bi].w:bi;\n"
    "        var bytePos=(bq.wi*(window.netNb-1)/(window.netNpts-1))|0;\n"
    "        var v=window.netMdata[bq.wi*window.netNm+bi];\n"
    "        fwShow(e,'MODEL '+lbl+'<br>BYTE ~'+bytePos+'<br>'\n"
    "          +'<span style=\"color:'+(v>0.001?'#34d399'\n"
    "          :v<-0.001?'#f87171':'#9a9aad')+'\">NET '\n"
    "          +(v>=0?'+':'')+v.toFixed(2)+' b/B</span>');\n"
    "      });\n"
    "      kfCv.addEventListener('mouseleave',fwHide);\n"
    "    }\n"
    "    kfSize();\n"
    "    flyStop();rotoStop();voxStop();tnStop();blkStop();\n"
    "    fireStop();twStop();ecStop();\n"
    "    kfOn=true;\n"
    "    document.body.classList.add('kefr');\n"
    "    showToast('THE MODELS RAISE THE BARS');\n"
    "    if(!kfRaf) kfRaf=requestAnimationFrame(kfFrame);\n"
    "  }\n"
    "  function kfStop(){\n"
    "    kfOn=false;\n"
    "    document.body.classList.remove('kefr');\n"
    "    fwHide();\n"
    "    if(kfRaf){cancelAnimationFrame(kfRaf);kfRaf=null;}\n"
    "  }\n"
    "  /* echo bridges: the file as a road at night. every backward\n"
    "     match is a suspension bridge from the copy back to its\n"
    "     source - the span IS the match distance, the glow is the\n"
    "     match length, and the color is the verdict: green, the\n"
    "     models caught the repeat and the copy was nearly free;\n"
    "     red, the coder paid for bytes it had already seen. red\n"
    "     arches are the model set's unfinished business */\n"
    "  var ecOn=false,ecCv=null,ecCtx=null,ecRaf=null,ecT=0;\n"
    "  var ecArcs=null,ecN=0,ecCam=0,ecDir=1;\n"
    "  var ecPlay=-1,ecKick=0,ecFollowT=0;\n"
    "  function ecBuild(){\n"
    "    if(ecArcs) return;\n"
    "    var ml=window.repML,md=window.repMD;\n"
    "    ecN=Math.min(BD.length,ml.length);\n"
    "    var pre=new Float64Array(ecN+1);\n"
    "    for(var i=0;i<ecN;i++) pre[i+1]=pre[i]+BD[i].c;\n"
    "    /* greedy parse: one bridge per repeated region, not one\n"
    "       per position - overlapping matches are the same echo */\n"
    "    ecArcs=[];\n"
    "    var i=0;\n"
    "    while(i<ecN){\n"
    "      var l=ml[i];\n"
    "      if(l>=8){\n"
    "        if(i+l>ecN) l=ecN-i;\n"
    "        ecArcs.push({d:i,s:i-md[i],l:l,\n"
    "          a:(pre[i+l]-pre[i])/l});\n"
    "        i+=l;\n"
    "      } else i++;\n"
    "    }\n"
    "    if(!ecArcs.length) return;\n"
    "    /* too many bridges is fog: keep the longest */\n"
    "    ecArcs.sort(function(a,b){return b.l-a.l;});\n"
    "    if(ecArcs.length>400) ecArcs=ecArcs.slice(0,400);\n"
    "    /* landmarks: the longest spans, plus the priciest repeat\n"
    "       - the place the coder most regrets */\n"
    "    for(var k=0;k<ecArcs.length&&k<5;k++) ecArcs[k].lb=1;\n"
    "    var wi=-1,wc=0;\n"
    "    for(var k=0;k<ecArcs.length;k++){\n"
    "      var tc=ecArcs[k].a*ecArcs[k].l;\n"
    "      if(ecArcs[k].a>=4&&tc>wc){wc=tc;wi=k;}\n"
    "    }\n"
    "    if(wi>=0) ecArcs[wi].pw=1;\n"
    "  }\n"
    "  function ecSize(){\n"
    "    if(!ecCv) return;\n"
    "    var sf=Math.min(.5,960/window.innerWidth);\n"
    "    ecCv.width=Math.max(2,(window.innerWidth*sf)|0);\n"
    "    ecCv.height=Math.max(2,(window.innerHeight*sf)|0);\n"
    "  }\n"
    "  /* a repeat should be nearly free; anything above ~4 b/B\n"
    "     means the models never noticed */\n"
    "  function ecCol(a){\n"
    "    return a<2?'52,211,153':a<4.5?'251,191,36':'248,113,113';\n"
    "  }\n"
    "  function ecView(){return ecN<1200?ecN:Math.max(1200,ecN/3);}\n"
    "  function ecFrame(){\n"
    "    if(!ecOn||!document.body.classList.contains('party')){\n"
    "      ecStop();return;\n"
    "    }\n"
    "    var W=ecCv.width,H=ecCv.height,ctx=ecCtx;\n"
    "    ecT+=.016;ecKick*=.9;\n"
    "    var view=ecView();\n"
    "    /* the camera drifts the file; when the music plays it\n"
    "       goes where the needle is */\n"
    "    if(view>=ecN){\n"
    "      ecCam=(ecN-view)/2;\n"
    "    } else if(ecPlay>=0&&Date.now()-ecFollowT<2500){\n"
    "      ecCam+=((ecPlay-view/2)-ecCam)*.04;\n"
    "      if(ecCam>ecN-view)ecCam=ecN-view;\n"
    "      if(ecCam<0)ecCam=0;\n"
    "    } else {\n"
    "      ecCam+=ecDir*view*.0012;\n"
    "      if(ecCam>ecN-view){ecCam=ecN-view;ecDir=-1;}\n"
    "      if(ecCam<0){ecCam=0;ecDir=1;}\n"
    "    }\n"
    "    var px=W/view;\n"
    "    ctx.clearRect(0,0,W,H);\n"
    "    var roadY=(H*.8)|0;\n"
    "    /* the road: the cost ramp underfoot */\n"
    "    for(var x=0;x<W;x++){\n"
    "      var b=(ecCam+x/px)|0;\n"
    "      if(b<0||b>=ecN) continue;\n"
    "      ctx.fillStyle='rgba('+ecCol(BD[b].c)+',.9)';\n"
    "      ctx.fillRect(x,roadY,1,2);\n"
    "    }\n"
    "    /* mileposts every 512 bytes */\n"
    "    ctx.font='6px \"Press Start 2P\",monospace';\n"
    "    ctx.textAlign='left';\n"
    "    var mb=(((Math.max(0,ecCam)|0)+511)&~511);\n"
    "    var me=Math.min(ecN,ecCam+view);\n"
    "    for(var b=mb;b<me;b+=512){\n"
    "      var x=(b-ecCam)*px;\n"
    "      ctx.fillStyle='rgba(99,99,111,.7)';\n"
    "      ctx.fillRect(x|0,roadY+3,1,4);\n"
    "      ctx.fillStyle='rgba(154,154,173,.75)';\n"
    "      ctx.fillText('0x'+blkHex(b),x+2,roadY+12);\n"
    "    }\n"
    "    /* bridges, longest first so the short ones read on top */\n"
    "    ctx.lineCap='round';\n"
    "    ctx.font='7px \"Press Start 2P\",monospace';\n"
    "    var drawn=[];\n"
    "    for(var k=0;k<ecArcs.length;k++){\n"
    "      var a=ecArcs[k];\n"
    "      var x1=(a.s-ecCam)*px,x2=(a.d-ecCam)*px;\n"
    "      if(x2<-20||x1>W+20) continue;\n"
    "      var span=x2-x1;\n"
    "      var hgt=span*.42+6; if(hgt>roadY-16) hgt=roadY-16;\n"
    "      var lit=(ecPlay>=a.d&&ecPlay<a.d+a.l)||\n"
    "        (ecPlay>=a.s&&ecPlay<a.s+a.l);\n"
    "      var lw=.6+Math.log(a.l)*.25;\n"
    "      var al=lit?1:.28+Math.min(.4,a.l/600);\n"
    "      ctx.strokeStyle='rgba('+ecCol(a.a)+','+al.toFixed(2)+')';\n"
    "      ctx.lineWidth=lit?lw+1:lw;\n"
    "      if(lit){ctx.shadowColor='rgb('+ecCol(a.a)+')';\n"
    "        ctx.shadowBlur=8;}\n"
    "      ctx.beginPath();\n"
    "      ctx.moveTo(x1,roadY);\n"
    "      ctx.quadraticCurveTo((x1+x2)/2,roadY-2*hgt,x2,roadY);\n"
    "      ctx.stroke();\n"
    "      ctx.shadowBlur=0;\n"
    "      ctx.fillStyle=ctx.strokeStyle;\n"
    "      ctx.fillRect(x1|0,roadY-3,1,3);\n"
    "      ctx.fillRect(x2|0,roadY-3,1,3);\n"
    "      if((a.lb||a.pw)&&span>30){\n"
    "        var ax=(x1+x2)/2,ay=roadY-hgt-4;\n"
    "        if(ay>8){\n"
    "          var ok=1;\n"
    "          for(var j=0;j<drawn.length;j++){\n"
    "            if(Math.abs(drawn[j][0]-ax)<52&&\n"
    "               Math.abs(drawn[j][1]-ay)<10){ok=0;break;}\n"
    "          }\n"
    "          if(ok){\n"
    "            drawn.push([ax,ay]);\n"
    ...
    "              ctx.fillText('PAID TWICE',ax,ay);\n"
    "            } else {\n"
    "              ctx.fillStyle='rgba(232,232,240,.8)';\n"
    "              ctx.fillText(a.l+'B \\u2190 0x'+blkHex(a.s),ax,ay);\n"
    "            }\n"
    "            ctx.textAlign='left';\n"
    "          }\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    /* the needle: where the music is, while it plays */\n"
    "    if(ecPlay>=0&&ecKick>.03){\n"
    "      var xp=(ecPlay-ecCam)*px;\n"
    "      if(xp>=0&&xp<W){\n"
    "        ctx.fillStyle='rgba(255,227,63,'\n"
    "          +Math.min(1,ecKick).toFixed(2)+')';\n"
    "        ctx.fillRect(xp|0,(roadY-H*.45)|0,1,(H*.45+2)|0);\n"
    "      }\n"
    "    }\n"
    "    ecRaf=requestAnimationFrame(ecFrame);\n"
    "  }\n"
    "  function ecToggle(){\n"
    "    if(ecOn){ecStop();flyStart();showToast('BRIDGES BURNED');return;}\n"
    "    if(!document.body.classList.contains('party')) return;\n"
    "    if(!window.repML||typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO ECHOES IN THIS FILE');return;}\n"
    "    ecBuild();\n"
    "    if(!ecArcs||!ecArcs.length){\n"
    "      showToast('NOTHING REPEATS. RESPECT.');return;}\n"
    "    egg('echoes');\n"
    "    if(!ecCv){\n"
    "      ecCv=document.getElementById('echoes');\n"
    "      ecCtx=ecCv.getContext('2d');\n"
    "      window.addEventListener('resize',ecSize);\n"
    "      ecCv.addEventListener('mousemove',function(e){\n"
    "        if(!ecOn||!ecArcs){fwHide();return;}\n"
    "        var r=ecCv.getBoundingClientRect();\n"
    "        var mx=(e.clientX-r.left)/r.width*ecCv.width;\n"
    "        var my=(e.clientY-r.top)/r.height*ecCv.height;\n"
    "        var W=ecCv.width,H=ecCv.height,roadY=(H*.8)|0;\n"
    "        var px=W/ecView();\n"
    "        var best=null,bd2=12;\n"
    "        for(var k=0;k<ecArcs.length;k++){\n"
    "          var a=ecArcs[k];\n"
    "          var x1=(a.s-ecCam)*px,x2=(a.d-ecCam)*px;\n"
    "          if(mx<x1-2||mx>x2+2) continue;\n"
    "          var span=x2-x1;\n"
    "          var hgt=span*.42+6; if(hgt>roadY-16) hgt=roadY-16;\n"
    "          var t=span>0?(mx-x1)/span:0;\n"
    "          var cy=roadY-4*hgt*t*(1-t);\n"
    "          var dd=my>cy?my-cy:cy-my;\n"
    "          if(dd<bd2){bd2=dd;best=a;}\n"
    "        }\n"
    "        if(!best){fwHide();return;}\n"
    "        var st=best.a<2\n"
    "          ?'<span style=\"color:#34d399\">CAUGHT</span>'\n"
    "          :best.a<4.5\n"
    "          ?'<span style=\"color:#fbbf24\">PARTLY</span>'\n"
    "          :'<span style=\"color:#f87171\">MISSED</span>';\n"
    "        fwShow(e,'MATCH 0x'+blkHex(best.d)+' \\u2190 0x'\n"
    "          +blkHex(best.s)+'<br>'+best.l+' B \\u00b7 DIST '\n"
    "          +(best.d-best.s)+'<br>AVG '+best.a.toFixed(2)\n"
    "          +' b/B \\u00b7 '+st);\n"
    "      });\n"
    "      ecCv.addEventListener('mouseleave',fwHide);\n"
    "    }\n"
    "    ecSize();\n"
    "    flyStop();rotoStop();voxStop();tnStop();blkStop();\n"
    "    fireStop();twStop();kfStop();\n"
    "    ecOn=true;\n"
    "    document.body.classList.add('echo');\n"
    "    showToast('EVERY BRIDGE IS A DEJA VU');\n"
    "    if(!ecRaf) ecRaf=requestAnimationFrame(ecFrame);\n"
    "  }\n"
    "  function ecStop(){\n"
    "    ecOn=false;\n"
    ...
    "  }\n"
    "  /* forward pass: the whole model pipeline - same hashes, same\n"
    "     table, same mixer, same counter updates as the real decoder\n"
    "     - but the bits are already known, so there is no range\n"
    "     coder. hand it bytes, it hands you the bill, in bits, per\n"
    "     byte. this is the scoring engine compose mode will lean on;\n"
    ...
    "  var fpLU=new Uint32Array(1),fpLF=new Float32Array(fpLU.buffer);\n"
    "  var FPC0=Math.fround(1.42286530448213);\n"
    "  var FPC1=Math.fround(-0.58208536795165);\n"
    "  var FPC2=Math.fround(0.15922006346951);\n"
    "  function fpLog2(x){\n"
    "    fpLF[0]=x;\n"
    "    var u=fpLU[0];\n"
    "    var e=(u>>>23)-127;\n"
    "    fpLU[0]=(u&0x7FFFFF)|0x3F800000;\n"
    "    var m=Math.fround(fpLF[0]-1);\n"
    "    return Math.fround(e+Math.fround(m*Math.fround(FPC0\n"
    "      +Math.fround(m*Math.fround(FPC1\n"
    "      +Math.fround(m*FPC2))))));\n"
    "  }\n"
    "  function fpScore(bytes,wantM,mo){\n"
    "    if(!fpHdr()) return null;\n"
    "    /* mo: an audition lineup {num,weights,ext} instead of the\n"
    "       shipped one */\n"
    "    var num=mo?mo.num:fpH.num;\n"
    "    var weights=mo?mo.weights:fpH.weights;\n"
    "    var ext=mo?mo.ext:fpH.ext;\n"
    "    /* the table is kept, and the slots the last run touched are\n"
    "       journaled, so cleanup zeroes half a million entries instead\n"
    "       of sweeping 2^db slots - at -H 30 that is the difference\n"
    "       between a keystroke and a coffee break. oversized runs\n"
    "       (journal past 8M probes) fall back to the full sweep */\n"
    "    if(!fpDT) fpDT=new Uint8Array(2*(1<<PAQ.db));\n"
    "    else if(fpJN>0){\n"
    "      for(var i=0;i<fpJN;i++){var j=fpJ[i];fpDT[j]=0;fpDT[j+1]=0;}\n"
    "    } else if(fpJN<0) fpDT.fill(0);\n"
    "    fpJN=0;\n"
    "    var dt=fpDT;\n"
    "    var nb=bytes.length,bitlen=nb*8+1;\n"
    "    var jn=bitlen*num,useJ=jn<=8388608;\n"
    "    if(useJ&&(!fpJ||fpJ.length<jn)) fpJ=new Uint32Array(jn);\n"
    "    if(wantM){\n"
    "      if(!fpM||fpM.length<nb*num) fpM=new Float32Array(nb*num);\n"
    "      else fpM.fill(0,0,nb*num);\n"
    "      /* coder stats, mirroring the C stats pass field for field */\n"
    "      if(!fpS||fpS.bb.length<nb*8||fpS.mH.length<num){\n"
    "        fpS={bb:new Float32Array(nb*8),conf:new Uint32Array(11),\n"
    "          calC:new Uint32Array(20),cal1:new Uint32Array(20),\n"
    "          calP:new Float64Array(20),\n"
    "          bpC:new Uint32Array(8),bpS:new Float64Array(8),\n"
    "          mH:new Uint32Array(num),mM:new Uint32Array(num),\n"
    "          occS:new Uint32Array(257)};\n"
    "      } else {\n"
    "        fpS.bb.fill(0);fpS.conf.fill(0);\n"
    "        fpS.calC.fill(0);fpS.cal1.fill(0);fpS.calP.fill(0);\n"
    "        fpS.bpC.fill(0);fpS.bpS.fill(0);\n"
    "        fpS.mH.fill(0);fpS.mM.fill(0);\n"
    "      }\n"
    "      fpS.occN=0;fpS.occRun=0;fpS.occI=((bitlen/256)|0)+1;\n"
    "      fpS.occ=0;fpS.satL=0;fpS.satS=0;fpS.satB=0;fpS.satM=0;\n"
    "      fpS.nbits=bitlen;\n"
    "    }\n"
    "    fp={buf:new Uint8Array(nb+16)};\n"
    "    fp.buf.set(bytes,8);\n"
    "    var perByte=new Float32Array(nb);\n"
    "    var total=0,sl=new Int32Array(num||1);\n"
    "    for(var bp=0;bp<bitlen;bp++){\n"
    "      var b2=bp-1;\n"
    "      /* bit zero is the stop bit, always 1, priced but unowned */\n"
    ...
    "        var sh=(1-(((e0+255)&(e1+255))>>8))*2+weights[m];\n"
    "        if(wantM){\n"
    "          if(e0|e1){\n"
    "            fpS.mH[m]++;\n"
    "            if(bp>0){\n"
    "              /* the same sequential attribution the C side bakes\n"
    "                 into BAND.q: how far this model moved the mix */\n"
    "              var bc=bit?p1:p0,bt=p0+p1;\n"
    "              p0+=e0<<sh;p1+=e1<<sh;\n"
    "              fpM[(b2>>3)*num+m]+=fpLog2((bit?p1:p0)/(p0+p1))\n"
    "                -fpLog2(bc/bt);\n"
    "            } else {p0+=e0<<sh;p1+=e1<<sh;}\n"
    "          } else {\n"
    "            fpS.mM[m]++;fpS.occRun++;\n"
    "            p0+=e0<<sh;p1+=e1<<sh;\n"
    "          }\n"
    ...
    "        sl[m]=s2;\n"
    "      }\n"
    "      var q=Math.fround(Math.fround(bit?p1:p0)/Math.fround(p0+p1));\n"
    "      var c=-fpLog2(q);\n"
    "      total+=c;\n"
    "      if(bp>0) perByte[b2>>3]+=c;\n"
    "      if(wantM){\n"
    "        var pt2=p0+p1,cf=(bit?p1:p0)/pt2;\n"
    "        var cb2=cf<0.5?0:(((cf-0.5)*20)|0)+1;\n"
    "        if(cb2>10)cb2=10;\n"
    "        fpS.conf[cb2]++;\n"
    "        var p1f=p1/pt2,kb=(p1f*20)|0;\n"
    "        if(kb>19)kb=19;\n"
    "        fpS.calC[kb]++;fpS.cal1[kb]+=bit;fpS.calP[kb]+=p1f;\n"
    "        var bps=bp&7;\n"
    "        fpS.bpC[bps]++;fpS.bpS[bps]+=c;\n"
    "        if(bp>0) fpS.bb[b2]=c;\n"
    "        if((bp+1)%%fpS.occI===0&&fpS.occN<256)\n"
    "          fpS.occS[fpS.occN++]=fpS.occRun;\n"
    "      }\n"
    "      for(var m=0;m<num;m++){\n"
    "        var s2=sl[m];\n"
    "        dt[s2+bit]=dt[s2+bit]+1;\n"
    "        if(dt[s2+1-bit]>1) dt[s2+1-bit]>>=1;\n"
    "      }\n"
    "    }\n"
    "    if(wantM){\n"
    "      if(fpS.occN<257) fpS.occS[fpS.occN++]=fpS.occRun;\n"
    "      if(useJ){\n"
    "        /* occupancy + saturation, exactly: classify each touched\n"
    "           slot once by zeroing it as we go, then restore - the\n"
    "           journal makes the dedup free */\n"
    "        var tl=[];\n"
    "        for(var k2=0;k2<fpJN;k2++){\n"
    "          var j2=fpJ[k2],a2=dt[j2],b3=dt[j2+1];\n"
    "          if(a2|b3){\n"
    "            fpS.occ++;\n"
    "            if(!a2||!b3) fpS.satL++;\n"
    "            else if(a2>b3*4||b3>a2*4) fpS.satS++;\n"
    "            else if(a2<=b3*2&&b3<=a2*2) fpS.satB++;\n"
    "            else fpS.satM++;\n"
    "            tl.push(j2,a2,b3);\n"
    "            dt[j2]=0;dt[j2+1]=0;\n"
    "          }\n"
    "        }\n"
    "        for(var k2=0;k2<tl.length;k2+=3){\n"
    "          dt[tl[k2]]=tl[k2+1];dt[tl[k2]+1]=tl[k2+2];\n"
    "        }\n"
    "      } else fpS.occ=-1; /* no journal, no exact census */\n"
    "    }\n"
    "    if(!useJ) fpJN=-1; /* dirty, full sweep next time */\n"
    "    fp=null;\n"
    "    return {bits:total,perByte:perByte,perModel:wantM?fpM:null,\n"
    "      stats:wantM?fpS:null};\n"
    "  }\n"
    "  window.fpScore=fpScore; /* console: score any Uint8Array */\n"
    "  function fpStart(){\n"
    "    if(typeof PAQ==='undefined'||typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO PAYLOAD ABOARD');return;}\n"
    "    var bytes=new Uint8Array(BD.length);\n"
    "    for(var i=0;i<BD.length;i++) bytes[i]=parseInt(BD[i].h,16);\n"
    ...
    "    }\n"
    "    var ok=mx===0;\n"
    "    /* a report written before c carried nine digits has every\n"
    "       cost on a two-decimal grid - that is a stale artifact,\n"
    "       not a broken engine, and it should say so, not guru */\n"
    "    var stale=!ok;\n"
    "    if(stale) for(var i=0;i<BD.length;i++){\n"
    "      var t2=BD[i].c*100;\n"
    "      if(Math.abs(t2-Math.round(t2))>1e-6){stale=false;break;}\n"
    "    }\n"
    "    egg('forward');\n"
    "    var ex=document.getElementById('fpbox');\n"
    "    if(ex) ex.remove();\n"
    "    function row(a,b){return '<div class=\"cr\"><span>'+a\n"
    "      +'</span><span>'+b+'</span></div>';}\n"
    "    var h2='<div class=\"cbx\"><div class=\"ct\">*** FORWARD PASS ***"
    "</div>'\n"
    "      +row('BYTES SCORED',BD.length)\n"
    "      +row('MODELS',fpH.num)\n"
    ...
    "        :stale?'OLD REPORT. REBUILD AND REGENERATE'\n"
    "        :auSwapped?'NEW LINEUP. THE REPORT PRICES THE OLD ONE'\n"
    "        :'THE FORWARD PASS DISSENTS')\n"
    "      +'<div class=\"cf\">SAME TABLE, SAME MIXER, NO CODER'\n"
    "      +'<br>COMPOSE MODE WILL SEND ITS EDITS HERE</div></div>';\n"
    "    var ov=document.createElement('div');\n"
    "    ov.id='fpbox';ov.innerHTML=h2;\n"
    "    ov.addEventListener('click',function(){ov.remove();});\n"
    "    document.body.appendChild(ov);\n"
    "    dragBox(ov.querySelector('.cbx'),ov.querySelector('.ct'));\n"
    "    if(ok) showToast('RE-PRICED. THE NUMBERS HOLD');\n"
    "    else if(stale)\n"
    "      showToast('TWO-DECIMAL DATA. REGENERATE THE REPORT');\n"
    "    else if(auSwapped)\n"
    "      showToast('DIFFERENT BAND, DIFFERENT BILL');\n"
    "    else showGuru();\n"
    "  }\n"
    "  /* the tuner: edit the file, the model re-bills it live. every\n"
    "     keystroke runs the whole forward pass - real tables, real\n"
    "     mixer - so the price is the truth, ripple and all. make it\n"
    "     cheaper and you have composed a lighter file. it will not\n"
    "     be the same file. that is the joke */\n"
    "  var tuOn=false,tuOv=null,tuCv=null,tuCtx=null;\n"
    "  var tuDCv=null,tuDCtx=null,tuHead=null,tuInfo=null;\n"
    "  var tuBytes=null,tuOrig=null,tuC0=null,tuCN=null;\n"
    "  var tuT0=0,tuTN=0,tuSel=0,tuNib=-1,tuUndo=null;\n"
    "  var tuMs=0,tuN=0,tuLit=false,tuHot=false,tuBlip=false;\n"
    "  var tuM=null,tuNM=0,tuBandMap=null,tuPlay=-1;\n"
    "  var tuBase=null,tuTBase=0,tuSV0=null,tuCB0=null,tuAB=false;\n"
    "  var TUCOLS=64,TUCS=9,TUMAX=16384;\n"
    "  function hx2(v){return (v<16?'0':'')+v.toString(16).toUpperCase();}\n"
    "  function tuVis(){return Math.min(tuBytes.length,TUMAX);}\n"
    "  function tuHeadFmt(){\n"
    "    if(tuAB) return '<span style=\"color:#22d3ee\">'\n"
    "      +'AUDITION: THE ORIGINAL (RELEASE O)</span>';\n"
    "    var d=tuTN-tuT0;\n"
    "    var cl=d>0.0001?'#f87171':d<-0.0001?'#34d399':'var(--fg2)';\n"
    "    return 'ORIG '+(tuT0/8).toFixed(1)+' B&nbsp; NOW '\n"
    "      +(tuTN/8).toFixed(1)+' B&nbsp; <span style=\"color:'+cl+'\">'\n"
    "      +(d>=0?'+':'')+d.toFixed(1)+' BITS</span>&nbsp; '\n"
    "      +tuN+' EDITS&nbsp; '+tuMs.toFixed(0)+' MS';\n"
    "  }\n"
    "  function tuInfoFmt(){\n"
    "    var i=tuSel,d=tuCN[i]-tuC0[i],el=(tuTN-tuT0)-d;\n"
    "    var s='0x'+i.toString(16).toUpperCase()+': '+hx2(tuOrig[i])\n"
    "      +(tuBytes[i]!==tuOrig[i]?' &gt; '+hx2(tuBytes[i]):'')\n"
    "      +' &nbsp;COST '+tuC0[i].toFixed(2)+' &gt; '+tuCN[i].toFixed(2)\n"
    "      +' ('+(d>=0?'+':'')+d.toFixed(2)+')'\n"
    "      +' &nbsp;ELSEWHERE '+(el>=0?'+':'')+el.toFixed(2)+' BITS';\n"
    "    if(tuNib>=0) s+=' &nbsp;TYPING '+tuNib.toString(16).toUpperCase()+'_';\n"
    "    return s;\n"
    "  }\n"
    "  function tuStrip(){\n"
    "    var n=tuVis(),W=tuDCv.width,H=tuDCv.height,mid=H/2,i;\n"
    "    tuDCtx.fillStyle='#04040a';tuDCtx.fillRect(0,0,W,H);\n"
    "    tuDCtx.fillStyle='#3a3a48';tuDCtx.fillRect(0,mid,W,1);\n"
    "    var acc=new Float32Array(W),mx=0.001;\n"
    "    for(i=0;i<n;i++) acc[(i*W/n)|0]+=tuCN[i]-tuC0[i];\n"
    "    for(i=0;i<W;i++){var a=Math.abs(acc[i]);if(a>mx)mx=a;}\n"
    "    for(i=0;i<W;i++){\n"
    "      var d=acc[i];if(!d)continue;\n"
    "      var h2=Math.min(mid,Math.abs(d)/mx*(mid-2)+1);\n"
    "      tuDCtx.fillStyle=d>0?'#f87171':'#34d399';\n"
    "      tuDCtx.fillRect(i,d>0?mid-h2:mid,1,h2);\n"
    "    }\n"
    "  }\n"
    "  function tuDraw(){\n"
    "    var n=tuVis(),i;\n"
    "    var C=(tuAB&&tuCB0)?tuCB0:tuCN;\n"
    "    tuCtx.fillStyle='#04040a';tuCtx.fillRect(0,0,tuCv.width,tuCv.height);\n"
    "    for(i=0;i<n;i++){\n"
    "      var x=(i%%TUCOLS)*TUCS,y=((i/TUCOLS)|0)*TUCS;\n"
    "      tuCtx.globalAlpha=.25+.75*Math.min(1,C[i]/8);\n"
    "      tuCtx.fillStyle=C[i]<3?'#34d399':C[i]<6?'#fbbf24':'#f87171';\n"
    "      tuCtx.fillRect(x,y,TUCS-1,TUCS-1);\n"
    "      tuCtx.globalAlpha=1;\n"
    "      if(tuBytes[i]!==tuOrig[i]){\n"
    "        tuCtx.fillStyle='#fff';tuCtx.fillRect(x,y,2,2);\n"
    "      }\n"
    "    }\n"
    "    if(sonify&&tuPlay>=0&&tuPlay<n){\n"
    "      var px2=(tuPlay%%TUCOLS)*TUCS,py2=((tuPlay/TUCOLS)|0)*TUCS;\n"
    "      tuCtx.strokeStyle='#22d3ee';\n"
    "      tuCtx.strokeRect(px2-.5,py2-.5,TUCS,TUCS);\n"
    "    }\n"
    "    var sx=(tuSel%%TUCOLS)*TUCS,sy=((tuSel/TUCOLS)|0)*TUCS;\n"
    "    tuCtx.strokeStyle='#ffe33f';\n"
    "    tuCtx.strokeRect(sx-.5,sy-.5,TUCS,TUCS);\n"
    "    tuStrip();\n"
    ...
    "  function tuRepaint(){\n"
    "    tuSyncT=null;\n"
    "    /* frequency + bigram tables follow the bytes, then every\n"
    "       registered card rebuilds and wears its TUNED chip */\n"
    "    if(typeof BF!=='undefined'){\n"
    "      for(var v=0;v<256;v++) BF[v]=0;\n"
    "      for(var i=0;i<tuBytes.length;i++) BF[tuBytes[i]]++;\n"
    "    }\n"
    "    if(typeof BG!=='undefined'&&BG.fill){\n"
    "      BG.fill(0);\n"
    "      for(var i=0;i+1<tuBytes.length;i++)\n"
    "        BG[tuBytes[i]*256+tuBytes[i+1]]++;\n"
    "    }\n"
    "    var t=new Uint8Array(tuBytes.length),n2=0;\n"
    "    var tv=new Uint8Array(256);\n"
    "    for(var i=0;i<tuBytes.length;i++)\n"
    "      if(tuBytes[i]!==tuBase[i]){\n"
    "        t[i]=1;n2++;\n"
    "        tv[tuBytes[i]]=1;tv[tuBase[i]]=1;\n"
    "      }\n"
    "    window.TUNED=n2?t:null;\n"
    "    window.TUNEDV=n2?tv:null; /* affected byte VALUES */\n"
    "    if(window.retune) window.retune();\n"
    "    tuBadge(n2);\n"
    "  }\n"
    "  function tuBadge(n2){\n"
    "    var b=document.getElementById('tuned-badge');\n"
    "    if(!n2){if(b)b.remove();return;}\n"
    "    if(!b){\n"
    "      b=document.createElement('div');\n"
    "      b.id='tuned-badge';\n"
    "      b.style.cssText='position:fixed;top:10px;right:12px;'\n"
    "        +'z-index:9996;font-family:var(--pix);font-size:8px;'\n"
    "        +'color:#ff3fd8;background:rgba(0,0,4,.85);'\n"
    "        +'border:1px solid #ff3fd8;padding:4px 8px;'\n"
    "        +'letter-spacing:1px;cursor:pointer;';\n"
    "      b.title='the report shows a tuned file. click for the tuner';\n"
    "      b.addEventListener('click',function(){if(!tuOn)tuStart();});\n"
    "      document.body.appendChild(b);\n"
    "    }\n"
    "    var d=tuTN-tuTBase;\n"
    "    b.textContent='TUNED '+n2+'B '+(d>=0?'+':'')+d.toFixed(1)+' BITS';\n"
    "  }\n"
    "  /* svg glyph overlays: the C side stamps printable chars onto\n"
    "     cmap/attr/bytefreq cells with class markers, so the rebuild\n"
    "     can drop and re-stamp exactly those and nothing else */\n"
    "  var SVGNS='http://www.w3.org/2000/svg';\n"
    "  function tuGlyph(svg,cls,cx,cy,fs,ch,fill){\n"
    "    var t3=document.createElementNS(SVGNS,'text');\n"
    "    t3.setAttribute('class',cls);\n"
    "    t3.setAttribute('x',cx);\n"
    "    t3.setAttribute('y',cy);\n"
    "    t3.setAttribute('text-anchor','middle');\n"
    "    t3.setAttribute('font-size',fs);\n"
    "    t3.setAttribute('fill',fill);\n"
    "    t3.setAttribute('pointer-events','none');\n"
    "    t3.setAttribute('opacity','.8');\n"
    "    t3.textContent=ch;\n"
    "    svg.appendChild(t3);\n"
    "  }\n"
    "  function tuKillGlyphs(svg,cls){\n"
    "    var o=svg.querySelectorAll('text.'+cls);\n"
    "    for(var k=0;k<o.length;k++) o[k].parentNode.removeChild(o[k]);\n"
    "  }\n"
    "  function tuGlyphOk(ch){\n"
    "    return !!ch&&'<>&\\\"'.indexOf(ch)<0;\n"
    "  }\n"
    "  /* the cmap rebuild: recolor through its own slider pipeline,\n"
    "     and outline the cells that have been touched */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-cmap',\n"
    "    fn:function(){\n"
    "      if(typeof BD==='undefined') return;\n"
    "      var rects=document.querySelectorAll('#cmap-svg rect[data-c]');\n"
    "      var t=window.TUNED;\n"
    "      for(var i=0;i<rects.length&&i<BD.length;i++){\n"
    "        rects[i].setAttribute('data-c',BD[i].c.toFixed(2));\n"
    "        if(t&&t[i]){\n"
    "          rects[i].setAttribute('stroke','#ff3fd8');\n"
    "          rects[i].setAttribute('stroke-width','1');\n"
    "        } else rects[i].removeAttribute('stroke');\n"
    "      }\n"
    "      var svg3=document.getElementById('cmap-svg');\n"
    "      if(svg3&&rects.length&&(+rects[0].getAttribute('width'))>=6){\n"
    "        tuKillGlyphs(svg3,'cmg');\n"
    "        var mx3=8;\n"
    "        var sl3=document.getElementById('cmap-slider');\n"
    "        if(sl3) mx3=parseFloat(sl3.value)||8;\n"
    "        for(var g=0;g<rects.length&&g<BD.length;g++){\n"
    "          var d3=BD[g];\n"
    "          if(!tuGlyphOk(d3.ch)) continue;\n"
    "          var w3=+rects[g].getAttribute('width');\n"
    "          tuGlyph(svg3,'cmg',\n"
    "            +rects[g].getAttribute('x')+w3/2,\n"
    "            +rects[g].getAttribute('y')+w3/2+(w3>=10?3:2),\n"
    "            w3>=10?7:w3>=8?6:5,d3.ch,\n"
    "            (d3.c/mx3)<0.6?'#0c0e14':'rgba(255,255,255,.7)');\n"
    "        }\n"
    "      }\n"
    "      var sl2=document.getElementById('cmap-slider');\n"
    "      if(sl2) sl2.dispatchEvent(new Event('input'));\n"
    "    }});\n"
    "  /* byte frequency: same log ramp the C side baked */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-bytefreq',\n"
    "    fn:function(){\n"
    "      if(typeof BF==='undefined') return;\n"
    "      var fmax=0;\n"
    "      for(var v=0;v<256;v++) if(BF[v]>fmax) fmax=BF[v];\n"
    "      var lf=Math.log2(fmax>0?fmax:1);\n"
    "      var rects=document.querySelectorAll('#bfreq-svg rect[data-b]');\n"
    "      var gl=document.querySelectorAll('#bfreq-svg text.bfg');\n"
    "      var gi=0;\n"
    "      for(var k=0;k<rects.length;k++){\n"
    "        var bv=+rects[k].getAttribute('data-b');\n"
    "        var fq=BF[bv],t=0;\n"
    "        if(fq){\n"
    "          t=(Math.log2(fq)+1)/(lf+1);\n"
    "          if(t<0.08)t=0.08; if(t>1)t=1;\n"
    "          rects[k].setAttribute('fill','rgb('+Math.round(13+21*t)+','\n"
    "            +Math.round(40+171*t)+','+Math.round(60+178*t)+')');\n"
    "        } else rects[k].setAttribute('fill','var(--bg3)');\n"
    "        if(window.TUNEDV&&window.TUNEDV[bv]){\n"
    "          rects[k].setAttribute('stroke','#ff3fd8');\n"
    "          rects[k].setAttribute('stroke-width','1');\n"
    "        } else rects[k].removeAttribute('stroke');\n"
    "        /* glyphs are per byte value, so only contrast moves */\n"
    "        if(bv>=32&&bv<=126&&\n"
    "           '<>&\\\"'.indexOf(String.fromCharCode(bv))<0){\n"
    "          if(gl[gi]) gl[gi].setAttribute('fill',\n"
    "            t>0.35?'#0c0e14':'#6b7186');\n"
    "          gi++;\n"
    "        }\n"
    "      }\n"
    "    }});\n"
    "  /* attribution map: recompute dominant/worst from BD.m, same\n"
    "     concave curve, and outline the tuned cells */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-attr',\n"
    "    fn:function(){\n"
    "      if(typeof BD==='undefined'||!BD.length||!BD[0].m) return;\n"
    "      if(typeof ATTR_PAL==='undefined') return;\n"
    "      var rects=document.querySelectorAll('#attr-svg rect[data-i]');\n"
    "      if(!rects.length) return;\n"
    "      var nm=BD[0].m.length,gm=0.001;\n"
    "      for(var i=0;i<BD.length;i++){\n"
    "        var mm=BD[i].m;\n"
    "        for(var m=0;m<nm;m++) if(mm[m]>gm) gm=mm[m];\n"
    "      }\n"
    "      var t2=window.TUNED;\n"
    "      for(var k=0;k<rects.length;k++){\n"
    "        var i=+rects[k].getAttribute('data-i');\n"
    "        if(!BD[i]) continue;\n"
    "        var mm=BD[i].m,bm=-1,wm=-1,bv=0,wv=0;\n"
    "        for(var m=0;m<nm;m++){\n"
    "          var v=mm[m];\n"
    "          if(v>bv){bv=v;bm=m;}\n"
    "          if(v<wv){wv=v;wm=m;}\n"
    "        }\n"
    "        rects[k].setAttribute('data-bm',bm);\n"
    "        rects[k].setAttribute('data-wm',wm);\n"
    "        if(bm>=0&&bv>0.001){\n"
    "          var t=bv/gm;if(t>1)t=1;t=1-(1-t)*(1-t);\n"
    "          var pc=ATTR_PAL[bm%%ATTR_PAL.length];\n"
    "          rects[k].setAttribute('fill',\n"
    "            'rgb('+pc[0]+','+pc[1]+','+pc[2]+')');\n"
    "          rects[k].setAttribute('fill-opacity',(0.15+0.85*t).toFixed(3));\n"
    "        } else {\n"
    "          rects[k].setAttribute('fill','rgb(128,128,128)');\n"
    "          rects[k].setAttribute('fill-opacity','0.10');\n"
    "        }\n"
    "        if(t2&&t2[i]){\n"
    "          rects[k].setAttribute('stroke','#ff3fd8');\n"
    "          rects[k].setAttribute('stroke-width','1');\n"
    "        } else rects[k].removeAttribute('stroke');\n"
    "      }\n"
    "      var svg4=document.getElementById('attr-svg');\n"
    "      if(svg4&&rects.length&&(+rects[0].getAttribute('width'))>=6){\n"
    "        tuKillGlyphs(svg4,'atg');\n"
    "        for(var g=0;g<rects.length&&g<BD.length;g++){\n"
    "          var d4=BD[g];\n"
    "          if(!tuGlyphOk(d4.ch)) continue;\n"
    "          var w4=+rects[g].getAttribute('width');\n"
    "          var mt=/rgb\\((\\d+),(\\d+),(\\d+)/.exec(\n"
    "            rects[g].getAttribute('fill'));\n"
    "          var lum=mt?(+mt[1]*299+ +mt[2]*587+ +mt[3]*114)/1000:128;\n"
    "          tuGlyph(svg4,'atg',\n"
    "            +rects[g].getAttribute('x')+w4/2,\n"
    "            +rects[g].getAttribute('y')+w4/2+(w4>=10?3:2),\n"
    "            w4>=10?7:w4>=8?6:5,d4.ch,\n"
    "            lum>120?'#0c0e14':'rgba(255,255,255,.7)');\n"
    "        }\n"
    "      }\n"
    "    }});\n"
    "  /* model contribution donut: rebuilt whole from BD.m sums */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-mctrib',\n"
    "    fn:function(){\n"
    "      if(typeof BD==='undefined'||typeof MI==='undefined') return;\n"
    "      if(!BD.length||!BD[0].m||typeof ATTR_PAL==='undefined') return;\n"
    "      var wrap=document.getElementById('mctrib-wrap');\n"
    "      if(!wrap) return;\n"
    "      var nm=MI.length,sv=new Float64Array(nm);\n"
    "      for(var i=0;i<BD.length;i++){\n"
    "        var mm=BD[i].m;\n"
    "        for(var m=0;m<nm&&m<mm.length;m++) sv[m]+=mm[m];\n"
    "      }\n"
    "      var ord=[];\n"
    "      for(var m=0;m<nm;m++) ord.push(m);\n"
    "      ord.sort(function(a,b){return sv[b]-sv[a];});\n"
    "      var tot=0;\n"
    "      for(var m=0;m<nm;m++) if(sv[m]>0) tot+=sv[m];\n"
    "      if(tot<1) tot=1;\n"
    "      /* which models' takes actually moved since the encode */\n"
    "      var chg=null;\n"
    "      if(tuSV0){\n"
    "        chg=[];\n"
    "        for(var m5=0;m5<nm;m5++)\n"
    "          chg.push(Math.abs(sv[m5]-tuSV0[m5])>0.5);\n"
    "      }\n"
    "      var C2=2*Math.PI*52,cum=0;\n"
    "      function pal(m){return ATTR_PAL[m%%ATTR_PAL.length];}\n"
    "      var h='<svg width=\"140\" height=\"140\" viewBox=\"0 0 140 140\"'\n"
    "        +' style=\"flex-shrink:0\">'\n"
    "        +'<circle cx=\"70\" cy=\"70\" r=\"52\" fill=\"none\"'\n"
    "        +' stroke=\"var(--bg3)\" stroke-width=\"18\"/>'\n"
    "        +'<g transform=\"rotate(-90 70 70)\">';\n"
    "      for(var k=0;k<nm;k++){\n"
    "        var m=ord[k];\n"
    "        if(sv[m]<=0) continue;\n"
    "        var arc=C2*sv[m]/tot,pc=pal(m);\n"
    "        h+='<circle cx=\"70\" cy=\"70\" r=\"52\" fill=\"none\" stroke=\"rgb('\n"
    "          +pc[0]+','+pc[1]+','+pc[2]+')\" stroke-width=\"18\"'\n"
    "          +' stroke-dasharray=\"'+arc.toFixed(2)+' '+(C2-arc).toFixed(2)\n"
    "          +'\" stroke-dashoffset=\"'+(-cum).toFixed(2)+'\"/>';\n"
    "        cum+=arc;\n"
    "      }\n"
    "      h+='</g><text x=\"70\" y=\"74\" text-anchor=\"middle\" font-size=\"22\"'\n"
    "        +' font-weight=\"600\" fill=\"var(--fg)\"'\n"
    "        +' font-family=\"var(--sans)\">'+(tot/8).toFixed(0)+'</text>'\n"
    "        +'<text x=\"70\" y=\"90\" text-anchor=\"middle\" font-size=\"8\"'\n"
    "        +' fill=\"var(--fg3)\" letter-spacing=\"1.2px\">BYTES SAVED</text>'\n"
    "        +'</svg>';\n"
    "      function lrow(m){\n"
    "        var pc=pal(m);\n"
    "        return '<div style=\"display:flex;align-items:center;gap:10px\">'\n"
    "          +'<span style=\"display:inline-block;width:10px;height:10px;'\n"
    "          +'border-radius:2px;background:rgb('+pc[0]+','+pc[1]+','+pc[2]\n"
    "          +');flex-shrink:0'+(chg&&chg[m]\n"
    "            ?';outline:1px solid #ff3fd8;outline-offset:1px':'')\n"
    "          +'\"></span>'\n"
    "          +'<span style=\"flex:1;color:var(--fg2);font-family:var(--mono);'\n"
    "          +'font-size:11px\">'+MI[m].mask+':'+MI[m].w+'</span>'\n"
    "          +'<span style=\"font-family:var(--mono);color:'\n"
    "          +(sv[m]<=0?'var(--fg3)':'var(--fg)')+';font-weight:500;'\n"
    "          +'font-size:11px\">'+(sv[m]/8).toFixed(0)+' B</span>'\n"
    "          +'<span style=\"font-family:var(--mono);color:var(--fg3);'\n"
    "          +'min-width:42px;text-align:right;font-size:11px\">'\n"
    "          +(100*sv[m]/tot).toFixed(1)+'%%</span></div>';\n"
    "      }\n"
    "      var lg='<div style=\"flex:1;display:flex;flex-direction:column;'\n"
    "        +'gap:6px;font-size:12px;min-width:0\">';\n"
    "      var show=Math.min(nm,6);\n"
    "      for(var k=0;k<show;k++) lg+=lrow(ord[k]);\n"
    "      if(nm>show){\n"
    "        var rb=0;\n"
    "        for(var k=show;k<nm;k++) if(sv[ord[k]]>0) rb+=sv[ord[k]];\n"
    "        lg+='<details><summary class=\"mc-more\" style=\"display:flex;'\n"
    "          +'align-items:center;gap:10px;color:var(--fg3);font-size:11px\">'\n"
    "          +'<span class=\"mc-caret\" style=\"display:inline-block;width:10px;'\n"
    "          +'flex-shrink:0;text-align:center;font-size:9px\">\\u25B8</span>'\n"
    "          +'<span style=\"flex:1\">+'+(nm-show)+' more</span>'\n"
    "          +'<span style=\"font-family:var(--mono)\">'+(rb/8).toFixed(0)\n"
    "          +' B</span>'\n"
    "          +'<span style=\"font-family:var(--mono);min-width:42px;'\n"
    "          +'text-align:right\">'+(100*rb/tot).toFixed(1)+'%%</span>'\n"
    "          +'</summary>'\n"
    "          +'<div style=\"display:flex;flex-direction:column;gap:6px;'\n"
    "          +'margin-top:6px\">';\n"
    "        for(var k=show;k<nm;k++) lg+=lrow(ord[k]);\n"
    "        lg+='</div></details>';\n"
    "      }\n"
    "      lg+='</div>';\n"
    "      wrap.innerHTML=h+lg;\n"
    "    }});\n"
    "  /* coder-stat cards: rebuilt from the forward pass's own\n"
    "     bookkeeping (window.FPSTATS), same buckets as the C pass */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-conf',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS; if(!S) return;\n"
    "      var tb=document.querySelector('#sec-conf table'); if(!tb) return;\n"
    "      var L=['&lt;50%%','50-55%%','55-60%%','60-65%%','65-70%%',\n"
    "        '70-75%%','75-80%%','80-85%%','85-90%%','90-95%%','95-100%%'];\n"
    "      var mx=0,tot=0,i;\n"
    "      for(i=0;i<11;i++){if(S.conf[i]>mx)mx=S.conf[i];tot+=S.conf[i];}\n"
    "      if(!tot||!mx) return;\n"
    "      var h='<tr><th>Range</th><th class=\"r\">Bits</th>'\n"
    "        +'<th class=\"r\">%%</th><th>Distribution</th></tr>';\n"
    "      for(i=0;i<11;i++){\n"
    "        if(!S.conf[i]) continue;\n"
    "        var bc=i>=8?'bar-grn':i>=5?'bar-acc':i>=2?'bar-ylw'\n"
    "          :i>=1?'bar-orn':'bar-red';\n"
    "        h+='<tr><td class=\"n\">'+L[i]+'</td>'\n"
    "          +'<td class=\"r\">'+S.conf[i]+'</td>'\n"
    "          +'<td class=\"r\">'+(100*S.conf[i]/tot).toFixed(1)+'</td>'\n"
    "          +'<td class=\"bar-cell\"><div class=\"bar '+bc\n"
    "          +'\" style=\"width:'+(100*S.conf[i]/mx).toFixed(0)\n"
    "          +'%%;opacity:.25\"></div>'\n"
    "          +'<span class=\"bar-label\">&nbsp;</span></td></tr>';\n"
    "      }\n"
    "      tb.innerHTML=h;\n"
    "    }});\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-bytepos',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS; if(!S) return;\n"
    "      var tb=document.querySelector('#sec-bytepos table');\n"
    "      if(!tb) return;\n"
    "      var mx=0,i,avg;\n"
    "      for(i=0;i<8;i++) if(S.bpC[i]){\n"
    "        avg=S.bpS[i]/S.bpC[i]; if(avg>mx) mx=avg;\n"
    "      }\n"
    "      if(mx<=0) return;\n"
    "      var h='<tr><th>Bit</th><th class=\"r\">Count</th>'\n"
    "        +'<th class=\"r\">Avg Cost</th><th>Cost</th></tr>';\n"
    "      for(i=0;i<8;i++){\n"
    "        if(!S.bpC[i]) continue;\n"
    "        avg=S.bpS[i]/S.bpC[i];\n"
    "        var cls=avg<mx*0.4?'c-grn':avg<mx*0.6?'c-blu'\n"
    "          :avg<mx*0.8?'c-ylw':'c-orn';\n"
    "        var bc=avg<mx*0.4?'bar-grn':avg<mx*0.6?'bar-acc'\n"
    "          :avg<mx*0.8?'bar-ylw':'bar-orn';\n"
    "        h+='<tr><td class=\"n\">Bit '+i+'</td>'\n"
    "          +'<td class=\"r\">'+S.bpC[i]+'</td>'\n"
    "          +'<td class=\"r '+cls+'\">'+avg.toFixed(3)+'</td>'\n"
    "          +'<td class=\"bar-cell\"><div class=\"bar '+bc\n"
    "          +'\" style=\"width:'+(100*avg/mx).toFixed(0)\n"
    "          +'%%;opacity:.2\"></div>'\n"
    "          +'<span class=\"bar-label\">&nbsp;</span></td></tr>';\n"
    "      }\n"
    "      tb.innerHTML=h;\n"
    "    }});\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-hash',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS;\n"
    "      if(!S||S.occ<0||typeof PAQ==='undefined') return;\n"
    "      var tb=document.querySelector('#sec-hash table'); if(!tb) return;\n"
    "      var hs=Math.pow(2,PAQ.db);\n"
    "      var ld=100*S.occ/hs;\n"
    "      var ts=hs*2,un='B';\n"
    "      if(ts>=1024){ts/=1024;un='KiB';}\n"
    "      if(ts>=1024){ts/=1024;un='MiB';}\n"
    "      if(ts>=1024){ts/=1024;un='GiB';}\n"
    "      tb.innerHTML='<tr><td>Table size</td><td>'+hs+' slots ('\n"
    "        +ts.toFixed(1)+' '+un+', 2 B/slot)</td></tr>'\n"
    "        +'<tr><td>Occupied</td><td class=\"'\n"
    "        +(ld<50?'c-grn':ld<75?'c-ylw':'c-orn')+'\">'+S.occ\n"
    "        +' ('+ld.toFixed(1)+'%%)</td></tr>';\n"
    "    }});\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-sat',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS;\n"
    "      if(!S||S.occ<0) return;\n"
    "      var tb=document.querySelector('#sec-sat table'); if(!tb) return;\n"
    "      var cats=[['Lopsided (one=0)',S.satL,'bar-grn'],\n"
    "        ['Strong (&gt;4:1)',S.satS,'bar-acc'],\n"
    "        ['Balanced (&lt;2:1)',S.satB,'bar-ylw'],\n"
    "        ['Mixed (2:1\\u20134:1)',S.satM,'bar-orn']];\n"
    "      var tot=S.satL+S.satS+S.satB+S.satM,mx=0,i;\n"
    "      if(!tot) return;\n"
    "      for(i=0;i<4;i++) if(cats[i][1]>mx) mx=cats[i][1];\n"
    "      if(!mx) return;\n"
    "      var h='<tr><th>Category</th><th class=\"r\">Count</th>'\n"
    "        +'<th class=\"r\">%%</th><th>Distribution</th></tr>';\n"
    "      for(i=0;i<4;i++){\n"
    "        h+='<tr><td class=\"n\">'+cats[i][0]+'</td>'\n"
    "          +'<td class=\"r\">'+cats[i][1]+'</td>'\n"
    "          +'<td class=\"r\">'+(100*cats[i][1]/tot).toFixed(1)+'</td>'\n"
    "          +'<td class=\"bar-cell\"><div class=\"bar '+cats[i][2]\n"
    "          +'\" style=\"width:'+(100*cats[i][1]/mx).toFixed(0)\n"
    "          +'%%;opacity:.25\"></div>'\n"
    "          +'<span class=\"bar-label\">&nbsp;</span></td></tr>';\n"
    "      }\n"
    "      tb.innerHTML=h;\n"
    "    }});\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-calib',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS; if(!S) return;\n"
    "      var svg=document.getElementById('calib-svg'); if(!svg) return;\n"
    "      var tot=0,i;\n"
    "      for(i=0;i<20;i++) tot+=S.calC[i];\n"
    "      if(!tot) return;\n"
    "      var h='<rect x=\"38\" y=\"10\" width=\"272\" height=\"276\"'\n"
    "        +' fill=\"var(--bg3)\" rx=\"4\"/>';\n"
    "      for(i=0;i<=4;i++){\n"
    "        var v=i/4,x=38+((v*272)|0),y=10+276-((v*276)|0);\n"
    "        h+='<line x1=\"'+x+'\" y1=\"10\" x2=\"'+x+'\" y2=\"286\"'\n"
    "          +' stroke=\"var(--bdr)\" stroke-width=\"0.5\"/>'\n"
    "          +'<line x1=\"38\" y1=\"'+y+'\" x2=\"310\" y2=\"'+y+'\"'\n"
    "          +' stroke=\"var(--bdr)\" stroke-width=\"0.5\"/>'\n"
    "          +'<text x=\"'+x+'\" y=\"299\" text-anchor=\"middle\"'\n"
    "          +' font-size=\"9\" fill=\"var(--fg3)\">'+v.toFixed(2)+'</text>'\n"
    "          +'<text x=\"34\" y=\"'+(y+3)+'\" text-anchor=\"end\"'\n"
    "          +' font-size=\"9\" fill=\"var(--fg3)\">'+v.toFixed(2)+'</text>';\n"
    "      }\n"
    "      h+='<line x1=\"38\" y1=\"286\" x2=\"310\" y2=\"10\"'\n"
    "        +' stroke=\"var(--fg3)\" stroke-width=\"1\"'\n"
    "        +' stroke-dasharray=\"5,3\" opacity=\".6\"/>';\n"
    "      var ece=0;\n"
    "      window.CAL=[];\n"
    "      for(i=0;i<20;i++){\n"
    "        if(!S.calC[i]){window.CAL.push(0);continue;}\n"
    "        var pr2=S.calP[i]/S.calC[i],ob=S.cal1[i]/S.calC[i];\n"
    "        var dv=Math.abs(pr2-ob);\n"
    "        ece+=S.calC[i]/tot*dv;\n"
    "        window.CAL.push([pr2,ob,S.calC[i]]);\n"
    "        var r3=2+Math.log2(S.calC[i]+1)*0.5; if(r3>7)r3=7;\n"
    "        h+='<circle cx=\"'+(38+pr2*272).toFixed(1)+'\" cy=\"'\n"
    "          +(10+276-ob*276).toFixed(1)+'\" r=\"'+r3.toFixed(1)\n"
    "          +'\" fill=\"'+(dv<0.02?'#34d399':dv<0.06?'#fbbf24':'#f87171')\n"
    "          +'\" fill-opacity=\".8\" data-cb=\"'+i\n"
    "          +'\" style=\"cursor:pointer\"/>';\n"
    "      }\n"
    "      h+='<text x=\"174\" y=\"316\" text-anchor=\"middle\" font-size=\"10\"'\n"
    "        +' fill=\"var(--fg3)\">predicted P(1)</text>'\n"
    "        +'<text x=\"12\" y=\"148\" text-anchor=\"middle\" font-size=\"10\"'\n"
    "        +' fill=\"var(--fg3)\" transform=\"rotate(-90 12 148)\">'\n"
    "        +'observed P(1)</text>';\n"
    "      svg.innerHTML=h;\n"
    "      var dsc=document.querySelector('#sec-calib .desc');\n"
    "      if(dsc) dsc.innerHTML='Predicted P(bit=1) vs observed rate,'\n"
    "        +' 5%% buckets over '+tot+' bits. Points on the diagonal ='\n"
    "        +' honest probabilities. ECE = '+ece.toFixed(4)\n"
    "        +' (lower is better).';\n"
    "    }});\n"
    "  /* per-model table: cells updated in place so sorting, hover\n"
    "     sparklines and the attribution cross-links keep working */\n"
    "  if(window.retuneHooks) window.retuneHooks.push({id:'sec-models',\n"
    "    fn:function(){\n"
    "      var S=window.FPSTATS; if(!S) return;\n"
    "      if(typeof BD==='undefined'||!BD.length||!BD[0].m) return;\n"
    "      var rows=document.querySelectorAll('#sec-models tr.pm-row');\n"
    "      if(!rows.length) return;\n"
    "      var nm=BD[0].m.length,sv=new Float64Array(nm);\n"
    "      for(var i=0;i<BD.length;i++){\n"
    "        var mm=BD[i].m;\n"
    "        for(var m=0;m<nm;m++) sv[m]+=mm[m];\n"
    "      }\n"
    "      var mxs=0.0001,totS=0,m;\n"
    "      for(m=0;m<nm;m++){\n"
    "        var ab=Math.abs(sv[m]);\n"
    "        if(ab>mxs) mxs=ab;\n"
    "        totS+=sv[m];\n"
    "      }\n"
    "      for(var k=0;k<rows.length;k++){\n"
    "        var r2=rows[k];\n"
    "        m=+r2.getAttribute('data-bm');\n"
    "        var td=r2.children;\n"
    "        var hits=S.mH[m]||0,miss=S.mM[m]||0,tot2=hits+miss;\n"
    "        var pct=tot2?100*hits/tot2:0;\n"
    "        td[3].setAttribute('data-v',hits);\n"
    "        td[3].textContent=hits;\n"
    "        td[4].setAttribute('data-v',pct.toFixed(4));\n"
    "        td[4].textContent=pct.toFixed(1);\n"
    "        td[5].setAttribute('data-v',miss);\n"
    "        td[5].textContent=miss;\n"
    "        var bits=sv[m];\n"
    "        td[6].setAttribute('data-v',bits.toFixed(4));\n"
    "        var bar=td[6].querySelector('.bar');\n"
    "        var lab=td[6].querySelector('.bar-label');\n"
    "        if(bar){\n"
    "          bar.style.width=(100*Math.abs(bits)/mxs).toFixed(0)+'%%';\n"
    "          bar.className='bar '+(bits>0?'bar-grn':'bar-red');\n"
    "        }\n"
    "        if(lab){\n"
    "          lab.textContent=bits.toFixed(1);\n"
    "          lab.className='bar-label '+(bits>mxs*0.3?'c-grn'\n"
    "            :bits>mxs*0.1?'c-acc':bits>0?'c-ylw':'c-red');\n"
    "        }\n"
    "        td[7].setAttribute('data-v',(bits/8).toFixed(4));\n"
    "        td[7].textContent=(bits/8).toFixed(1);\n"
    "        r2.style.boxShadow=(tuSV0&&Math.abs(bits-tuSV0[m])>0.5)\n"
    "          ?'inset 2px 0 0 #ff3fd8':'';\n"
    "      }\n"
    "      var trs=document.querySelectorAll('#sec-models table tr');\n"
    "      var last=trs[trs.length-1];\n"
    "      if(last&&!last.classList.contains('pm-row')&&last.children[1]){\n"
    "        last.children[1].textContent=totS.toFixed(1);\n"
    "        last.children[2].textContent=(totS/8).toFixed(1);\n"
    "      }\n"
    "      if(window.pmSortBy&&window.reportState)\n"
    "        window.pmSortBy(window.reportState.sortCol,\n"
    "          window.reportState.sortDir);\n"
    "    }});\n"
    "  /* the forward pass, off the main thread: the worker is built\n"
    "     from this function's own source (toString -> blob url, no\n"
    "     external file), keeps its own table + journal, and scores\n"
    "     while the page - and the music - never stall. it must stay\n"
    "     arithmetically identical to fpScore: same crc, same fround\n"
    "     discipline, same counter updates */\n"
    "  function fpWorkerMain(){\n"
    "    var H=null,dt=null,J=null,JN=0,CT=null;\n"
    "    var lb=new Uint32Array(1),lf=new Float32Array(lb.buffer);\n"
    "    var C0=Math.fround(1.42286530448213);\n"
    "    var C1=Math.fround(-0.58208536795165);\n"
    "    var C2=Math.fround(0.15922006346951);\n"
    "    function crcInit(){\n"
    ...
    "      }\n"
    "    }\n"
    "    function crc(h,b){return (CT[(h^b)&255]^(h>>>8))>>>0;}\n"
    "    function lg2(x){\n"
    "      lf[0]=x;\n"
    "      var u=lb[0],e=(u>>>23)-127;\n"
    "      lb[0]=(u&0x7FFFFF)|0x3F800000;\n"
    "      var m=Math.fround(lf[0]-1);\n"
    "      return Math.fround(e+Math.fround(m*Math.fround(C0\n"
    "        +Math.fround(m*Math.fround(C1+Math.fround(m*C2))))));\n"
    "    }\n"
    "    function hash0(mask){\n"
    "      var h=crc(mask>>>0,0),cm=mask&255;\n"
    "      while(cm){if(cm&128)h=crc(h,0);cm=(cm<<1)&255;}\n"
    "      return h;\n"
    "    }\n"
    "    function hashB(buf,bp2,mask){\n"
    "      var cm=mask&255,p=8+(bp2>>3),h=mask>>>0;\n"
    "      h=crc(h,(0x100|buf[p])>>((~bp2&7)+1));\n"
    "      while(cm){p--;if(cm&128)h=crc(h,buf[p]);cm=(cm<<1)&255;}\n"
    "      return h;\n"
    "    }\n"
    ...
    "      if(d.init){\n"
    "        H=d;crcInit();\n"
    "        dt=new Uint8Array(2*(1<<H.db));J=null;JN=0;\n"
    "        return;\n"
    "      }\n"
    "      var bytes=new Uint8Array(d.b);\n"
    "      var num=H.num,weights=H.weights,ext=H.ext,db=H.db,bpr=H.bp;\n"
    "      if(JN>0){for(var i=0;i<JN;i++){var j=J[i];dt[j]=0;dt[j+1]=0;}}\n"
    "      else if(JN<0) dt.fill(0);\n"
    "      JN=0;\n"
    "      var nb=bytes.length,bitlen=nb*8+1;\n"
    "      var jn=bitlen*num,useJ=jn<=8388608;\n"
    "      if(useJ&&(!J||J.length<jn)) J=new Uint32Array(jn);\n"
    "      var buf=new Uint8Array(nb+16);buf.set(bytes,8);\n"
    "      var perByte=new Float32Array(nb);\n"
    "      var pm=new Float32Array(nb*num);\n"
    "      var bb=new Float32Array(nb*8);\n"
    "      var conf=new Uint32Array(11);\n"
    "      var calC=new Uint32Array(20),cal1=new Uint32Array(20);\n"
    "      var calP=new Float64Array(20);\n"
    "      var bpC=new Uint32Array(8),bpS=new Float64Array(8);\n"
    "      var mH=new Uint32Array(num),mM=new Uint32Array(num);\n"
    "      var occS=new Uint32Array(257),occN=0,occRun=0;\n"
    "      var occI=((bitlen/256)|0)+1;\n"
    "      var sl=new Int32Array(num||1),total=0;\n"
    "      for(var bp=0;bp<bitlen;bp++){\n"
    "        var b2=bp-1;\n"
    "        var bit=(bp===0)?1:(buf[8+(b2>>3)]>>(7-(b2&7)))&1;\n"
    "        var p0=bpr,p1=bpr;\n"
    "        for(var m=0;m<num;m++){\n"
    "          var h=(bp===0)?hash0(ext[m]):hashB(buf,b2,ext[m]);\n"
    "          var s2=(h>>>(32-db))*2;\n"
    "          if(useJ) J[JN++]=s2;\n"
    "          var e0=dt[s2],e1=dt[s2+1];\n"
    "          var sh=(1-(((e0+255)&(e1+255))>>8))*2+weights[m];\n"
    ...
    "              var bc=bit?p1:p0,bt=p0+p1;\n"
    "              p0+=e0<<sh;p1+=e1<<sh;\n"
    "              pm[(b2>>3)*num+m]+=lg2((bit?p1:p0)/(p0+p1))\n"
    "                -lg2(bc/bt);\n"
    "            } else {p0+=e0<<sh;p1+=e1<<sh;}\n"
    "          } else {\n"
    "            mM[m]++;occRun++;\n"
    ...
    "          sl[m]=s2;\n"
    "        }\n"
    "        var q=Math.fround(Math.fround(bit?p1:p0)\n"
    "          /Math.fround(p0+p1));\n"
    "        var c=-lg2(q);\n"
    "        total+=c;\n"
    "        if(bp>0) perByte[b2>>3]+=c;\n"
    ...
    "        var p1f=p1/pt2,kb=(p1f*20)|0;\n"
    "        if(kb>19)kb=19;\n"
    "        calC[kb]++;cal1[kb]+=bit;calP[kb]+=p1f;\n"
    "        var bps=bp&7;\n"
    "        bpC[bps]++;bpS[bps]+=c;\n"
    "        if(bp>0) bb[b2]=c;\n"
    "        if((bp+1)%%occI===0&&occN<256) occS[occN++]=occRun;\n"
    "        for(var m=0;m<num;m++){\n"
    "          var s2=sl[m];\n"
    "          dt[s2+bit]=dt[s2+bit]+1;\n"
    "          if(dt[s2+1-bit]>1) dt[s2+1-bit]>>=1;\n"
    "        }\n"
    "      }\n"
    "      if(occN<257) occS[occN++]=occRun;\n"
    "      var occ=-1,satL=0,satS=0,satB=0,satM=0;\n"
    "      if(useJ){\n"
    "        occ=0;var tl=[];\n"
    "        for(var k2=0;k2<JN;k2++){\n"
    "          var j2=J[k2],a2=dt[j2],b3=dt[j2+1];\n"
    "          if(a2|b3){\n"
    "            occ++;\n"
    "            if(!a2||!b3) satL++;\n"
    "            else if(a2>b3*4||b3>a2*4) satS++;\n"
    "            else if(a2<=b3*2&&b3<=a2*2) satB++;\n"
    "            else satM++;\n"
    "            tl.push(j2,a2,b3);\n"
    "            dt[j2]=0;dt[j2+1]=0;\n"
    "          }\n"
    ...
    "      }\n"
    "      if(!useJ) JN=-1;\n"
    "      postMessage({bits:total,perByte:perByte,perModel:pm,bb:bb,\n"
    "        conf:conf,calC:calC,cal1:cal1,calP:calP,bpC:bpC,bpS:bpS,\n"
    "        mH:mH,mM:mM,occS:occS,occN:occN,occI:occI,occ:occ,\n"
    "        satL:satL,satS:satS,satB:satB,satM:satM,nbits:bitlen},\n"
    "        [perByte.buffer,pm.buffer,bb.buffer]);\n"
    "    };\n"
    "  }\n"
    "  var tuWk=null,tuWkOk=false,tuBusy=false,tuNextB=null,tuSt0=0;\n"
    "  function tuWkInit(){\n"
    "    if(tuWk||tuWkOk||!window.Worker||!fpHdr()) return;\n"
    "    if(typeof PAQ==='undefined') return;\n"
    "    try{\n"
    "      var src='('+fpWorkerMain.toString()+')()';\n"
    "      var url=URL.createObjectURL(\n"
    "        new Blob([src],{type:'text/javascript'}));\n"
    "      tuWk=new Worker(url);\n"
    "      tuWk.postMessage({init:1,db:PAQ.db,bp:PAQ.bp,num:fpH.num,\n"
    "        weights:fpH.weights,ext:fpH.ext});\n"
    "      tuWk.onmessage=function(ev){tuWkApply(ev.data);};\n"
    "      tuWk.onerror=function(){tuWkOk=false;tuWk=null;};\n"
    "      tuWkOk=true;\n"
    "    }catch(e){tuWk=null;tuWkOk=false;}\n"
    "  }\n"
    "  function tuWkApply(r){\n"
    "    tuMs=performance.now()-tuSt0;\n"
    "    tuCN=r.perByte;tuTN=r.bits;tuM=r.perModel;tuNM=fpH.num;\n"
    "    window.FPSTATS=r;\n"
    "    tuAfterScore();\n"
    "    tuBusy=false;\n"
    "    if(tuNextB){\n"
    "      var nx=tuNextB;tuNextB=null;\n"
    "      tuBusy=true;tuSt0=performance.now();\n"
    "      tuWk.postMessage({b:nx.buffer},[nx.buffer]);\n"
    "    }\n"
    "  }\n"
    "  function tuScore(){\n"
    "    tuWkInit();\n"
    "    if(tuWkOk){\n"
    "      var snap=new Uint8Array(tuBytes);\n"
    "      if(tuBusy){tuNextB=snap;return;} /* coalesce rapid edits */\n"
    "      tuBusy=true;tuSt0=performance.now();\n"
    "      tuWk.postMessage({b:snap.buffer},[snap.buffer]);\n"
    "      return;\n"
    "    }\n"
    "    var t0=performance.now();\n"
    "    var r=fpScore(tuBytes,true);\n"
    "    tuMs=performance.now()-t0;\n"
    "    tuCN=r.perByte;tuTN=r.bits;tuM=r.perModel;\n"
    "    window.FPSTATS=r.stats;\n"
    "    tuAfterScore();\n"
    "  }\n"
    "  function tuAfterScore(){\n"
    "    tuN=0;\n"
    "    for(var i=0;i<tuBytes.length;i++)\n"
    "      if(tuBytes[i]!==tuOrig[i]) tuN++;\n"
    "    if(tuTN<tuT0&&!tuLit){\n"
    ...
    "    if(tuBlip){\n"
    "      tuBlip=false;\n"
    "      blip(mf(noteFor(mstep,tuCN[tuSel])),.05,.12,panOf(tuSel));\n"
    "    }\n"
    "    tuSync();\n"
    "    tuDraw();\n"
    "  }\n"
    "  function tuSet(v){\n"
    "    tuUndo.push({i:tuSel,v:tuBytes[tuSel]});\n"
    "    tuBytes[tuSel]=v&255;\n"
    "    /* hear the edit when its real price arrives - in key if a\n"
    "       key is pinned */\n"
    "    tuBlip=true;\n"
    "    tuScore();\n"
    "  }\n"
    ...
    "    if(!tuOn) return;\n"
    "    var k=e.key;\n"
    "    if(k==='Escape'){tuStop();e.preventDefault();return;}\n"
    "    /* the panel owns the keyboard only while the pointer is on\n"
    "       it - everywhere else, letters still spell cheat words\n"
    "       (cmajor, band, ...) instead of typing hex into the file */\n"
    "    if(!tuHot) return;\n"
    "    if(k==='m'||k==='M') return; /* mute stays global */\n"
    "    e.stopPropagation();\n"
    "    var n=tuVis(),moved=false;\n"
    "    if(k==='ArrowLeft'&&tuSel>0){tuSel--;moved=true;}\n"
    "    else if(k==='ArrowRight'&&tuSel<n-1){tuSel++;moved=true;}\n"
    "    else if(k==='ArrowUp'&&tuSel>=TUCOLS){tuSel-=TUCOLS;moved=true;}\n"
    "    else if(k==='ArrowDown'&&tuSel+TUCOLS<n){tuSel+=TUCOLS;moved=true;}\n"
    "    if(moved){tuNib=-1;tuDraw();e.preventDefault();return;}\n"
    "    if(k===' '){\n"
    "      sonify=!sonify;\n"
    "      if(sonify) sonPos=tuSel;\n"
    "      else tuPlay=-1;\n"
    "      sonFlush(); /* start or stop lands this instant */\n"
    "      sonClear();trkShow(sonify);\n"
    "      var hr=document.querySelector('.hero-ring');\n"
    "      if(hr) hr.classList.toggle('vinyl',sonify);\n"
    "      tuDraw();\n"
    "      showToast(sonify?'THE TUNED FILE, PLAYING':'NEEDLE UP');\n"
    "      e.preventDefault();return;}\n"
    "    if(k==='p'){\n"
    "      sonPos=tuSel;\n"
    "      sonFlush();\n"
    "      showToast(sonify?'FROM HERE':'CUED. SPACE PLAYS');\n"
    "      e.preventDefault();return;}\n"
    "    if(k==='o'||k==='O'){\n"
    "      if(!tuAB){\n"
    "        tuAB=true;\n"
    "        /* replay the same passage: cue a short lead-in before\n"
    "           the selection so the edit lands on a beat */\n"
    "        sonPos=Math.max(0,tuSel-8);\n"
    "        sonFlush(); /* kill the queued future: the flip is NOW */\n"
    "        tuDraw();\n"
    "      }\n"
    "      e.preventDefault();return;}\n"
    "    if(k==='+'||k==='='){tuSet(tuBytes[tuSel]+1);e.preventDefault();return;}\n"
    "    if(k==='-'){tuSet(tuBytes[tuSel]+255);e.preventDefault();return;}\n"
    "    if(k==='Backspace'){\n"
    "      tuNib=-1;tuSet(tuOrig[tuSel]);e.preventDefault();return;}\n"
    "    if(k==='u'){\n"
    "      var p=tuUndo.pop();\n"
    "      if(p){tuBytes[p.i]=p.v;tuSel=p.i;tuNib=-1;tuScore();}\n"
    "      e.preventDefault();return;}\n"
    "    if(k==='r'){\n"
    "      /* reset is undoable: current values go onto the stack */\n"
    "      for(var i2=0;i2<tuBytes.length;i2++)\n"
    "        if(tuBytes[i2]!==tuOrig[i2])\n"
    "          tuUndo.push({i:i2,v:tuBytes[i2]});\n"
    "      tuBytes.set(tuOrig);tuNib=-1;tuScore();\n"
    "      showToast('THE ORIGINAL STANDS (U UNDOES)');\n"
    "      e.preventDefault();return;}\n"
    "    if(k.length===1){\n"
    "      var hx='0123456789abcdef'.indexOf(k.toLowerCase());\n"
    "      if(hx>=0){\n"
    "        if(tuNib<0){tuNib=hx;tuInfo.innerHTML=tuInfoFmt();}\n"
    "        else{var v=(tuNib<<4)|hx;tuNib=-1;tuSet(v);}\n"
    "        e.preventDefault();return;\n"
    "      }\n"
    "    }\n"
    "  }\n"
    "  window.addEventListener('keydown',tuKey,true);\n"
    "  /* release anywhere ends the audition, even off-panel */\n"
    "  window.addEventListener('keyup',function(e){\n"
    "    if(tuAB&&(e.key==='o'||e.key==='O')){\n"
    "      tuAB=false;\n"
    "      /* same lead-in on release: the tuned take of the same bars */\n"
    "      sonPos=Math.max(0,tuSel-8);\n"
    "      sonFlush();\n"
    "      tuDraw();\n"
    "    }\n"
    "  },true);\n"
    "  /* the playhead visits the tuner: mark the byte that is\n"
    "     sounding now, courtesy of the beat bus */\n"
    "  onBeat(function(e2){\n"
    "    if(tuOn&&sonify){tuPlay=e2.i;tuDraw();}\n"
    "  });\n"
    "  function tuStop(){\n"
    "    if(!tuOn) return;\n"
    "    tuOn=false;tuPlay=-1;tuAB=false;\n"
    "    if(tuOv){tuOv.remove();tuOv=null;}\n"
    "  }\n"
    "  function tuStart(){\n"
    "    if(tuOn){tuStop();return;}\n"
    "    if(auOn){showToast('THE AUDITION HAS THE STAGE');return;}\n"
    "    if(typeof PAQ==='undefined'||typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO PAYLOAD ABOARD');return;}\n"
    "    tuBytes=new Uint8Array(BD.length);\n"
    "    for(var i=0;i<BD.length;i++) tuBytes[i]=parseInt(BD[i].h,16);\n"
    "    var t0=performance.now();\n"
    "    var r=fpScore(tuBytes,true);\n"
    "    tuMs=performance.now()-t0;\n"
    "    if(!r){showToast('THE PASS DID NOT SURVIVE');return;}\n"
    "    tuC0=new Float32Array(r.perByte);tuCN=r.perByte;\n"
    "    tuT0=r.bits;tuTN=r.bits;\n"
    "    if(!tuBase){\n"
    "      tuBase=new Uint8Array(tuBytes);tuTBase=r.bits;\n"
    "      tuCB0=new Float32Array(r.perByte); /* original costs, for A/B */\n"
    "      /* per-model baseline, so cards can mark whose take moved */\n"
    "      if(r.perModel&&fpH){\n"
    "        tuSV0=new Float64Array(fpH.num);\n"
    "        for(var b0=0;b0<tuBytes.length;b0++)\n"
    "          for(var m0=0;m0<fpH.num;m0++)\n"
    "            tuSV0[m0]+=r.perModel[b0*fpH.num+m0];\n"
    "      }\n"
    "    }\n"
    "    /* revert and reset target the TRUE original, not whatever\n"
    "       tuned state this session happened to open on */\n"
    "    tuOrig=tuBase;\n"
    "    tuWkInit(); /* warm the scoring worker before the first edit */\n"
    "    tuSel=0;tuNib=-1;tuUndo=[];tuN=0;tuLit=false;\n"
    "    tuNM=fpH.num;tuM=r.perModel;tuPlay=-1;\n"
    "    /* band channels name models as MASK:W; find each in header\n"
    "       order so live contributions land on the right channel */\n"
    "    tuBandMap=null;\n"
    "    if(typeof BAND!=='undefined'&&BAND.ch.length){\n"
    "      tuBandMap=[];\n"
    "      for(var ci=0;ci<BAND.ch.length;ci++){\n"
    "        var hm=-1;\n"
    "        for(var m2=0;m2<fpH.num;m2++){\n"
    "          if(hx2(fpH.ext[m2]&255)+':'+fpH.weights[m2]\n"
    "             ===BAND.ch[ci]){hm=m2;break;}\n"
    "        }\n"
    "        tuBandMap.push(hm);\n"
    "      }\n"
    "    }\n"
    ...
    "      +'pointer-events:auto;';\n"
    "    bx.addEventListener('mouseenter',function(){tuHot=true;});\n"
    "    bx.addEventListener('mouseleave',function(){tuHot=false;});\n"
    "    var tt=document.createElement('div');\n"
    "    tt.id='tu-ttl';\n"
    "    tt.style.cssText='color:var(--acc);font-size:10px;'\n"
    "      +'margin-bottom:8px;text-shadow:2px 2px 0 rgba(255,63,216,.5);';\n"
    "    tt.textContent='*** PAQ TUNER ***';\n"
    "    tuHead=document.createElement('div');\n"
    "    tuHead.style.cssText='margin:6px 0;line-height:1.9;';\n"
    "    tuCv=document.createElement('canvas');\n"
    "    tuCv.width=TUCOLS*TUCS;tuCv.height=rows*TUCS;\n"
    "    tuCv.style.cssText='display:block;cursor:crosshair;';\n"
    "    tuCtx=tuCv.getContext('2d');\n"
    "    tuDCv=document.createElement('canvas');\n"
    "    tuDCv.width=TUCOLS*TUCS;tuDCv.height=36;\n"
    "    tuDCv.style.cssText='display:block;margin-top:6px;';\n"
    "    tuDCtx=tuDCv.getContext('2d');\n"
    "    tuInfo=document.createElement('div');\n"
    "    tuInfo.style.cssText='margin:6px 0;color:var(--yel);'\n"
    "      +'line-height:1.9;min-height:14px;';\n"
    "    var ft=document.createElement('div');\n"
    "    ft.style.cssText='color:var(--fg3);font-size:7px;line-height:1.9;';\n"
    "    ft.innerHTML='ARROWS MOVE - HEX TYPES - +/- NUDGE - WHEEL SPINS'\n"
    "      +'<br>BKSP REVERT - U UNDO - R RESET - SPACE LISTEN - P CUE'\n"
    "      +' - ESC OUT'\n"
    "      +'<br>HOLD O = THE ORIGINAL, FOR EARS AND EYES'\n"
    "      +'<br>KEYS BITE ONLY WHILE THE POINTER IS ON THE PANEL'\n"
    "      +(tuBytes.length>TUMAX?'<br>SHOWING FIRST 16K OF '\n"
    "        +tuBytes.length+' (ALL SCORED)':'');\n"
    "    tuCv.addEventListener('click',function(e){\n"
    "      var cx=(e.offsetX/TUCS)|0,cy=(e.offsetY/TUCS)|0;\n"
    "      var i2=cy*TUCOLS+cx;\n"
    "      if(i2>=0&&i2<tuVis()){tuSel=i2;tuNib=-1;tuDraw();}\n"
    "    });\n"
    "    tuCv.addEventListener('wheel',function(e){\n"
    "      e.preventDefault();\n"
    "      tuSet(tuBytes[tuSel]+(e.deltaY<0?1:255));\n"
    "    },{passive:false});\n"
    "    bx.appendChild(tt);bx.appendChild(tuHead);\n"
    "    bx.appendChild(tuCv);bx.appendChild(tuDCv);\n"
    "    bx.appendChild(tuInfo);bx.appendChild(ft);\n"
    "    tuOv.appendChild(bx);\n"
    "    document.body.appendChild(tuOv);\n"
    "    dragBox(bx,tt);\n"
    "    tuOn=true;\n"
    "    egg('tuner');\n"
    "    tuDraw();\n"
    "    showToast('COMPOSE. THE MODEL IS LISTENING');\n"
    "  }\n"
    "  /* any sub-window drags by its title: on grab the box leaves\n"
    "     the flex-centered overlay for fixed coordinates and follows\n"
    "     the pointer, clamped to the viewport */\n"
    "  function dragBox(bx2,hd,rp){\n"
    "    if(!bx2||!hd) return;\n"
    "    hd.style.cursor='move';\n"
    "    hd.style.userSelect='none';\n"
    "    hd.style.touchAction='none';\n"
    "    var sx=0,sy=0,bx0=0,by0=0,drag=false,moved=false;\n"
    "    hd.addEventListener('pointerdown',function(e){\n"
    "      var r=bx2.getBoundingClientRect();\n"
    "      /* rp: escape the dock - position:fixed is broken inside a\n"
    "         transformed ancestor (deejay's scale), so reparent. keep\n"
    "         the visual scale it had inside, or it halves mid-grab */\n"
    "      if(rp&&bx2.parentNode!==document.body){\n"
    "        var sc=bx2.offsetWidth?r.width/bx2.offsetWidth:1;\n"
    "        document.body.appendChild(bx2);\n"
    "        bx2.style.zIndex='9997';\n"
    "        bx2.classList.add('rk-free');\n"
    "        if(sc>1.01||sc<0.99){\n"
    "          bx2.style.transform='scale('+sc.toFixed(3)+')';\n"
    "          bx2.style.transformOrigin='0 0';\n"
    "          bx2.style.imageRendering='pixelated';\n"
    "        }\n"
    "      }\n"
    "      bx2.style.position='fixed';\n"
    "      bx2.style.left=r.left+'px';\n"
    "      bx2.style.top=r.top+'px';\n"
    "      bx2.style.margin='0';\n"
    "      sx=e.clientX;sy=e.clientY;bx0=r.left;by0=r.top;\n"
    "      drag=true;moved=false;\n"
    "      try{hd.setPointerCapture(e.pointerId);}catch(e2){}\n"
    "      e.preventDefault();\n"
    "    });\n"
    ...
    "      var nl=bx0+dx,nt=by0+dy;\n"
    "      if(nl<60-bx2.offsetWidth) nl=60-bx2.offsetWidth;\n"
    "      if(nl>innerWidth-60) nl=innerWidth-60;\n"
    "      if(nt<0) nt=0;\n"
    "      if(nt>innerHeight-40) nt=innerHeight-40;\n"
    "      bx2.style.left=nl+'px';\n"
    "      bx2.style.top=nt+'px';\n"
    "    });\n"
    "    hd.addEventListener('pointerup',function(e){\n"
    "      drag=false;\n"
    "      try{hd.releasePointerCapture(e.pointerId);}catch(e2){}\n"
    "    });\n"
    "    /* only a real drag swallows the click - clean clicks still\n"
    "       reach the tracker egg / modal dismiss. window capture runs\n"
    "       before any same-element listener */\n"
    "    window.addEventListener('click',function(e){\n"
    "      if(moved&&(e.target===hd||hd.contains(e.target))){\n"
    "        e.stopPropagation();e.preventDefault();moved=false;\n"
    "      }\n"
    "    },true);\n"
    "  }\n"
    "  /* the rack: scopes drag by their whole body (display-only),\n"
    "     the tracker by its header (rows stay clickable) */\n"
    "  (function(){\n"
    "    var rk=['paqscope','paqspec','paqsg','paqvec'];\n"
    "    for(var ri=0;ri<rk.length;ri++){\n"
    "      var re2=document.getElementById(rk[ri]);\n"
    "      if(re2) dragBox(re2,re2,true);\n"
    "    }\n"
    "  })();\n"
    "  /* synth: the sonification, patchable. every knob writes\n"
    "     straight into the live engine - the next scheduled step\n"
    "     plays the new patch. defaults are the shipped sound */\n"
    "  var synOn=false,synOv=null;\n"
    "  var SYNDEF={bd:.125,tr:0,wave:'square',lg:.04,ld:.12,fmi:1,\n"
    "    bi:4,bw:'square',bg:.05,st:8,bgx:1,pw:.8,\n"
    "    t1:1.5,t2:3,t3:5,cs:4,prog:0,rv:0,rt:1.8,\n"
    "    dw:0,dt:3,df:.35,fc:20000,fq:.7,wr:0,wa:0,\n"
    "    sw:0,atk:0,cb:16,cmp:1,ah:0,gr:0,pts:0,mg:0,me:0,\n"
    "    fx:0,fe:0,pp:0,str:1,dtn:0,pmp:0,ww:0,sf:0,gen:0};\n"
    "  var syn={};\n"
    "  function synReset(){for(var k in SYNDEF) syn[k]=SYNDEF[k];}\n"
    "  synReset();\n"
    "  var SYNPR=[\n"
    "    {n:'AM F C G',c:[[57,60,64],[53,57,60],[48,52,55],[55,59,62]],\n"
    "     l:['AM','F','C','G']},\n"
    "    {n:'ANDALUSIAN',c:[[57,60,64],[55,59,62],[53,57,60],[52,56,59]],\n"
    "     l:['AM','G','F','E']},\n"
    "    {n:'AXIS',c:[[48,52,55],[55,59,62],[57,60,64],[53,57,60]],\n"
    "     l:['C','G','AM','F']},\n"
    "    {n:'PLUNGE',c:[[57,60,64],[52,55,59],[53,57,60],[55,59,62]],\n"
    "     l:['AM','EM','F','G']}];\n"
    "  function synProg(p){\n"
    "    syn.prog=p;\n"
    "    for(var k=0;k<4;k++){\n"
    ...
    "    }\n"
    "  }\n"
    "  /* the dice: a curated random patch. musical bounds only -\n"
    "     tempo, volume, thresholds and the user's toggles are left\n"
    "     alone. the panel rebuilds to show the roll */\n"
    "  function synRandom(){\n"
    "    var W2=['square','sawtooth','triangle','sine'];\n"
    "    syn.wave=W2[(Math.random()*4)|0];\n"
    "    syn.bw=W2[(Math.random()*4)|0];\n"
    "    syn.ld=.06+Math.random()*.25;\n"
    "    syn.atk=Math.random()<.5?0:Math.random()*.12;\n"
    "    syn.fmi=.5+Math.random()*1.5;\n"
    "    syn.dtn=Math.random()<.5?(Math.random()*18)|0:0;\n"
    "    syn.rv=Math.random()<.4?Math.random()*.5:0;\n"
    "    syn.dw=Math.random()<.5?Math.random()*.45:0;\n"
    "    syn.dt=[2,3,4,6,8][(Math.random()*5)|0];\n"
    "    syn.df=.15+Math.random()*.5;\n"
    "    syn.fc=300*Math.pow(60,Math.random());\n"
    "    syn.fq=.7+Math.random()*6;\n"
    "    syn.fe=Math.random()<.5?Math.random():0;\n"
    "    syn.wr=Math.random()<.3?.5+Math.random()*4:0;\n"
    "    syn.wa=syn.wr?.2+Math.random()*.6:0;\n"
    "    syn.sw=Math.random()<.35?.2+Math.random()*.35:0;\n"
    "    syn.cb=Math.random()<.25?5+((Math.random()*6)|0):16;\n"
    "    syn.pw=.4+Math.random()*.6;\n"
    "    syn.ww=Math.random()<.3?Math.random()*.3:0;\n"
    "    syn.pmp=Math.random()<.4?.3+Math.random()*.5:0;\n"
    "    synProg((Math.random()*4)|0);\n"
    "    filtApply();dlApply();crushApply();cmpApply();\n"
    "    rvApply();rvDecay();\n"
    "    if(synOn){synOn=false;\n"
    "      if(synOv){synOv.remove();synOv=null;}\n"
    "      synStart(true);}\n"
    "    showToast('A PATCH FROM THE VOID');\n"
    "  }\n"
    "  /* the genres: a kit pattern (16-bit masks over the bar, bit =\n"
    "     step), a kick recipe [f0,f1,dur,gain,drive], a bass style,\n"
    "     a tempo, a few patch nudges. the file still sings the lead\n"
    "     - same bytes, different club */\n"
    "  /* per genre: kit masks, kick recipe [f0,f1,dur,gain,drive,\n"
    "     click], bass style/wave/gain, snare gain, and the LEAD\n"
    "     CONTRACT - lm is the rhythm mask the melody may speak on,\n"
    "     ln its note length, oc its register. the genre owns WHEN,\n"
    "     the file owns WHAT */\n"
    "  var GENRES=[null,\n"
    "    {n:'HOUSE',bpm:124,k:0x1111,s:0x1010,hc:0xAAAA,ho:0x4444,\n"
    "     /* clap backbeat, 909 metal, M1 stabs on the offbeats,\n"
    "        the i-VI-III-VII merry-go-round */\n"
    "     bs:'ob',kp:[120,45,.12,.11,0,.015],fl:1,arr:1,\n"
    "     clp:1,h9:1,stb:0x0808,prog:[0,5,2,6],am:0x1111,\n"
    "     np:[0,3,7,10],lm:0x4444,ln:.16,\n"
    "     po:{pmp:.35,sw:.12}},\n"
    "    {n:'TECHNO',bpm:132,k:0x1111,s:0,hc:0xAAAA,ho:0x4444,\n"
    "     /* rumble under the kick, a 3-against-4 tick, no chords:\n"
    "        techno moves by not moving */\n"
    "     bs:'ob',kp:[105,38,.15,.16,1,.02],arr:1,\n"
    "     rmb:1,h9:1,pp3:0x0249,rd:0x5555,am:0x1111,\n"
    "     np:[0,0,12,7],lm:0x2222,ln:.1,\n"
    "     po:{pmp:.3}},\n"
    "    {n:'PSYTRANCE',bpm:145,k:0x1111,s:0,hc:0xAAAA,ho:0x4444,\n"
    "     /* filtered rubber gallop, zaps at the turnarounds */\n"
    "     bs:'psy',kp:[170,52,.09,.13,0,.03],fl:1,rs:1,arr:1,\n"
    "     zp:1,\n"
    "     np:[0,3,7,12],lm:0x8888,ln:.08,oc:1,\n"
    "     po:{ld:.09,pmp:.45}},\n"
    "    {n:'HARDSTYLE',bpm:150,k:0x1111,s:0,hc:0x4444,ho:0,\n"
    "     /* the screech: long detuned lead over the kick-bass,\n"
    "        whose tail now slides the bassline */\n"
    "     bs:'ton',kp:[380,55,.2,.42,1,.03],rb:1,fl:1,rs:1,\n"
    "     arr:1,ss:1,prog:[0,5,0,6],\n"
    "     np:[0,3,5,7],lm:0x0101,ln:.5,\n"
    "     po:{wave:'sawtooth',pmp:.3,dtn:14}},\n"
    "    {n:'HARDCORE',bpm:180,k:0x1111,s:0,hc:0xAAAA,ho:0,\n"
    "     /* the 7x drive and a hoover on every phrase */\n"
    "     bs:'none',kp:[430,50,.22,.5,2,.04],fl:1,arr:1,hv:1,\n"
    "     np:[0,0,3,12],lm:0x4444,ln:.09,\n"
    "     po:{ld:.07,pmp:.2}},\n"
    "    {n:'DRUM+BASS',bpm:174,k:0x0401,s:0x1010,hc:0x5555,ho:0x4000,\n"
    "     /* reese bass, ghost snares: the break chops itself */\n"
    "     bs:'reese',kp:[130,42,.11,.14,0,.025],fl:1,sg:.075,\n"
    "     arr:1,gh:1,prog:[0,3,5,4],\n"
    "     np:[0,3,5,7,10],lm:0x0808,ln:.2,\n"
    "     po:{}},\n"
    "    {n:'AMBIENT',bpm:72,k:0,s:0,hc:0,ho:0,\n"
    "     /* a drone on the floor, sevenths in the pad, and the\n"
    "        filter drifting like weather */\n"
    "     bs:'none',pad:1,pad7:1,drn:1,kp:[0,0,0,0,0,0],\n"
    "     prog:[0,5,3,4],\n"
    "     np:[0,7,12,17],lm:0x0001,ln:1.2,\n"
    "     po:{atk:.12,ld:.38,rv:.45,rt:3,fc:2600,wr:.07,wa:.25}},\n"
    "    {n:'TRANCE',bpm:138,k:0x1111,s:0,hc:0xAAAA,ho:0x4444,\n"
    "     /* full 16th arp lead - trance IS the arp - rolling\n"
    "        off-16th bass, a three-saw supersaw, dotted\n"
    "        ping-pong delay, and the anthem progression */\n"
    "     bs:'roll',kp:[125,46,.13,.13,0,.02],fl:1,rs:1,arr:1,\n"
    "     ss:1,h9:1,prog:[0,5,2,6],am:0x1111,mm:'arp',\n"
    "     np:[0,3,7,12,15,19],lm:0xFFFF,ln:.11,\n"
    "     po:{wave:'sawtooth',dtn:12,atk:.02,rv:.3,rt:2.2,\n"
    "       dw:.35,dt:3,df:.45,pp:1,pmp:.45}},\n"
    "    {n:'ACID',bpm:130,k:0x1111,s:0x1010,hc:0xAAAA,ho:0,\n"
    "     /* no bass voice: the LEAD is the 303, running 16ths\n"
    "        through its own squelching filter - slides tie the\n"
    "        plain notes, cost writes the accents */\n"
    "     bs:'none',kp:[115,42,.12,.12,0,.015],fl:1,arr:1,\n"
    "     clp:1,a303:1,sl3:1,prog:[0,0,5,0],\n"
    "     np:[0,0,12,0,3,10],lm:0xFFFF,ln:.12,\n"
    "     po:{wave:'sawtooth',fq:5.5,ld:.13}},\n"
    "    {n:'DUBSTEP',bpm:140,k:0x0081,s:0x0100,hc:0x2222,ho:0,\n"
    "     /* half-time: snare on 3. the wub re-aims each half-bar\n"
    "        and the sub slips past the filter on the post road */\n"
    "     bs:'wub',kp:[110,40,.14,.3,1,.02],arr:1,\n"
    "     wq:1,sub:1,bigsn:1,prog:[0,0,5,4],\n"
    "     np:[0,3,1,7,10,13],lm:0x1090,ln:.25,\n"
    "     po:{wr:2.5,wa:.75,fc:1100,fq:5}},\n"
    "    {n:'LOFI',bpm:84,k:0x0041,s:0x1010,hc:0xAAAA,ho:0,\n"
    "     /* boom-bap kicks, loose hats, rimshot backbeat,\n"
    "        sevenths in the pad, worn tape, worn shellac */\n"
    "     bs:'sub',kp:[95,44,.11,.1,0,0],\n"
    "     rim:1,hm:1,pad:1,pad7:1,prog:[0,5,1,4],\n"
    "     np:[0,3,5,7,10],lm:0x0912,ln:.3,\n"
    "     po:{sw:.32,ww:.12,sf:.5,fc:2400,ld:.3,atk:.03,\n"
    "       rv:.25,wave:'triangle'}},\n"
    "    {n:'SPEEDCORE',bpm:300,k:0x1111,s:0,hc:0,ho:0,\n"
    "     /* the 7x drive, and the flood: the last two bars of\n"
    "        every phrase are all kick */\n"
    "     bs:'none',kp:[500,55,.16,.55,2,.05],fld:1,\n"
    "     np:[0,12,3,15],lm:0xFFFF,ln:.06,\n"
    "     po:{ld:.05}}];\n"
    "  /* genre default key: unpinned dance floors live in A minor.\n"
    "     a typed key (aminor, cmajor...) overrides it */\n"
    "  var GKEY={r:57,mi:1};\n"
    "  /* genre melody: a per-phrase MOTIF. the first bar of every\n"
    "     4-bar phrase is sampled live from the data - cost picks\n"
    "     positions in the genre's note pool - and the next three\n"
    "     bars repeat it. repetition is what makes it a tune, and\n"
    "     the tune is still the file's: new phrase, new bytes, new\n"
    "     motif. downbeats resolve to the root unless the byte\n"
    "     truly hurts */\n"
    "  var MTF=new Int8Array(16);\n"
    "  /* a pinned major key majors the pool's minor intervals */\n"
    "  function pmap(v,mi){\n"
    "    return mi?v:(v%%12===3?v+1:v%%12===10?v+1:v);\n"
    "  }\n"
    "  /* diatonic plumbing: a scale degree to semitones, and the\n"
    "     triad on it. the progression speaks degrees, not notes,\n"
    "     so every chord it lands on is in key */\n"
    "  function gdOff(sk,d){\n"
    "    var o=0;while(d>=7){d-=7;o+=12;}\n"
    "    return KEYSC[sk.mi][d]+o;\n"
    "  }\n"
    "  function gtriad(sk,d){\n"
    "    return [sk.r+gdOff(sk,d),sk.r+gdOff(sk,d+2),\n"
    "      sk.r+gdOff(sk,d+4)];\n"
    "  }\n"
    "  /* the bar's root offset under a genre progression, or -1\n"
    "     when the genre stays put */\n"
    "  function gprog(G,step,sk){\n"
    "    return G.prog?gdOff(sk,G.prog[(step>>4)&3]):-1;\n"
    "  }\n"
    "  function gnote(step,c,G){\n"
    "    var sk=sonKey||GKEY;\n"
    "    var go=gprog(G,step,sk);if(go<0)go=0;\n"
    "    var np=G.np||[0,3,7,12];\n"
    "    var sp2=step&15,bar3=step>>4;\n"
    "    if((bar3&3)===0){\n"
    "      var pi=c<syn.t1?((sp2&2)?1:0)\n"
    "        :c<syn.t2?1+(sp2&1)\n"
    "        :c<syn.t3?np.length-2:np.length-1;\n"
    "      if(pi>=np.length)pi=np.length-1;\n"
    "      MTF[sp2]=pi;\n"
    "    }\n"
    "    var pi2=MTF[sp2];\n"
    "    if(pi2<0||pi2>=np.length)pi2=0;\n"
    "    if((sp2&7)===0&&c<syn.t3)pi2=0;\n"
    "    var n2=sk.r+go+pmap(np[pi2],sk.mi)+24;\n"
    "    /* tense bytes lift the arp an octave: the sparkle */\n"
    "    if(G.mm==='arp'&&c>=syn.t2&&c<syn.t3)n2+=12;\n"
    "    return n2;\n"
    "  }\n"
    "  /* genre bass: tight and disposable */\n"
    "  function gbass(fq,g,d2,t,w,dest){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type=w||'sawtooth';o.frequency.value=fq;\n"
    "    gn.gain.setValueAtTime(g,t);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+d2);\n"
    "    o.connect(gn);gn.connect(dest||gainM);\n"
    "    o.start(t);o.stop(t+d2+.02);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  /* the riser: one bar of noise climbing into the drop */\n"
    "  function riser(t,sp){\n"
    "    var s=actx.createBufferSource(),g=actx.createGain();\n"
    "    var f=actx.createBiquadFilter();\n"
    "    s.buffer=nzBuf();s.loop=true;\n"
    "    f.type='bandpass';f.frequency.value=300+sp*260;\n"
    "    f.Q.value=2;\n"
    "    g.gain.setValueAtTime(.001,t);\n"
    "    g.gain.linearRampToValueAtTime(.018+.005*sp,t+beatDur);\n"
    "    s.connect(f);f.connect(g);g.connect(gainM);\n"
    "    s.start(t);s.stop(t+beatDur+.02);\n"
    "    vreg(t,[s]);\n"
    "  }\n"
    "  /* the crash: the drop's first step wears it */\n"
    "  function crash(t){\n"
    "    var s=actx.createBufferSource(),g=actx.createGain();\n"
    "    var f=actx.createBiquadFilter();\n"
    "    s.buffer=nzBuf();s.loop=true;\n"
    ...
    "    vreg(t,[s]);\n"
    "  }\n"
    "  /* hardstyle: the kick's tonal tail. the note IS the kick,\n"
    "     and the tail slides in from the last note - the bassline\n"
    "     is one long kick */\n"
    "  var gtPrev=0;\n"
    "  function gtail(fq,t){\n"
    "    var o=actx.createOscillator(),gn=actx.createGain();\n"
    "    o.type='square';\n"
    "    o.frequency.setValueAtTime(gtPrev||fq,t+.07);\n"
    "    o.frequency.exponentialRampToValueAtTime(fq,t+.16);\n"
    "    gtPrev=fq;\n"
    "    var d2=beatDur*4-.09;if(d2<.1)d2=.1;\n"
    "    gn.gain.setValueAtTime(.0001,t+.07);\n"
    "    gn.gain.linearRampToValueAtTime(.12,t+.1);\n"
    "    gn.gain.exponentialRampToValueAtTime(.001,t+.07+d2);\n"
    "    o.connect(gn);gn.connect(drv||gainM);\n"
    "    o.start(t+.07);o.stop(t+.1+d2);\n"
    "    vreg(t,[o]);\n"
    "  }\n"
    "  function genreApply(g){\n"
    "    syn.gen=g;\n"
    "    var G=GENRES[g];\n"
    "    /* every switch starts from the shipped patch - chip\n"
    "       included. bpm, filters, swing: the last club's smoke\n"
    "       stays at the door */\n"
    "    var GK=['wave','dtn','atk','rv','rt','dw','dt','df','pp',\n"
    "      'pmp','sw','ww','sf','fc','fq','wr','wa','ld','fx','fe'];\n"
    "    for(var gi=0;gi<GK.length;gi++) syn[GK[gi]]=SYNDEF[GK[gi]];\n"
    "    syn.bd=SYNDEF.bd;\n"
    "    if(G){\n"
    "      syn.bd=60/(G.bpm*4);\n"
    "      if(G.po) for(var k in G.po) syn[k]=G.po[k];\n"
    "    }\n"
    "    beatDur=document.body.classList.contains('mourning')\n"
    "      ?syn.bd*2:syn.bd;\n"
    "    filtApply();dlApply();crushApply();cmpApply();rvApply();\n"
    "    rvDecay(); /* rt changed hands: the tail regrows to fit */\n"
    "    /* clean slate on the floor: the set restarts at bar one\n"
    "       and the queued future - still wearing the old patch -\n"
    "       dies before it sounds */\n"
    "    mstep=0;gtPrev=0;fxLast=0;\n"
    "    if(actx) sonFlush();\n"
    "    if(synOn){synOn=false;\n"
    "      if(synOv){synOv.remove();synOv=null;}\n"
    "      synStart(true);}\n"
    "    showToast(!G?'BACK TO THE CHIP'\n"
    "      :G.bpm>=250?G.n+' / '+G.bpm+' BPM. SEE A PHYSICIAN.'\n"
    "      :G.n+' / '+G.bpm+' BPM');\n"
    "  }\n"
    ...
    "     file. built lazily; costs nothing while the knob sits at 0 */\n"
    "  var rvConv=null,rvGain=null;\n"
    "  /* the master chain: crusher -> filter(+lfo) -> post -> limiter.\n"
    "     every stage sits at a transparent setting until its knob\n"
    "     moves, so the shipped sound is untouched by default */\n"
    "  var crush=null,filt=null,post=null,comp=null;\n"
    "  var lfo=null,lfoG=null,dl=null,dlG=null,dlF=null;\n"
    "  var dl2=null,dlPL=null,dlPR=null;\n"
    "  var drv=null,drv2=null; /* tanh drives for genre kicks:\n"
    "     drv is the shipped 3.5x, drv2 is hardcore's 7x */\n"
    "  function filtApply(){\n"
    "    if(!filt) return;\n"
    "    filt.frequency.value=syn.fc;\n"
    "    filt.Q.value=syn.fq;\n"
    "    if(lfo){\n"
    "      lfo.frequency.value=syn.wr;\n"
    "      lfoG.gain.value=syn.wa*syn.fc*.7;\n"
    "    }\n"
    "  }\n"
    "  function dlApply(){\n"
    "    if(!dl) return;\n"
    "    var dt2=Math.min(2,beatDur*syn.dt);\n"
    "    dl.delayTime.value=dt2;\n"
    "    if(dl2) dl2.delayTime.value=dt2;\n"
    "    dlG.gain.value=syn.dw;\n"
    "    dlF.gain.value=syn.df;\n"
    "    /* ping-pong: first tap lands left, crosses to a second\n"
    "       tap on the right, feedback returns home. mono when off.\n"
    "       rewired live - webaudio dedups identical connections */\n"
    "    try{\n"
    "      dl.disconnect();\n"
    "      if(syn.pp&&dl2&&dlPL){\n"
    "        dl.connect(dlPL);dl.connect(dl2);\n"
    "        dl2.disconnect();\n"
    "        dl2.connect(dlPR);dl2.connect(dlF);\n"
    "        dlPL.connect(dlG);dlPR.connect(dlG);\n"
    "      } else {\n"
    "        if(dl2) dl2.disconnect();\n"
    "        dl.connect(dlG);dl.connect(dlF);\n"
    "      }\n"
    "    }catch(e){}\n"
    "  }\n"
    "  function crushApply(){\n"
    "    if(!crush) return;\n"
    "    if(syn.cb>=16){crush.curve=null;return;}\n"
    "    var lv=Math.pow(2,syn.cb-1),cv=new Float32Array(1025);\n"
    "    for(var i=0;i<1025;i++){\n"
    "      var x=i/512-1;\n"
    "      cv[i]=Math.round(x*lv)/lv;\n"
    "    }\n"
    "    crush.curve=cv;\n"
    "  }\n"
    "  function cmpApply(){\n"
    ...
    "      comp.knee.value=6;comp.attack.value=.003;\n"
    ...
    "  }\n"
    "  function rvIR(){\n"
    "    var sr=actx.sampleRate,len=Math.max(1,(sr*syn.rt)|0);\n"
    "    var ir=actx.createBuffer(2,len,sr);\n"
    "    for(var ch2=0;ch2<2;ch2++){\n"
    "      var d=ir.getChannelData(ch2);\n"
    "      for(var i=0;i<len;i++)\n"
    ...
    "  }\n"
    "  function rvApply(){\n"
    "    if(!actx||!gainM) return;\n"
    "    if(syn.rv>0&&!rvConv){\n"
    "      try{\n"
    "        rvConv=actx.createConvolver();\n"
    "        rvConv.buffer=rvIR();\n"
    "        rvGain=actx.createGain();\n"
    "        (post||gainM).connect(rvConv);\n"
    "        rvConv.connect(rvGain);\n"
    "        rvGain.connect(comp||actx.destination);\n"
    "      }catch(e){rvConv=null;return;}\n"
    "    }\n"
    "    if(rvGain) rvGain.gain.value=syn.rv;\n"
    "  }\n"
    "  function rvDecay(){\n"
    "    if(rvConv&&actx) rvConv.buffer=rvIR();\n"
    "  }\n"
    "  /* the protracker skin: typed on, typed off. the whole report\n"
    "     re-dresses as PT2.3D; nothing underneath changes */\n"
    "  var ptOn=false;\n"
    "  function ptToggle(){\n"
    "    ptOn=!ptOn;\n"
    "    document.body.classList.toggle('ptsyn',ptOn);\n"
    "    var ttl=document.getElementById('syn-ttl');\n"
    "    if(ttl) ttl.textContent=ptOn?'PAQSYNTH V2.3D'\n"
    "      :'*** PAQ SYNTH ***';\n"
    "    showToast(ptOn?'THE YEAR IS 1994':'BACK TO NEON');\n"
    "  }\n"
    "  function synStop(){\n"
    "    if(!synOn) return;\n"
    "    synOn=false;\n"
    "    if(synOv){synOv.remove();synOv=null;}\n"
    "  }\n"
    "  function synRow(id,lab,mn,mx,st2,val,txt){\n"
    "    return '<div style=\"display:flex;align-items:center;gap:6px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'+lab\n"
    "      +'</span><span class=\"sy-ar\" data-t=\"'+id+'\" data-d=\"-1\">'\n"
    "      +'\\u25c2</span>'\n"
    "      +'<input id=\"syn-'+id+'\" type=\"range\" min=\"'+mn\n"
    "      +'\" max=\"'+mx+'\" step=\"'+st2+'\" value=\"'+val\n"
    "      +'\" style=\"flex:1;accent-color:#ff3fd8;min-width:64px\">'\n"
    "      +'<span class=\"sy-ar\" data-t=\"'+id+'\" data-d=\"1\">\\u25b8</span>'\n"
    "      +'<span id=\"synv-'+id+'\" style=\"width:58px;text-align:right;'\n"
    "      +'color:var(--yel);flex-shrink:0\">'+txt+'</span></div>';\n"
    "  }\n"
    "  function synSel(id,lab,opts,sel){\n"
    "    var h='<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'+lab\n"
    "      +'</span><select id=\"syn-'+id+'\" style=\"flex:1;'\n"
    "      +'background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2);font-family:inherit;'\n"
    "      +'font-size:8px;padding:2px\">';\n"
    "    for(var k=0;k<opts.length;k++)\n"
    "      h+='<option value=\"'+opts[k][0]+'\"'\n"
    "        +(String(opts[k][0])===String(sel)?' selected':'')+'>'\n"
    "        +opts[k][1]+'</option>';\n"
    "    return h+'</select></div>';\n"
    "  }\n"
    "  function synBind(id,fn){\n"
    "    var el=document.getElementById('syn-'+id);\n"
    "    if(el) el.addEventListener('input',function(){fn(el);});\n"
    "  }\n"
    "  function synV(id,txt){\n"
    "    var el=document.getElementById('synv-'+id);\n"
    "    if(el) el.textContent=txt;\n"
    "  }\n"
    "  function synStart(quiet){\n"
    "    if(synOn){synStop();return;}\n"
    "    synOn=true;\n"
    "    synOv=document.createElement('div');\n"
    "    synOv.id='synth-ov';\n"
    "    synOv.style.cssText='position:fixed;inset:0;z-index:10003;'\n"
    "      +'display:flex;align-items:center;justify-content:flex-start;'\n"
    "      +'padding-left:216px;pointer-events:none;';\n"
    "    var bx=document.createElement('div');\n"
    "    bx.id='syn-bx';\n"
    "    bx.style.cssText='background:#0b0b10;border:2px solid var(--pnk);'\n"
    "      +'padding:14px 18px;font-family:var(--pix);font-size:8px;'\n"
    ...
    "    var mg=gainM?Math.round(gainM.gain.value*100):50;\n"
    "    var bpm=Math.round(60/(syn.bd*4));\n"
    "    var h='<div id=\"syn-ttl\" style=\"color:var(--acc);font-size:10px;'\n"
    "      +'margin-bottom:8px;text-shadow:2px 2px 0 rgba(255,63,216,.5)\">'\n"
    "      +(ptOn?'PAQSYNTH V2.3D':'*** PAQ SYNTH ***')+'</div>'\n"
    "      +synRow('bpm','TEMPO',40,240,1,bpm,bpm+' BPM')\n"
    "      +synSel('gen','GENRE',[[0,'CHIP'],[1,'HOUSE'],\n"
    "        [2,'TECHNO'],[8,'TRANCE'],[3,'PSYTRANCE'],\n"
    "        [9,'ACID'],[4,'HARDSTYLE'],[5,'HARDCORE'],\n"
    "        [12,'SPEEDCORE'],[6,'DRUM+BASS'],[10,'DUBSTEP'],\n"
    "        [11,'LOFI'],[7,'AMBIENT']],syn.gen)\n"
    "      +synRow('mas','MASTER',0,100,1,mg,mg+'%%')\n"
    "      +synRow('tr','TRANSPOSE',-24,24,1,syn.tr,syn.tr+' ST')\n"
    "      +synSel('str','NEEDLE',[[1,'FORWARD'],[-1,'REVERSE'],\n"
    "        [2,'SKIP 2'],[-2,'BACK 2'],[4,'SKIP 4']],syn.str)\n"
    "      +synSel('wave','LEAD WAVE',[['square','SQUARE'],\n"
    "        ['sawtooth','SAW'],['triangle','TRIANGLE'],\n"
    "        ['sine','SINE']],syn.wave)\n"
    "      +synRow('lg','LEAD GAIN',0,15,1,Math.round(syn.lg*100),\n"
    "        Math.round(syn.lg*100)+'%%')\n"
    "      +synRow('ld','DECAY',40,400,10,Math.round(syn.ld*1000),\n"
    "        Math.round(syn.ld*1000)+' MS')\n"
    "      +synRow('dtn','DETUNE',0,25,1,syn.dtn,syn.dtn+' CT')\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'FX COLUMN</span><button id=\"syn-fx\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.fx?'ON':'OFF')+'</button></div>'\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'FM ENGINE</span><button id=\"syn-fm\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(fmOn?'ON':'OFF')+'</button></div>'\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'GROOVE</span><button id=\"syn-gr\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.gr?'ON':'OFF')+'</button></div>'\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'BAND PARTS</span><button id=\"syn-pts\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.pts?'ON':'OFF')+'</button></div>'\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'MASK GATES</span><button id=\"syn-mg\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.mg?'ON':'OFF')+'</button></div>'\n"
    "      +synRow('fmi','FM DEPTH',25,300,5,Math.round(syn.fmi*100),\n"
    "        Math.round(syn.fmi*100)+'%%')\n"
    "      +synSel('bi','BASS EVERY',[[0,'OFF'],[2,'2 STEPS'],\n"
    "        [4,'4 STEPS'],[8,'8 STEPS']],syn.bi)\n"
    "      +synSel('bw','BASS WAVE',[['square','SQUARE'],\n"
    "        ['sawtooth','SAW'],['triangle','TRIANGLE'],\n"
    "        ['sine','SINE']],syn.bw)\n"
    "      +synRow('bg','BASS GAIN',0,15,1,Math.round(syn.bg*100),\n"
    "        Math.round(syn.bg*100)+'%%')\n"
    "      +synRow('st','SNARE AT',3,13,1,syn.st>12?13:syn.st,\n"
    "        syn.st>12?'OFF':syn.st+' BITS')\n"
    "      +synRow('bgx','BAND GAIN',0,200,5,Math.round(syn.bgx*100),\n"
    "        Math.round(syn.bgx*100)+'%%')\n"
    "      +synRow('pw','PAN WIDTH',0,100,5,Math.round(syn.pw*100),\n"
    "        Math.round(syn.pw*100)+'%%')\n"
    "      +synRow('rv','REVERB',0,100,5,Math.round(syn.rv*100),\n"
    "        Math.round(syn.rv*100)+'%%')\n"
    "      +synRow('rt','ROOM',3,40,1,Math.round(syn.rt*10),\n"
    "        syn.rt.toFixed(1)+' S')\n"
    "      +synRow('dw','DELAY',0,100,5,Math.round(syn.dw*100),\n"
    "        Math.round(syn.dw*100)+'%%')\n"
    "      +synSel('dt','DELAY TIME',[[2,'2 STEPS'],[3,'3 STEPS'],\n"
    "        [4,'4 STEPS'],[6,'6 STEPS'],[8,'8 STEPS']],syn.dt)\n"
    "      +synRow('df','FEEDBACK',0,85,5,Math.round(syn.df*100),\n"
    "        Math.round(syn.df*100)+'%%')\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'MATCH ECHO</span><button id=\"syn-me\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.me?'ON':'OFF')+'</button></div>'\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'PING-PONG</span><button id=\"syn-pp\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.pp?'ON':'OFF')+'</button></div>'\n"
    "      +synRow('fc','CUTOFF',0,100,1,\n"
    "        Math.round(100*Math.log(syn.fc/100)/Math.log(200)),\n"
    "        syn.fc>=1000?(syn.fc/1000).toFixed(1)+' K'\n"
    "          :Math.round(syn.fc)+' HZ')\n"
    "      +synRow('fq','RESONANCE',5,200,5,Math.round(syn.fq*10),\n"
    "        syn.fq.toFixed(1))\n"
    "      +synRow('fe','FILTER ENV',0,100,5,Math.round(syn.fe*100),\n"
    "        Math.round(syn.fe*100)+'%%')\n"
    "      +synRow('wr','WOBBLE HZ',0,80,1,Math.round(syn.wr*10),\n"
    "        syn.wr.toFixed(1)+' HZ')\n"
    "      +synRow('wa','WOBBLE AMT',0,100,5,Math.round(syn.wa*100),\n"
    "        Math.round(syn.wa*100)+'%%')\n"
    "      +synRow('sw','SWING',0,60,2,Math.round(syn.sw*100),\n"
    "        Math.round(syn.sw*100)+'%%')\n"
    "      +synRow('pmp','PUMP',0,100,5,Math.round(syn.pmp*100),\n"
    "        Math.round(syn.pmp*100)+'%%')\n"
    "      +synRow('atk','ATTACK',0,200,5,Math.round(syn.atk*1000),\n"
    "        Math.round(syn.atk*1000)+' MS')\n"
    "      +synRow('cb','CRUSH',4,16,1,syn.cb,\n"
    "        syn.cb>=16?'OFF':syn.cb+' BITS')\n"
    "      +synRow('ww','TAPE WOW',0,50,2,Math.round(syn.ww*100),\n"
    "        Math.round(syn.ww*100)+' CT')\n"
    "      +synRow('sf','SURFACE',0,100,5,Math.round(syn.sf*100),\n"
    "        Math.round(syn.sf*100)+'%%')\n"
    "      +'<div style=\"display:flex;align-items:center;gap:8px;'\n"
    "      +'margin:5px 0\"><span style=\"width:92px;flex-shrink:0\">'\n"
    "      +'LIMITER</span><button id=\"syn-cmp\" style=\"flex:1;'\n"
    "      +'font-family:inherit;font-size:8px;padding:3px;'\n"
    "      +'cursor:pointer;background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">'\n"
    "      +(syn.cmp?'ON':'OFF')+'</button></div>'\n"
    "      +synRow('t1','CALM BELOW',2,30,1,Math.round(syn.t1*10),\n"
    "        syn.t1.toFixed(1))\n"
    "      +synRow('t2','TENSE AT',5,60,1,Math.round(syn.t2*10),\n"
    "        syn.t2.toFixed(1))\n"
    "      +synRow('t3','PAIN AT',10,80,1,Math.round(syn.t3*10),\n"
    "        syn.t3.toFixed(1))\n"
    "      +synSel('cs','CHORD EVERY',[[3,'8 STEPS'],[4,'16 STEPS'],\n"
    "        [5,'32 STEPS']],syn.cs)\n"
    ...
    "      +'background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">FACTORY PATCH</button>'\n"
    "      +'<button id=\"syn-rnd\" style=\"flex:1;font-family:inherit;'\n"
    "      +'font-size:8px;padding:4px;cursor:pointer;'\n"
    "      +'background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2)\">RANDOM PATCH</button></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;line-height:1.9;'\n"
    "      +'margin-top:8px\">LIVE ON THE NEXT SCHEDULED STEP - ESC OUT'\n"
    "      +'</div>';\n"
    "    bx.innerHTML=h;\n"
    "    synOv.appendChild(bx);\n"
    "    document.body.appendChild(synOv);\n"
    "    dragBox(bx,document.getElementById('syn-ttl'));\n"
    "    /* pt arrow steppers: a click nudges the slider one step\n"
    "       and fires the same input path the slider fires, so\n"
    "       every binding works unchanged */\n"
    "    bx.addEventListener('click',function(e){\n"
    "      var a=e.target.closest('.sy-ar');\n"
    "      if(!a) return;\n"
    "      var el=document.getElementById('syn-'\n"
    "        +a.getAttribute('data-t'));\n"
    "      if(!el) return;\n"
    "      el.value=(+el.value)+(parseFloat(el.step)||1)\n"
    "        *(+a.getAttribute('data-d'));\n"
    "      el.dispatchEvent(new Event('input'));\n"
    "    });\n"
    "    synBind('bpm',function(el){\n"
    "      syn.bd=60/((+el.value)*4);beatDur=syn.bd;\n"
    "      dlApply(); /* the delay stays tempo-synced */\n"
    "      synV('bpm',el.value+' BPM');});\n"
    "    synBind('gen',function(el){genreApply(+el.value);});\n"
    "    synBind('mas',function(el){\n"
    "      if(gainM) gainM.gain.value=(+el.value)/100;\n"
    "      synV('mas',el.value+'%%');});\n"
    "    synBind('tr',function(el){syn.tr=+el.value;\n"
    "      synV('tr',el.value+' ST');});\n"
    "    synBind('wave',function(el){syn.wave=el.value;});\n"
    "    synBind('lg',function(el){syn.lg=(+el.value)/100;\n"
    "      synV('lg',el.value+'%%');});\n"
    "    synBind('ld',function(el){syn.ld=(+el.value)/1000;\n"
    "      synV('ld',el.value+' MS');});\n"
    "    synBind('fmi',function(el){syn.fmi=(+el.value)/100;\n"
    "      synV('fmi',el.value+'%%');});\n"
    "    synBind('bi',function(el){syn.bi=+el.value;});\n"
    "    synBind('bw',function(el){syn.bw=el.value;});\n"
    "    synBind('bg',function(el){syn.bg=(+el.value)/100;\n"
    "      synV('bg',el.value+'%%');});\n"
    "    synBind('st',function(el){\n"
    "      var v=+el.value;syn.st=v>12?99:v;\n"
    "      synV('st',v>12?'OFF':v+' BITS');});\n"
    "    synBind('bgx',function(el){syn.bgx=(+el.value)/100;\n"
    "      synV('bgx',el.value+'%%');});\n"
    "    synBind('pw',function(el){syn.pw=(+el.value)/100;\n"
    "      synV('pw',el.value+'%%');});\n"
    "    synBind('rv',function(el){syn.rv=(+el.value)/100;\n"
    "      rvApply();synV('rv',el.value+'%%');});\n"
    "    synBind('rt',function(el){syn.rt=(+el.value)/10;\n"
    "      rvDecay();synV('rt',syn.rt.toFixed(1)+' S');});\n"
    "    synBind('dw',function(el){syn.dw=(+el.value)/100;\n"
    "      dlApply();synV('dw',el.value+'%%');});\n"
    "    synBind('dt',function(el){syn.dt=+el.value;dlApply();});\n"
    "    synBind('df',function(el){syn.df=(+el.value)/100;\n"
    "      dlApply();synV('df',el.value+'%%');});\n"
    "    synBind('fc',function(el){\n"
    "      syn.fc=100*Math.pow(200,(+el.value)/100);\n"
    "      filtApply();\n"
    "      synV('fc',syn.fc>=1000?(syn.fc/1000).toFixed(1)+' K'\n"
    "        :Math.round(syn.fc)+' HZ');});\n"
    "    synBind('fq',function(el){syn.fq=(+el.value)/10;\n"
    "      filtApply();synV('fq',syn.fq.toFixed(1));});\n"
    "    synBind('fe',function(el){syn.fe=(+el.value)/100;\n"
    "      synV('fe',el.value+'%%');});\n"
    "    synBind('wr',function(el){syn.wr=(+el.value)/10;\n"
    "      filtApply();synV('wr',syn.wr.toFixed(1)+' HZ');});\n"
    "    synBind('wa',function(el){syn.wa=(+el.value)/100;\n"
    "      filtApply();synV('wa',el.value+'%%');});\n"
    "    synBind('sw',function(el){syn.sw=(+el.value)/100;\n"
    "      synV('sw',el.value+'%%');});\n"
    "    synBind('atk',function(el){syn.atk=(+el.value)/1000;\n"
    "      synV('atk',el.value+' MS');});\n"
    "    synBind('cb',function(el){\n"
    "      syn.cb=+el.value;crushApply();\n"
    "      synV('cb',syn.cb>=16?'OFF':syn.cb+' BITS');});\n"
    "    synBind('str',function(el){syn.str=+el.value;});\n"
    "    synBind('dtn',function(el){syn.dtn=+el.value;\n"
    "      synV('dtn',el.value+' CT');});\n"
    "    synBind('pmp',function(el){syn.pmp=(+el.value)/100;\n"
    "      synV('pmp',el.value+'%%');});\n"
    "    synBind('ww',function(el){syn.ww=(+el.value)/100;\n"
    "      synV('ww',el.value+' CT');});\n"
    "    synBind('sf',function(el){syn.sf=(+el.value)/100;\n"
    "      synV('sf',el.value+'%%');});\n"
    "    synBind('t1',function(el){syn.t1=(+el.value)/10;\n"
    "      synV('t1',syn.t1.toFixed(1));});\n"
    "    synBind('t2',function(el){syn.t2=(+el.value)/10;\n"
    "      synV('t2',syn.t2.toFixed(1));});\n"
    "    synBind('t3',function(el){syn.t3=(+el.value)/10;\n"
    "      synV('t3',syn.t3.toFixed(1));});\n"
    "    synBind('cs',function(el){syn.cs=+el.value;});\n"
    "    synBind('prog',function(el){synProg(+el.value);});\n"
    "    var fb2=document.getElementById('syn-fm');\n"
    "    if(fb2) fb2.addEventListener('click',function(){\n"
    "      fmToggle();fb2.textContent=fmOn?'ON':'OFF';});\n"
    "    var gb2=document.getElementById('syn-gr');\n"
    "    if(gb2) gb2.addEventListener('click',function(){\n"
    "      syn.gr=syn.gr?0:1;\n"
    "      gb2.textContent=syn.gr?'ON':'OFF';});\n"
    "    var pb2=document.getElementById('syn-pts');\n"
    "    if(pb2) pb2.addEventListener('click',function(){\n"
    "      syn.pts=syn.pts?0:1;\n"
    "      pb2.textContent=syn.pts?'ON':'OFF';\n"
    "      if(syn.pts) egg('parts');\n"
    "      showToast(syn.pts?'THE BAND LEARNS THEIR PARTS'\n"
    "        :'BACK TO THE CHORD');});\n"
    "    var mgb=document.getElementById('syn-mg');\n"
    "    if(mgb) mgb.addEventListener('click',function(){\n"
    "      syn.mg=syn.mg?0:1;\n"
    "      mgb.textContent=syn.mg?'ON':'OFF';\n"
    "      showToast(syn.mg?'THE MASKS CALL THE STEPS'\n"
    "        :'EVERY STEP FOR EVERYONE');});\n"
    "    var meb=document.getElementById('syn-me');\n"
    "    if(meb) meb.addEventListener('click',function(){\n"
    "      syn.me=syn.me?0:1;\n"
    "      meb.textContent=syn.me?'ON':'OFF';\n"
    "      if(!syn.me){sonEcoEnd=-1;\n"
    "        if(dlG){try{dlG.gain.value=syn.dw;}catch(e){}}}\n"
    "      showToast(syn.me?'THE FILE ECHOES ITSELF'\n"
    "        :'DRY, AS SHIPPED');});\n"
    "    var fxb=document.getElementById('syn-fx');\n"
    "    if(fxb) fxb.addEventListener('click',function(){\n"
    "      syn.fx=syn.fx?0:1;\n"
    "      fxb.textContent=syn.fx?'ON':'OFF';\n"
    "      showToast(syn.fx?'THE PATTERN PLAYS ITS EFFECTS'\n"
    "        :'NOTES ONLY');});\n"
    "    var ppb=document.getElementById('syn-pp');\n"
    "    if(ppb) ppb.addEventListener('click',function(){\n"
    "      syn.pp=syn.pp?0:1;dlApply();\n"
    "      ppb.textContent=syn.pp?'ON':'OFF';\n"
    "      showToast(syn.pp?'THE ECHO CROSSES THE FIELD'\n"
    "        :'THE ECHO STAYS HOME');});\n"
    "    var cb3=document.getElementById('syn-cmp');\n"
    ...
    "      if(synOv){synOv.remove();synOv=null;}\n"
    "      synStart(true);\n"
    "      showToast('FACTORY PATCH');});\n"
    "    var rn2=document.getElementById('syn-rnd');\n"
    "    if(rn2) rn2.addEventListener('click',synRandom);\n"
    "    egg('synth');\n"
    "    if(!quiet) showToast('PATCH AWAY');\n"
    "  }\n"
    "  /* the war for the table: the real decoder run again, watched\n"
    "     from the memory bus. the direct-mapped table is lossy - two\n"
    "     contexts hashing to one slot silently share a counter - and\n"
    "     the C side can never see it happen, because detecting it\n"
    "     costs memory the encoder cannot spend. the browser can:\n"
    "     a 16-bit shadow tag per slot (the hash bits the slot index\n"
    "     throws away, so detection is exact for -H >= 16) catches\n"
    "     every takeover as it happens. cyan = claimed, pink =\n"
    "     contested, and the verdict at the end is the one number\n"
    "     no card can print: how many predictions rode a counter\n"
    "     that two contexts were writing to */\n"
    "  var hwOn=false,hwOv=null,hwCv=null,hwCtx=null,hwRaf=null;\n"
    "  var hw=null,hwFr=0,hwTurbo=false,hwDone=false,hwHold=0;\n"
    "  var hwPB=0,hwSP=1,hwNPix=0,hwSlots=0;\n"
    "  var hwImg=null,hwPix=null,hwOccPx=null,hwCtPx=null,hwHeat=null;\n"
    "  var hwOcc=0,hwColl=0,hwCsl=0,hwTaint=0,hwProbes=0;\n"
    "  var hwTop=null,hwTopN=0,hwCollPrev=0;\n"
    "  var hwRegs=null,hwStat=null;\n"
    "  function hwInit(){\n"
    "    var H=paqHdr();\n"
    ...
    "    for(var i=0;i<31;i++) hw.value=((hw.value<<1)|cBit(hw,hw.cpos++))>>>0;\n"
    "  }\n"
    "  function hwStep(){\n"
    ...
    "      if(e0|e1){\n"
    "        if(d.tags[si]!==tag){\n"
    "          /* a different context is standing in this slot */\n"
    "          hwColl++;\n"
    "          d.tags[si]=tag;\n"
    "          var cbi=si>>3,cbm=1<<(si&7);\n"
    "          if(!(d.cb[cbi]&cbm)){\n"
    "            d.cb[cbi]|=cbm;hwCsl++;\n"
    "            hwCtPx[si>>>hwPB]=1;\n"
    "          }\n"
    "          if(hwTop[si]!==undefined) hwTop[si]++;\n"
    "          else if(hwTopN<4096){hwTop[si]=1;hwTopN++;}\n"
    "          hwHeat[si>>>hwPB]=255;\n"
    "        }\n"
    "        if(d.cb[si>>3]&(1<<(si&7))) hwTaint++;\n"
    "      } else {\n"
    "        d.tags[si]=tag;\n"
    "        hwOcc++;\n"
    "        var pi=si>>>hwPB;\n"
    "        if(hwOccPx[pi]<255) hwOccPx[pi]++;\n"
    "        if(hwHeat[pi]<90) hwHeat[pi]=90;\n"
    "      }\n"
    "      var sh=(1-(((e0+255)&(e1+255))>>8))*2+d.weights[m];\n"
    "      p0+=e0<<sh;p1+=e1<<sh;\n"
    "      d.sl[m]=s2;\n"
    ...
    "    for(var m=0;m<num;m++){\n"
    "      var s2=d.sl[m];\n"
    "      d.dt[s2+bit]=d.dt[s2+bit]+1;\n"
    "      if(d.dt[s2+1-bit]>1) d.dt[s2+1-bit]>>=1;\n"
    "    }\n"
    "    if(d.bp>0&&bit){var b2=d.bp-1;d.buf[8+(b2>>3)]|=1<<(7-(b2&7));}\n"
    "    d.bp++;\n"
    "  }\n"
    "  function hwTipHide(){ovTipHide();}\n"
    "  function hwTipAt(e,html){ovTipAt(hwOv,'var(--acc)',e,html);}\n"
    "  function hwVerdict(){\n"
    "    var pc=hwProbes?100*hwTaint/hwProbes:0;\n"
    "    var line;\n"
    "    if(!hwColl)\n"
    "      line='NO SLOT WAS EVER SHARED. THE TABLE WAS ROOMY.';\n"
    "    else if(pc<1)\n"
    "      line='MOSTLY PEACE: '+pc.toFixed(2)+'%% OF PREDICTIONS TAINTED';\n"
    "    else if(pc<5)\n"
    "      line=pc.toFixed(1)+'%% OF PREDICTIONS RODE A SHARED COUNTER';\n"
    "    else\n"
    "      line=pc.toFixed(1)+'%% TAINTED. THE TABLE IS TOO SMALL '\n"
    "        +'FOR THIS FILE';\n"
    "    var ws=-1,wc2=0;\n"
    "    for(var k in hwTop){\n"
    "      if(hwTop[k]>wc2){wc2=hwTop[k];ws=+k;}\n"
    "    }\n"
    "    if(ws>=0)\n"
    "      line+='<br>WORST SLOT 0x'+dcHex8(ws).slice(2)\n"
    "        +' \\u00b7 '+wc2+' TAKEOVERS';\n"
    "    hwStat.innerHTML=line;\n"
    "    hwStat.style.color=!hwColl?'#34d399'\n"
    "      :pc<1?'#34d399':pc<5?'#fbbf24':'#f87171';\n"
    "    /* the armies go home; the map of the war stays up */\n"
    "    hw.tags=null;hw.dt=null;hw.cb=null;\n"
    "  }\n"
    "  function hwStop(){\n"
    "    hwOn=false;hwDone=false;hwHold=0;\n"
    "    if(hwRaf){cancelAnimationFrame(hwRaf);hwRaf=null;}\n"
    "    if(hwOv){hwOv.remove();hwOv=null;}\n"
    "    hw=null;hwOccPx=null;hwCtPx=null;hwHeat=null;\n"
    "    hwPix=null;hwImg=null;hwTop=null;\n"
    "  }\n"
    "  function hwStart(){\n"
    "    if(hwOn) return;\n"
    "    if(typeof PAQ==='undefined'||!PAQ.b64){\n"
    ...
    "    ensureAudio();\n"
    "    try{hwInit();}catch(e){\n"
    "      showToast('THE BROWSER REFUSED THE BATTLEFIELD');\n"
    "      hw=null;return;}\n"
    "    egg('hashwar');\n"
    "    hwPB=PAQ.db>18?PAQ.db-18:0;\n"
    "    hwSP=1<<hwPB;\n"
    "    hwNPix=1<<Math.min(18,PAQ.db);\n"
    "    hwSlots=1<<PAQ.db;\n"
    "    hwOccPx=new Uint8Array(262144);\n"
    "    hwCtPx=new Uint8Array(262144);\n"
    "    hwHeat=new Uint8Array(262144);\n"
    "    hwOcc=0;hwColl=0;hwCsl=0;hwTaint=0;hwProbes=0;\n"
    "    hwTop={};hwTopN=0;hwCollPrev=0;\n"
    "    hwDone=false;hwTurbo=false;hwFr=0;hwHold=0;\n"
    "    hwOv=document.createElement('div');\n"
    "    hwOv.id='hashwar';\n"
    "    hwOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,4,.9);display:flex;align-items:center;'\n"
    "      +'justify-content:center';\n"
    "    hwOv.innerHTML='<div style=\"background:#0b0b10;'\n"
    "      +'border:2px solid var(--acc);padding:14px 18px;'\n"
    "      +'box-shadow:8px 8px 0 rgba(0,0,0,.6);'\n"
    "      +'font-family:var(--pix);font-size:8px;letter-spacing:1px;'\n"
    "      +'color:var(--fg)\">'\n"
    "      +'<div style=\"display:flex;justify-content:space-between;'\n"
    ...
    "      +'*** THE WAR FOR THE TABLE ***'\n"
    "      +'<span id=\"hw-x\" style=\"cursor:pointer\">[X]</span></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:6px\">'\n"
    "      +'HASH SPACE \\u00b7 2^'+PAQ.db+' SLOTS \\u00b7 '\n"
    "      +(hwSP>1?hwSP+' SLOTS/PIXEL':'1 SLOT/PIXEL')+'</div>'\n"
    "      +'<canvas id=\"hw-cv\" width=\"512\" height=\"512\" '\n"
    "      +'style=\"display:block;width:min(70vw,440px);aspect-ratio:1;'\n"
    "      +'image-rendering:pixelated;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);cursor:crosshair\"></canvas>'\n"
    "      +'<div style=\"display:flex;gap:14px;margin-top:6px;'\n"
    "      +'font-size:7px;color:var(--fg3)\">'\n"
    "      +'<span><span style=\"display:inline-block;width:8px;height:8px;'\n"
    "      +'background:#16b8d8\"></span> CLAIMED</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:8px;height:8px;'\n"
    "      +'background:#e05aba\"></span> CONTESTED</span>'\n"
    "      +'<span><span style=\"display:inline-block;width:8px;height:8px;'\n"
    "      +'background:#0a0a12;border:1px solid #26262f\"></span>'\n"
    "      +' EMPTY</span></div>'\n"
    "      +'<div id=\"hw-regs\" style=\"margin-top:8px;line-height:2.1;'\n"
    "      +'color:var(--fg2)\"></div>'\n"
    "      +'<div id=\"hw-st\" style=\"margin-top:4px;color:var(--yel);'\n"
    "      +'min-height:14px\"></div>'\n"
    "      +'<div style=\"margin-top:8px;color:var(--fg3);font-size:7px\">'\n"
    "      +'THE REAL DECODER, WATCHED FROM THE MEMORY BUS \\u00b7 '\n"
    "      +'SPACE = FFWD \\u00b7 ESC = LOOK AWAY</div></div>';\n"
    "    document.body.appendChild(hwOv);\n"
    "    hwCv=document.getElementById('hw-cv');\n"
    "    hwCtx=hwCv.getContext('2d');\n"
    "    hwImg=hwCtx.createImageData(512,512);\n"
    "    hwPix=new Uint32Array(hwImg.data.buffer);\n"
    "    hwRegs=document.getElementById('hw-regs');\n"
    "    hwStat=document.getElementById('hw-st');\n"
    "    document.getElementById('hw-x').addEventListener('click',hwStop);\n"
    "    hwCv.addEventListener('mousemove',function(e){\n"
    "      if(!hwOn){hwTipHide();return;}\n"
    "      var r=hwCv.getBoundingClientRect();\n"
    "      var x=((e.clientX-r.left)*(512/r.width))|0;\n"
    "      var y=((e.clientY-r.top)*(512/r.height))|0;\n"
    "      if(x<0||x>511||y<0||y>511){hwTipHide();return;}\n"
    "      var pi=y*512+x;\n"
    "      if(pi>=hwNPix){hwTipHide();return;}\n"
    "      var o=hwOccPx[pi];\n"
    "      hwTipAt(e,'SLOTS 0x'+dcHex8(pi*hwSP).slice(2)\n"
    "        +'\\u2013 0x'+dcHex8(pi*hwSP+hwSP-1).slice(2)\n"
    "        +'<br>CLAIMED '+o+'/'+hwSP+'<br>'\n"
    "        +(hwCtPx[pi]\n"
    "          ?'<span style=\"color:#f87171\">CONTESTED</span>'\n"
    "          :o?'<span style=\"color:#34d399\">AT PEACE</span>'\n"
    "          :'<span style=\"color:#63636f\">EMPTY LAND</span>'));\n"
    "    });\n"
    "    hwCv.addEventListener('mouseleave',hwTipHide);\n"
    "    hwOn=true;\n"
    "    showToast('EVERY CONTEXT NEEDS A HOME');\n"
    "    if(!hwRaf) hwRaf=requestAnimationFrame(hwFrame);\n"
    "  }\n"
    "  function hwFrame(){\n"
    "    if(!hwOn||!document.body.classList.contains('party')){\n"
    "      hwStop();return;\n"
    "    }\n"
    "    hwFr++;\n"
    "    if(!hwDone){\n"
    "      var n2=Math.max(8,Math.ceil(hw.bitlen/1100))*(hwTurbo?16:1);\n"
    "      while(hw.bp<hw.bitlen&&n2--) hwStep();\n"
    "      if(hw.bp>=hw.bitlen){hwDone=true;hwVerdict();}\n"
    "    }\n"
    "    /* a low knock when new fights broke out this frame */\n"
    "    if(hwColl>hwCollPrev&&!(hwFr&7))\n"
    "      blip(110+Math.random()*40,.02,.05);\n"
    "    hwCollPrev=hwColl;\n"
    "    /* compose the battlefield */\n"
    "    for(var i=0;i<262144;i++){\n"
    "      var h2=hwHeat[i];\n"
    "      if(h2) hwHeat[i]=h2=(h2*.88)|0;\n"
    "      var o=hwOccPx[i];\n"
    "      if(!o&&!h2){hwPix[i]=0xFF090306;continue;}\n"
    "      var t=o/hwSP; if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(hwCtPx[i]){r=140+100*t;g=45+30*t;b=120+60*t;}\n"
    "      else{r=8+30*t;g=28+180*t;b=40+195*t;}\n"
    "      if(h2){r+=h2;g+=h2*(hwCtPx[i]?.3:.8);b+=h2*(hwCtPx[i]?.4:.9);}\n"
    "      if(r>255)r=255; if(g>255)g=255; if(b>255)b=255;\n"
    "      hwPix[i]=0xFF000000|((b|0)<<16)|((g|0)<<8)|(r|0);\n"
    "    }\n"
    "    hwCtx.putImageData(hwImg,0,0);\n"
    "    var done=hw.bp>0?Math.min(hw.nb,(hw.bp-1)>>3):0;\n"
    "    hwRegs.innerHTML='BIT '+dfFmt(hw.bp)+'/'+dfFmt(hw.bitlen)\n"
    "      +' \\u00b7 BYTE '+dfFmt(done)+'/'+dfFmt(hw.nb)\n"
    "      +(hwTurbo?' \\u00b7 <span style=\"color:var(--yel)\">FFWD</span>':'')\n"
    "      +'<br>CLAIMED <span style=\"color:var(--acc)\">'+dfFmt(hwOcc)\n"
    "      +'</span> ('+(100*hwOcc/hwSlots).toFixed(2)+'%%)'\n"
    "      +' \\u00b7 MODELS '+hw.num\n"
    "      +'<br>COLLISIONS <span style=\"color:var(--pnk)\">'+dfFmt(hwColl)\n"
    "      +'</span> \\u00b7 CONTESTED '+dfFmt(hwCsl)\n"
    "      +(hwOcc?' ('+(100*hwCsl/hwOcc).toFixed(2)+'%% OF CLAIMED)':'')\n"
    "      +'<br>TAINTED PREDICTIONS '+dfFmt(hwTaint)\n"
    "      +(hwProbes?' ('+(100*hwTaint/hwProbes).toFixed(2)+'%%)':'');\n"
    "    if(hwDone){\n"
    "      /* let the last flashes cool, then hold the map still */\n"
    "      if(!hwHold) hwHold=hwFr;\n"
    "      if(hwFr-hwHold>120){hwRaf=null;return;}\n"
    "    }\n"
    "    hwRaf=requestAnimationFrame(hwFrame);\n"
    "  }\n"
    "  /* what if: the real decode runs once, shadowed by smaller\n"
    "     tables - same models, same stream, same update rule, just\n"
    "     fewer index bits. the real table decides every bit; the\n"
    "     shadows only pay for it, accumulating -log2 p of the bit\n"
    "     that actually happened. the codec is symmetric, so each\n"
    "     shadow's total IS the compressed size a real -H encode at\n"
    "     that size would emit for these models. the loader-RAM\n"
    "     question, answered in bytes, from inside the report */\n"
    "  var wfOn=false,wfOv=null,wfCv=null,wfCtx=null,wfRaf=null;\n"
    "  var wf=null,wfFr=0,wfTurbo=false,wfDone=false,wfHold=0;\n"
    "  var wfCost=null,wfSamp=null,wfLast=0;\n"
    "  var wfRegs=null,wfStat=null;\n"
    "  var wfCols=['#22d3ee','#fbbf24','#fb923c','#f87171'];\n"
    "  function wfRam(db2){\n"
    "    var b=2<<db2;\n"
    ...
    "      range:0x80000000,low:0,value:0,cpos:0,bp:0};\n"
    "    for(var i=0;i<31;i++) wf.value=((wf.value<<1)|cBit(wf,wf.cpos++))>>>0;\n"
    "  }\n"
    "  function wfStep(){\n"
    ...
    "        var t=d.tabs[k];\n"
    "        var e0=t[s2],e1=t[s2+1];\n"
    "        var sh=(1-(((e0+255)&(e1+255))>>8))*2+d.weights[m];\n"
    "        P0[k]+=e0<<sh;P1[k]+=e1<<sh;\n"
    "        d.sl[m*ns+k]=s2;\n"
    "      }\n"
    "    }\n"
    "    /* the real table (k=0) decides; the shadows only pay */\n"
    "    var p0=P0[0],p1=P1[0],total=p0+p1;\n"
    "    var th=Number(BigInt(d.range)*BigInt(p1)/BigInt(total));\n"
    "    var bit;\n"
    "    if(((d.value-d.low)>>>0)<th){d.range=th;bit=1;}\n"
    "    else{d.low=(d.low+th)>>>0;d.range=(d.range-th)>>>0;bit=0;}\n"
    ...
    "    }\n"
    "    for(var k=0;k<ns;k++){\n"
    "      wfCost[k]-=Math.log2((bit?P1[k]:P0[k])/(P0[k]+P1[k]));\n"
    "      var t=d.tabs[k];\n"
    "      for(var m=0;m<num;m++){\n"
    "        var s2=d.sl[m*ns+k];\n"
    "        t[s2+bit]=t[s2+bit]+1;\n"
    "        if(t[s2+1-bit]>1) t[s2+1-bit]>>=1;\n"
    "      }\n"
    "    }\n"
    "    if(d.bp>0&&bit){var b2=d.bp-1;d.buf[8+(b2>>3)]|=1<<(7-(b2&7));}\n"
    "    d.bp++;\n"
    ...
    "  function wfVerdict(){\n"
    "    var hdrB=wf.co;\n"
    "    var a=hdrB+wfCost[0]/8;\n"
    "    var fileB=(typeof REPORT_ID!=='undefined')?REPORT_ID.outb:0;\n"
    "    var line='MEASURED '+a.toFixed(1)+' B';\n"
    "    if(fileB){\n"
    "      line+=' \\u00b7 THE FILE IS '+fileB+' B';\n"
    "      if(a-fileB<2.5&&fileB-a<2.5) line+=' \\u00b7 THE MATH HOLDS';\n"
    "    }\n"
    "    if(wf.ns>1){\n"
    "      var worst=(wfCost[wf.ns-1]-wfCost[0])/8;\n"
    "      line+='<br>'+(worst<1\n"
    "        ?'THE BIG TABLE IS VANITY. SHIP THE SMALL ONE.'\n"
    "        :worst<8?'A FEW BYTES BUY A LOT OF RAM'\n"
    "        :'RAM WELL SPENT');\n"
    "      wfStat.style.color=worst<1?'#34d399'\n"
    "        :worst<8?'#fbbf24':'#f87171';\n"
    "    } else {\n"
    "      line+='<br>THE TABLE IS ALREADY TINY';\n"
    "      wfStat.style.color='#34d399';\n"
    "    }\n"
    "    wfStat.innerHTML=line;\n"
    "    wf.tabs=null; /* the shadows go home */\n"
    "  }\n"
    "  function wfStop(){\n"
    "    wfOn=false;wfDone=false;wfHold=0;\n"
    "    if(wfRaf){cancelAnimationFrame(wfRaf);wfRaf=null;}\n"
    "    if(wfOv){wfOv.remove();wfOv=null;}\n"
    "    wf=null;wfCost=null;wfSamp=null;\n"
    "  }\n"
    "  function wfStart(){\n"
    "    if(wfOn) return;\n"
    "    if(typeof PAQ==='undefined'||!PAQ.b64){\n"
    ...
    "    ensureAudio();\n"
    "    try{wfInit();}catch(e){\n"
    "      showToast('THE BROWSER REFUSED THE LEDGER');\n"
    "      wf=null;return;}\n"
    "    egg('whatif');\n"
    "    wfCost=new Float64Array(wf.ns);\n"
    "    wfSamp=new Float32Array(wf.nb*wf.ns);\n"
    "    wfLast=0;wfDone=false;wfTurbo=false;wfFr=0;wfHold=0;\n"
    "    wfOv=document.createElement('div');\n"
    "    wfOv.id='whatif';\n"
    "    wfOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,4,.9);display:flex;align-items:center;'\n"
    "      +'justify-content:center';\n"
    "    wfOv.innerHTML='<div style=\"background:#0b0b10;'\n"
    "      +'border:2px solid var(--blu);padding:14px 18px;'\n"
    "      +'box-shadow:8px 8px 0 rgba(0,0,0,.6);'\n"
    "      +'font-family:var(--pix);font-size:8px;letter-spacing:1px;'\n"
    "      +'color:var(--fg)\">'\n"
    "      +'<div style=\"display:flex;justify-content:space-between;'\n"
    "      +'gap:30px;color:var(--blu);font-size:10px;margin-bottom:10px;'\n"
    "      +'text-shadow:2px 2px 0 rgba(255,63,216,.5)\">'\n"
    "      +'*** WHAT IF: A SMALLER TABLE ***'\n"
    "      +'<span id=\"wf-x\" style=\"cursor:pointer\">[X]</span></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:6px\">'\n"
    "      +'CUMULATIVE COMPRESSED BYTES OVER THE FILE, PER TABLE</div>'\n"
    "      +'<canvas id=\"wf-cv\" width=\"460\" height=\"220\" '\n"
    "      +'style=\"display:block;width:min(70vw,460px);'\n"
    "      +'background:#05050a;border:1px solid var(--bdr2)\"></canvas>'\n"
    "      +'<div id=\"wf-regs\" style=\"margin-top:8px;line-height:2.1;'\n"
    "      +'color:var(--fg2)\"></div>'\n"
    "      +'<div id=\"wf-st\" style=\"margin-top:4px;color:var(--yel);'\n"
    "      +'min-height:14px\"></div>'\n"
    "      +'<div style=\"margin-top:8px;color:var(--fg3);font-size:7px\">'\n"
    "      +'SAME MODELS \\u00b7 SAME STREAM \\u00b7 FEWER INDEX BITS \\u00b7 '\n"
    "      +'SPACE = FFWD \\u00b7 ESC = CLOSE</div></div>';\n"
    "    document.body.appendChild(wfOv);\n"
    "    wfCv=document.getElementById('wf-cv');\n"
    "    wfCtx=wfCv.getContext('2d');\n"
    "    wfRegs=document.getElementById('wf-regs');\n"
    "    wfStat=document.getElementById('wf-st');\n"
    "    document.getElementById('wf-x').addEventListener('click',wfStop);\n"
    "    wfOn=true;\n"
    "    showToast('SAME BITS, SMALLER HOUSE');\n"
    "    if(!wfRaf) wfRaf=requestAnimationFrame(wfFrame);\n"
    "  }\n"
    "  function wfFrame(){\n"
    "    if(!wfOn||!document.body.classList.contains('party')){\n"
    "      wfStop();return;\n"
    "    }\n"
    "    wfFr++;\n"
    "    if(!wfDone){\n"
    "      var n2=Math.max(8,Math.ceil(wf.bitlen/1100))*(wfTurbo?16:1);\n"
    "      while(wf.bp<wf.bitlen&&n2--) wfStep();\n"
    "      if(wf.bp>=wf.bitlen){wfDone=true;wfVerdict();}\n"
    "    }\n"
    "    var done=wf.bp>0?Math.min(wf.nb,(wf.bp-1)>>3):0;\n"
    "    for(var bi=wfLast;bi<done;bi++)\n"
    "      for(var k=0;k<wf.ns;k++) wfSamp[bi*wf.ns+k]=wfCost[k]/8;\n"
    "    wfLast=done;\n"
    "    var W=wfCv.width,H=wfCv.height,ctx=wfCtx;\n"
    "    ctx.fillStyle='#05050a';ctx.fillRect(0,0,W,H);\n"
    "    var maxB=1;\n"
    "    for(var k=0;k<wf.ns;k++) if(wfCost[k]/8>maxB) maxB=wfCost[k]/8;\n"
    "    maxB*=1.06;\n"
    "    ctx.strokeStyle='rgba(58,58,72,.6)';ctx.lineWidth=1;\n"
    "    for(var g2=1;g2<4;g2++){\n"
    "      var gy=(H*g2/4)|0;\n"
    "      ctx.beginPath();ctx.moveTo(0,gy);ctx.lineTo(W,gy);ctx.stroke();\n"
    "    }\n"
    "    if(wfLast>1){\n"
    "      var ds=Math.max(1,Math.ceil(wf.nb/W));\n"
    "      for(var k=wf.ns-1;k>=0;k--){\n"
    "        ctx.strokeStyle=wfCols[k];\n"
    "        ctx.lineWidth=k===0?1.6:1.1;\n"
    "        ctx.beginPath();\n"
    "        var first=true;\n"
    "        for(var bi=0;bi<wfLast;bi+=ds){\n"
    "          var x=bi/wf.nb*W;\n"
    "          var y=H-2-(wfSamp[bi*wf.ns+k]/maxB)*(H-6);\n"
    ...
    "    }\n"
    "    var hdrB=wf.co;\n"
    "    var d0=hdrB+wfCost[0]/8;\n"
    "    var h='BYTE '+dfFmt(done)+'/'+dfFmt(wf.nb)\n"
    "      +(wfTurbo?' \\u00b7 <span style=\"color:var(--yel)\">FFWD</span>':'')\n"
    "      +'<br>';\n"
    "    for(var k=0;k<wf.ns;k++){\n"
    "      var by=hdrB+wfCost[k]/8;\n"
    "      h+='<span style=\"color:'+wfCols[k]+'\">2^'+wf.dbs[k]+'</span> '\n"
    "        +by.toFixed(1)+' B'\n"
    "        +(k===0?' \\u00b7 THE REAL ONE'\n"
    "          :' \\u00b7 +'+(by-d0).toFixed(1)+' B')\n"
    "        +' \\u00b7 '+wfRam(wf.dbs[k])+'<br>';\n"
    ...
    "    wfRegs.innerHTML=h;\n"
    "    if(wfDone){\n"
    "      if(!wfHold) wfHold=wfFr;\n"
    "      if(wfFr-wfHold>30){wfRaf=null;return;}\n"
    "    }\n"
    "    wfRaf=requestAnimationFrame(wfFrame);\n"
    "  }\n"
    "  /* sabotage: flip one bit of the compressed stream and run\n"
    "     the real decoder on the wound. output is diffed against\n"
    "     the truth live - perfect until the flipped bit is read,\n"
    "     wreckage after. the proof that maximal compression means\n"
    "     zero redundancy: every bit is load-bearing, the stream is\n"
    "     one long conspiracy, and one defector ends it */\n"
    "  var sbOn=false,sbOv=null,sbCv=null,sbCtx=null,sbRaf=null;\n"
    "  var sbCC=null,sbCCtx=null,sb=null,sbFr=0,sbTurbo=false;\n"
    "  var sbRun=false,sbDone=false,sbWj=-1,sbWb=0,sbDiv=-1;\n"
    "  var sbMatch=0,sbDrawn=0;\n"
    "  var sbCols=64,sbCell=4,sbCCols=64,sbCCell=2,sbNbc=0;\n"
    "  var sbRegs=null,sbStat=null;\n"
    "  function sbInit(){\n"
    "    var H=paqHdr();\n"
    ...
    "  }\n"
    "  function sbPaintIn(){\n"
    "    var cpad=sbCCell>2?1:0;\n"
    "    for(var i=0;i<sbNbc;i++){\n"
    "      if(i===sbWj){\n"
    "        sbCCtx.fillStyle='#ffe33f';\n"
    "      } else {\n"
    "        sbCCtx.fillStyle=grayB(sb.cd0[sb.co+i]);\n"
    "      }\n"
    "      sbCCtx.fillRect((i%%sbCCols)*sbCCell,((i/sbCCols)|0)*sbCCell,\n"
    "        sbCCell-cpad,sbCCell-cpad);\n"
    "    }\n"
    "  }\n"
    "  function sbWoundAt(j){\n"
    "    sbWj=j;sbWb=(Math.random()*8)|0;\n"
    "    egg('sabotage');\n"
    "    sb.cd.set(sb.cd0);\n"
    "    sb.cd[sb.co+j]^=1<<sbWb;\n"
    "    sb.dt.fill(0);sb.buf.fill(0);\n"
    "    sb.range=0x80000000;sb.low=0;sb.value=0;sb.cpos=0;sb.bp=0;\n"
    "    for(var i=0;i<31;i++) sb.value=((sb.value<<1)|cBit(sb,sb.cpos++))>>>0;\n"
    "    sbRun=true;sbDone=false;sbDiv=-1;sbMatch=0;sbDrawn=0;\n"
    "    sbCtx.fillStyle='#05050a';\n"
    "    sbCtx.fillRect(0,0,sbCv.width,sbCv.height);\n"
    "    sbPaintIn();\n"
    "    sbStat.style.color='var(--yel)';\n"
    "    sbStat.textContent='BIT '+sbWb+' OF 0x'+blkHex(j)+' FLIPPED. '\n"
    "      +'DECODING THE WOUNDED STREAM.';\n"
    "    blip(700,.04,.08);\n"
    "  }\n"
    "  function sbStop(){\n"
    "    sbOn=false;sbRun=false;sbDone=false;\n"
    "    if(sbRaf){cancelAnimationFrame(sbRaf);sbRaf=null;}\n"
    "    if(sbOv){sbOv.remove();sbOv=null;}\n"
    "    sb=null; /* the table goes with it */\n"
    "  }\n"
    "  function sbStart(){\n"
    "    if(sbOn) return;\n"
    "    if(typeof PAQ==='undefined'||!PAQ.b64||\n"
    "       typeof BD==='undefined'||!BD.length){\n"
    "      showToast('NO PAYLOAD ABOARD');return;}\n"
    "    ensureAudio();\n"
    "    try{sbInit();}catch(e){\n"
    "      showToast('THE BROWSER REFUSED THE OPERATING THEATER');\n"
    "      sb=null;return;}\n"
    "    sbWj=-1;sbWb=0;sbRun=false;sbDone=false;sbDiv=-1;\n"
    "    sbMatch=0;sbDrawn=0;sbTurbo=false;sbFr=0;\n"
    "    var cw=Math.min(window.innerWidth-72,600);\n"
    "    var szs=[8,7,6,5,4,3,2];\n"
    "    sbCell=2;\n"
    "    for(var k=0;k<szs.length;k++){\n"
    "      var c2=(cw/szs[k])|0;\n"
    "      if(Math.ceil(sb.nb/c2)*szs[k]<=260){sbCell=szs[k];break;}\n"
    "    }\n"
    "    sbCols=Math.max(1,(cw/sbCell)|0);\n"
    "    var rows=Math.max(1,Math.ceil(sb.nb/sbCols));\n"
    "    sbNbc=sb.cd0.length-sb.co;\n"
    "    var cszs=[6,5,4,3,2,1];\n"
    "    sbCCell=1;\n"
    "    for(var k=0;k<cszs.length;k++){\n"
    "      var c3=(cw/cszs[k])|0;\n"
    "      if(Math.ceil(sbNbc/c3)*cszs[k]<=90){sbCCell=cszs[k];break;}\n"
    "    }\n"
    "    sbCCols=Math.max(1,(cw/sbCCell)|0);\n"
    "    var crows=Math.max(1,Math.ceil(sbNbc/sbCCols));\n"
    "    sbOv=document.createElement('div');\n"
    "    sbOv.id='sabotage';\n"
    "    sbOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,4,.9);display:flex;align-items:center;'\n"
    "      +'justify-content:center';\n"
    "    sbOv.innerHTML='<div style=\"background:#0b0b10;'\n"
    "      +'border:2px solid var(--red);padding:14px 18px;'\n"
    "      +'box-shadow:8px 8px 0 rgba(0,0,0,.6);'\n"
    "      +'font-family:var(--pix);font-size:8px;letter-spacing:1px;'\n"
    "      +'color:var(--fg)\">'\n"
    "      +'<div style=\"display:flex;justify-content:space-between;'\n"
    "      +'gap:30px;color:var(--red);font-size:10px;margin-bottom:10px;'\n"
    "      +'text-shadow:2px 2px 0 rgba(255,63,216,.5)\">*** SABOTAGE ***'\n"
    "      +'<span id=\"sb-x\" style=\"cursor:pointer\">[X]</span></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:4px\">'\n"
    "      +'COMPRESSED IN \\u00b7 CLICK A BYTE TO FLIP ONE OF ITS BITS</div>'\n"
    "      +'<canvas id=\"sb-cc\" width=\"'+(sbCCols*sbCCell)+'\" height=\"'\n"
    "      +(crows*sbCCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);margin-bottom:8px;'\n"
    "      +'cursor:crosshair\"></canvas>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:4px\">'\n"
    "      +'DECODED OUT VS THE TRUTH \\u00b7 '\n"
    "      +'<span style=\"color:#34d399\">MATCHES</span> \\u00b7 '\n"
    "      +'<span style=\"color:#f87171\">WRECKAGE</span></div>'\n"
    "      +'<canvas id=\"sb-cv\" width=\"'+(sbCols*sbCell)+'\" height=\"'\n"
    "      +(rows*sbCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2)\"></canvas>'\n"
    "      +'<div id=\"sb-regs\" style=\"margin-top:8px;line-height:2.1;'\n"
    "      +'color:var(--fg2)\"></div>'\n"
    "      +'<div id=\"sb-st\" style=\"margin-top:4px;color:var(--yel);'\n"
    "      +'min-height:14px\">PICK YOUR BIT. ANY BIT.</div>'\n"
    "      +'<div style=\"margin-top:8px;color:var(--fg3);font-size:7px\">'\n"
    "      +'THE REAL DECODER ON A WOUNDED STREAM \\u00b7 '\n"
    "      +'SPACE = FFWD \\u00b7 ESC = WALK AWAY</div></div>';\n"
    "    document.body.appendChild(sbOv);\n"
    "    sbCv=document.getElementById('sb-cv');\n"
    "    sbCtx=sbCv.getContext('2d');\n"
    "    sbCC=document.getElementById('sb-cc');\n"
    "    sbCCtx=sbCC.getContext('2d');\n"
    "    sbRegs=document.getElementById('sb-regs');\n"
    "    sbStat=document.getElementById('sb-st');\n"
    "    document.getElementById('sb-x').addEventListener('click',sbStop);\n"
    "    sbCC.addEventListener('click',function(e){\n"
    "      if(!sbOn||!sb) return;\n"
    "      var j=dcCellIdx(e,sbCC,sbCCols,sbCCell);\n"
    "      if(j>=0&&j<sbNbc) sbWoundAt(j);\n"
    "    });\n"
    "    sbPaintIn();\n"
    "    sbOn=true;\n"
    "    showToast('PICK YOUR BIT. ANY BIT.');\n"
    "    if(!sbRaf) sbRaf=requestAnimationFrame(sbFrame);\n"
    "  }\n"
    "  function sbFrame(){\n"
    "    if(!sbOn||!document.body.classList.contains('party')){\n"
    "      sbStop();return;\n"
    "    }\n"
    "    sbFr++;\n"
    "    if(sbRun&&!sbDone){\n"
    "      var n2=Math.max(8,Math.ceil(sb.bitlen/560))*(sbTurbo?16:1);\n"
    "      while(sb.bp<sb.bitlen&&n2--) decStep(sb);\n"
    "      if(sb.bp>=sb.bitlen) sbDone=true;\n"
    "    }\n"
    "    var done=sb.bp>0?Math.min(sb.nb,(sb.bp-1)>>3):0;\n"
    "    var pad=sbCell>2?1:0;\n"
    "    for(;sbDrawn<done;sbDrawn++){\n"
    "      var rb=dcRefByte(sbDrawn);\n"
    "      var ok=rb<0||sb.buf[8+sbDrawn]===rb;\n"
    "      if(ok) sbMatch++;\n"
    "      else if(sbDiv<0){\n"
    "        sbDiv=sbDrawn;\n"
    "        showToast('THE ROT SETS IN AT 0x'+blkHex(sbDrawn));\n"
    "        blip(90,.05,.3);\n"
    "      }\n"
    "      sbCtx.fillStyle=ok?'#1d7a55':'#c8323c';\n"
    "      sbCtx.fillRect((sbDrawn%%sbCols)*sbCell,\n"
    "        ((sbDrawn/sbCols)|0)*sbCell,sbCell-pad,sbCell-pad);\n"
    "    }\n"
    "    if(sbWj<0)\n"
    "      sbRegs.innerHTML='THE STREAM IS PRISTINE.';\n"
    "    else\n"
    "      sbRegs.innerHTML='WOUND: BIT '+sbWb+' OF IN 0x'+blkHex(sbWj)\n"
    "        +' \\u00b7 BYTE '+dfFmt(done)+'/'+dfFmt(sb.nb)\n"
    "        +(sbTurbo?' \\u00b7 <span style=\"color:var(--yel)\">'\n"
    "          +'FFWD</span>':'');\n"
    "    if(sbDone&&sbRun){\n"
    "      sbRun=false;\n"
    "      var surv=sbDiv<0?sb.nb:sbDiv;\n"
    "      var lucky=sbMatch-surv;\n"
    "      if(sbDiv<0){\n"
    "        sbStat.style.color='#34d399';\n"
    "        sbStat.textContent='NO DAMAGE. YOU FLIPPED DEAD AIR.';\n"
    "      } else {\n"
    "        sbStat.style.color='#f87171';\n"
    "        sbStat.innerHTML=surv+' BYTES SURVIVED \\u00b7 '\n"
    "          +(sb.nb-sbMatch)+' RUINED \\u00b7 '+lucky+' LUCKY MATCH'\n"
    "          +(lucky===1?'':'ES')+' IN THE WRECKAGE';\n"
    "        showToast('ONE BIT. THAT IS ALL IT TAKES.');\n"
    "      }\n"
    "    }\n"
    "    sbRaf=requestAnimationFrame(sbFrame);\n"
    "  }\n"
    "  /* recrunch: the missing half of decrunch - the ENCODER, ported\n"
    "     the same way. same header, same models, same table, same\n"
    "     mixer, plus the range coder run forward, mirroring the C side\n"
    "     call for call. it presses the file as it stands (tuner edits\n"
    "     and all) into a real .paq and swaps it in as PAQ, so decrunch,\n"
    "     hashwar, whatif and sabotage all read the new pressing. the\n"
    "     model set stays the shipped one - the search lives in C and\n"
    "     is not invited */\n"
    "  var rcOn=false,rcOv=null,rcCv=null,rcCtx=null,rcCC=null,rcCCtx=null;\n"
    "  var rcRaf=null,rc=null,rcFr=0,rcHold=0,rcTurbo=false,rcPaused=false;\n"
    "  var rcDone=false,rcDrawn=0,rcODrawn=0,rcDirty=null;\n"
    "  var rcCols=64,rcCell=4,rcCCols=64,rcCCell=2,rcNbc=0,rcCmax=8;\n"
    "  var rcRegs=null,rcStat=null,rcBlipT=0,rcBytes=null;\n"
    "  function rcB64(u8){\n"
    "    var s='';\n"
    ...
    "    return btoa(s);\n"
    "  }\n"
    "  /* propagate_carry, verbatim - flips below the write head are\n"
    "     real carry ripples into settled bytes, and those get flagged\n"
    "     for the yellow flash */\n"
    "  function rcCarry(pos){\n"
    "    var out=rc.out,head=(rc.bitPos>0?rc.bitPos>>3:0);\n"
    "    while(--pos>=0){\n"
    "      var mk=1<<(pos&7),i=pos>>3,by=out[i];\n"
    "      out[i]=by^mk;\n"
    "      if(i<head&&rcDirty) rcDirty.push({i:i,f:rcFr});\n"
    "      if(!(by&mk)) break;\n"
    "    }\n"
    "  }\n"
    "  /* one bit through the press: same table, same mixer as the\n"
    "     decoder, then the exact mirror of its threshold split -\n"
    "     thresh = range*p1/total, a 1 takes the low interval, a 0\n"
    "     takes the rest and may carry into bits already written */\n"
    "  function rcStep(){\n"
    "    var d=rc,p0=PAQ.bp,p1=PAQ.bp,num=d.num;\n"
    "    var bp=d.bp,b2=bp-1;\n"
    "    var bit=(bp===0)?1:(d.buf[8+(b2>>3)]>>(7-(b2&7)))&1;\n"
    "    for(var m=0;m<num;m++){\n"
    "      var h=(bp===0)?dcHash0(d.ext[m]):ctxHash(d.buf,b2,d.ext[m]);\n"
    "      var s2=(h>>>(32-PAQ.db))*2;\n"
    "      var e0=d.dt[s2],e1=d.dt[s2+1];\n"
    "      var sh=(1-(((e0+255)&(e1+255))>>8))*2+d.weights[m];\n"
    "      p0+=e0<<sh;p1+=e1<<sh;\n"
    "      d.sl[m]=s2;\n"
    "    }\n"
    "    var th=Number(BigInt(d.range)*BigInt(p1)/BigInt(p0+p1));\n"
    "    if(bit){d.range=th;}\n"
    "    else{\n"
    "      var ol=d.low;\n"
    "      d.low=(d.low+th)>>>0;\n"
    "      if(d.low<ol) rcCarry(d.bitPos);\n"
    "      d.range=(d.range-th)>>>0;\n"
    "    }\n"
    "    while(d.range<0x80000000){\n"
    "      d.bitPos++;\n"
    "      if(d.low>>>31) rcCarry(d.bitPos);\n"
    "      d.low=(d.low<<1)>>>0;\n"
    "      d.range=(d.range<<1)>>>0;\n"
    "    }\n"
    "    for(var m=0;m<num;m++){\n"
    ...
    "      if(d.dt[s2+1-bit]>1) d.dt[s2+1-bit]>>=1;\n"
    "    }\n"
    "    if(bp>0) d.costs[b2>>3]+=-Math.log2((bit?p1:p0)/(p0+p1));\n"
    "    d.bp++;\n"
    "  }\n"
    "  function rcFinishUp(){\n"
    "    rcDone=true;\n"
    "    var d=rc;\n"
    "    /* arith_finish, verbatim */\n"
    "    var pos=d.bitPos;\n"
    "    if(d.low>0){\n"
    "      if(((d.low+d.range-1)>>>0)>=d.low) pos++;\n"
    "      rcCarry(pos);\n"
    "    }\n"
    "    d.bits=pos;\n"
    "    d.pay=(pos+7)>>3;\n"
    "    var H=d.H;\n"
    "    var oldPay=H.cd.length-H.co,oldFile=H.cd.length;\n"
    "    var same=d.pay===oldPay;\n"
    "    if(same) for(var i=0;i<d.pay;i++)\n"
    "      if(H.cd[H.co+i]!==d.out[i]){same=false;break;}\n"
    "    var tn=0;\n"
    "    if(window.TUNED)\n"
    "      for(var i=0;i<window.TUNED.length;i++)\n"
    "        if(window.TUNED[i]) tn++;\n"
    "    if(!tn&&!rcBytes&&!d.mo&&!same){\n"
    "      /* pressing the untouched file must reproduce the shipped\n"
    "         payload bit for bit. anything else is a real bug */\n"
    "      try{console.error('recrunch self-test failed: payload '\n"
    "        +d.pay+' B vs shipped '+oldPay+' B');}catch(e){}\n"
    "      rcStat.style.color='#f87171';\n"
    "      rcStat.textContent='PAYLOAD '+d.pay+' B \\u2260 SHIPPED '\n"
    "        +oldPay+' B. THE ENCODER DISSENTS.';\n"
    "      showToast('THE ENCODER DISSENTS');\n"
    "      showGuru();\n"
    "      d.dt=null;d.sl=null;\n"
    "      return;\n"
    "    }\n"
    "    /* the swap: from here on PAQ IS the new pressing and every\n"
    "       instrument reads it. codec overlays holding the old stream\n"
    "       close first. reload the page for the shipped file */\n"
    "    if(dcOn) dcStop();\n"
    "    if(hwOn) hwStop();\n"
    "    if(wfOn) wfStop();\n"
    "    if(sbOn) sbStop();\n"
    "    var file;\n"
    "    if(d.mo){\n"
    "      /* new lineup, new header: the bitlen field is kept, the\n"
    "         wmask and masks are rewritten exactly as the C side\n"
    "         lays them out */\n"
    "      var lbB=PAQ.lf?4:2,hco=lbB+4+d.mo.masks.length;\n"
    "      file=new Uint8Array(hco+d.pay);\n"
    "      file.set(H.cd.subarray(0,lbB));\n"
    "      file[lbB]=d.mo.wmask&255;\n"
    "      file[lbB+1]=(d.mo.wmask>>>8)&255;\n"
    "      file[lbB+2]=(d.mo.wmask>>>16)&255;\n"
    "      file[lbB+3]=(d.mo.wmask>>>24)&255;\n"
    "      for(var i=0;i<d.mo.masks.length;i++)\n"
    "        file[lbB+4+i]=d.mo.masks[i];\n"
    "      file.set(d.out.subarray(0,d.pay),hco);\n"
    "    } else {\n"
    "      file=new Uint8Array(H.co+d.pay);\n"
    "      file.set(H.cd.subarray(0,H.co));\n"
    "      file.set(d.out.subarray(0,d.pay),H.co);\n"
    "    }\n"
    "    var c=0xFFFFFFFF;\n"
    "    for(var i=0;i<d.bytes.length;i++) c=dcCrc(c,d.bytes[i]);\n"
    "    PAQ.b64=rcB64(file);\n"
    "    paqH=null;fpH=null;\n"
    "    if(d.mo){\n"
    "      /* the lineup changed with the header: the tuner's scoring\n"
    "         worker still holds the old one - it goes home and gets\n"
    "         re-hired against the new header on next use */\n"
    "      if(tuWk){try{tuWk.terminate();}catch(e){}\n"
    "        tuWk=null;tuWkOk=false;}\n"
    "      auSet=null;auFlags=null;auSwapped=true;\n"
    "      if(auOn) auStop();\n"
    "    }\n"
    "    rcBytes=d.bytes;\n"
    "    if(typeof REPORT_ID!=='undefined'){\n"
    "      REPORT_ID.crc='0x'+dcHex8((c^0xFFFFFFFF)>>>0);\n"
    "      REPORT_ID.outb=file.length;\n"
    "      REPORT_ID.ratio=100*file.length/REPORT_ID.inb;\n"
    "    }\n"
    "    egg('recrunch');\n"
    "    var dd=file.length-oldFile;\n"
    "    rcStat.style.color='#34d399';\n"
    "    rcStat.textContent=(same?'BYTE-IDENTICAL TO THE SHIPPED'\n"
    "      +' PAYLOAD. ':'')+'PAYLOAD '+oldPay+' \\u2192 '+d.pay+' B ('\n"
    "      +(dd>=0?'+':'')+dd+' B). PAQ SWAPPED IN.'\n"
    "      +(d.mo?' NEW LINEUP: '+d.mo.masks.length+' MODELS.':'');\n"
    "    showToast(same?'SAME BITS. THE PRESS IS TRUE'\n"
    "      :'A NEW PRESSING. DECRUNCH IT');\n"
    "    blip(523,.05,.12);\n"
    "    setTimeout(function(){blip(659,.05,.12);},130);\n"
    "    setTimeout(function(){blip(784,.05,.12);},260);\n"
    "    setTimeout(function(){blip(1046,.07,.3);},390);\n"
    "    /* the table goes home; the finished picture stays up */\n"
    "    d.dt=null;d.sl=null;\n"
    "  }\n"
    "  function rcStepTick(){\n"
    "    var now=Date.now();\n"
    "    if(now-rcBlipT<40) return;\n"
    "    rcBlipT=now;\n"
    "    blip(300+((rc&&rc.bp>1&&rc.costs[(rc.bp-2)>>3]>4)?180:0),.02,.03);\n"
    "  }\n"
    "  function rcStepOne(quiet){\n"
    "    if(!rcOn||rcDone||!rc) return;\n"
    "    if(!rcPaused){rcPaused=true;\n"
    "      showToast('THE PRESS HOLDS ITS BREATH');}\n"
    "    if(rc.bp<rc.bitlen) rcStep();\n"
    "    if(rc.bp>=rc.bitlen&&!rcDone) rcFinishUp();\n"
    "    if(!quiet) rcStepTick();\n"
    "  }\n"
    "  function rcStepByte(){\n"
    "    if(!rcOn||rcDone||!rc) return;\n"
    "    do{rcStepOne(true);}while(rcOn&&!rcDone&&((rc.bp-1)&7)!==0);\n"
    "    rcStepTick();\n"
    "  }\n"
    "  function rcRamp(c){\n"
    "    var t=c/rcCmax;if(t<0)t=0;if(t>1)t=1;\n"
    "    var r,g,b;\n"
    "    if(t<0.5){var u=t*2;r=16+164*u;g=185-45*u;b=129-89*u;}\n"
    "    else{var u=(t-0.5)*2;r=180+68*u;g=140-27*u;b=40+73*u;}\n"
    "    return 'rgb('+(r|0)+','+(g|0)+','+(b|0)+')';\n"
    "  }\n"
    "  function rcPaintOut(i,clr){\n"
    "    if(i>=rcNbc) return;\n"
    "    var cpad=rcCCell>2?1:0;\n"
    "    if(!clr){\n"
    "      var ob=rcDone?rc.pay:(rc.bitPos>0?(rc.bitPos>>3):0);\n"
    "      if(i<ob) clr=cynB(rc.out[i]);\n"
    "      else clr='#05050a';\n"
    "    }\n"
    "    rcCCtx.fillStyle=clr;\n"
    "    rcCCtx.fillRect((i%%rcCCols)*rcCCell,((i/rcCCols)|0)*rcCCell,\n"
    "      rcCCell-cpad,rcCCell-cpad);\n"
    "  }\n"
    "  function rcTipHide(){ovTipHide();}\n"
    "  function rcTipAt(e,html){ovTipAt(rcOv,'#34d399',e,html);}\n"
    "  function rcStop(){\n"
    "    rcOn=false;rcDone=false;rcPaused=false;rcHold=0;\n"
    "    if(rcRaf){cancelAnimationFrame(rcRaf);rcRaf=null;}\n"
    "    if(rcOv){rcOv.remove();rcOv=null;}\n"
    "    rc=null;rcDirty=null; /* the table goes with it */\n"
    "  }\n"
    "  function rcStart(){\n"
    "    if(rcOn) return;\n"
    "    if(typeof PAQ==='undefined'||!PAQ.b64||\n"
    ...
    "    var H=paqHdr();\n"
    "    if(!H||BD.length!==H.nb){\n"
    "      showToast('THE FILE CHANGED SHAPE. NOT TODAY.');return;}\n"
    "    ensureAudio();\n"
    "    var mo=(auLive&&auSet)?auSet:null;\n"
    "    var bytes=new Uint8Array(BD.length);\n"
    "    for(var i=0;i<BD.length;i++) bytes[i]=parseInt(BD[i].h,16);\n"
    "    var nb=bytes.length;\n"
    "    try{\n"
    "      rc={bytes:bytes,H:H,nb:nb,bitlen:nb*8+1,mo:mo,\n"
    "        num:mo?mo.num:H.num,\n"
    "        weights:mo?mo.weights:H.weights,\n"
    "        ext:mo?mo.ext:H.ext,\n"
    "        buf:new Uint8Array(nb+16),\n"
    "        dt:new Uint8Array(2*(1<<PAQ.db)),\n"
    "        out:new Uint8Array(nb+1024),\n"
    "        costs:new Float32Array(nb),\n"
    "        sl:new Int32Array((mo?mo.num:H.num)||1),\n"
    "        range:0x80000000,low:0,bitPos:-1,bp:0};\n"
    "    }catch(e){\n"
    "      showToast('THE BROWSER REFUSED THE PRESSING PLANT');\n"
    "      rc=null;return;}\n"
    "    rc.buf.set(bytes,8);\n"
    "    rcCmax=8;\n"
    "    for(var i=0;i<BD.length;i++) if(BD[i].c>rcCmax) rcCmax=BD[i].c;\n"
    "    rcDone=false;rcTurbo=false;rcFr=0;rcHold=0;\n"
    "    rcDrawn=0;rcODrawn=0;rcDirty=[];\n"
    "    var tn=0;\n"
    "    if(window.TUNED)\n"
    "      for(var i=0;i<window.TUNED.length;i++)\n"
    "        if(window.TUNED[i]) tn++;\n"
    "    var ft=fitCells(nb,300,[8,7,6,5,4,3,2]);\n"
    "    rcCell=ft.cell;rcCols=ft.cols;\n"
    "    var rows=ft.rows;\n"
    "    /* the out strip is laid out by the shipped payload; a fatter\n"
    "       pressing simply runs past the right edge of the picture */\n"
    "    rcNbc=H.cd.length-H.co;\n"
    "    var f2=fitCells(rcNbc,110,[6,5,4,3,2,1]);\n"
    "    rcCCell=f2.cell;rcCCols=f2.cols;\n"
    "    var crows=f2.rows;\n"
    "    rcOv=document.createElement('div');\n"
    "    rcOv.id='rcov';\n"
    "    rcOv.style.cssText='position:fixed;inset:0;z-index:10002;'\n"
    "      +'background:rgba(0,0,4,.9);display:flex;align-items:center;'\n"
    "      +'justify-content:center';\n"
    "    rcOv.innerHTML='<div style=\"background:#0b0b10;'\n"
    "      +'border:2px solid #34d399;padding:14px 18px;'\n"
    "      +'box-shadow:8px 8px 0 rgba(0,0,0,.6);'\n"
    "      +'font-family:var(--pix);font-size:8px;letter-spacing:1px;'\n"
    "      +'color:var(--fg)\">'\n"
    "      +'<div style=\"display:flex;justify-content:space-between;'\n"
    "      +'gap:30px;color:#34d399;font-size:10px;margin-bottom:10px;'\n"
    "      +'text-shadow:2px 2px 0 rgba(255,63,216,.5)\">*** RECRUNCH ***'\n"
    "      +'<span id=\"rc-x\" style=\"cursor:pointer\">[X]</span></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin-bottom:4px\">'\n"
    "      +'PLAINTEXT IN \\u00b7 '+nb+' B'\n"
    "      +(tn?' \\u00b7 '+tn+' EDIT'+(tn===1?'':'S')+' IN THE MIX':'')\n"
    "      +(mo?' \\u00b7 AUDITION LINEUP: '+mo.num+' MODELS':'')\n"
    "      +'</div>'\n"
    "      +'<canvas id=\"rc-cv\" width=\"'+(rcCols*rcCell)+'\" height=\"'\n"
    "      +(rows*rcCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);cursor:crosshair\"></canvas>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;margin:8px 0 4px\">'\n"
    "      +'COMPRESSED OUT \\u00b7 SHIPPED '+rcNbc+' B</div>'\n"
    "      +'<canvas id=\"rc-cc\" width=\"'+(rcCCols*rcCCell)+'\" height=\"'\n"
    "      +(crows*rcCCell)+'\" style=\"display:block;background:#05050a;'\n"
    "      +'border:1px solid var(--bdr2);cursor:crosshair\"></canvas>'\n"
    "      +'<div id=\"rc-regs\" style=\"margin-top:10px;line-height:2.1;'\n"
    "      +'color:var(--fg2)\"></div>'\n"
    "      +'<div id=\"rc-st\" style=\"margin-top:4px;color:var(--yel);'\n"
    "      +'min-height:14px\"></div>'\n"
    "      +'<div style=\"margin-top:8px;color:var(--fg3);font-size:7px\">'\n"
    "      +'THE REAL ENCODER, RUN BEFORE YOUR EYES \\u00b7 '\n"
    "      +'SPACE = ROLL / TURBO \\u00b7 . = ONE BIT \\u00b7 '\n"
    "      +', = ONE BYTE \\u00b7 ESC = WALK AWAY</div></div>';\n"
    "    document.body.appendChild(rcOv);\n"
    "    rcCv=document.getElementById('rc-cv');\n"
    "    rcCtx=rcCv.getContext('2d');\n"
    "    rcCC=document.getElementById('rc-cc');\n"
    "    rcCCtx=rcCC.getContext('2d');\n"
    "    rcRegs=document.getElementById('rc-regs');\n"
    "    rcStat=document.getElementById('rc-st');\n"
    "    document.getElementById('rc-x').addEventListener('click',rcStop);\n"
    "    /* the plaintext at rest: gray cells, textured by byte value.\n"
    "       the press paints each one with its price as it passes */\n"
    "    var gpad=rcCell>2?1:0;\n"
    "    for(var i=0;i<nb;i++){\n"
    "      rcCtx.fillStyle=grayB(bytes[i]);\n"
    "      rcCtx.fillRect((i%%rcCols)*rcCell,((i/rcCols)|0)*rcCell,\n"
    "        rcCell-gpad,rcCell-gpad);\n"
    "    }\n"
    "    rcCv.addEventListener('mousemove',function(e){\n"
    "      if(!rc){rcTipHide();return;}\n"
    "      var i=dcCellIdx(e,rcCv,rcCols,rcCell);\n"
    "      if(i<0||i>=rc.nb){rcTipHide();return;}\n"
    "      var done=rc.bp>0?Math.min(rc.nb,(rc.bp-1)>>3):0;\n"
    "      var v=rc.bytes[i];\n"
    "      var ch=(v>=0x20&&v<=0x7E&&v!==60&&v!==62&&v!==38)\n"
    "        ?(' \\''+String.fromCharCode(v)+'\\''):'';\n"
    "      var h='IN '+i+' \\u00b7 BYTE '+dcHex2(v)+ch+'<br>';\n"
    "      if(i<done){\n"
    "        var c=rc.costs[i];\n"
    "        var clr=c<3?'#34d399':c<6?'#fbbf24':'#f87171';\n"
    "        h+='<span style=\"color:'+clr+'\">COST '+c.toFixed(2)\n"
    "          +' BITS</span>';\n"
    "        if(typeof BD!=='undefined'&&BD[i])\n"
    "          h+='<br><span style=\"color:var(--fg3)\">FORWARD PASS SAID '\n"
    "            +BD[i].c.toFixed(2)+'</span>';\n"
    "      } else if(i===done&&!rcDone){\n"
    "        h+='<span style=\"color:var(--yel)\">UNDER THE PRESS</span>';\n"
    "      } else {\n"
    "        h+='<span style=\"color:var(--fg3)\">NOT YET PRESSED</span>';\n"
    "      }\n"
    "      rcTipAt(e,h);\n"
    "    });\n"
    "    rcCv.addEventListener('mouseleave',rcTipHide);\n"
    "    rcCC.addEventListener('mousemove',function(e){\n"
    "      if(!rc){rcTipHide();return;}\n"
    "      var j=dcCellIdx(e,rcCC,rcCCols,rcCCell);\n"
    "      if(j<0||j>=rcNbc){rcTipHide();return;}\n"
    "      var ob=rcDone?rc.pay:(rc.bitPos>0?(rc.bitPos>>3):0);\n"
    "      var h='OUT 0x'+dcHex8(j).slice(4)\n"
    "        +' <span style=\"color:var(--fg3)\">(FILE 0x'\n"
    "        +dcHex8(rc.H.co+j).slice(4)+')</span><br>';\n"
    "      if(j<ob){\n"
    "        h+='BYTE '+dcHex2(rc.out[j])\n"
    "          +(rcDone?'':'<br><span style=\"color:var(--fg3)\">STILL'\n"
    "            +' SUBJECT TO CARRIES</span>');\n"
    "      } else {\n"
    "        h+='<span style=\"color:var(--fg3)\">STILL IN THE FUTURE'\n"
    "          +'</span>';\n"
    "      }\n"
    "      rcTipAt(e,h);\n"
    "    });\n"
    "    rcCC.addEventListener('mouseleave',rcTipHide);\n"
    "    /* armed, not rolling: space releases it */\n"
    "    rcPaused=true;\n"
    "    rcStat.textContent=nb+' BYTES STAGED. SPACE ROLLS THE PRESS.';\n"
    "    rcOn=true;\n"
    "    showToast('ARMED');\n"
    "    if(!rcRaf) rcRaf=requestAnimationFrame(rcFrame);\n"
    "  }\n"
    "  function rcFrame(){\n"
    "    if(!rcOn||!document.body.classList.contains('party')){\n"
    "      rcStop();return;\n"
    "    }\n"
    "    rcFr++;\n"
    "    if(!rcDone&&!rcPaused){\n"
    "      /* ~18s for the whole file in real time, x16 under turbo */\n"
    "      var n=Math.max(1,Math.ceil(rc.bitlen/1100))*(rcTurbo?16:1)*8;\n"
    "      while(rc.bp<rc.bitlen&&n--) rcStep();\n"
    "      if(rc.bp>=rc.bitlen&&!rcDone) rcFinishUp();\n"
    "    }\n"
    "    /* IN: consumed bytes wear their price */\n"
    "    var done=rc.bp>0?Math.min(rc.nb,(rc.bp-1)>>3):0;\n"
    "    var pad=rcCell>2?1:0;\n"
    "    for(;rcDrawn<done;rcDrawn++){\n"
    "      var x=(rcDrawn%%rcCols)*rcCell,y=((rcDrawn/rcCols)|0)*rcCell;\n"
    "      rcCtx.fillStyle=rcRamp(rc.costs[rcDrawn]);\n"
    "      rcCtx.fillRect(x,y,rcCell-pad,rcCell-pad);\n"
    "    }\n"
    "    if(!rcDone&&done<rc.nb){\n"
    "      var x=(done%%rcCols)*rcCell,y=((done/rcCols)|0)*rcCell;\n"
    "      rcCtx.fillStyle=(rcFr&8)?'#ffe33f':'#fff';\n"
    "      rcCtx.fillRect(x,y,rcCell-pad,rcCell-pad);\n"
    "      /* the press hums its progress: pitch is the last byte's pain */\n"
    "      if(!rcPaused&&!(rcFr&3)&&done>0)\n"
    "        blip(160+Math.min(900,rc.costs[done-1]*90),.02,.04);\n"
    "    }\n"
    "    /* OUT: the stream materializes, carries flash yellow */\n"
    "    var ob=rcDone?rc.pay:(rc.bitPos>0?(rc.bitPos>>3):0);\n"
    "    for(;rcODrawn<Math.min(ob,rcNbc);rcODrawn++)\n"
    "      rcPaintOut(rcODrawn,null);\n"
    "    if(rcDirty)\n"
    "      for(var k=rcDirty.length-1;k>=0;k--){\n"
    "        var en=rcDirty[k];\n"
    "        if(en.i>=rcNbc||rcFr-en.f>=10){\n"
    "          if(en.i<rcNbc) rcPaintOut(en.i,null);\n"
    "          rcDirty.splice(k,1);\n"
    "        } else rcPaintOut(en.i,'#ffe33f');\n"
    "      }\n"
    "    if(!rcDone&&ob<rcNbc){\n"
    "      rcCCtx.fillStyle=(rcFr&8)?'#ffe33f':'#ff3fd8';\n"
    "      var cpad2=rcCCell>2?1:0;\n"
    "      rcCCtx.fillRect((ob%%rcCCols)*rcCCell,\n"
    "        ((ob/rcCCols)|0)*rcCCell,rcCCell-cpad2,rcCCell-cpad2);\n"
    "    }\n"
    "    rcRegs.innerHTML='RANGE <span style=\"color:var(--acc)\">'\n"
    "      +dcHex8(rc.range)+'</span> LOW <span style=\"color:var(--acc)\">'\n"
    "      +dcHex8(rc.low)+'</span><br>BYTE <span style=\"color:var(--yel)\">'\n"
    "      +done+'</span>/'+rc.nb+' \\u00b7 OUT '+ob+' B'\n"
    "      +(done?' \\u00b7 RATIO '+(100*ob/done).toFixed(1)+'%%':'')\n"
    "      +(rcTurbo?' \\u00b7 TURBO':'')\n"
    "      +(rcPaused&&!rcDone?' \\u00b7 <span style=\"color:var(--yel)\">'\n"
    "        +'STEP</span>':'');\n"
    "    if(rcDone){\n"
    "      /* let the last carries cool, then hold the picture still */\n"
    "      if(!rcHold) rcHold=rcFr;\n"
    "      if(rcFr-rcHold>120){rcRaf=null;return;}\n"
    "    }\n"
    "    rcRaf=requestAnimationFrame(rcFrame);\n"
    "  }\n"
    "  /* the audition: the model set, editable. bench a model, nudge\n"
    "     a weight, hire a new mask - every change re-bills the whole\n"
    "     file through the forward pass, header bytes included, with\n"
    "     the wmask re-encoded exactly like the C side (the extended\n"
    "     masks carry wmask residue into the hash seeds, so weights\n"
    "     move every slot). REHEARSE is optimize_weights, ported: the\n"
    "     same cyclic hill-climb the search runs, live on stage. the\n"
    "     chosen lineup goes to recrunch, which writes it a real\n"
    "     header. the shipped search stays in C; this is the stage\n"
    "     where its lineup defends the job */\n"
    "  var auOn=false,auOv=null,auSet=null,auLive=true,auSwapped=false;\n"
    "  var auBase=null,auFlags=null,auWts=null,auXtra=null,auBy=null;\n"
    "  var auReh=null,auRowsEl=null,auHeadEl=null,auStatEl=null;\n"
    "  /* encode_weight_mask, verbatim (terminate=1) */\n"
    "  function auEncW(ms){\n"
    "    var wmask=0,remaining=ms.length,bit=31,masks=[];\n"
    "    for(var w=0;remaining;w++){\n"
    "      for(var m=0;m<ms.length;m++){\n"
    "        if(ms[m].w===w){masks.push(ms[m].mask);remaining--;bit--;}\n"
    "      }\n"
    "      wmask=(wmask|(1<<bit))>>>0;bit--;\n"
    "    }\n"
    "    while(bit>=0){wmask=(wmask|(1<<bit))>>>0;bit--;}\n"
    "    var p=0,x=wmask&255;\n"
    "    while(x){p^=x&1;x>>=1;}\n"
    "    return {wmask:(wmask&((-2+(1^p))>>>0))>>>0,masks:masks};\n"
    "  }\n"
    "  /* decode_weight_mask, the same walk paqHdr does */\n"
    "  function auDecW(wm,masks){\n"
    "    var w=wm>>>0,wv=0,weights=[],ext=[];\n"
    "    for(var n=0;n<masks.length;n++){\n"
    "      while(w&0x80000000){w=(w<<1)>>>0;wv++;}\n"
    "      w=(w<<1)>>>0;\n"
    "      weights.push(wv);\n"
    "      ext.push((masks[n]|(w&0xFFFFFF00))>>>0);\n"
    "    }\n"
    "    return {weights:weights,ext:ext};\n"
    "  }\n"
    "  function auBuild(ms){\n"
    "    var ew=auEncW(ms);\n"
    "    var dw=auDecW(ew.wmask,ew.masks);\n"
    "    return {num:ew.masks.length,masks:ew.masks,wmask:ew.wmask,\n"
    "      weights:dw.weights,ext:dw.ext};\n"
    "  }\n"
    "  /* the candidate list: enabled shipped models in header order,\n"
    "     then the hires. src/i let the rehearsal write weights back */\n"
    "  function auModels(){\n"
    "    var ms=[];\n"
    "    for(var m=0;m<auBase.num;m++)\n"
    "      if(auFlags[m])\n"
    "        ms.push({mask:auBase.ext[m]&255,w:auWts[m],src:'b',i:m});\n"
    "    for(var k=0;k<auXtra.length;k++)\n"
    "      if(auXtra[k].on)\n"
    "        ms.push({mask:auXtra[k].mask,w:auXtra[k].w,src:'x',i:k});\n"
    "    return ms;\n"
    "  }\n"
    "  function auSameAsShip(set){\n"
    "    if(set.num!==auBase.num) return false;\n"
    "    for(var m=0;m<set.num;m++)\n"
    "      if(set.masks[m]!==(auBase.ext[m]&255)||\n"
    "         set.weights[m]!==auBase.weights[m]) return false;\n"
    "    return true;\n"
    "  }\n"
    "  function auFileB(bits,n){return (PAQ.lf?4:2)+4+n+bits/8;}\n"
    "  function auScore(set,wantM){return fpScore(auBy,wantM,set);}\n"
    "  function auRefresh(){\n"
    "    if(!auOn) return;\n"
    "    var ms=auModels();\n"
    "    var set=auBuild(ms);\n"
    "    var r=auScore(set,true);\n"
    "    if(!r){auStatEl.textContent='THE PASS DID NOT SURVIVE';return;}\n"
    "    auSet=auSameAsShip(set)?null:set;\n"
    "    /* per-model net bits under THIS lineup, from the pass */\n"
    "    var sums=new Float64Array(set.num);\n"
    "    if(r.perModel)\n"
    "      for(var i=0;i<auBy.length;i++)\n"
    "        for(var m=0;m<set.num;m++)\n"
    "          sums[m]+=r.perModel[i*set.num+m];\n"
    "    var used=new Uint8Array(set.num||1);\n"
    "    function takeSum(mask,w){\n"
    "      for(var m=0;m<set.num;m++)\n"
    "        if(!used[m]&&set.masks[m]===mask&&set.weights[m]===w){\n"
    "          used[m]=1;return sums[m];\n"
    "        }\n"
    "      return 0;\n"
    "    }\n"
    "    var shipB=auBase.cd.length;\n"
    "    var nowB=auFileB(r.bits,set.num);\n"
    "    var dd=nowB-shipB;\n"
    "    var dc2=dd>0.05?'#f87171':dd<-0.05?'#34d399':'var(--fg2)';\n"
    "    auHeadEl.innerHTML='SHIPPED '+auBase.num+' MODELS \\u00b7 '\n"
    "      +shipB+' B<br>THIS LINEUP '+set.num+' \\u00b7 '\n"
    "      +nowB.toFixed(1)+' B \\u00b7 <span style=\"color:'+dc2+'\">'\n"
    "      +(dd>=0?'+':'')+dd.toFixed(1)+' B</span>'\n"
    "      +(auSet?'':' <span style=\"color:var(--fg3)\">(THE SHIPPED'\n"
    "      +' SET)</span>');\n"
    "    function rowH(on,lbl,sv,acts){\n"
    "      return '<div style=\"display:flex;gap:8px;align-items:center;'\n"
    "        +'margin:3px 0'+(on?'':';opacity:.45')+'\">'\n"
    "        +'<span '+acts[0]+' style=\"cursor:pointer;color:'\n"
    "        +(on?'#34d399':'var(--fg3)')+'\">['+(on?'X':' ')+']</span>'\n"
    "        +'<span style=\"width:48px;color:var(--acc)\">'+lbl+'</span>'\n"
    "        +'<span '+acts[1]+' style=\"cursor:pointer;'\n"
    "        +'color:var(--pnk)\">[-]</span>'\n"
    "        +'<span '+acts[2]+' style=\"cursor:pointer;'\n"
    "        +'color:var(--pnk)\">[+]</span>'\n"
    "        +'<span style=\"flex:1;text-align:right;color:'\n"
    "        +(sv>0?'var(--fg)':'#f87171')+'\">'\n"
    "        +(on?(sv/8).toFixed(1)+' B':'\\u2014')+'</span>'+acts[3]\n"
    "        +'</div>';\n"
    "    }\n"
    "    var h='';\n"
    "    for(var m=0;m<auBase.num;m++){\n"
    "      var mk=auBase.ext[m]&255;\n"
    "      h+=rowH(auFlags[m],hx2(mk)+':'+auWts[m],\n"
    "        auFlags[m]?takeSum(mk,auWts[m]):0,\n"
    "        ['data-act=\"tog\" data-m=\"'+m+'\"',\n"
    "         'data-act=\"dn\" data-m=\"'+m+'\"',\n"
    "         'data-act=\"up\" data-m=\"'+m+'\"','']);\n"
    "    }\n"
    "    for(var k=0;k<auXtra.length;k++){\n"
    "      h+=rowH(auXtra[k].on,hx2(auXtra[k].mask)+':'+auXtra[k].w+'*',\n"
    "        auXtra[k].on?takeSum(auXtra[k].mask,auXtra[k].w):0,\n"
    "        ['data-act=\"tog\" data-x=\"'+k+'\"',\n"
    "         'data-act=\"dn\" data-x=\"'+k+'\"',\n"
    "         'data-act=\"up\" data-x=\"'+k+'\"',\n"
    "         '<span data-act=\"fire\" data-x=\"'+k+'\" style=\"'\n"
    "         +'cursor:pointer;color:#f87171\">[FIRE]</span>']);\n"
    "    }\n"
    "    auRowsEl.innerHTML=h;\n"
    "    if(set.num===0)\n"
    "      auStatEl.textContent='NO BAND. THE FILE SINGS A CAPPELLA.';\n"
    "    else if(!auReh) auStatEl.textContent='';\n"
    "  }\n"
    "  /* optimize_weights, ported: cyclic coordinate descent over the\n"
    "     enabled weights, one trial per tick so the page breathes */\n"
    "  function auRehearse(){\n"
    "    if(auReh){auReh=null;\n"
    "      auStatEl.textContent='REHEARSAL CUT SHORT.';return;}\n"
    "    var ms=auModels();\n"
    "    if(!ms.length){showToast('NO ONE TO REHEARSE');return;}\n"
    "    var r0=auScore(auBuild(ms),false);\n"
    "    if(!r0) return;\n"
    "    auReh={ms:ms,idx:ms.length-1,dir:1,last:ms.length-1,\n"
    "      best:r0.bits,trials:0};\n"
    "    egg('audition');\n"
    "    auStatEl.textContent='REHEARSING...';\n"
    "    setTimeout(auRehStep,0);\n"
    "  }\n"
    "  function auRehStep(){\n"
    "    var st=auReh;\n"
    "    if(!st||!auOn){auReh=null;return;}\n"
    "    var m=st.ms[st.idx];\n"
    "    var nw=m.w+st.dir,improved=false;\n"
    "    if(nw>=0&&nw<=9){\n"
    "      var old=m.w;m.w=nw;\n"
    "      var tr=auScore(auBuild(st.ms),false);\n"
    "      st.trials++;\n"
    "      if(tr&&tr.bits<st.best){st.best=tr.bits;st.last=st.idx;\n"
    "        improved=true;}\n"
    "      else m.w=old;\n"
    "    }\n"
    "    if(!improved){\n"
    "      if(st.dir===1&&m.w>0) st.dir=-1;\n"
    "      else{\n"
    "        st.dir=1;st.idx--;\n"
    "        if(st.idx<0) st.idx=st.ms.length-1;\n"
    "        if(st.idx===st.last){\n"
    "          auReh=null;\n"
    "          for(var k=0;k<st.ms.length;k++){\n"
    "            var e=st.ms[k];\n"
    "            if(e.src==='b') auWts[e.i]=e.w;\n"
    "            else auXtra[e.i].w=e.w;\n"
    "          }\n"
    "          auRefresh();\n"
    "          auStatEl.textContent='REHEARSED. '+st.trials+' TRIALS.';\n"
    "          showToast('THE BAND IS TIGHTER NOW');\n"
    "          return;\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "    auStatEl.textContent='REHEARSING '+hx2(st.ms[st.idx].mask)\n"
    "      +' \\u00b7 TRIAL '+st.trials+' \\u00b7 BEST '\n"
    "      +auFileB(st.best,st.ms.length).toFixed(1)+' B';\n"
    "    setTimeout(auRehStep,0);\n"
    "  }\n"
    "  function auStop(){\n"
    "    if(!auOn) return;\n"
    "    auOn=false;auReh=null;\n"
    "    if(auOv){auOv.remove();auOv=null;}\n"
    "  }\n"
    "  function auStart(){\n"
    "    if(auOn){auStop();return;}\n"
    "    if(typeof PAQ==='undefined'||typeof BD==='undefined'||\n"
    "       !BD.length){showToast('NO PAYLOAD ABOARD');return;}\n"
    "    if(tuOn){showToast('ONE STAGE AT A TIME. CLOSE THE TUNER');\n"
    "      return;}\n"
    "    auBase=paqHdr();\n"
    "    if(!auBase){showToast('NO HEADER TO AUDITION');return;}\n"
    "    auBy=new Uint8Array(BD.length);\n"
    "    for(var i=0;i<BD.length;i++) auBy[i]=parseInt(BD[i].h,16);\n"
    "    /* the session survives close/open unless the header moved */\n"
    "    if(!auFlags||auFlags.length!==auBase.num){\n"
    "      auFlags=[];auWts=[];auXtra=[];\n"
    "      for(var m=0;m<auBase.num;m++){\n"
    "        auFlags.push(true);auWts.push(auBase.weights[m]);\n"
    "      }\n"
    "    }\n"
    "    auOv=document.createElement('div');\n"
    "    auOv.style.cssText='position:fixed;inset:0;z-index:10003;'\n"
    "      +'display:flex;align-items:center;justify-content:flex-end;'\n"
    "      +'padding-right:24px;pointer-events:none;';\n"
    "    var bx=document.createElement('div');\n"
    "    bx.id='au-bx';\n"
    "    bx.style.cssText='background:#0b0b10;border:2px solid var(--acc);'\n"
    "      +'padding:14px 18px;font-family:var(--pix);font-size:8px;'\n"
    "      +'color:var(--fg2);letter-spacing:1px;max-height:92vh;'\n"
    "      +'overflow-y:auto;box-shadow:8px 8px 0 rgba(0,0,0,.6);'\n"
    "      +'pointer-events:auto;min-width:300px;';\n"
    "    bx.innerHTML='<div id=\"au-ttl\" style=\"color:var(--acc);'\n"
    "      +'font-size:10px;margin-bottom:8px;'\n"
    "      +'text-shadow:2px 2px 0 rgba(255,63,216,.5)\">'\n"
    "      +'*** THE AUDITION ***</div>'\n"
    "      +'<div id=\"au-head\" style=\"margin:6px 0;line-height:1.9;'\n"
    "      +'color:var(--fg)\"></div>'\n"
    "      +'<div id=\"au-rows\" style=\"margin:8px 0\"></div>'\n"
    "      +'<div style=\"display:flex;gap:8px;align-items:center;'\n"
    "      +'margin:8px 0\">HIRE:'\n"
    "      +'<input id=\"au-mask\" maxlength=\"2\" placeholder=\"FF\" '\n"
    "      +'style=\"width:30px;font-family:inherit;font-size:8px;'\n"
    "      +'background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2);padding:3px\">'\n"
    "      +'W<input id=\"au-w\" maxlength=\"1\" placeholder=\"2\" '\n"
    "      +'style=\"width:16px;font-family:inherit;font-size:8px;'\n"
    "      +'background:var(--bg3);color:var(--fg);'\n"
    "      +'border:1px solid var(--bdr2);padding:3px\">'\n"
    "      +'<span id=\"au-hire\" style=\"cursor:pointer;'\n"
    "      +'color:#34d399\">[HIRE]</span></div>'\n"
    "      +'<div style=\"display:flex;gap:12px;margin:8px 0\">'\n"
    "      +'<span id=\"au-reh\" style=\"cursor:pointer;'\n"
    "      +'color:var(--yel)\">[REHEARSE]</span>'\n"
    "      +'<span id=\"au-rst\" style=\"cursor:pointer;'\n"
    "      +'color:var(--fg2)\">[RESET]</span>'\n"
    "      +'<span id=\"au-live\" style=\"cursor:pointer;'\n"
    "      +'color:var(--pnk)\"></span></div>'\n"
    "      +'<div id=\"au-stat\" style=\"margin:6px 0;color:var(--yel);'\n"
    "      +'min-height:12px\"></div>'\n"
    "      +'<div style=\"color:var(--fg3);font-size:7px;line-height:1.9\">'\n"
    "      +'EVERY NUMBER IS A FULL FORWARD PASS, HEADER INCLUDED'\n"
    "      +'<br>[X] BENCHES A MODEL \\u00b7 * MARKS A HIRE'\n"
    "      +'<br>REHEARSE = THE C WEIGHT HILL-CLIMB, LIVE'\n"
    "      +'<br>RECRUNCH PRESSES THE LINEUP FOR REAL \\u00b7 ESC OUT'\n"
    "      +'</div>';\n"
    "    auOv.appendChild(bx);\n"
    "    document.body.appendChild(auOv);\n"
    "    auHeadEl=document.getElementById('au-head');\n"
    "    auRowsEl=document.getElementById('au-rows');\n"
    "    auStatEl=document.getElementById('au-stat');\n"
    "    var lv=document.getElementById('au-live');\n"
    "    function lvPaint(){\n"
    "      lv.textContent='[RECRUNCH: '+(auLive?'THIS LINEUP':'SHIPPED')\n"
    "        +']';\n"
    "    }\n"
    "    lvPaint();\n"
    "    lv.addEventListener('click',function(){\n"
    "      auLive=!auLive;lvPaint();\n"
    "      showToast(auLive?'THE LINEUP WILL PRESS'\n"
    "        :'THE SHIPPED SET WILL PRESS');\n"
    "    });\n"
    "    auRowsEl.addEventListener('click',function(e){\n"
    "      var el=e.target.closest('[data-act]');\n"
    "      if(!el) return;\n"
    "      if(auReh){showToast('MID-REHEARSAL. HANDS OFF.');return;}\n"
    "      var act=el.getAttribute('data-act');\n"
    "      var m=el.getAttribute('data-m'),x=el.getAttribute('data-x');\n"
    "      if(m!==null){\n"
    "        m=+m;\n"
    "        if(act==='tog'){\n"
    "          if(!auFlags[m]&&auModels().length>=21){\n"
    "            showToast('THE STAGE IS FULL (21)');return;}\n"
    "          auFlags[m]=!auFlags[m];\n"
    "        }\n"
    "        else if(act==='up'&&auWts[m]<9) auWts[m]++;\n"
    "        else if(act==='dn'&&auWts[m]>0) auWts[m]--;\n"
    "      } else if(x!==null){\n"
    "        x=+x;\n"
    "        if(act==='tog'){\n"
    "          if(!auXtra[x].on&&auModels().length>=21){\n"
    "            showToast('THE STAGE IS FULL (21)');return;}\n"
    "          auXtra[x].on=!auXtra[x].on;\n"
    "        }\n"
    "        else if(act==='up'&&auXtra[x].w<9) auXtra[x].w++;\n"
    "        else if(act==='dn'&&auXtra[x].w>0) auXtra[x].w--;\n"
    "        else if(act==='fire') auXtra.splice(x,1);\n"
    "      }\n"
    "      egg('audition');\n"
    "      auRefresh();\n"
    "    });\n"
    "    document.getElementById('au-hire').addEventListener('click',\n"
    "      function(){\n"
    "        if(auReh){showToast('MID-REHEARSAL. HANDS OFF.');return;}\n"
    "        var mv=parseInt(document.getElementById('au-mask').value,16);\n"
    "        var wv=parseInt(document.getElementById('au-w').value,10);\n"
    "        if(isNaN(mv)||mv<0||mv>255||isNaN(wv)||wv<0||wv>9){\n"
    "          showToast('MASK 00-FF, WEIGHT 0-9');return;}\n"
    "        if(auModels().length>=21){\n"
    "          showToast('THE STAGE IS FULL (21)');return;}\n"
    "        auXtra.push({mask:mv,w:wv,on:true});\n"
    "        document.getElementById('au-mask').value='';\n"
    "        document.getElementById('au-w').value='';\n"
    "        egg('audition');\n"
    "        auRefresh();\n"
    "      });\n"
    "    document.getElementById('au-reh').addEventListener('click',\n"
    "      auRehearse);\n"
    "    document.getElementById('au-rst').addEventListener('click',\n"
    "      function(){\n"
    "        if(auReh){showToast('MID-REHEARSAL. HANDS OFF.');return;}\n"
    "        auFlags=[];auWts=[];auXtra=[];\n"
    "        for(var m=0;m<auBase.num;m++){\n"
    ...
    "        }\n"
    "        auRefresh();\n"
    "        showToast('THE SHIPPED SET STANDS');\n"
    "      });\n"
    "    dragBox(bx,document.getElementById('au-ttl'));\n"
    "    auOn=true;\n"
    "    auRefresh();\n"
    "    showToast('THE STAGE IS YOURS');\n"
    "  }\n"
    "  /* the bus routes: what each backdrop does when a byte plays.\n"
    "     every route reads the same event, so the sky, the fire,\n"
    "     the bars and the column all point at the same offset of\n"
    "     the file at the same audio instant */\n"
    "  onBeat(function(e){\n"
    "    var sn=e.c>=8;\n"
    "    if(kfOn&&kfCv){\n"
    "      var np=window.netNpts,H=kfCv.height;\n"
    "      var wi=e.i<np?e.i:np-1;\n"
    "      kfKickY=wi*(H-1)/(np-1);\n"
    "      kfKick=sn?1.6:.7;\n"
    "    }\n"
    "    if(fireOn&&fireHeat){\n"
    "      var fx=(e.i*fireW/BD.length)|0;\n"
    "      var fb=(fireH-1)*fireW,fv=sn?36:30;\n"
    "      fireHeat[fb+fx]=fv;\n"
    "      if(fx>0)fireHeat[fb+fx-1]=fv-2;\n"
    "      if(fx<fireW-1)fireHeat[fb+fx+1]=fv-2;\n"
    "    }\n"
    "    if(flyRaf&&!flyConst){flyPulse=sn?1:.45;flyCur=e.i;}\n"
    "    if(twOn)twKick=sn?1:.4;\n"
    "    if(rotoOn&&sn)rotoKick=.9;\n"
    "    if(tnOn&&sn)tnKick=7;\n"
    "    if(ecOn){ecPlay=e.i;ecKick=sn?1.2:.6;\n"
    "      ecFollowT=Date.now();}\n"
    "  });\n"
    ...
    "  if(glz){\n"
    "    /* the cube can be carried; it does not mind. it can also be\n"
    "       thrown; it minds slightly */\n"
    "    var dragOn=false,dragged=false,gdx=0,gdy=0,gsx=0,gsy=0;\n"
    "    var mvx=0,mvy=0,mlx=0,mly=0,mlt=0;\n"
    "    var phys=null,fvx=0,fvy=0,fpx=0,fpy=0;\n"
    "    function fly(){\n"
    "      if(!document.body.classList.contains('party')||\n"
    "         glz.classList.contains('dead')||dragOn){phys=null;return;}\n"
    "      /* position is tracked here, not read back from the DOM: the\n"
    "         3D spin makes the projected bounding box breathe every frame */\n"
    "      fpx+=fvx;fpy+=fvy;\n"
    "      var W=window.innerWidth,H=window.innerHeight;\n"
    "      var gw=glz.offsetWidth,gh=glz.offsetHeight;\n"
    "      if(fpx<0){fpx=0;fvx=-fvx*.7;}\n"
    "      if(fpx+gw>W){fpx=W-gw;fvx=-fvx*.7;}\n"
    "      if(fpy<0){fpy=0;fvy=-fvy*.7;}\n"
    "      if(fpy+gh>H){fpy=H-gh;fvy=-fvy*.7;}\n"
    "      glz.style.left=fpx+'px';glz.style.top=fpy+'px';\n"
    "      glz.style.right='auto';glz.style.bottom='auto';\n"
    "      fvx*=.985;fvy*=.985;\n"
    "      if(Math.abs(fvx)+Math.abs(fvy)>.15)\n"
    "        phys=requestAnimationFrame(fly);\n"
    "      else phys=null;\n"
    "    }\n"
    "    glz.addEventListener('pointerdown',function(e){\n"
    "      if(!document.body.classList.contains('party')) return;\n"
    "      if(glz.classList.contains('dead')) return;\n"
    "      if(phys){cancelAnimationFrame(phys);phys=null;}\n"
    "      mvx=0;mvy=0;mlt=0;\n"
    ...
    "        var mdt=tnow-mlt;\n"
    "        if(mdt>0){\n"
    "          mvx=.7*((e.clientX-mlx)/mdt)+.3*mvx;\n"
    "          mvy=.7*((e.clientY-mly)/mdt)+.3*mvy;\n"
    "        }\n"
    "      }\n"
    "      mlx=e.clientX;mly=e.clientY;mlt=tnow;\n"
    "    });\n"
    ...
    "            }\n"
    "          }\n"
    "        } else if(Math.abs(mvx)+Math.abs(mvy)>.3&&\n"
    "                  Date.now()-mlt<120&&\n"
    "                  !glz.classList.contains('dead')){\n"
    "          fvx=Math.max(-40,Math.min(40,mvx*16));\n"
    "          fvy=Math.max(-40,Math.min(40,mvy*16));\n"
    "          fpx=glz.offsetLeft;fpy=glz.offsetTop;\n"
    "          dragOn=false;\n"
    "          if(!phys) phys=requestAnimationFrame(fly);\n"
    "        }\n"
    "      }\n"
    "      dragOn=false;\n"
    ...
    "        beatDur=syn.bd*2; /* mourning halves whatever tempo is set */\n"
    "        killedN++;egg('killed');\n"
    "        showToast('YOU MONSTER');\n"
    "        blip(90,.05,.4);\n"
    "        setTimeout(function(){\n"
    "          glz.classList.remove('dead');\n"
    "          document.body.classList.remove('mourning');\n"
    "          beatDur=syn.bd;\n"
    "        },10000);\n"
    "        return;\n"
    "      }\n"
    ...
    "      if(glzN===10){egg('pleased');showToast('THE CUBE IS PLEASED');}\n"
    "    });\n"
    "  }\n"
    ...
    "      /* a scratch ends in a click; the tap counter looks away */\n"
    "      if(vsSkip){vsSkip=false;return;}\n"
    "      var now=Date.now();\n"
...
static void write_html_report(const char *path, const CompStats *s) {
  ...
  fprintf(f,
    ...
    "#paqtrk .pt-row.b4{background:rgba(127,140,170,.07)}\n"
    "#paqtrk .pt-row.b16{background:rgba(127,140,170,.13)}\n"
    "#paqtrk .pt-row.pb{border-top:1px solid rgba(255,63,216,.4)}\n"
    ...
    "#paqtrk .pt-row .in{width:14px;color:var(--pnk)}\n"
    "#paqtrk .pt-row .vl{width:14px;color:var(--grn)}\n"
    ...
    "/* crt mode: the picture warps, the glass sits on top. tuned\n"
    "   for speed: nothing animates ON the filtered layer (a flicker\n"
    "   there re-rasters the whole warp every 90ms), the refresh\n"
    "   band rides a composited transform instead of repainting its\n"
    "   background, and the ornaments inside the tube hold still */\n"
    ...
    "@keyframes crtflick{from{opacity:1}to{opacity:.55}}\n"
    ...
    "rgba(255,255,255,.05) 26%%,rgba(255,255,255,.09) 50%%,"
    "rgba(255,255,255,.05) 74%%,transparent 100%%);"
    "will-change:transform;"
    "animation:crtband 6s linear infinite}\n"
    "@keyframes crtband{from{transform:translateY(-260px)}"
    "to{transform:translateY(100vh)}}\n"
    "/* the warp re-rasters whenever anything inside it paints, so\n"
    "   the always-on ornaments pause while the tube is on */\n"
    "body.crt .card::before{animation-play-state:paused}\n"
    "body.crt .hero-ring svg{animation-play-state:paused}\n"
    "body.crt .hero-info h1::after{animation-play-state:paused}\n"
    "body.crt .hero-info::after{animation-play-state:paused}\n"
    ...
    "/* paqsynth's protracker skin: worn only while body.ptsyn is set\n"
    "   (the typed keyword). the palette is PT2.3D's shipped default,\n"
    "   verified against the pt2-clone source: 000/555/888/BBB grays,\n"
    "   black gui text, 3344FF pattern text on black, DD0044 cursor,\n"
    "   FFDD00 quadrascope. the font chain tries the reconstructed\n"
    "   protracker 8x8 and amiga topaz remakes if installed */\n"
    "#synth-ov .sy-ar{display:none}\n"
    "body.ptsyn #syn-bx,body.ptsyn #tu-bx,body.ptsyn #au-bx,"
    "body.ptsyn #paqtrk,body.ptsyn #synth-ov button,"
    "body.ptsyn #synth-ov select{"
    "font-family:'Protracker','ProTracker','TopazPlus a600a1200a4000',"
    "'Topaz a600a1200a4000','Topaz',var(--pix)!important}\n"
    "body.ptsyn #syn-bx{background:#888!important;"
    "border:2px solid!important;"
    "border-color:#bbb #555 #555 #bbb!important;"
    "color:#000!important;"
    "box-shadow:none!important}\n"
    "body.ptsyn #syn-ttl{background:#888!important;"
    ...
    "body.ptsyn #syn-bx>div:last-child{color:#000!important}\n"
    "body.ptsyn #synth-ov button{background:#888!important;"
    "color:#000!important;border:2px solid!important;"
    "border-color:#bbb #555 #555 #bbb!important;"
    "text-transform:uppercase}\n"
    "body.ptsyn #synth-ov button:active{"
    "border-color:#555 #bbb #bbb #555!important;"
    "background:#777!important}\n"
    "body.ptsyn #synth-ov select{background:#000!important;"
    "color:#3344ff!important;border:1px solid!important;"
    "border-color:#555 #bbb #bbb #555!important}\n"
    "body.ptsyn #synth-ov span[id^=\"synv-\"]{background:#000;"
    "color:#3344ff!important;"
    "border:1px solid;border-color:#555 #bbb #bbb #555;"
    "padding:1px 3px}\n"
    "body.ptsyn #synth-ov .sy-ar{display:block;width:13px;flex-shrink:0;"
    "text-align:center;background:#888;border:1px solid;"
    "border-color:#bbb #555 #555 #bbb;color:#000;"
    "cursor:pointer;font-size:7px;line-height:1.5;user-select:none}\n"
    "body.ptsyn #synth-ov .sy-ar:active{"
    "border-color:#555 #bbb #bbb #555;background:#777}\n"
    "body.ptsyn #synth-ov input[type=range]{-webkit-appearance:none;"
    "appearance:none;height:11px;background:#000;border:1px solid;"
    "border-color:#555 #bbb #bbb #555;border-radius:0}\n"
    "body.ptsyn #synth-ov input[type=range]::-webkit-slider-thumb{"
    "-webkit-appearance:none;width:16px;height:9px;background:#888;"
    "border:1px solid;border-color:#bbb #555 #555 #bbb;"
    "border-radius:0}\n"
    "body.ptsyn #synth-ov input[type=range]::-moz-range-thumb{"
    "width:16px;height:9px;"
    "background:#888;border:1px solid #555;border-radius:0}\n"
    "/* the rest of the stage: gray chassis, black displays. pattern\n"
    "   text goes PT blue; the cost column keeps its data colors */\n"
    "body.ptsyn #tu-bx,body.ptsyn #au-bx{"
    "border:2px solid!important;"
    "border-color:#bbb #555 #555 #bbb!important;"
    "box-shadow:none!important}\n"
    "body.ptsyn #tu-ttl,body.ptsyn #au-ttl{background:#888!important;"
    "color:#000!important;text-shadow:none!important;text-align:center;"
    "padding:4px 8px;border:1px solid;"
    "border-color:#555 #bbb #bbb #555}\n"
    "body.ptsyn #paqtrk.on,body.ptsyn #paqscope.on,"
    "body.ptsyn #paqspec.on,body.ptsyn #paqsg.on,"
    "body.ptsyn #paqvec.on{"
    "border:2px solid;border-color:#bbb #555 #555 #bbb;"
    "background:#000;"
    "box-shadow:none}\n"
    "body.ptsyn #paqtrk.on::before,body.ptsyn #paqscope.on::before,"
    "body.ptsyn #paqspec.on::before,body.ptsyn #paqsg.on::before,"
    "body.ptsyn #paqvec.on::before{animation:none;background:#888}\n"
    "body.ptsyn #paqtrk .pt-head{background:#888;color:#000;"
    "text-shadow:none;border-bottom:1px solid #555}\n"
    "body.ptsyn #paqtrk .pt-head span{color:#000;text-shadow:none}\n"
    "body.ptsyn #paqtrk .pt-row,body.ptsyn #paqtrk .pt-row .o,"
    "body.ptsyn #paqtrk .pt-row .nt,body.ptsyn #paqtrk .pt-row .by,"
    "body.ptsyn #paqtrk .pt-row .in,body.ptsyn #paqtrk .pt-row .vl,"
    "body.ptsyn #paqtrk .pt-row .fx{color:#3344ff}\n"
    "body.ptsyn #paqtrk .pt-row.b4{background:rgba(136,136,136,.12)}\n"
    "body.ptsyn #paqtrk .pt-row.b16{background:rgba(136,136,136,.22)}\n"
    "body.ptsyn #paqtrk .pt-row.pb{border-top:1px solid #555}\n"
    "body.ptsyn #paqtrk .pt-row.cur{background:rgba(221,0,68,.30);"
    "outline:1px solid #dd0044}\n"
    "body.ptsyn #paqtrk .pt-row.cur .nt{color:#fff}\n"
    "/* the WHOLE ui follows the keyword: the report re-dressed as\n"
    "   PT2.3D. one variable override does most of it - gray chassis,\n"
    "   black display wells, black gui text, blue accents - and the\n"
    "   rest is de-neoning: no glows, no copper, no sky. data colors\n"
    "   (cost ramps, model palette) keep their meaning */\n"
    "body.party.ptsyn{\n"
    "  --bg:#888888;--bg2:#888888;--bg3:#000000;--bg4:#777777;\n"
    "  --bdr:#555555;--bdr2:#bbbbbb;\n"
    "  --fg:#000000;--fg2:#111111;--fg3:#444444;\n"
    "  --acc:#3344ff;--blu:#3344ff;--pnk:#dd0044;--yel:#ffdd00;\n"
    "  --shadow:none;scrollbar-color:#888 #000;\n"
    "  --sans:'Protracker','ProTracker','TopazPlus a600a1200a4000',"
    "'Topaz a600a1200a4000','Topaz',var(--pix);\n"
    "  --mono:'Protracker','ProTracker','TopazPlus a600a1200a4000',"
    "'Topaz a600a1200a4000','Topaz',var(--pix);\n"
    "}\n"
    "body.party.ptsyn #stars,body.party.ptsyn #raster,"
    "body.party.ptsyn #flyby{display:none}\n"
    "body.party.ptsyn::before,body.party.ptsyn #copper2{"
    "animation:none;background:#555}\n"
    "body.party.ptsyn .card{border:2px solid;"
    "border-color:#bbb #555 #555 #bbb}\n"
    "body.party.ptsyn .card::before{animation:none;background:#bbb}\n"
    "body.party.ptsyn .card h2,body.party.ptsyn .hero-info h1{"
    "text-shadow:none}\n"
    "body.party.ptsyn .card:hover h2{animation:none}\n"
    "body.party.ptsyn .card h2::before{background:#555;box-shadow:none}\n"
    "body.party.ptsyn .hero-ring svg{animation:none}\n"
    "body.party.ptsyn .hero-pct .big{text-shadow:none}\n"
    "body.party.ptsyn .sb-brand .dot{background:#3344ff;"
    "box-shadow:none}\n"
    "body.party.ptsyn .sb-brand span:not(.dot){text-shadow:none}\n"
    "body.party.ptsyn .sidebar a:hover{text-shadow:none;"
    "background:rgba(0,0,0,.08)}\n"
    "body.party.ptsyn .slider-row button,"
    "body.party.ptsyn .theme-toggle{background:#888;color:#000;"
    "border:2px solid;border-color:#bbb #555 #555 #bbb;"
    "box-shadow:none}\n"
    "body.party.ptsyn .slider-row button:hover{background:#999;"
    "color:#000}\n"
    "body.party.ptsyn .slider-row button:active{"
    "border-color:#555 #bbb #bbb #555}\n"
    "body.party.ptsyn .cd-panel{color:#3344ff}\n"
    "body.party.ptsyn .cd-panel .cd-byte{color:#fff}\n"
    "body.party.ptsyn .scrolltext span{text-shadow:none}\n"
    "/* fixups. charts position their text with coordinates computed\n"
    "   for compact glyphs, so svg text keeps the mono font - the 8x8\n"
    "   would overflow every cell. grids that paint translucent cells\n"
    "   were designed against a dark card: they get their black\n"
    "   display wells back, which is also what PT would do. text\n"
    "   inputs were --fg on --bg3 = black on black under this skin */\n"
    "body.party.ptsyn svg text{"
    "font-family:'JetBrains Mono',ui-monospace,monospace}\n"
    "body.party.ptsyn #corr-svg,body.party.ptsyn #bfreq-svg,"
    "body.party.ptsyn #attr-svg,body.party.ptsyn #mg-svg,"
    "body.party.ptsyn #cmap-svg{background:#000}\n"
    "body.party.ptsyn #corr-svg text[fill=\"var(--fg2)\"]{"
    "fill:#9a9aad}\n"
    "body.party.ptsyn #corr-svg text[fill=\"var(--fg3)\"],"
    "body.party.ptsyn #bfreq-svg text[fill=\"var(--fg3)\"],"
    "body.party.ptsyn #mg-svg text[fill=\"var(--fg3)\"],"
    "body.party.ptsyn #attr-svg text[fill=\"var(--fg3)\"],"
    "body.party.ptsyn #cmap-svg text[fill=\"var(--fg3)\"]{fill:#999}\n"
    "body.party.ptsyn input:not([type=range]){"
    "background:#000!important;color:#3344ff!important;"
    "border:1px solid!important;border-radius:0!important;"
    "border-color:#555 #bbb #bbb #555!important}\n"
    "/* four colours cannot cast a shadow or round a corner - the\n"
    "   real PT gui was born flat, so the skin goes flat too */\n"
    "body.party.ptsyn svg rect{rx:0}\n"
    "body.party.ptsyn .hover-tip{box-shadow:none}\n"
    "body.party.ptsyn .sidebar a,body.party.ptsyn .bar,"
    "body.party.ptsyn .cd-bar-track,"
    "body.party.ptsyn .cd-bar-fill{border-radius:0}\n"
    "body.party.ptsyn ::-webkit-scrollbar-thumb{background:#888;"
    "border:2px solid #000;border-radius:0}\n"
    "body.party.ptsyn ::-webkit-scrollbar-track{background:#000}\n"
    "</style></head><body>\n"
    "<div id=\"stars\"></div>\n"
