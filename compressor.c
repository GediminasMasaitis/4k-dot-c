...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "      buf=(buf+k).slice(-10); /* longest keyword: devcheats (9) */\n"
    ...
    "      +'</span><input id=\"syn-'+id+'\" type=\"range\" min=\"'+mn\n"
    "      +'\" max=\"'+mx+'\" step=\"'+st2+'\" value=\"'+val\n"
    "      +'\" style=\"flex:1;accent-color:#ff3fd8;min-width:80px\">'\n"
    "      +'<span id=\"synv-'+id+'\" style=\"width:60px;text-align:right;'\n"
    "      +'color:var(--yel)\">'+txt+'</span></div>';\n"
    "  }\n"
