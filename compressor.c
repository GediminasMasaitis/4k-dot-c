...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "    return btoa(s);\n"
    "  }\n"
    "  function rcCarry(out,pos){\n"
    "    while(--pos>=0){\n"
    "      var mk=1<<(pos&7),by=out[pos>>3];\n"
    "      out[pos>>3]=by^mk;\n"
    "      if(!(by&mk)) break;\n"
    "    }\n"
    "  }\n"
    "  function rcEncode(bytes){\n"
    "    var H=paqHdr();\n"
    "    if(!H||bytes.length!==H.nb) return null;\n"
    "    var num=H.num,weights=H.weights,ext=H.ext;\n"
    "    var nb=bytes.length,bitlen=nb*8+1;\n"
    "    var buf=new Uint8Array(nb+16);\n"
    "    buf.set(bytes,8);\n"
    "    var dt=new Uint8Array(2*(1<<PAQ.db));\n"
    "    var out=new Uint8Array(nb+1024);\n"
    "    var sl=new Int32Array(num||1);\n"
    "    var range=0x80000000,low=0,bitPos=-1;\n"
    "    for(var bp=0;bp<bitlen;bp++){\n"
    "      var b2=bp-1;\n"
    "      var bit=(bp===0)?1:(buf[8+(b2>>3)]>>(7-(b2&7)))&1;\n"
    "      var p0=PAQ.bp,p1=PAQ.bp;\n"
    "      for(var m=0;m<num;m++){\n"
    "        var h=(bp===0)?dcHash0(ext[m]):ctxHash(buf,b2,ext[m]);\n"
    "        var s2=(h>>>(32-PAQ.db))*2;\n"
    "        var e0=dt[s2],e1=dt[s2+1];\n"
    "        var sh=(1-(((e0+255)&(e1+255))>>8))*2+weights[m];\n"
    "        p0+=e0<<sh;p1+=e1<<sh;\n"
    "        sl[m]=s2;\n"
    "      }\n"
    "      /* the exact mirror of the decoder: thresh = range*p1/total,\n"
    "         a 1 takes the low interval, a 0 takes the rest and may\n"
    "         carry into bits already written */\n"
    "      var th=Number(BigInt(range)*BigInt(p1)/BigInt(p0+p1));\n"
    "      if(bit){range=th;}\n"
    "      else{\n"
    "        var ol=low;\n"
    "        low=(low+th)>>>0;\n"
    "        if(low<ol) rcCarry(out,bitPos);\n"
    "        range=(range-th)>>>0;\n"
    "      }\n"
    "      while(range<0x80000000){\n"
    "        bitPos++;\n"
    "        if(low>>>31) rcCarry(out,bitPos);\n"
    "        low=(low<<1)>>>0;\n"
    "        range=(range<<1)>>>0;\n"
    "      }\n"
    "      for(var m=0;m<num;m++){\n"
    ...
    "    var pos=bitPos;\n"
    "    if(low>0){\n"
    "      if(((low+range-1)>>>0)>=low) pos++;\n"
    "      rcCarry(out,pos);\n"
    "    }\n"
    "    var pay=(pos+7)>>3;\n"
    "    var file=new Uint8Array(H.co+pay);\n"
    "    file.set(H.cd.subarray(0,H.co));\n"
    "    file.set(out.subarray(0,pay),H.co);\n"
    "    return {file:file,bits:pos,pay:pay};\n"
    "  }\n"
    "  function rcStart(){\n"
    "    if(typeof PAQ==='undefined'||!PAQ.b64||\n"
    "       typeof BD==='undefined'||!BD.length){\n"
    ...
    "    var bytes=new Uint8Array(BD.length);\n"
    "    for(var i=0;i<BD.length;i++) bytes[i]=parseInt(BD[i].h,16);\n"
    "    var t0=performance.now(),r=null;\n"
    "    try{r=rcEncode(bytes);}catch(e){}\n"
    "    var ms=performance.now()-t0;\n"
    "    if(!r){showToast('THE PRESS JAMMED');return;}\n"
    "    var oldPay=H.cd.length-H.co,oldFile=H.cd.length;\n"
    "    var same=r.pay===oldPay;\n"
    "    if(same) for(var i=0;i<r.pay;i++)\n"
    "      if(H.cd[H.co+i]!==r.file[H.co+i]){same=false;break;}\n"
    "    var tn=0;\n"
    "    if(window.TUNED)\n"
    "      for(var i=0;i<window.TUNED.length;i++)\n"
    "        if(window.TUNED[i]) tn++;\n"
    ...
    "        +r.pay+' B vs shipped '+oldPay+' B');}catch(e){}\n"
    ...
    "    if(sbOn) sbStop();\n"
    "    var c=0xFFFFFFFF;\n"
    "    for(var i=0;i<bytes.length;i++) c=dcCrc(c,bytes[i]);\n"
    "    PAQ.b64=rcB64(r.file);\n"
    "    paqH=null;fpH=null;\n"
    "    rcBytes=bytes;\n"
    "    if(typeof REPORT_ID!=='undefined'){\n"
    "      REPORT_ID.crc='0x'+dcHex8((c^0xFFFFFFFF)>>>0);\n"
    "      REPORT_ID.outb=r.file.length;\n"
    "      REPORT_ID.ratio=100*r.file.length/REPORT_ID.inb;\n"
    "    }\n"
    "    egg('recrunch');\n"
    "    var d=r.file.length-oldFile;\n"
    "    var ex=document.getElementById('fpbox');\n"
    "    if(ex) ex.remove();\n"
    "    function row(a,b){return '<div class=\"cr\"><span>'+a\n"
    "      +'</span><span>'+b+'</span></div>';}\n"
    "    var h2='<div class=\"cbx\"><div class=\"ct\">*** RECRUNCH ***</div>'\n"
    "      +row('BYTES PRESSED',bytes.length)\n"
    "      +row('EDITS IN THE MIX',tn)\n"
    "      +row('PAYLOAD',oldPay+' \\u2192 '+r.pay+' B')\n"
    "      +row('WHOLE FILE',oldFile+' \\u2192 '+r.file.length+' B')\n"
    "      +row('DELTA',(d>=0?'+':'')+d+' B')\n"
    "      +row('TIME',ms.toFixed(1)+' MS')\n"
    "      +row('VERDICT',same?'BYTE-IDENTICAL TO THE SHIPPED PAYLOAD'\n"
    "        :'A NEW PRESSING. PAQ SWAPPED IN')\n"
    "      +'<div class=\"cf\">SAME MODELS, SAME TABLE - THE SEARCH STAYS'\n"
    "      +' IN C<br>DECRUNCH NOW PROVES THIS PRESSING. RELOAD FOR THE'\n"
    "      +' SHIPPED ONE</div></div>';\n"
    "    var ov=document.createElement('div');\n"
    "    ov.id='fpbox';ov.innerHTML=h2;\n"
    "    ov.addEventListener('click',function(){ov.remove();});\n"
