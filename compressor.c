...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "    var phys=null,fvx=0,fvy=0;\n"
    "    function fly(){\n"
    "      if(!document.body.classList.contains('party')||\n"
    "         glz.classList.contains('dead')||dragOn){phys=null;return;}\n"
    "      var r=glz.getBoundingClientRect();\n"
    "      var x=r.left+fvx,y=r.top+fvy;\n"
    "      var W=window.innerWidth,H=window.innerHeight;\n"
    "      if(x<0){x=0;fvx=-fvx*.7;}\n"
    "      if(x+r.width>W){x=W-r.width;fvx=-fvx*.7;}\n"
    "      if(y<0){y=0;fvy=-fvy*.7;}\n"
    "      if(y+r.height>H){y=H-r.height;fvy=-fvy*.7;}\n"
    "      glz.style.left=x+'px';glz.style.top=y+'px';\n"
