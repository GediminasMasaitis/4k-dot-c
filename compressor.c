...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "  var dfPc=null,dfWin=null,dfKA=0;\n"
    ...
    "  var blkW=null,blkU=null,blkV=null,blkT=0,blkP=0,blkRows=0;\n"
    "  function blkHex(i){\n"
    ...
    "  function blkBuild(){\n"
    "    if(blkW) return;\n"
    "    var nb=Math.min(BD.length,16384);\n"
    "    var BW2=64,tw=16,th=8,hMax=70;\n"
    ...
    "    blkU={x:BW2*tw/2,y:BW2*th/2};\n"
    "    blkV={x:-rows*tw/2,y:rows*th/2};\n"
    ...
    "            :'rgba(255,255,255,.85)';\n"
    "          wc.font='7px \"Press Start 2P\",monospace';\n"
    "          wc.fillText(BD[i].ch,sx,ty+2.5);\n"
    "        }\n"
    "        /* the offset, on the flank of the tall ones */\n"
    "        if(h>=20){\n"
    "          wc.fillStyle='rgba(255,255,255,.4)';\n"
    "          wc.font='6px Consolas,monospace';\n"
    "          wc.fillText(blkHex(i),sx,ty+th/2+8);\n"
    "        }\n"
    "      }\n"
    "    }\n"
    "  }\n"
    ...
    "    blkT+=.016;\n"
    "    blkP+=.2; if(blkP>=blkRows) blkP-=blkRows;\n"
    "    var sw=Math.sin(blkT*.18)*10;\n"
    "    var px=-8*blkP+8*sw, py=4*blkP+4*sw;\n"
    "    blkCtx.clearRect(0,0,W,H);\n"
    "    var bx0=(W-blkW.width)/2, by0=(H-blkW.height)/2;\n"
    "    for(var m=-2;m<=2;m++){\n"
    "      for(var k=-2;k<=2;k++){\n"
    "        var bx=(bx0+k*blkU.x+m*blkV.x-px)|0;\n"
    "        var by=(by0+k*blkU.y+m*blkV.y-py)|0;\n"
    "        if(bx>W||by>H||bx+blkW.width<0||by+blkW.height<0)\n"
    "          continue;\n"
