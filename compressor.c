...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "        }\n"
    "      }\n"
    "      if((GEN.s>>sp)&1) snare(t,0,GEN.sg||.05);\n"
    "      if((GEN.hc>>sp)&1) hat(t,.013,false);\n"
    "      if((GEN.ho>>sp)&1) hat(t,.02,true);\n"
    "      if(GEN.pad&&sp===0) pad(t,ch);\n"
    ...
    "      if(GEN.rs&&bar2===0&&sp===0&&step>15) crash(t);\n"
    "      var grt=(sonify&&sonKey?sonKey.r:ch[0]);\n"
    "      /* hardstyle reverse bass: the offbeat 'uh', driven */\n"
    "      if(GEN.rb&&(sp&3)===2)\n"
    "        gbass(mf(grt-12),.07,.12,t,'sawtooth',drv);\n"
    "      if(GEN.bs==='ob'){\n"
    "        /* offbeat eighths: the house/techno/trance pulse */\n"
    "        if((0x4444>>sp)&1)\n"
    "          gbass(mf(grt-12),GEN.bg||.08,.2,t,GEN.bw2||'square');\n"
    "      } else if(GEN.bs==='psy'){\n"
    "        /* rolling 16ths on every non-kick step: the psy gallop */\n"
    "        if(!((GEN.k>>sp)&1))\n"
    "          gbass(mf(grt-12),.09,beatDur*.85,t);\n"
    "      } else if(GEN.bs==='sub'){\n"
    "        if(sp===0||sp===8) gbass(mf(grt-24),.1,.45,t,'sine');\n"
    "      } else if(GEN.bs==='ton'){\n"
    "        if((GEN.k>>sp)&1) gtail(mf(grt-24),t);\n"
    "      } else if(GEN.bs==='wub'){\n"
    "        /* one long note per half-bar; the wobble lfo on the\n"
    "           master filter does the talking */\n"
    "        if(sp===0||sp===8)\n"
    ...
    "          var te=envA(g,(syn.lg+.001)*acc,t,ldv);\n"
    "          fxLead(o,mf(nn),t,syn.atk+ldv,leadC,step);\n"
    "          o.connect(g);g.connect(panTo(notePan));o.start(t);\n"
    "          o.stop(te);\n"
    "          vreg(t,[o]);\n"
    "          if(syn.dtn>0){\n"
    ...
    "        env open, resonance up, fx column on for the slides */\n"
    "     bs:'none',kp:[115,42,.12,.12,0,.015],fl:1,\n"
    "     np:[0,0,12,0,3,10],lm:0xFFFF,ln:.12,\n"
    "     po:{wave:'sawtooth',fx:1,fe:.9,fq:5.5,fc:420,ld:.13}},\n"
    "    {n:'DUBSTEP',bpm:140,k:0x0081,s:0x0100,hc:0x2222,ho:0,\n"
    "     /* half-time: snare on 3. the global wobble lfo IS the\n"
    "        wub - the whole mix talks */\n"
    "     bs:'wub',kp:[110,40,.14,.3,1,.02],\n"
    "     np:[0,3,1,7,10,13],lm:0x1090,ln:.25,\n"
    "     po:{wr:2.5,wa:.75,fc:1100,fq:5}},\n"
    "    {n:'LOFI',bpm:84,k:0x0041,s:0x1010,hc:0xAAAA,ho:0,\n"
    "     /* boom-bap kicks, swing, worn tape, worn shellac */\n"
    ...
    "    var n2=sk.r+pmap(np[pi2],sk.mi)+24;\n"
    ...
    "  var drv=null; /* tanh drive for genre kicks */\n"
    "  function filtApply(){\n"
    "    if(!filt) return;\n"
