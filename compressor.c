...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "      (i/(BD.length-1))*1.6-0.8:0;\n"
    "  }\n"
    "  function bandPan(ci){\n"
    "    var n2=BAND.ch.length;\n"
    "    return n2>1?(ci/(n2-1))*1.6-0.8:0;\n"
    "  }\n"
    ...
    "      var dg=c<1.5?0:c<3?[0,2,4][step%%3]:c<5?[1,3,5][step%%3]:6;\n"
    "      return sonKey.r+sc[dg]+24;\n"
    "    }\n"
    "    var ch=CHORDS[(step>>4)&3];\n"
    "    if(c<1.5) return ch[0]+24;          /* calm: root */\n"
    "    if(c<3) return ch[step%%3]+24;      /* fine: chord tone */\n"
    "    if(c<5) return ch[step%%3]+26;      /* tense: color tone */\n"
    ...
    "      n=noteFor(step,c);\n"
    "      notePan=panOf(si);\n"
    "      if(c>=8) snare(t,notePan); /* pain: percussion */\n"
    ...
    "            fmVoice(mf(bandNote(ci,step)),.02+.05*(q/15),.12,t,\n"
    "              panTo(bandPan(ci)),FMR[ci%%8],1+3*(q/15));\n"
    "            continue;\n"
    "          }\n"
    ...
    "        var fi2=leadC<0?2:leadC<1.5?.4:leadC<3?1.5:leadC<5?4:8;\n"
    "        fmVoice(mf(n),.055,.12,t,panTo(notePan),1,fi2);\n"
    "      }else{\n"
    "        var o=actx.createOscillator(),g=actx.createGain();\n"
    "        o.type='square';o.frequency.value=mf(n);\n"
    "        g.gain.setValueAtTime(.04,t);\n"
    "        g.gain.exponentialRampToValueAtTime(.001,t+.12);\n"
    "        o.connect(g);g.connect(panTo(notePan));o.start(t);o.stop(t+.13);\n"
    "      }\n"
    "    }\n"
    "    if(step%%4===0){\n"
    ...
    "      if(fmOn){\n"
    "        fmVoice(mf(bn),.07,.22,t,gainM,1,1.2);\n"
    "      }else{\n"
    ...
    "    /* the tuner's forward pass and the retune repaint can hold\n"
    "       the main thread for a few hundred ms each; a deeper queue\n"
    "       rides over both stalls without a dropout */\n"
    "    while(nextT<now+(tuOn?0.8:0.25)){\n"
    ...
    "  var eggSet={},eggN=0,EGG_TOTAL=55;\n"
    ...
    "  var tuMs=0,tuN=0,tuLit=false,tuHot=false;\n"
    ...
    "    window.FPSTATS=r.stats; /* coder-stat cards rebuild from this */\n"
    "    tuN=0;\n"
    "    for(var i=0;i<tuBytes.length;i++)\n"
    ...
    "    tuBytes[tuSel]=v&255;\n"
    "    tuScore();\n"
    "    /* hear the edit: the byte's new price, as the song would\n"
    "       play it - in key if a key is pinned */\n"
