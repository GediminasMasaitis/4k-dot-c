...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "    showToast('TWENTY BANDS OF TRUTH');\n"
    ...
    "    if(zoomOn) egg('melomaniac');\n"
    "    showToast(zoomOn?'ENHANCE':'BACK TO SCALE');\n"
    "  }\n"
    "  var dockW=248;\n"
    "  function dockSync(){\n"
    "    var w=248;\n"
    ...
    "      var m2=trkEl.getBoundingClientRect().width|0;\n"
    "      if(m2>248) w=m2;\n"
    "    }\n"
    "    if(w===dockW) return;\n"
    "    dockW=w;\n"
    ...
    "      buf=(buf+k).slice(-10); /* longest keyword: melomaniac (10) */\n"
    ...
    "        /* two m's in it, so the transient mute flips cancel */\n"
    "        if(buf.slice(-10)==='melomaniac'){buf='';zoomToggle();return;}\n"
    ...
    "      ['melomaniac','THE RACK, TWICE THE SIZE'],\n"
