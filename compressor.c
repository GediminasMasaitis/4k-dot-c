...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "  var voxHf=null,voxSh=null,voxPR=null,voxPG=null,voxPB=null;\n"
    ...
    "      }\n"
    "    }\n"
    "    /* baked slope shading: south faces lit, north faces shaded */\n"
    "    voxSh=new Float32Array(n);\n"
    "    for(var i=0;i<n;i++){\n"
    "      var sl=1+(voxHf[i]-voxHf[(i+voxTW)%%n])*.05;\n"
    "      if(sl<.5)sl=.5; if(sl>1.4)sl=1.4;\n"
    "      voxSh[i]=sl;\n"
    "    }\n"
    "    /* hypsometric palette: altitude = cost = the cmap ramp */\n"
    "    voxPR=new Uint8Array(256);voxPG=new Uint8Array(256);\n"
    "    voxPB=new Uint8Array(256);\n"
    "    for(var i=0;i<256;i++){\n"
    "      var t=(i-10)/160; if(t<0)t=0; if(t>1)t=1;\n"
    "      var r,g,b;\n"
    "      if(t<0.5){var u=t*2;\n"
    "        r=16+(180-16)*u;g=185+(140-185)*u;b=129+(40-129)*u;}\n"
    "      else{var u=(t-0.5)*2;\n"
    "        r=180+(248-180)*u;g=140+(113-140)*u;b=40+(113-40)*u;}\n"
    "      voxPR[i]=r|0;voxPG[i]=g|0;voxPB[i]=b|0;\n"
    "    }\n"
    "  }\n"
    "  function voxSize(){\n"
    "    if(!voxCv) return;\n"
    "    voxCv.width=Math.max(2,window.innerWidth>>2);\n"
    "    voxCv.height=Math.max(2,window.innerHeight>>2);\n"
    "    voxImg=voxCtx.createImageData(voxCv.width,voxCv.height);\n"
    "    voxPix=new Uint32Array(voxImg.data.buffer);\n"
    "    voxYB=new Int16Array(voxCv.width);\n"
    "    /* dusk gradient: indigo overhead to haze at the horizon, a\n"
    "       darker floor below it (seen only past the view distance) */\n"
    ...
    "      }else{var t=(y-hz)/(H-hz||1);\n"
    "        r=46-36*t;g=20-14*t;b=58-42*t;\n"
    "      }\n"
    ...
    "    var camx=(voxTW>>1)+Math.sin(voxT*.21)*40;\n"
    "    var ang=Math.sin(voxT*.13)*.55;\n"
    "    var sinA=Math.sin(ang),cosA=Math.cos(ang);\n"
    "    var mw=voxTW-1,mh=voxTH-1;\n"
    "    /* high seat + low horizon = gently pitched down: relief reads\n"
    "       as landscape below instead of walls ahead */\n"
    "    var gi=((voxY|0)&mh)*voxTW+((camx|0)&mw);\n"
    "    voxCH+=((voxHf[gi]+105+Math.sin(voxT*.9)*5)-voxCH)*.06;\n"
    "    var camH=voxCH;\n"
    ...
    "      var invz=scale/z;\n"
    "      /* quadratic fog toward the horizon haze: vivid up close,\n"
    "         terrain melts seamlessly into the sky at the far plane */\n"
    "      var af=z/maxZ;af=af*af*.95;\n"
    "      var fr=1-af;\n"
    "      var aR=46*af,aG=20*af,aB=58*af;\n"
    "      for(var x=0;x<W;x++){\n"
    "        /* bilinear height - nearest sampling is what shimmers */\n"
    "        var ix=plx|0,iy=ply|0;\n"
    "        var fx=plx-ix,fy=ply-iy;\n"
    "        var x0=ix&mw,x1=(ix+1)&mw;\n"
    "        var y0=(iy&mh)*voxTW,y1=((iy+1)&mh)*voxTW;\n"
    "        var h00=voxHf[y0+x0],h01=voxHf[y0+x1];\n"
    "        var h10=voxHf[y1+x0],h11=voxHf[y1+x1];\n"
    "        var h=h00+(h01-h00)*fx+(h10-h00)*fy\n"
    "          +(h00-h01-h10+h11)*fx*fy;\n"
    "        var yt=((camH-h)*invz)+horizon|0;\n"
    "        if(yt<0) yt=0;\n"
    "        if(yt<ybuf[x]){\n"
    "          var hi=h|0;\n"
    "          var sh=voxSh[y0+x0]*fr;\n"
    "          var col=0xFF000000|\n"
    "            (((voxPB[hi]*sh+aB)|0)<<16)|\n"
    "            (((voxPG[hi]*sh+aG)|0)<<8)|\n"
    "            ((voxPR[hi]*sh+aR)|0);\n"
    ...
    "        plx+=dx;ply+=dy;\n"
    "      }\n"
    "      z+=dz;dz+=.05; /* coarser depth steps far away */\n"
    "    }\n"
    "    voxCtx.putImageData(voxImg,0,0);\n"
