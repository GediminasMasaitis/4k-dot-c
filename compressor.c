...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "            :' != expected '+want)\n"
    "          +'; '+mm+'/'+dc.nb+' decoded bytes disagree with BD'\n"
    "          +(m0>=0?'; first at 0x'+m0.toString(16).toUpperCase()\n"
    "            +': decoded '+dcHex2(dc.buf[8+m0])+' vs BD '+BD[m0].h:'')\n"
    "          +(tn?'; TUNED: '+tn+' edited byte(s) committed to BD'\n"
    "            +' - BD is the tuned file, the stream is the original'\n"
    "            +' encode':''));\n"
    "      }catch(e){}\n"
