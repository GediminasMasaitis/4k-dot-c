...
static void ChangeWeight_job(int job_idx, void *vctx) {
  ...
  for (int i = 0; i < ctx->packages_per_job && pkg_base + i < ctx->num_packages;
    ...
    for (int k = 0; k < 4; k++) {
      ...
      int right_exp = (int)(rb >> 23);
      int total_exp = (int)(tb >> 23);

      uint32_t rm = (rb & 0x7fffffu) | 0x3f800000u;
      uint32_t tm = (tb & 0x7fffffu) | 0x3f800000u;
      float rmf, tmf;
      memcpy(&rmf, &rm, sizeof(float));
      memcpy(&tmf, &tm, sizeof(float));

      float rx = rmf - 1.0f;
      float tx = tmf - 1.0f;
      float right_log = rx * (LOG2_C0 + rx * (LOG2_C1 + rx * LOG2_C2));
      float total_log = tx * (LOG2_C0 + tx * (LOG2_C1 + tx * LOG2_C2));

      int frac_part =
          (int)lrintf((total_log - right_log) * (float)TABLE_BIT_PRECISION);
      new_pkg_size +=
          ((total_exp - right_exp) << TABLE_BIT_PRECISION_BITS) + frac_part;
    }
