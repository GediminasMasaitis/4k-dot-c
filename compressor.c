...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "  var voxH=null,voxR=null,voxG=null,voxB=null;\n"
    ...
    "       upsample x4 so slopes are ramps instead of steps */\n"
    "    var BW=64;\n"
    "    var rows=Math.ceil(nb/BW);\n"
    "    var BH=64; while(BH<rows&&BH<512) BH<<=1;\n"
    "    var bn=BW*BH;\n"
    "    var bh=new Float32Array(bn);\n"
    "    var br=new Float32Array(bn),bg=new Float32Array(bn);\n"
    "    var bb=new Float32Array(bn);\n"
    "    var cmax=0.01;\n"
    ...
    "      a.set(tmp);\n"
    "    }\n"
    "    blur(bh);blur(bh);blur(br);blur(bg);blur(bb);\n"
    ...
    "    var n=voxTW*voxTH;\n"
    "    voxH=new Uint8Array(n);voxR=new Uint8Array(n);\n"
    "    voxG=new Uint8Array(n);voxB=new Uint8Array(n);\n"
    ...
    "        var i00=y0*BW+x0,i01=y0*BW+x1;\n"
    "        var i10=y1*BW+x0,i11=y1*BW+x1;\n"
    "        var w00=(1-tx)*(1-ty),w01=tx*(1-ty);\n"
    "        var w10=(1-tx)*ty,w11=tx*ty;\n"
    "        var o=y*voxTW+x;\n"
    "        voxH[o]=(bh[i00]*w00+bh[i01]*w01+bh[i10]*w10+bh[i11]*w11)|0;\n"
    "        voxR[o]=(br[i00]*w00+br[i01]*w01+br[i10]*w10+br[i11]*w11)|0;\n"
    "        voxG[o]=(bg[i00]*w00+bg[i01]*w01+bg[i10]*w10+bg[i11]*w11)|0;\n"
    "        voxB[o]=(bb[i00]*w00+bb[i01]*w01+bb[i10]*w10+bb[i11]*w11)|0;\n"
    "      }\n"
    "    }\n"
    "    /* baked slope shading: south faces lit, north faces shaded */\n"
    "    for(var i=0;i<n;i++){\n"
    "      var sl=1+(voxH[i]-voxH[(i+voxTW)%%n])*.04;\n"
    "      if(sl<.55)sl=.55; if(sl>1.35)sl=1.35;\n"
    "      var r=voxR[i]*sl,g=voxG[i]*sl,b=voxB[i]*sl;\n"
    "      voxR[i]=r>255?255:r|0;\n"
    "      voxG[i]=g>255?255:g|0;\n"
    "      voxB[i]=b>255?255:b|0;\n"
    "    }\n"
    "  }\n"
    "  function voxSize(){\n"
    "    if(!voxCv) return;\n"
    ...
    "    voxT+=.016;\n"
    "    voxY-=.9; /* forward is -y in classic voxelspace */\n"
    ...
    "    /* terrain-following camera, smoothed so ridges feel like lift */\n"
    "    var gi=((voxY|0)&mh)*voxTW+((camx|0)&mw);\n"
    "    voxCH+=((voxH[gi]+55+Math.sin(voxT*.9)*5)-voxCH)*.06;\n"
    "    var camH=voxCH;\n"
    "    var horizon=(H*.42)|0;\n"
    ...
    "    while(z<maxZ){\n"
    "      var plx=(-cosA-sinA)*z+camx,ply=(sinA-cosA)*z+voxY;\n"
    "      var prx=(cosA-sinA)*z+camx,pry=(-sinA-cosA)*z+voxY;\n"
    ...
    "      for(var x=0;x<W;x++){\n"
    "        var mi=((ply|0)&mh)*voxTW+((plx|0)&mw);\n"
    "        var yt=((camH-voxH[mi])*invz)+horizon|0;\n"
    "        if(yt<0) yt=0;\n"
    "        if(yt<ybuf[x]){\n"
    "          var col=0xFF000000|\n"
    "            (((voxB[mi]*fr+aB)|0)<<16)|\n"
