...
static void write_html_report(const char *path, const CompStats *s) {
  ...
  fprintf(f,
    ...
    "/* paqtracker: sonify's pattern view, docked past the sidebar */\n"
    "#paqtrk{display:none}\n"
    "body.party #paqtrk.on{display:block;position:fixed;left:200px;"
    "bottom:16px;width:248px;z-index:9997;background:rgba(4,4,8,.93);"
    "border:1px solid var(--bdr2);padding:0 0 6px;"
    "font-family:var(--pix);letter-spacing:.5px;"
    "box-shadow:5px 5px 0 rgba(0,0,0,.45)}\n"
    "@media(max-width:900px){body.party #paqtrk.on{left:16px}}\n"
...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "        if(buf.slice(-5)==='amiga'){buf='';boingToggle();return;}\n"
    ...
    "        if(buf.slice(-3)==='man'){buf='';egg('man');\n"
    ...
    "  var eggSet={},eggN=0,EGG_TOTAL=27;\n"
