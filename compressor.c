...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "      ['protracker','THE SYNTH IN AMIGA STEEL'],\n"
    ...
    "  }\n"
    "  /* the protracker skin: typed on, typed off. the synth is\n"
    "     unchanged underneath - only the clothes */\n"
    "  var ptOn=false;\n"
    "  function ptToggle(){\n"
    "    ptOn=!ptOn;\n"
    ...
    "      :'*** PAQ SYNTH ***';\n"
    "    if(ptOn&&!synOn) synStart(true);\n"
    "    showToast(ptOn?'STEEL AND TOPAZ':'BACK TO NEON');\n"
    "  }\n"
    "  function synStop(){\n"
    "    if(!synOn) return;\n"
    "    synOn=false;\n"
