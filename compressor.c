...
#undef CLAMP
  ...
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "        sl[m]=s2;\n"
    "      }\n"
    "      var c=-Math.log2((bit?p1:p0)/(p0+p1));\n"
    "      total+=c;\n"
    ...
    "    /* the report priced bits with a cubic log2 and rounded to\n"
    "       two decimals; we price with the real thing. small drift\n"
    "       is expected, so it is reported, not hidden */\n"
    ...
    "      +row('LOG DRIFT',(mine-rep).toFixed(2)+' BITS')\n"
