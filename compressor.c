...
static void emit_report_scripts(FILE *f) {
  fprintf(f, "</div><!-- wrap -->\n"
    ...
    "        dlG.gain.value=0;\n"
    "        post.connect(dl);dl.connect(dlG);dlG.connect(comp);\n"
    "        dl.connect(dlF);dlF.connect(dl);\n"
    ...
    "    sw:0,atk:0,cb:16,cmp:1,ah:0,gr:0,pts:0,mg:1,me:1};\n"
    "  var syn={};\n"
    ...
    "  function dlApply(){\n"
    "    if(!dl) return;\n"
    "    dl.delayTime.value=Math.min(2,beatDur*syn.dt);\n"
