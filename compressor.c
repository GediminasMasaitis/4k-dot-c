...
#undef CLAMP
  ...
  /* ── Per-Model Statistics ── */
  {
    ...
    fprintf(f, "%s",
      ...
      "/* build per-model sparkline from Net chart's mdata (if present) */\n"
      "function sparkline(m){\n"
      "  if(!window.netMdata) return '';\n"
      "  var nm=window.netNm, np=window.netNpts, nb=window.netNb;\n"
      "  var W=200,H=44,P=2;\n"
      "  /* find vmax for this model */\n"
      "  var vmax=0.001;\n"
      "  for(var i=0;i<np;i++){var v=Math.abs(window.netMdata[i*nm+m]);"
      "if(v>vmax)vmax=v;}\n"
      "  var midY=H/2;\n"
      "  var pc=(window.netPal&&window.netPal[m])||[100,100,100];\n"
      "  var clr='rgb('+pc[0]+','+pc[1]+','+pc[2]+')';\n"
      "  /* build area path positive + negative */\n"
      "  var d='M'+P+','+midY;\n"
      "  for(var i=0;i<np;i++){\n"
      "    var x=P+(i*(W-2*P)/(np>1?np-1:1));\n"
      "    var v=window.netMdata[i*nm+m];\n"
      "    var y=midY-(v/vmax)*(midY-P);\n"
      "    d += ' L'+x.toFixed(1)+','+y.toFixed(1);\n"
      "  }\n"
      "  d += ' L'+(W-P)+','+midY+' Z';\n"
      "  /* build line path for clarity */\n"
      "  var ld='';\n"
      "  for(var i=0;i<np;i++){\n"
      "    var x=P+(i*(W-2*P)/(np>1?np-1:1));\n"
      "    var v=window.netMdata[i*nm+m];\n"
      "    var y=midY-(v/vmax)*(midY-P);\n"
      "    ld += (i?'L':'M')+x.toFixed(1)+','+y.toFixed(1)+' ';\n"
      "  }\n"
      ...
      "    +'\" stroke=\"rgba(255,255,255,.2)\" stroke-width=\"0.5\"/>'\n"
      "    +'<path d=\"'+d+'\" fill=\"'+clr+'\" fill-opacity=\".25\"/>'\n"
      "    +'<path d=\"'+ld+'\" fill=\"none\" stroke=\"'+clr\n"
      "    +'\" stroke-width=\"1\"/>'\n"
      "    +'<text x=\"4\" y=\"9\" font-size=\"8\" fill=\"#6b7186\">+'\n"
      "    +vmax.toFixed(2)+'</text>'\n"
      "    +'<text x=\"4\" y=\"'+(H-3)+'\" font-size=\"8\" fill=\"#6b7186\">\\u2212'\n"
      "    +vmax.toFixed(2)+'</text>'\n"
      "    +'<text x=\"'+(W-4)+'\" y=\"'+(H-3)+'\" text-anchor=\"end\" '\n"
      "    +'font-size=\"8\" fill=\"#6b7186\">'+nb+' B</text>'\n"
      "    +'</svg>';\n"
      "}\n"
      "for(var i=0;i<rows.length;i++){\n"
      "  rows[i].addEventListener('click',function(){\n"
