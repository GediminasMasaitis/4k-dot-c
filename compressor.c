...
static void write_html_report(const char *path, const CompStats *s) {
  ...
  fprintf(f,
    ...
    "#cmap-detail .cd-byte{font-family:var(--mono);font-size:13px;"
    "font-weight:500;color:var(--fg2)}\n"
    "#cmap-detail .cd-sub{font-size:11px;color:var(--fg3)}\n"
    "#cmap-detail .cd-cost{font-family:var(--mono);font-size:22px;"
    "font-weight:700}\n"
  ...
  if (s->byte_costs && s->num_data_bytes > 0) {
    ...
    fprintf(f,
      ...
      "  var total=0;\n"
      "  if(d.m) for(var i=0;i<d.m.length;i++) total+=d.m[i];\n"
      ...
      "  h+='<span class=\"cd-sub\">offset '+d.o+'  &middot;  8.00 &minus; '+total.toFixed(2)+' saved</span>';\n"
      "  h+='</div>';\n"
      "  if(d.m && MI.length){\n"
      "    var mx=0;\n"
